...
| `cose` `nodeOverlap` / `nodeRepulsion` | multipliers on a point repulsion; overlap avoidance was a side effect of strength | `force` reads the node boxes under `avoidOverlap` (116.1 — a short-range contact term keeps every pair apart in the sim, labels on request) and `repulsion` is the one strength knob; there is no overlap multiplier to tune |
