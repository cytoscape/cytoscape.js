...
| `cose` `nodeOverlap` / `nodeRepulsion` | multipliers on a point repulsion; overlap avoidance was a side effect of strength | `force` separates the node boxes exactly at the settle under `avoidOverlap` (default true, labels on request), and `avoidOverlapInSim: true` makes the sim itself keep them apart (116.1's short-range contact term; opt-in since 117 — it roughly triples the iterations, and on a dense graph the settle does the clearing anyway); `repulsion` is the one strength knob; there is no overlap multiplier to tune |
| `cose` / `force` with `boundingBox` | v3 cose stretched the node centres to fill the box, up or down, size-blind; bodies could overhang it | `force` **scales down only** so every body lies inside, then centres the drawing (116.2 — flow's rule); a pinned node or constraints hold it back, as they hold the component re-pack |
