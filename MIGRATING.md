...
| `cose` `nodeOverlap` / `nodeRepulsion` | multipliers on a point repulsion; overlap avoidance was a side effect of strength | `force` separates the node boxes exactly at the settle under `avoidOverlap` (default true, labels on request); `avoidOverlap: 'sim'` makes the sim itself keep them apart with a separation sweep after every tick (118.2; opt-in — the settle's pass is cheaper for a one-shot run), `'both'` does both |
