...
- **Labels are model-space only** (no viewport-fixed mode).  `font-size` and
  ...
  answered editorially, not by a coordinate system).  Open sub-decision
  (visibility, not sizing): `min-zoomed-font-size` cull threshold evaluated
  at **export scale** vs **screen zoom** — lean export-scale for a
  self-consistent figure; parked with the label work.
