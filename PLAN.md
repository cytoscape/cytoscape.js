...
- **`renderScale` (added)**: render at a fraction of native resolution into an offscreen target + Catmull-Rom bicubic upscale to the canvas (9 bilinear taps; negative lobes keep SDF borders/hairlines crisp). Fill cost ~scale²: ndex-x-large fit-all at dpr 2 38 → 26.4 (0.75) → 10.6 ms (0.5, 25 → 72 fps). Picking stays native; `labelMinPx` option hard-culls unreadably small labels in the glyph cull predicate.
