...
- **Culling/LOD**: VS conservative off-viewport collapse + degenerate tombstones; edge width floor with alpha compensation; plain-disc nodes below ~3 px; sub-pixel size flooring with alpha compensation; optional zoom-based edge dimming (harness toggle). Compute culling + drawIndirect remain the documented follow-up. Added after GPU-time profiling: **far-zoom edge decimation** — once floored edges fall below half alpha, a hash-stable 1-in-N subset draws at N× alpha (N ≤ 64), preserving aggregate density while removing the far-zoom blend-serialization worst case (465k edges massed on a few hundred pixels: ~33 ms → ~8 ms; fit-all at dpr 1: 18.5 → 13.1 ms).
