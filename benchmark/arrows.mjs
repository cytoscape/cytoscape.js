...
const sheets = ( arrows ) => [ 3, 5 ].map( width => ( {
  nodes: { width: 30, height: 30 },
  edges: { ...edgeStyle( arrows ), width }
} ) );

const build = arrows => cytoscape( {
  elements: elements(),
  style: { nodes: { width: 30, height: 30 }, edges: edgeStyle( arrows ) }
} );

console.log( `arrows: ${PAIRS * 2} nodes, ${EDGES} edges (${PER_PAIR} per pair)\n` );
...
const buildA1 = time( () => { arrowed = build( true ); } );
const buildP1 = time( () => { plain = build( false ); } );
const spareP = ( () => { let cy; const ms = time( () => { cy = build( false ); } ); cy.destroy(); return ms; } )();
const spareA = ( () => { let cy; const ms = time( () => { cy = build( true ); } ); cy.destroy(); return ms; } )();
...
rows.push( {
  label: 'build (style apply + arrow fold)', unit: 'edge', count: EDGES,
  arrowedMs: arrowedBuildMs, plainMs: plainBuildMs, ratio: arrowedBuildMs / plainBuildMs
} );
...
compare( `cy.style() re-apply x ${APPLIES}`, 'edge-apply', EDGES * APPLIES, cy => {
...
compare( `node.position x ${DRAGS} (drag)`, 'move', DRAGS, cy => {
  for( let i = 0; i < DRAGS; i++ ){
    ...
    cy.$id( `a${j}` ).position( { x: ( j % 200 ) * 60 + ( i % 7 ), y: Math.floor( j / 200 ) * 60 } );
  }
} );
...
compare( `edge.sourceEndpoint x ${WORK}`, 'edge', WORK, cy => {
  ...
  for( let i = 0; i < WORK; i++ ){ x += cy.$id( edgeId( i ) ).sourceEndpoint().x; }
...
compare( `edge.targetEndpoint x ${WORK}`, 'edge', WORK, cy => {
  ...
  for( let i = 0; i < WORK; i++ ){ x += cy.$id( edgeId( i ) ).targetEndpoint().x; }
...
compare( `edge.midpoint x ${WORK}`, 'edge', WORK, cy => {
  ...
  for( let i = 0; i < WORK; i++ ){ x += cy.$id( edgeId( i ) ).midpoint().x; }
...
const curvedEndpointMs = Math.min( time( () => readEndpoints( curvedArrowed ) ),
  time( () => readEndpoints( curvedArrowed ) ) );
const straightEndpointMs = Math.min( time( () => readEndpoints( arrowed ) ),
...
rows.push( {
  label: 'control: sourceEndpoint, bezier vs straight', unit: 'edge', count: WORK,
  arrowedMs: curvedEndpointMs, plainMs: straightEndpointMs,
...
for( const s of edgeSpecs.filter( s => s.id.startsWith( 'edge.arrow' ) || s.id === 'edge.width' ) ){
  console.log( `  ${s.id.padEnd( 22 )} ${s.ctor.name.padEnd( 13 )} x${s.components}  ${s.bytesPerSlot} B/edge` );
}
...
finishManualRun( 'arrows', [
  ...rows.map( r => ( {
    ...
    benches: [ { name: 'arrowed', ms: r.arrowedMs }, { name: 'none', ms: r.plainMs } ]
  } ) ),
  { name: 'arrow premium: edge column bytes', benches: [ { name: 'B/edge', ms: bytesPerEdge } ] }
] );
