...
//
//   BENCH_N=200000 node --import tsx benchmark/compaction.mjs
...
const columnBytes = (cy, g) =>
  columnSpecsForGroup(g).reduce(
    (sum, spec) =>
      sum +
      spec.components * spec.ctor.BYTES_PER_ELEMENT * cy._store.capacity(g),
    0,
  );
...
console.log(
  `held-collection first-touch repair (${held.length} moved refs): ${repairMs.toFixed(1)} ms one-shot`,
);
console.log(
  `store-level removal (no trigger): ${storeRemovalMs.toFixed(0)} ms; ` +
...
group('cpu pick: background miss (full highWater walk)', () => {
  summary(() => {
    bench('peak', () =>
      do_not_optimize(pickNodeAt(peak._store, frame, missX, 0)));
    bench('compacted', () =>
      do_not_optimize(pickNodeAt(compacted._store, frame, missX, 0)));
  });
});
...
group('cpu pick: node hit', () => {
  ...
  summary(() => {
    bench('peak', () =>
      do_not_optimize(pickNodeAt(peak._store, frame, peakAt.x, peakAt.y)));
    bench('compacted', () =>
      do_not_optimize(
        pickNodeAt(compacted._store, frame, compactedAt.x, compactedAt.y),
      ));
  });
});
...
group('isCurrent: current ref (forwards present vs absent)', () => {
  ...
  summary(() => {
    bench('no forwards (peak)', () =>
      do_not_optimize(peak._store.isCurrent(peakRef)));
    bench('forwards present (compacted)', () =>
      do_not_optimize(compacted._store.isCurrent(compactedRef)));
  });
});
...
group('control: nodes({ selected }) scan (order-list, expected parity)', () => {
  ...
  summary(() => {
    ...
    bench('compacted', () =>
      do_not_optimize(compacted.nodes({ selected: true })));
  });
});
...
group('control: whole-graph boundingBox (expected parity)', () => {
  summary(() => {
    bench('peak', () =>
      do_not_optimize(peak.elements().boundingBox({ includeLabels: false })));
    bench('compacted', () =>
      do_not_optimize(
        compacted.elements().boundingBox({ includeLabels: false }),
      ));
  });
});
