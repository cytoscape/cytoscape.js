...
const build = (curveStyle) =>
  cytoscape({
    elements: elements(),
    style: {
      nodes: { width: 30, height: 30 },
      edges: { 'curve-style': curveStyle, width: 3 },
    },
  });

console.log(
  `curves: ${PAIRS * 2} nodes, ${EDGES} edges (${BUNDLE} parallel per pair)\n`,
);
...
const curvedBuildMs = time(() => {
  curved = build('bezier');
});
const straightBuildMs = time(() => {
...
rows.push({
  label: 'build (style apply + derivation)',
  unit: 'edge',
  count: EDGES,
  curvedMs: curvedBuildMs,
  straightMs: straightBuildMs,
  ratio: curvedBuildMs / straightBuildMs,
});
...
compare('first read after a bulk move (cold)', 'call', 1, (cy) => {
  cy.nodes().positions((n, i) => ({
    x: (i % 200) * (60 + moveParity),
    y: Math.floor(i / 200) * (60 + moveParity),
  }));
...
finishManualRun(
  ...
  rows.map((r) => ({
    ...
    benches: [
      { name: 'curved', ms: r.curvedMs },
      { name: 'straight', ms: r.straightMs },
    ],
  })),
);
