...
// for v3's selectors.
//
//   node --import tsx benchmark/data.mjs              # N=2000
//   BENCH_N=200000 BENCH_OP=write node --import tsx benchmark/data.mjs
//
...
if (has('write')) {
  ...
  group('data: bulk write, numeric column', () => {
    summary(() => {
      bench('v3', () => {
        v3Nodes.data('num', (i++ % 7) + 0.5);
      });
      bench('gpu', () => {
        nodes.data('num', (i++ % 7) + 0.5);
      });
    });
  });
  ...
  group('data: bulk write, dictionary string column (4 values)', () => {
    summary(() => {
      bench('v3', () => {
        v3Nodes.data('cat', CATS[i++ & 3]);
      });
      bench('gpu', () => {
        nodes.data('cat', CATS[i++ & 3]);
      });
    });
  });
  ...
  group('data: removeData (whole collection)', () => {
    summary(() => {
      bench('v3', () => {
        v3Nodes.data('tmp', 1);
        v3Nodes.removeData('tmp');
      });
      bench('gpu', () => {
        nodes.data('tmp', 1);
        nodes.removeData('tmp');
      });
    });
  });
}
...
if (has('read')) {
  ...
  cmpRead('data: read one string key (dictionary decode)', (n) =>
    n.data('cat'),
  );
...
if (has('query')) {
  ...
  group('query: data comparison ([weight > 3] vs { gt })', () => {
    summary(() => {
      ...
      bench('gpu', () =>
        do_not_optimize(gpu.nodes({ data: { weight: { gt: 3 } } })));
    });
  });
  ...
  group('query: two keys AND-ed', () => {
    summary(() => {
      ...
      bench('gpu', () =>
        do_not_optimize(
          gpu.nodes({ data: { weight: { gt: 2 }, bucket: { lt: 5 } } }),
        ));
    });
  });
  ...
  group('query: membership ([bucket in ...] vs { in })', () => {
    summary(() => {
      bench('v3', () =>
        do_not_optimize(v3.nodes('[bucket = 1], [bucket = 2], [bucket = 5]')));
      bench('gpu', () =>
        do_not_optimize(gpu.nodes({ data: { bucket: { in: [1, 2, 5] } } })));
    });
  });
  ...
  group('query: predicate function (both sides)', () => {
    summary(() => {
      bench('v3', () =>
        do_not_optimize(v3.nodes().filter((n) => n.data('weight') > 3)));
      bench('gpu', () =>
        do_not_optimize(gpu.nodes().filter((n) => n.data('weight') > 3)));
    });
  });
}
