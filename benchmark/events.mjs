...
//
//   node --import tsx benchmark/events.mjs            # N=2000
//   BENCH_N=20000 BENCH_OP=emit node --import tsx benchmark/events.mjs
//
...
if (has('phase')) {
  ...
  group('emit: flat vs phased (compound bubbling, 14.5)', () => {
    summary(() => {
      bench('orphan (flat emit)', () => {
        orphan.position({ x: 500 + (i++ & 1), y: 500 });
      });
      bench('child of 2 ancestors (phased)', () => {
        child.position({ x: i++ & 1, y: 0 });
      });
    });
  });
  ...
  group('emit: flat vs phased, no listeners', () => {
    summary(() => {
      bench('orphan (flat)', () => {
        qOrphan.position({ x: 500 + (i++ & 1), y: 500 });
      });
      bench('child (phased)', () => {
        qChild.position({ x: i++ & 1, y: 0 });
      });
    });
  });
}
...
if (has('anim')) {
  ...
  group('anim: start + stop, one element', () => {
    summary(() => {
      bench('v3', () => {
        v3Node.animate({ position: { x: 5, y: 5 }, duration: 1000 });
        v3Node.stop();
      });
      bench('gpu', () => {
        gpuNode.animate({ position: { x: 5, y: 5 }, duration: 1000 });
        gpuNode.stop();
      });
    });
  });
  ...
  // where start cost actually shows up at scale
  {
    ...
    group(`anim: start + stop, ${band.length}-node collection`, () => {
      summary(() => {
        bench('v3', () => {
          v3Band.animate({ position: { x: 5, y: 5 }, duration: 1000 });
          v3Band.stop();
        });
        bench('gpu', () => {
          band.animate({ position: { x: 5, y: 5 }, duration: 1000 });
          band.stop();
        });
      });
    });
  ...
  group('anim: viewport animate + stop', () => {
    summary(() => {
      bench('v3', () => {
        v3.animate({ zoom: 2, duration: 1000 });
        v3.stop();
      });
      bench('gpu', () => {
        gpu.animate({ zoom: 2, duration: 1000 });
        gpu.stop();
      });
    });
  });
