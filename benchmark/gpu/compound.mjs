...
//
//   BENCH_N=20000 node --import tsx benchmark/gpu/compound.mjs
//   BENCH_N=200000 BENCH_OP=child node --import tsx benchmark/gpu/compound.mjs
//
...
function buildCompoundElements( n = N ){
  ...
  nodes.push( { data: { id: 'seed', parent: 'spare' }, position: { x: -500, y: -500 } } );
  ...
  for( let j = 0; j < n; j++ ){ // one intra-cluster edge per leaf
    const c = j % clusters;
...
function cmp( name, setup, fn ){
  ...
  const a = makeV3( elements, { styleEnabled: true, layout: { name: 'preset' } } );
...
cmp( 'parent drag (shift subtree + bb settle)',
...
cmp( 'child drag (move + parent re-derive)',
