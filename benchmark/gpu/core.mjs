...
function cmp( name, setup, op, gpuOp = op ){
  ...
  group( name, () => {
    summary( () => {
      ...
      bench( 'gpu', () => { const k = ( i++ ) & MASK; return do_not_optimize( gpuOp( gs[ k ], k ) ); } );
    } );
  } );
}
...
cmp( 'core: id lookup ($("#id") vs $id())', cyOf,
  ( c, k ) => c.$( '#n' + ( MIDNUM + k ) ),
  ( c, k ) => c.$id( 'n' + ( MIDNUM + k ) ) );
cmp( 'core: nodes()',            cyOf, c => c.nodes() );
cmp( 'core: edges()',            cyOf, c => c.edges() );
cmp( 'core: elements()',         cyOf, c => c.elements() );
cmp( 'core: nodes ($("node") vs filter({group}))', cyOf,
  c => c.$( 'node' ),
  c => c.filter( { group: 'nodes' } ) );
cmp( 'core: selected ($(":selected") vs filter({selected}))', cyOf,
  c => c.$( ':selected' ),
