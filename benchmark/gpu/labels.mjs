...
label rewritten by a data write).  Run:
  ...
  node --import tsx benchmark/gpu/labels.mjs           # 100k
  BENCH_N=20000 node --import tsx benchmark/gpu/labels.mjs
*/
...
const opts = {
  wrap: WRAP_WRAP, maxWidth: 80, overflowWrap: OFLOW_WHITESPACE,
  justification: JUSTIFY_CENTER, lineHeight: 1.2
};
...
time( `breakLines x ${N}`, () => {
  ...
  for( const text of texts ){ lines += breakLines( text, adv, opts ).length; }
...
time( `estimateBlock x ${N}`, () => {
  ...
  for( const text of texts ){ w += estimateBlock( text, 16, opts ).width; }
...
const entry = text => ( {
  text, fontSize: 16, color: 0xff000000, anchorY: 19, marginX: 0, marginY: 0,
  outlineWidth: 0, outlineColor: 0, bgColor: 0, bgPadding: 0, bgShape: 0,
  bgBorderColor: 0, bgBorderWidth: 0, anchorX: 0, halignShift: 0, valignShift: 0,
  endOffset: 0, minZoomedFontSize: 0, rotate: false,
  wrap: 1, maxWidth: 80, lineHeight: 1.2, overflowWrap: 0, justification: 1
} );
...
time( `setLabel (build) x ${N}`, () => {
  for( let i = 0; i < N; i++ ){ store.setLabel( slots[ i ], entry( texts[ i ] ), 'nodes' ); }
} );
