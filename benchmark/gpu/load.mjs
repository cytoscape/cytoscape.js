...
//
//   node --import tsx benchmark/gpu/load.mjs             # N=2000
//   BENCH_N=20000 node --import tsx benchmark/gpu/load.mjs
//
...
// v3's default layout is **grid**, so a plain `cytoscape( { elements } )`
...
const columnar = cytoscapeGpu.toColumnarElements( elements );
const wire = cytoscapeGpu.serializeElements( columnar );
...
if( has( 'init' ) ){
  group( 'load: init from the definition form (construct + dispose)', () => {
    summary( () => {
      bench( 'v3', () => {
        const cy = cytoscape( { elements: clone( elements ), ...V3_OPTS } );

        cy.destroy();
      } );
      bench( 'gpu', () => {
        const cy = cytoscapeGpu( { elements: clone( elements ) } );
...
if( has( 'forms' ) ){
  group( 'load: v4 ingest forms (construct + dispose)', () => {
    summary( () => {
      ...
      bench( 'columnar form', () => {
        const cy = cytoscapeGpu( { elements: columnar } );

        cy.destroy();
      } );
      bench( 'wire buffer', () => {
        const cy = cytoscapeGpu( { elements: wire } );
...
if( has( 'convert' ) ){
  group( 'load: conversion', () => {
    summary( () => {
      bench( 'toColumnarElements', () => do_not_optimize( cytoscapeGpu.toColumnarElements( elements ) ) );
      bench( 'serializeElements (from columnar)', () => do_not_optimize( cytoscapeGpu.serializeElements( columnar ) ) );
      bench( 'serializeElements (from defs)', () => do_not_optimize( cytoscapeGpu.serializeElements( elements ) ) );
      bench( 'deserializeElements', () => do_not_optimize( cytoscapeGpu.deserializeElements( wire ) ) );
    } );
  } );
}
...
{
  const gpu = cytoscapeGpu( { elements: clone( elements ) } );
  const v3 = cytoscape( { elements: clone( elements ), ...V3_OPTS } );
  ...
  if( has( 'add' ) ){
    ...
    const bandColumnar = cytoscapeGpu.toColumnarElements( band );
    const bandWire = cytoscapeGpu.serializeElements( bandColumnar );
