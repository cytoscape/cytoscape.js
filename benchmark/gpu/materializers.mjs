...
cmp( 'sweep: all nodes ($("node") vs filter({group}))',
  c => c.$( 'node' ), c => c.filter( { group: 'nodes' } ) );
cmp( 'sweep: selected ($(":selected") vs filter({selected}))',
  c => c.$( ':selected' ), c => c.filter( { selected: true } ) );
cmp( 'sweep: selected nodes ($("node:selected") vs filter({group, selected}))',
  c => c.$( 'node:selected' ), c => c.filter( { group: 'nodes', selected: true } ) );
cmp( 'sweep: nodes(":selected") vs nodes({selected})',
