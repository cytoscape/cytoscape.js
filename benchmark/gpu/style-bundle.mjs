...
through `build/cytoscape-gpu.esm.mjs` against the 2.0 µs the tsx path
...
older than `src/`.
  ...
  node benchmark/gpu/style-bundle.mjs
  node --import tsx benchmark/gpu/style-bundle.mjs   # identical; see below
...
const GPU_BUNDLE = join( ROOT, 'build/cytoscape-gpu.esm.mjs' );
const V3_BUNDLE = join( ROOT, 'build/cytoscape.esm.mjs' );
...
const newestSrc = ( dir, newest = 0 ) => {
  for( const entry of readdirSync( dir, { withFileTypes: true } ) ){
    ...
    if( entry.isDirectory() ){ newest = newestSrc( full, newest ); }
...
const { default: cytoscapeGpu } = await import( GPU_BUNDLE );
...
for( let i = 0; i < N; i++ ){
  elements.push( { data: { id: 'n' + i, w: i % 50 }, position: { x: i % 100 * 10, y: ( i / 100 | 0 ) * 10 } } );
}
...
const gpu = cytoscapeGpu( { elements } );

gpu.style( {
  nodes: {
    'background-color': '#c0392b', 'border-width': 3, 'border-color': '#2c3e50',
    'width': 40, 'height': 40, 'shape': 'round-rectangle', 'label': 'x',
    'text-wrap': 'wrap', 'text-max-width': 80
  },
  edges: {
    'line-color': '#7f8c8d', 'width': 2, 'curve-style': 'taxi',
    'taxi-radius': 8, 'target-distance-from-node': 4,
    'target-arrow-shape': 'triangle', 'target-arrow-color': '#333'
  }
} );
...
const POOL = 8;
const gpuNodes = Array.from( { length: POOL }, ( _, i ) => gpu.$id( 'n' + ( i * 7 + 3 ) ) );
const gpuEdges = Array.from( { length: POOL }, ( _, i ) => gpu.$id( 'e' + ( i * 7 + 3 ) ) );
const v3Nodes = Array.from( { length: POOL }, ( _, i ) => v3.getElementById( 'n' + ( i * 7 + 3 ) ) );
const v3Edges = Array.from( { length: POOL }, ( _, i ) => v3.getElementById( 'e' + ( i * 7 + 3 ) ) );
...
for( const [ label, pos, groupName, prop ] of PROPS ){
  ...
  summary( () => {
    group( `style( '${label}' )  (was ${pos} in the switch)`, () => {
      bench( 'v3', () => { do_not_optimize( v3Pool[ i++ & ( POOL - 1 ) ].style( prop ) ); } );
      bench( 'gpu', () => { do_not_optimize( gpuPool[ i++ & ( POOL - 1 ) ].style( prop ) ); } );
    } );
  } );
}
...
group( 'whole-object style() (gpu-only: v3 returns a different shape)', () => {
  bench( 'gpu node', () => { do_not_optimize( gpuNodes[ i++ & ( POOL - 1 ) ].style() ); } );
  bench( 'gpu edge', () => { do_not_optimize( gpuEdges[ i++ & ( POOL - 1 ) ].style() ); } );
} );
