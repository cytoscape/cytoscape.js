...
//
//   node --import tsx benchmark/gpu/surface.mjs           # N=2000
//   BENCH_OP=viewport node --import tsx benchmark/gpu/surface.mjs
//
...
const v3c = makeV3( compoundEles, { styleEnabled: true, layout: { name: 'preset' } } );
...
const ctx = {
  v3: { cy: v3, node: v3.getElementById( 'n' + MIDNUM ), edge: v3.edges()[ 0 ],
    nodes: v3.nodes(), eles: v3.elements(), band: v3.nodes().slice( 0, 100 ),
    other: v3.nodes().slice( 50, 150 ), c: v3c, parent: v3c.getElementById( 'p' ),
    child: v3c.getElementById( 'c1' ), curveEdge: null },
  gpu: { cy: gpu, node: gpu.$id( 'n' + MIDNUM ), edge: gpu.edges()[ 0 ],
    nodes: gpu.nodes(), eles: gpu.elements(), band: gpu.nodes().slice( 0, 100 ),
    other: gpu.nodes().slice( 50, 150 ), c: gpuc, parent: gpuc.$id( 'p' ),
    child: gpuc.$id( 'c1' ), curveEdge: gpuCurve.$id( 'e1' ) }
};
...
cmp( 'viewport', 'zoom( n ) set', c => { c.cy.zoom( 1 + ( c.i & 1 ) * 0.5 ); } );
cmp( 'viewport', 'pan() get', c => c.cy.pan() );
cmp( 'viewport', 'pan( p ) set', c => { c.cy.pan( { x: c.i & 7, y: 0 } ); } );
cmp( 'viewport', 'panBy()', c => { c.cy.panBy( { x: ( c.i & 1 ) ? 1 : -1, y: 0 } ); } );
...
cmp( 'viewport', 'minZoom/maxZoom get (2 calls)', c => c.cy.minZoom() + c.cy.maxZoom() );
cmp( 'viewport', 'center( eles )', c => { c.cy.center( c.band ); } );
cmp( 'viewport', 'reset()', c => { c.cy.reset(); } );
cmp( 'viewport', 'viewport({ zoom, pan })', c => { c.cy.viewport( { zoom: 1, pan: { x: 0, y: 0 } } ); } );
only( 'viewport', 'getCenterPan( eles )', c => c.cy.getCenterPan( c.band ) );

// core introspection + gating -------------------------------------------------
cmp( 'core', 'instanceString + headless + styleEnabled (3 calls)', c =>
  String( c.cy.instanceString() ) + c.cy.headless() + c.cy.styleEnabled() );
...
cmp( 'core', 'gating getters (5 calls)', c =>
  ...
  + c.cy.userZoomingEnabled() + c.cy.boxSelectionEnabled() );
cmp( 'core', 'gating setters round-trip (autolock/autoungrabify/autounselectify)', c => {
  c.cy.autolock( true ); c.cy.autolock( false );
  c.cy.autoungrabify( true ); c.cy.autoungrabify( false );
  c.cy.autounselectify( true ); c.cy.autounselectify( false );
} );
only( 'core', 'interaction tuning getters (5 calls)', c =>
  c.cy.wheelSensitivity() + c.cy.desktopTapThreshold() + c.cy.touchTapThreshold()
  + c.cy.tapholdDuration() + c.cy.multiClickDebounceTime() );
...
cmp( 'core', 'graph data() set + get (v3 restyles the graph)', c => { c.cy.data( 'k', c.i ); return c.cy.data( 'k' ); } );
cmp( 'core', 'graph scratch() set + get (v3 restyles the graph)', c => { c.cy.scratch( 'k', c.i ); return c.cy.scratch( 'k' ); } );
cmp( 'core', 'batch( fn ) (empty)', c => { c.cy.batch( () => {} ); } );
...
only( 'core', 'compact() (no dead slots — the early-out)', c => { c.cy.compact(); } );

// collection: identity + iteration --------------------------------------------
cmp( 'collection', 'element accessors (id/group/isNode/isEdge, 4 calls)', c =>
  c.node.id() + c.node.group() + c.node.isNode() + c.node.isEdge() );
cmp( 'collection', 'length + empty + nonempty (3 calls)', c =>
  c.nodes.length + c.nodes.empty() + c.nodes.nonempty() );
cmp( 'collection', 'eq/first/last (3 calls)', c =>
  ( c.nodes.eq( 5 ).length + c.nodes.first().length + c.nodes.last().length ) );
cmp( 'collection', 'slice()', c => c.nodes.slice( 10, 110 ) );
cmp( 'collection', 'toArray()', c => c.nodes.toArray() );
cmp( 'collection', 'sort( fn )', c => c.band.sort( ( a, b ) => a.id() < b.id() ? -1 : 1 ) );
cmp( 'collection', 'reduce( fn )', c => c.band.reduce( acc => acc + 1, 0 ) );
cmp( 'collection', 'min + max ( fn ) (2 calls)', c =>
  c.band.min( n => n.data( 'foo' ) ).value + c.band.max( n => n.data( 'foo' ) ).value );
cmp( 'collection', 'some + every (2 calls)', c =>
...
cmp( 'collection', 'symmetricDifference', c => c.band.symmetricDifference( c.other ) );
...
cmp( 'traversal', 'openNeighborhood + closedNeighborhood (2 calls)', c =>
  c.node.openNeighborhood().length + c.node.closedNeighborhood().length );
cmp( 'traversal', 'connectedNodes', c => c.edge.connectedNodes() );
cmp( 'traversal', 'sources + targets (2 calls)', c =>
  c.edge.sources().length + c.edge.targets().length );
cmp( 'traversal', 'source() + target() (2 calls)', c =>
  c.edge.source().length + c.edge.target().length );
cmp( 'traversal', 'edgesWith', c => c.band.edgesWith( c.other ) );
cmp( 'traversal', 'edgesTo', c => c.band.edgesTo( c.other ) );
cmp( 'traversal', 'parallelEdges + codirectedEdges (2 calls)', c =>
  c.edge.parallelEdges().length + c.edge.codirectedEdges().length );
cmp( 'traversal', 'roots + leaves (2 calls)', c => c.eles.roots().length + c.eles.leaves().length );
cmp( 'traversal', 'allAreNeighbors', c => c.band.allAreNeighbors( c.other ) );
cmp( 'traversal', 'isLoop + isSimple (2 calls)', c => c.edge.isLoop() + c.edge.isSimple() );

// collection: degree ----------------------------------------------------------
cmp( 'degree', 'indegree + outdegree (2 calls)', c => c.node.indegree() + c.node.outdegree() );
cmp( 'degree', 'minDegree + maxDegree (2 calls)', c =>
  c.nodes.minDegree( false ) + c.nodes.maxDegree( false ) );
cmp( 'degree', 'minIndegree + maxOutdegree (2 calls)', c =>
  c.nodes.minIndegree( false ) + c.nodes.maxOutdegree( false ) );
...
cmp( 'dimensions', 'width + height + outerWidth + outerHeight (4 calls)', c =>
  c.node.width() + c.node.height() + c.node.outerWidth() + c.node.outerHeight() );
cmp( 'dimensions', 'rendered dims (4 calls)', c =>
  c.node.renderedWidth() + c.node.renderedHeight()
  + c.node.renderedOuterWidth() + c.node.renderedOuterHeight() );
cmp( 'dimensions', 'renderedBoundingBox', c => c.node.renderedBoundingBox() );
cmp( 'dimensions', 'midpoint + renderedMidpoint (2 calls)', c =>
  c.edge.midpoint().x + c.edge.renderedMidpoint().x );
cmp( 'dimensions', 'source/targetEndpoint (2 calls)', c =>
  c.edge.sourceEndpoint().x + c.edge.targetEndpoint().x );
cmp( 'dimensions', 'rendered source/targetEndpoint (2 calls)', c =>
  c.edge.renderedSourceEndpoint().x + c.edge.renderedTargetEndpoint().x );
cmp( 'dimensions', 'shift + silentShift (round-trip)', c => {
  c.band.shift( { x: 1, y: 0 } );
  c.band.silentShift( { x: -1, y: 0 } );
} );
cmp( 'dimensions', 'silentPosition', c => { c.node.silentPosition( { x: c.i & 7, y: 0 } ); } );
cmp( 'dimensions', 'positions( fn ) over 100', c => { c.band.positions( ( n, k ) => ( { x: k, y: 0 } ) ); } );
cmp( 'dimensions', 'padding + paddedWidth + paddedHeight (3 calls)', c =>
  c.parent.padding() + c.parent.paddedWidth() + c.parent.paddedHeight() );

// collection: curve accessors (v4-only shapes) --------------------------------
only( 'curves', 'isBundledBezier', c => c.curveEdge.isBundledBezier() );
only( 'curves', 'controlPoints + renderedControlPoints (2 calls)', c =>
  ( c.curveEdge.controlPoints() ?? [] ).length + ( c.curveEdge.renderedControlPoints() ?? [] ).length );
only( 'curves', 'segmentPoints (undefined for bezier)', c => c.curveEdge.segmentPoints() );

// collection: compound --------------------------------------------------------
cmp( 'compound', 'parent + parents + ancestors (3 calls)', c =>
  c.child.parent().length + c.child.parents().length + c.child.ancestors().length );
cmp( 'compound', 'children + descendants + siblings (3 calls)', c =>
  c.parent.children().length + c.parent.descendants().length + c.child.siblings().length );
cmp( 'compound', 'orphans + nonorphans (2 calls)', c =>
  c.c.elements().orphans().length + c.c.elements().nonorphans().length );
cmp( 'compound', 'commonAncestors', c => c.c.nodes().commonAncestors() );
cmp( 'compound', 'isParent/isChild/isChildless/isOrphan (4 calls)', c =>
  c.parent.isParent() + c.child.isChild() + c.child.isChildless() + c.parent.isOrphan() );

// collection: flags -----------------------------------------------------------
cmp( 'flags', 'select + unselect + selected (band)', c => {
  c.band.select(); const s = c.band.selected(); c.band.unselect(); return s;
} );
cmp( 'flags', 'selectify + unselectify + selectable', c => {
  c.band.unselectify(); const s = c.band.selectable(); c.band.selectify(); return s;
} );
cmp( 'flags', 'lock + unlock + locked', c => {
  c.band.lock(); const s = c.band.locked(); c.band.unlock(); return s;
} );
cmp( 'flags', 'grabify + ungrabify + grabbable', c => {
  c.band.ungrabify(); const s = c.band.grabbable(); c.band.grabify(); return s;
} );
cmp( 'flags', 'show + hide + visible', c => {
  c.band.hide(); const s = c.band.visible(); c.band.show(); return s;
} );
cmp( 'flags', 'active/activate/unactivate (3 calls)', c => {
  c.band.activate(); const s = c.band.active(); c.band.unactivate(); return s;
} );
cmp( 'flags', 'panify + unpanify + pannable', c => {
  c.band.panify(); const s = c.band.pannable(); c.band.unpanify(); return s;
} );
cmp( 'flags', 'takesUpSpace + interactive + transparent (3 calls)', c =>
  c.node.takesUpSpace() + c.node.interactive() + c.node.transparent() );
cmp( 'flags', 'effectiveOpacity', c => c.node.effectiveOpacity() );

// collection: data / json / scratch -------------------------------------------
cmp( 'element data', 'scratch get + set', c => { c.node.scratch( 'k', c.i ); return c.node.scratch( 'k' ); } );
...
for( const row of ROWS ){
  ...
  for( const [ side, c ] of sides ){
    ...
    try {
      ...
      skipped.push( `${row.section}: ${row.label} [${side}] — ${err.message.slice( 0, 80 )}` );
    }
  }
}
...
for( const row of ROWS ){
  if( OP != null && !row.section.includes( OP ) && !row.label.includes( OP ) ){ continue; }

  const pair = row.v3 != null && row.runnable.v3 && row.runnable.gpu;

  if( !row.runnable.gpu ){ continue; }

  group( `${row.section}: ${row.label}`, () => {
    const body = () => {
      if( pair ){
        bench( 'v3', () => { ctx.v3.i++; return do_not_optimize( row.v3( ctx.v3 ) ); } );
      }

      bench( 'gpu', () => { ctx.gpu.i++; return do_not_optimize( row.gpu( ctx.gpu ) ); } );
    };

    if( pair ){ summary( body ); } else { body(); }
  } );
}
