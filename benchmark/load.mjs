...
if (has('forms')) {
  group('load: v4 ingest forms (construct + dispose)', () => {
    summary(() => {
      ...
      bench('def clone alone (control)', () =>
        do_not_optimize(clone(elements)));
    });
  });
}
...
if (has('convert')) {
  group('load: conversion', () => {
    summary(() => {
      bench('toColumnarElements', () =>
        do_not_optimize(cytoscape.toColumnarElements(elements)));
      bench('serializeElements (from columnar)', () =>
        do_not_optimize(cytoscape.serializeElements(columnar)));
      bench('serializeElements (from defs)', () =>
        do_not_optimize(cytoscape.serializeElements(elements)));
      bench('deserializeElements', () =>
        do_not_optimize(cytoscape.deserializeElements(wire)));
    });
  });
}
...
// -- export from a live graph -------------------------------------------------
{
  ...
  if (has('add')) {
    ...
    group(`load: add a ${BAND}-node band (add + remove)`, () => {
      summary(() => {
        bench('v3', () => {
          v3.add(clone(band)).remove();
        });
        bench('gpu', () => {
          gpu.add(clone(band)).remove();
        });
      });
    });
    ...
    group(`load: add a ${BAND}-node band, v4 forms`, () => {
      summary(() => {
        ...
        bench('columnar form', () => {
          gpu.add(bandColumnar).remove();
        });
        bench('wire buffer', () => {
          gpu.add(bandWire).remove();
        });
      });
    });
  }
