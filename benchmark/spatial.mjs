...
//
//   node --import tsx benchmark/spatial.mjs           # N=2000
//   BENCH_N=200000 BENCH_OP=pick node --import tsx benchmark/spatial.mjs
//
...
if (has('pick')) {
  const SHAPES = [
    ...
    [
      'polygon',
      { 'shape-polygon-points': [-1, -1, 1, -1, 1, 1, 0, 0.4, -1, 1] },
    ], // 13 C3 blob walk
  ];
  ...
  // them below.
  {
    ...
    group('pick: hit (the walk; one inside-test)', () => {
      summary(() => {
        bench('zoom 1', () =>
          do_not_optimize(pickNodeAt(store, frameAt(1), hit.x, hit.y)));
        bench('zoom 2', () =>
          do_not_optimize(pickNodeAt(store, frameAt(2), hit.x * 2, hit.y * 2)));
      });
    });
  }
  ...
  // itself the reason it is the cheap case.
  {
    const coincident = (shape, extra) => {
      ...
      for (let i = 0; i < N; i++) {
        eles.push({ data: { id: 'c' + i }, position: { x: 0, y: 0 } });
      }

      const cy = cytoscape({ elements: eles });
    ...
    for (const [shape, extra] of SHAPES) {
      ...
      if (pickNodeAt(store, frameAt(1), x, y) != null) {
        console.warn(
          `  !! ${shape}: the pick point is INSIDE the shape — that row would measure one test, not ${N}`,
        );
      }
    ...
    group(`pick: shape inside-test, ${N} coincident misses`, () => {
      summary(() => {
        for (const [shape] of SHAPES) {
          ...
          bench(shape, () =>
            do_not_optimize(pickNodeAt(store, frameAt(1), x, y)),
          );
        }
      });
    });
    ...
    // the one whose cost can move with the zoom
    {
      ...
      group(
        'pick: round-hexagon inside-test by zoom (not affine-invariant)',
        () => {
          summary(() => {
            bench('zoom 1', () =>
              do_not_optimize(
                pickNodeAt(store, frameAt(1), corner[0], corner[1]),
              ));
            bench('zoom 2', () =>
              do_not_optimize(
                pickNodeAt(store, frameAt(2), corner[0] * 2, corner[1] * 2),
              ));
          });
        },
      );
  ...
  // the one compaction.mjs measures before/after.
  {
    ...
    group('pick: background miss (full highWater walk)', () => {
      bench('gpu', () =>
        do_not_optimize(pickNodeAt(store, frameAt(1), 1e7, 0)));
    });
  }
  ...
  // 20.3), so a labelled graph pays a second box test per candidate
  {
    ...
    group('pick: label box in the scan (text-events)', () => {
      summary(() => {
        bench('text-events: no', () =>
          do_not_optimize(pickNodeAt(plain._store, frameAt(1), 1e7, 0)));
        bench('text-events: yes', () =>
          do_not_optimize(pickNodeAt(labelled._store, frameAt(1), 1e7, 0)));
      });
    });
  }
}
...
if (has('box')) {
  ...
  // that way measures a degenerate call on both sides.  benchmark/
  ...
  withLabels.style({
    nodes: { width: 30, height: 30, label: { data: 'foo' } },
  });
  ...
  for (const [inst, mode] of [
    [curvedContain, 'contain'],
    [curvedOverlap, 'overlap'],
  ]) {
    inst.style({
  ...
  const reallyCurved = curvedOverlap
    .edges()
  ...
  group('box: overlap vs contain (straight edges)', () => {
    summary(() => {
      bench('contain', () =>
        do_not_optimize(cy._elementsInGestureBox(...halfBox)));
      bench('overlap', () =>
        do_not_optimize(overlapCy._elementsInGestureBox(...halfBox)));
    });
  });
  ...
  group('box: overlap vs contain (curved edges — the segment walk)', () => {
    summary(() => {
      bench('contain', () =>
        do_not_optimize(curvedContain._elementsInGestureBox(...halfBox)));
      bench('overlap', () =>
        do_not_optimize(curvedOverlap._elementsInGestureBox(...halfBox)));
    });
  });
}
...
if (has('bounds')) {
  ...
  v3.style([
    { selector: 'node', style: { width: 30, height: 30, label: 'data(foo)' } },
  ]);
  ...
  group('bounds: boundingBox, labels in vs out (v4)', () => {
    summary(() => {
      bench('includeLabels: true (default)', () =>
        do_not_optimize(gpuEles.boundingBox()));
      bench('includeLabels: false', () =>
        do_not_optimize(gpuEles.boundingBox({ includeLabels: false })));
    });
  });
  ...
  group('bounds: boundingBoxAt (v4 only)', () => {
    bench('gpu', () =>
      do_not_optimize(gpuEles.boundingBoxAt(() => ({ x: 1, y: 2 }))));
  });
