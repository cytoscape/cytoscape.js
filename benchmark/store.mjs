...
// none of them.
//
//   node --import tsx benchmark/store.mjs             # N=2000
//   BENCH_N=200000 BENCH_OP=id node --import tsx benchmark/store.mjs
//
...
if (has('id')) {
  ...
  group('id index: single-key operations', () => {
    summary(() => {
      ...
      bench('idAt (memoized hit)', () =>
        do_not_optimize(map.idAt('nodes', i++ % N)));
  ...
  group(
    'id index: remove + re-set (churn, drives the round-11 reclaim)',
    () => {
      bench('gpu', () => {
        const k = i++ % N;
...
if (has('adjacency')) {
  ...
  group('adjacency: reads', () => {
    summary(() => {
      ...
      bench('connectedEdges', () =>
        do_not_optimize(adj.connectedEdges(i++ % N)));
    });
  });
...
if (has('blob')) {
  ...
  group('blob: single-record operations', () => {
    summary(() => {
      bench('rewrite in place (same length)', () =>
        do_not_optimize(blob.write(i++ % N, record)));
...
if (has('dirty')) {
  ...
  group('dirty: mark (per-write span coalescing)', () => {
    summary(() => {
      bench('contiguous marks', () => {
        tracker.mark('node.position', i++ % N);
      });
      bench('scattered marks', () => {
        tracker.mark('node.position', (i++ * 977) % N);
      });
    });
  });
  ...
  group("dirty: take (a frame's drain)", () => {
    bench('gpu', () => {
      for (let k = 0; k < 64; k++) {
        tracker.mark('node.position', k * 7);
      }
...
if (has('image')) {
  const urls = Array.from(
    { length: 64 },
    (_, k) => `https://example.invalid/icon-${k}.svg`,
  );
    ...
    summary(() => {
      ...
      bench('acquire (new url) + release', () => {
        const id = registry.acquire(
          `https://example.invalid/fresh-${i++}.svg`,
          0,
        );
  ...
  group('charts: sheet apply, chart vs no chart', () => {
    summary(() => {
      bench('no chart', () => {
        cy.style({
          ...PLAIN,
          nodes: { ...PLAIN.nodes, width: 30 + (i++ & 1) },
        });
      });
      bench('pie from data (blob record per node)', () => {
        cy.style({
          ...CHART,
          nodes: { ...CHART.nodes, width: 30 + (i++ & 1) },
        });
      });
    });
  });
