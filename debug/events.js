...
(function () {
  ...
  function append(type, target) {
    ...
    const id =
      target != null && typeof target.id === 'function' ? target.id() : 'core';
  ...
  function flush() {
    ...
    while (el.childElementCount > MAX_ROWS) {
  ...
  window.onCy((cy) => {
    for (const [family, types] of Object.entries(FAMILIES)) {
      for (const type of types) {
        ...
        cy.on(type, (e) => {
          ...
          if (NOISY.has(type) && !$('#event-noisy-check').checked) {
            return;
          }
    ...
    cy.on(
      ...
      (e) => {
        if (enabled.has('tap')) {
          append('tap (predicate: isNode)', e.target);
        }
      },
    );
  });
