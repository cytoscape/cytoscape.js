...
(function(){
  ...
  const boolControl = ( sel, get, set ) => {
    ...
    window.onCy(cy => { el.checked = get(cy); });

    el.addEventListener('change', () => {
      const cy = window.cy;

      if(cy != null) { set(cy, el.checked); }
    });
  };

  boolControl('#panning-check', cy => cy.panningEnabled(), (cy, v) => cy.panningEnabled(v));
  boolControl('#user-panning-check', cy => cy.userPanningEnabled(), (cy, v) => cy.userPanningEnabled(v));
  boolControl('#zooming-check', cy => cy.zoomingEnabled(), (cy, v) => cy.zoomingEnabled(v));
  boolControl('#user-zooming-check', cy => cy.userZoomingEnabled(), (cy, v) => cy.userZoomingEnabled(v));
  boolControl('#box-selection-check', cy => cy.boxSelectionEnabled(), (cy, v) => cy.boxSelectionEnabled(v));
  boolControl('#autolock-check', cy => cy.autolock(), (cy, v) => cy.autolock(v));
  boolControl('#autoungrabify-check', cy => cy.autoungrabify(), (cy, v) => cy.autoungrabify(v));
  boolControl('#autounselectify-check', cy => cy.autounselectify(), (cy, v) => cy.autounselectify(v));
  // round 16.5 / 39.1: both v4 inventions, so v3's page has no equivalent
  boolControl('#box-labels-check', cy => cy.boxSelectionIncludesLabels(), (cy, v) => cy.boxSelectionIncludesLabels(v));
  ...
  const selectControl = ( sel, get, set ) => {
    ...
    window.onCy(cy => { el.value = String(get(cy)); });

    el.addEventListener('change', () => {
      const cy = window.cy;

      if(cy != null) { set(cy, el.value); }
    });
  };

  selectControl('#selection-type-select', cy => cy.selectionType(), (cy, v) => cy.selectionType(v));
  selectControl('#box-mode-select', cy => cy.boxSelectionMode(), (cy, v) => cy.boxSelectionMode(v));
  ...
  const numberControl = ( sel, get, set ) => {
    ...
    el.addEventListener('change', () => {
      ...
      if(cy != null) { set(cy, parseFloat(el.value)); }
    });
  };

  numberControl('#wheel-sensitivity-input', cy => cy.wheelSensitivity(), (cy, v) => cy.wheelSensitivity(v));
