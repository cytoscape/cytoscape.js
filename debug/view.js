...
(function(){
  ...
  on('#zoom-in-button', cy => cy.zoom({ level: cy.zoom() * 1.25, renderedPosition: centre(cy) }));
  on('#zoom-out-button', cy => cy.zoom({ level: cy.zoom() / 1.25, renderedPosition: centre(cy) }));
  ...
  on('#animate-fit-button', cy => cy.animate({ fit: { eles: scope(cy), padding: 30 } }, { duration: 600 }));
  on('#animate-center-button', cy => cy.animate({ center: { eles: scope(cy) } }, { duration: 600 }));
  on('#animate-panby-button', cy => cy.animate({ panBy: { x: 150, y: 0 } }, { duration: 600 }));
  ...
  on('#hide-bb-button', () => { $('#bb').style.display = 'none'; });
