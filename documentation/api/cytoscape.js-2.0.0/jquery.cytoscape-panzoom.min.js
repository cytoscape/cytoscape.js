...
if(!F){w()}R.zoom({level:S,renderedPosition:{x:M,y:K}})}var u;var z=function(R){R.stopPropagation();R.preventDefault();clearInterval(u);var S=y(R);if(isNaN(S.x)||isNaN(S.y)){h.hide();return}i(S);u=setInterval(function(){k.cytoscape("get").panBy(S)},c.panSpeed)};P.bind("mousedown",function(R){z(R);a(window).bind("mousemove",z)});P.bind("mouseup",function(){B()});a(window).bind("mouseup blur",function(){B()});p.bind("mousedown",function(){return false});var D;var n=false;var l=2;function I(ab,S){if(S===undefined){S=0
...
});a(window).bind("mouseup",function(){a(window).unbind("mousemove",O);n=false;G.removeClass("active");r()});return false});p.bind("mousedown",function(R){if(R.target!==G[0]){I(R);m(R)}});function C(){var S=k.cytoscape("get");var W=S.zoom();var U=c.minZoom;var aa=c.maxZoom;var Z=Math.log(U)/Math.log(aa);var R=Math.log(W)/Math.log(aa);var V=1-(R-Z)/(1-Z);var T=l;var Y=p.height()-G.height()-2*l;var X=V*(Y-T);if(X<T){X=T}if(X>Y){X=Y}G.css("top",X)}C();var s=k.cytoscape("get");s.on("zoom",function(){if(!n){C()
...
}if((W==c.maxZoom&&X==c.maxZoom)||(W==c.minZoom&&X==c.minZoom)){return}N(W)},c.zoomDelay);return false});a(window).bind("mouseup blur",function(){clearInterval(T);r()})}E(A,(1+c.zoomFactor));E(o,(1-c.zoomFactor));q.bind("mousedown",function(T){if(T.button!=0){return}var U=k.cytoscape("get");if(U.elements().size()===0){U.reset()}else{U.fit()}var S=Math.max(k.width(),k.height());var R=U.zoom()*(S-c.fitPadding*2)/S;U.zoom({level:R,renderedPosition:{x:k.width()/2,y:k.height()/2}});return false})})}};
if(e[d]){return e[d].apply(this,Array.prototype.slice.call(arguments,1))}else{if(typeof d=="object"||!d){return e.init.apply(this,arguments)}else{a.error("No such function `"+d+"` for jquery.cytoscapePanzoom")}}return a(this)};a.fn.cyPanzoom=a.fn.cytoscapePanzoom})(jQuery);