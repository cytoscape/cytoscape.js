...
if(E===D){i.splice(l,1);break}}}for(var w=0;w<C.length;w++){var j=C[w];j._private.removed=true;c.removeFromPool(j);A.push(j);if(j.isEdge()){var f=j.source()[0];var z=j.target()[0];m(f,j);m(z,j)}}var d={};for(var w=0;w<C.length;w++){var j=C[w];var t=j._private.group==="nodes";var v=j._private.parent;if(t&&v!==undefined&&!d[v]){d[v]=true;var g=c.getElementById(v);if(g&&g.length!==0&&!g._private.removed&&g.children().length===0){g.updateStyle()}}}var u=new b.Collection(this.cy(),A);if(u.size()>0){if(p){this.cy().notify({type:"remove",collection:u})
}u.trigger("remove")}return this}})(cytoscape);(function(a){a.fn.eles({animated:function(){var b=this[0];if(b){return b._private.animation.current.length>0}},clearQueue:function(){for(var b=0;b<this.length;b++){var c=this[b];c._private.animation.queue=[]}return this},delay:function(c,b){this.animate({delay:c},{duration:c,complete:b});return this},animate:function(j,e){var c=+new Date;var d=this._private.cy;var b=d.style();if(e===undefined){e={}}if(e.duration===undefined){e.duration=400}switch(e.duration){case"slow":e.duration=600;
...
})(cytoscape);(function(v){var u=("ontouchstart" in window)||window.DocumentTouch&&document instanceof DocumentTouch;var l=function(){return Date.now()};var b={};var o={};var c=w.prototype;var e=5,B=0,d=2,E=4,a=2;function w(F){this.data={select:[undefined,undefined,undefined,undefined,0],renderer:this,cy:F.cy,container:F.cy.container(),canvases:new Array(e),canvasRedrawReason:new Array(e),canvasNeedsRedraw:new Array(e),bufferCanvases:new Array(a)};this.hoverData={down:null,last:null,downTime:null,triggerMode:null,dragging:false,initialPan:[null,null],capture:false};
this.timeoutData={panTimeout:null};this.dragData={possibleDragElements:[]};this.touchData={start:null,capture:false,startPosition:[null,null,null,null,null,null],singleTouchStartTime:null,singleTouchMoved:true,now:[null,null,null,null,null,null],earlier:[null,null,null,null,null,null]};this.zoomData={freeToZoom:false,lastPointerX:null};this.redraws=0;this.init();for(var G=0;G<e;G++){this.data.canvases[G]=document.createElement("canvas");this.data.canvases[G].style.position="absolute";this.data.canvases[G].setAttribute("data-id","layer"+G);
this.data.canvases[G].style.zIndex=String(e-G);this.data.container.appendChild(this.data.canvases[G]);this.data.canvasRedrawReason[G]=new Array();this.data.canvasNeedsRedraw[G]=false}this.data.canvases[E].setAttribute("data-id","layer"+E+"-node");this.data.canvases[B].setAttribute("data-id","layer"+B+"-selectbox");this.data.canvases[d].setAttribute("data-id","layer"+d+"-drag");for(var G=0;G<a;G++){this.data.bufferCanvases[G]=document.createElement("canvas");this.data.bufferCanvases[G].style.position="absolute";
this.data.bufferCanvases[G].setAttribute("data-id","buffer"+G);this.data.bufferCanvases[G].style.zIndex=String(-G-1);this.data.bufferCanvases[G].style.visibility="visible";this.data.container.appendChild(this.data.bufferCanvases[G])}}w.prototype.notify=function(i){if(i.type=="add"||i.type=="remove"||i.type=="load"){this.updateNodesCache();this.updateEdgesCache()}if(i.type=="load"){this.load()}if(i.type=="viewport"){this.data.canvasNeedsRedraw[B]=true;this.data.canvasRedrawReason[B].push("viewchange")
}this.data.canvasNeedsRedraw[d]=true;this.data.canvasRedrawReason[d].push("notify");this.data.canvasNeedsRedraw[E]=true;this.data.canvasRedrawReason[E].push("notify");this.redraws++;this.redraw()};w.prototype.png=function(){var F=this.data;if(this.data.container.clientHeight>0&&this.data.container.clientWidth>0){context=F.bufferCanvases[1].getContext("2d");context.globalCompositeOperation="copy";context.drawImage(F.canvases[4],0,0);context.globalCompositeOperation="source-over";context.drawImage(F.canvases[2],0,0);
context.drawImage(F.canvases[0],0,0);context=F.bufferCanvases[0].getContext("2d");context.globalCompositeOperation="copy";context.drawImage(F.bufferCanvases[1],0,0)}var i=this.data.bufferCanvases[0];return i.toDataURL("image/png")};w.prototype.load=function(){var P=this;var N=function(aa,W,ab){if(!W){var Y=aa.parents();for(var Z=0;Z<Y.size();Z++){if(Y[Z]._private.selected){return}}}var ac=aa.descendants();for(var Z=0;Z<ac.size();Z++){if(W||!ac[Z]._private.selected){ac[Z]._private.rscratch.inDragLayer=true;
ab.push(ac[Z]);for(var X=0;X<ac[Z]._private.edges.length;X++){ac[Z]._private.edges[X]._private.rscratch.inDragLayer=true}}}};var Q=function(X,Y){X._private.grabbed=true;X._private.rscratch.inDragLayer=true;Y.push(X);for(var W=0;W<X._private.edges.length;W++){X._private.edges[W]._private.rscratch.inDragLayer=true}};var U=function(ab,Z){var aa=ab;while(aa.parent().nonempty()){aa=aa.parent()[0]}if(aa==ab&&Z){return}var W=aa.descendants().add(aa);for(var Y=0;Y<W.size();Y++){W[Y]._private.rscratch.inDragLayer=Z;
for(var X=0;X<W[Y]._private.edges.length;X++){W[Y]._private.edges[X]._private.rscratch.inDragLayer=Z}}};w.prototype.nodeIsDraggable=function(W){if(W._private.style.opacity.value!=0&&W._private.style.visibility.value=="visible"&&!W._private.locked&&W._private.grabbable){return true}return false};P.data.container.addEventListener("mousedown",function(Z){Z.preventDefault();P.hoverData.capture=true;var W=P.data.cy;var ac=P.projectIntoViewport(Z.pageX,Z.pageY);var ad=P.data.select;var Y=P.findNearestElement(ac[0],ac[1],true);
var af=P.hoverData.down;var aa=P.dragData.possibleDragElements;var ab=new v.Event(Z,{type:"grab"});if(Z.button==0){if(Y){Y.activate()}if(Y!=null&&P.nodeIsDraggable(Y)){if(Y._private.group=="nodes"&&Y._private.selected==false){aa=P.dragData.possibleDragElements=[];Q(Y,aa);Y.trigger(ab);if(Y._private.style.width.value=="auto"||Y._private.style.height.value=="auto"){N(Y,true,aa)}U(Y,true)}if(Y._private.group=="nodes"&&Y._private.selected==true){aa=P.dragData.possibleDragElements=[];var ag=false;var ae=W.$("node:selected");
for(var X=0;X<ae.length;X++){if(P.nodeIsDraggable(ae[X])){Q(ae[X],aa);if(!ag){Y.trigger(ab);ag=true}if(ae[X]._private.style.width.value=="auto"||ae[X]._private.style.height.value=="auto"){N(ae[X],false,aa)}U(ae[X],true)}}}Y.trigger(new v.Event(Z,{type:"mousedown"})).trigger(new v.Event(Z,{type:"tapstart"})).trigger(new v.Event(Z,{type:"vmousedown"}))}else{if(Y==null){W.trigger(new v.Event(Z,{type:"mousedown"})).trigger(new v.Event(Z,{type:"tapstart"})).trigger(new v.Event(Z,{type:"vmousedown"}))}}P.hoverData.down=Y;
P.hoverData.downTime=(new Date()).getTime();if(Y==null||Y.isEdge()){ad[4]=1;var ah=Math.max(0,400-(+new Date-P.hoverData.downTime));clearTimeout(P.bgActiveTimeout);P.bgActiveTimeout=setTimeout(function(){if(Y){Y.unactivate()}P.data.bgActivePosistion={x:ac[0],y:ac[1]};P.data.canvasNeedsRedraw[B]=true;P.data.canvasRedrawReason[B].push("bgactive");P.redraw()},ah)}}ad[0]=ad[2]=ac[0];ad[1]=ad[3]=ac[1];P.redraw()},false);window.addEventListener("mousemove",function(am){var ac=false;var Y=P.data.cy;var ab=P.projectIntoViewport(am.pageX,am.pageY);
var ag=P.data.select;var an=P.findNearestElement(ab[0],ab[1],true);var ae=P.hoverData.last;var af=P.hoverData.down;var aj=[ab[0]-ag[2],ab[1]-ag[3]];var ah=P.getCachedNodes();var X=P.getCachedEdges();var aa=P.dragData.possibleDragElements;var ai=P.hoverData.capture;var Z=am.shiftKey;if(!ai){var ad=P.findContainerPageCoords();if(am.pageX>ad[0]&&am.pageX<ad[0]+P.data.container.clientWidth&&am.pageY>ad[1]&&am.pageY<ad[1]+P.data.container.clientHeight){}else{return}}ac=true;var ak=new v.Event(am,{type:"mousemove"});
if(an!=null){an.trigger(ak)}else{if(an==null){Y.trigger(ak)}}if(P.hoverData.dragging){ac=true;if(Y.panningEnabled()){var W={x:aj[0]*Y.zoom(),y:aj[1]*Y.zoom()};Y.panBy(W)}ab=P.projectIntoViewport(am.pageX,am.pageY)}else{if(ag[4]==1&&(af==null||af.isEdge())&&(!Y.boxSelectionEnabled()||(new Date()).getTime()-P.hoverData.downTime>400)&&(Math.abs(ag[3]-ag[1])+Math.abs(ag[2]-ag[0])<4)&&Y.panningEnabled()){P.hoverData.dragging=true;ag[4]=0}else{clearTimeout(P.bgActiveTimeout);if(af&&af.isEdge()&&af.active()){af.unactivate()
}if(an!=ae){if(ae){ae.trigger(new v.Event(am,{type:"mouseout"}))}if(an){an.trigger(new v.Event(am,{type:"mouseover"}))}P.hoverData.last=an}if(af&&af.isNode()&&P.nodeIsDraggable(af)){P.dragData.didDrag=true;var ao=[];for(var al=0;al<aa.length;al++){if(aa[al]._private.group=="nodes"&&P.nodeIsDraggable(aa[al])){aa[al]._private.position.x+=aj[0];aa[al]._private.position.y+=aj[1];ao.push(aa[al])}}(new v.Collection(Y,ao)).trigger(new v.Event(am,{type:"drag"})).trigger(new v.Event(am,{type:"position"}));
if(ag[2]==ag[0]&&ag[3]==ag[1]){P.data.canvasNeedsRedraw[E]=true;P.data.canvasRedrawReason[E].push("Node(s) and edge(s) moved to drag layer")}P.data.canvasNeedsRedraw[d]=true;P.data.canvasRedrawReason[d].push("Nodes dragged")}if(Y.boxSelectionEnabled()){P.data.canvasNeedsRedraw[B]=true;P.data.canvasRedrawReason[B].push("Mouse moved, redraw selection box")}ac=true}}ag[2]=ab[0];ag[3]=ab[1];P.redraw();if(ac){if(am.stopPropagation){am.stopPropagation()}if(am.preventDefault){am.preventDefault()}am.cancelBubble=true;
am.returnValue=false;return false}},false);window.addEventListener("mouseup",function(ao){var ai=P.hoverData.capture;if(!ai){return}P.hoverData.capture=false;var X=P.data.cy;var ab=P.projectIntoViewport(ao.pageX,ao.pageY);var ag=P.data.select;var ap=P.findNearestElement(ab[0],ab[1],true);var ah=P.getCachedNodes();var W=P.getCachedEdges();var aa=P.dragData.possibleDragElements;var ad=P.hoverData.down;var Z=ao.shiftKey;P.data.bgActivePosistion=undefined;clearTimeout(P.bgActiveTimeout);if(ad){ad.unactivate()
}if((ad==null)&&!P.dragData.didDrag&&!(Math.pow(ag[2]-ag[0],2)+Math.pow(ag[3]-ag[1],2)>7&&ag[4])&&!P.hoverData.dragging){X.$(":selected").unselect();if(aa.length>0){P.data.canvasNeedsRedraw[E]=true;P.data.canvasRedrawReason[E].push("De-select")}P.dragData.possibleDragElements=aa=[]}if(Math.pow(ag[2]-ag[0],2)+Math.pow(ag[3]-ag[1],2)==0){if(ap!=null){ap.trigger(new v.Event(ao,{type:"click"})).trigger(new v.Event(ao,{type:"tap"})).trigger(new v.Event(ao,{type:"vclick"}))}else{if(ap==null){X.trigger(new v.Event(ao,{type:"click"})).trigger(new v.Event(ao,{type:"tap"})).trigger(new v.Event(ao,{type:"vclick"}))
}}}if(ap!=null){ap.trigger(new v.Event(ao,{type:"mouseup"})).trigger(new v.Event(ao,{type:"tapend"})).trigger(new v.Event(ao,{type:"vmouseup"}))}else{if(ap==null){X.trigger(new v.Event(ao,{type:"mouseup"})).trigger(new v.Event(ao,{type:"tapend"})).trigger(new v.Event(ao,{type:"vmouseup"}))}}if(ap==ad&&!P.dragData.didDrag){if(ap!=null&&ap._private.selectable){if(!Z){X.$(":selected").unselect()}if(ap.selected()){ap.unselect()}else{ap.select()}U(ap,false);P.data.canvasNeedsRedraw[E]=true;P.data.canvasRedrawReason[E].push("sglslct")
}}else{if(ap==ad){if(ap!=null&&ap._private.grabbed){var aj=X.$(":grabbed");for(var am=0;am<aj.length;am++){var ae=aj[am];ae._private.grabbed=false;var af=ae._private.edges;for(var al=0;al<af.length;al++){af[al]._private.rscratch.inDragLayer=false}U(ae,false)}var an=new v.Event(ao,{type:"free"});aj.trigger(an)}}}if(X.boxSelectionEnabled()&&Math.pow(ag[2]-ag[0],2)+Math.pow(ag[3]-ag[1],2)>7&&ag[4]){if(!Z){X.$(":selected").unselect()}var Y=[];var ac=P.getAllInBox(ag[0],ag[1],ag[2],ag[3]);var ak=new v.Event(ao,{type:"select"});
for(var am=0;am<ac.length;am++){if(ac[am]._private.selectable){aa.push(ac[am]);Y.push(ac[am])}}(new v.Collection(X,Y)).select();if(ac.length>0){P.data.canvasNeedsRedraw[E]=true;P.data.canvasRedrawReason[E].push("Selection")}}P.hoverData.dragging=false;if(!ag[4]){var an=new v.Event(ao,{type:"free"});for(var am=0;am<aa.length;am++){if(aa[am]._private.group=="nodes"){aa[am]._private.rscratch.inDragLayer=false;var af=aa[am]._private.edges;for(var al=0;al<af.length;al++){af[al]._private.rscratch.inDragLayer=false
}U(aa[am],false)}else{if(aa[am]._private.group=="edges"){aa[am]._private.rscratch.inDragLayer=false}}}if(ad){ad.trigger(an)}P.data.canvasNeedsRedraw[d]=true;P.data.canvasRedrawReason[d].push("Node/nodes back from drag");P.data.canvasNeedsRedraw[E]=true;P.data.canvasRedrawReason[E].push("Node/nodes back from drag")}ag[4]=0;P.hoverData.down=null;P.data.canvasNeedsRedraw[B]=true;P.data.canvasRedrawReason[B].push("Mouse up, selection box gone");P.dragData.didDrag=false;P.redraw()},false);var K=function(Y){var aa=P.data.cy;
var Z=P.projectIntoViewport(Y.pageX,Y.pageY);var X=[Z[0]*aa.zoom()+aa.pan().x,Z[1]*aa.zoom()+aa.pan().y];if(P.zoomData.freeToZoom){Y.preventDefault();var W=Y.wheelDeltaY/1000||Y.detail/-8.4;if(aa.panningEnabled()&&aa.zoomingEnabled()){aa.zoom({level:aa.zoom()*Math.pow(10,W),position:{x:X[0],y:X[1]}})}}};P.data.container.addEventListener("mousewheel",K,true);P.data.container.addEventListener("DOMMouseScroll",K,true);P.data.container.addEventListener("MozMousePixelScroll",function(W){if(P.zoomData.freeToZoom){W.preventDefault()
}},false);P.data.container.addEventListener("mousemove",function(W){if(P.zoomData.lastPointerX&&P.zoomData.lastPointerX!=W.pageX&&!P.zoomData.freeToZoom){P.zoomData.freeToZoom=true}P.zoomData.lastPointerX=W.pageX},false);P.data.container.addEventListener("mouseout",function(W){P.zoomData.freeToZoom=false;P.zoomData.lastPointerX=null},false);P.data.container.addEventListener("mouseout",function(W){P.data.cy.trigger(new v.Event(W,{type:"mouseout"}))},false);P.data.container.addEventListener("mouseover",function(W){P.data.cy.trigger(new v.Event(W,{type:"mouseover"}))
},false);var F,L,R,V;var S;var I,H;var J,T;var M=P.data.container.clientWidth,O=P.data.container.clientHeight;var i;function G(X,Z,W,Y){return Math.sqrt((W-X)*(W-X)+(Y-Z)*(Y-Z))}P.data.container.addEventListener("touchstart",function(an){an.preventDefault();P.touchData.capture=true;P.data.bgActivePosistion=undefined;var Z=P.data.cy;var ah=P.getCachedNodes();var Y=P.getCachedEdges();var X=P.touchData.now;var al=P.touchData.earlier;if(an.touches[0]){var ab=P.projectIntoViewport(an.touches[0].pageX,an.touches[0].pageY);
X[0]=ab[0];X[1]=ab[1]}if(an.touches[1]){var ab=P.projectIntoViewport(an.touches[1].pageX,an.touches[1].pageY);X[2]=ab[0];X[3]=ab[1]}if(an.touches[2]){var ab=P.projectIntoViewport(an.touches[2].pageX,an.touches[2].pageY);X[4]=ab[0];X[5]=ab[1]}if(an.touches[1]){var aa=P.findContainerPageCoords();T=aa[1];J=aa[0];F=an.touches[0].pageX-J;L=an.touches[0].pageY-T;R=an.touches[1].pageX-J;V=an.touches[1].pageY-T;i=0<=F&&F<=M&&0<=R&&R<=M&&0<=L&&L<=O&&0<=V&&V<=O;var ag=Z.pan();var W=Z.zoom();S=G(F,L,R,V);I=[(F+R)/2,(L+V)/2];
H=[(I[0]-ag.x)/W,(I[1]-ag.y)/W]}if(an.touches[2]){}else{if(an.touches[1]){}else{if(an.touches[0]){var ao=P.findNearestElement(X[0],X[1],true);if(ao!=null){ao.activate();P.touchData.start=ao;if(ao._private.group=="nodes"&&P.nodeIsDraggable(ao)){var ac=P.dragData.touchDragEles=[];Q(ao,ac);ao.trigger(new v.Event(an,{type:"grab"}));if(ao.selected()){ac=P.dragData.touchDragEles=[];var ad=Z.$("node:selected");for(var ai=0;ai<ad.length;ai++){var am=ad[ai];if(P.nodeIsDraggable(am)){ac.push(am);am._private.rscratch.inDragLayer=true;
var ae=am._private.edges;for(var aj=0;aj<ae.length;aj++){ae[aj]._private.rscratch.inDragLayer=true}if(am._private.style.width.value=="auto"||am._private.style.height.value=="auto"){N(am,false,ac)}U(am,true)}}}else{if(ao._private.style.width.value=="auto"||ao._private.style.height.value=="auto"){N(ao,true,ac)}U(ao,true)}}ao.trigger(new v.Event(an,{type:"touchstart"})).trigger(new v.Event(an,{type:"tapstart"})).trigger(new v.Event(an,{type:"vmousdown"}))}if(ao==null){Z.trigger(new v.Event(an,{type:"touchstart"})).trigger(new v.Event(an,{type:"tapstart"})).trigger(new v.Event(an,{type:"vmousedown"}));
P.data.bgActivePosistion={x:ab[0],y:ab[1]};P.data.canvasNeedsRedraw[B]=true;P.data.canvasRedrawReason[B].push("bgactive")}for(var ak=0;ak<X.length;ak++){al[ak]=X[ak];P.touchData.startPosition[ak]=X[ak]}P.touchData.singleTouchMoved=false;P.touchData.singleTouchStartTime=l();var af=setTimeout(function(){if(P.touchData.singleTouchMoved==false&&l()-P.touchData.singleTouchStartTime>250){if(P.touchData.start){P.touchData.start.trigger(new v.Event(an,{type:"taphold"}))}else{P.data.cy.trigger(new v.Event(an,{type:"taphold"}));
Z.$(":selected").unselect()}}},1000)}}}P.redraw()},false);window.addEventListener("touchmove",function(aE){var ax=P.data.select;var ak=P.touchData.capture;ak&&aE.preventDefault();var ai=P.data.cy;var at=P.getCachedNodes();var ac=P.getCachedEdges();var aw=P.touchData.now;var X=P.touchData.earlier;if(aE.touches[0]){var an=P.projectIntoViewport(aE.touches[0].pageX,aE.touches[0].pageY);aw[0]=an[0];aw[1]=an[1]}if(aE.touches[1]){var an=P.projectIntoViewport(aE.touches[1].pageX,aE.touches[1].pageY);aw[2]=an[0];
aw[3]=an[1]}if(aE.touches[2]){var an=P.projectIntoViewport(aE.touches[2].pageX,aE.touches[2].pageY);aw[4]=an[0];aw[5]=an[1]}var Y=[];for(var aB=0;aB<aw.length;aB++){Y[aB]=aw[aB]-X[aB]}if(ak&&aE.touches[2]&&ai.boxSelectionEnabled()){clearTimeout(this.threeFingerSelectTimeout);this.lastThreeTouch=+new Date;P.data.canvasNeedsRedraw[B]=true;P.data.canvasRedrawReason[B].push("Touch moved, redraw selection box");if(!ax||ax.length===0||ax[0]===undefined){ax[0]=(aw[0]+aw[2]+aw[4])/3;ax[1]=(aw[1]+aw[3]+aw[5])/3;
ax[2]=(aw[0]+aw[2]+aw[4])/3+1;ax[3]=(aw[1]+aw[3]+aw[5])/3+1}else{ax[2]=(aw[0]+aw[2]+aw[4])/3;ax[3]=(aw[1]+aw[3]+aw[5])/3}ax[4]=1}else{if(ak&&aE.touches[1]&&ai.zoomingEnabled()&&ai.panningEnabled()){var am=aE.touches[0].pageX-J,Z=aE.touches[0].pageY-T;var ah=aE.touches[1].pageX-J,W=aE.touches[1].pageY-T;var aq=G(am,Z,ah,W);var aC=aq/S;if(aC!=1&&i){var aG=am-F;var aF=Z-L;var ar=ah-R;var ap=W-V;var aA=(aG+ar)/2;var ay=(aF+ap)/2;var ab=ai.zoom();var aa=ab*aC;var ae=ai.pan();var av=H[0]*ab+ae.x;var au=H[1]*ab+ae.y;
var ad={x:-aa/ab*(av-ae.x-aA)+av,y:-aa/ab*(au-ae.y-ay)+au};ai._private.zoom=aa;ai._private.pan=ad;ai.trigger("pan zoom").notify("viewport");S=aq;F=am;L=Z;R=ah;V=W;P.pinching=true}if(aE.touches[0]){var an=P.projectIntoViewport(aE.touches[0].pageX,aE.touches[0].pageY);aw[0]=an[0];aw[1]=an[1]}if(aE.touches[1]){var an=P.projectIntoViewport(aE.touches[1].pageX,aE.touches[1].pageY);aw[2]=an[0];aw[3]=an[1]}if(aE.touches[2]){var an=P.projectIntoViewport(aE.touches[2].pageX,aE.touches[2].pageY);aw[4]=an[0];
aw[5]=an[1]}}else{if(aE.touches[0]){var al=P.touchData.start;var af=P.touchData.last;if(al!=null&&al._private.group=="nodes"&&P.nodeIsDraggable(al)){var ag=P.dragData.touchDragEles;for(var az=0;az<ag.length;az++){var aj=ag[az];if(P.nodeIsDraggable(aj)){P.dragData.didDrag=true;aj._private.position.x+=Y[0];aj._private.position.y+=Y[1]}}(new v.Collection(ai,ag)).trigger(new v.Event(aE,{type:"drag"})).trigger(new v.Event(aE,{type:"position"}));P.data.canvasNeedsRedraw[d]=true;P.data.canvasRedrawReason[d].push("touchdrag node");
if(P.touchData.startPosition[0]==X[0]&&P.touchData.startPosition[1]==X[1]){P.data.canvasNeedsRedraw[E]=true;P.data.canvasRedrawReason[E].push("node drag started")}}if(al!=null){al.trigger(new v.Event(aE,{type:"touchmove"}))}if(al==null){var ao=P.findNearestElement(aw[0],aw[1],true);if(ao!=null){ao.trigger(new v.Event(aE,{type:"touchmove"}))}if(ao==null){ai.trigger(new v.Event(aE,{type:"touchmove"}))}}if(ao!=af){if(af){af.trigger(new v.Event(aE,{type:"touchout"}))}if(ao){ao.trigger(new v.Event(aE,{type:"touchover"}))
}}P.touchData.last=ao;for(var aD=0;aD<aw.length;aD++){if(aw[aD]&&P.touchData.startPosition[aD]&&Math.abs(aw[aD]-P.touchData.startPosition[aD])>4){P.touchData.singleTouchMoved=true}}if(ak&&(al==null||al.isEdge())&&ai.panningEnabled()){if(al){al.unactivate();if(!P.data.bgActivePosistion){P.data.bgActivePosistion={x:aw[0],y:aw[1]}}P.data.canvasNeedsRedraw[B]=true;P.data.canvasRedrawReason[B].push("bgactive")}ai.panBy({x:Y[0]*ai.zoom(),y:Y[1]*ai.zoom()});var an=P.projectIntoViewport(aE.touches[0].pageX,aE.touches[0].pageY);
aw[0]=an[0];aw[1]=an[1]}}}}for(var aB=0;aB<aw.length;aB++){X[aB]=aw[aB]}P.redraw()},false);window.addEventListener("touchend",function(an){var ag=P.touchData.capture;if(!ag){return}P.touchData.capture=false;an.preventDefault();var ae=P.data.select;var Z=P.data.cy;var af=P.getCachedNodes();var Y=P.getCachedEdges();var X=P.touchData.now;var ak=P.touchData.earlier;var aa=P.touchData.start;if(an.touches[0]){var ab=P.projectIntoViewport(an.touches[0].pageX,an.touches[0].pageY);X[0]=ab[0];X[1]=ab[1]}if(an.touches[1]){var ab=P.projectIntoViewport(an.touches[1].pageX,an.touches[1].pageY);
X[2]=ab[0];X[3]=ab[1]}if(an.touches[2]){var ab=P.projectIntoViewport(an.touches[2].pageX,an.touches[2].pageY);X[4]=ab[0];X[5]=ab[1]}var W=+new Date;if(!an.touches[2]&&Z.boxSelectionEnabled()){clearTimeout(this.threeFingerSelectTimeout);this.threeFingerSelectTimeout=setTimeout(function(){var ao=[];var ar=P.getAllInBox(ae[0],ae[1],ae[2],ae[3]);ae[0]=undefined;ae[1]=undefined;ae[2]=undefined;ae[3]=undefined;ae[4]=0;P.data.canvasNeedsRedraw[B]=true;P.data.canvasRedrawReason[B].push("Touch moved, redraw selection box");
var aq=new v.Event(an,{type:"select"});for(var ap=0;ap<ar.length;ap++){if(ar[ap]._private.selectable){ao.push(ar[ap])}}(new v.Collection(Z,ao)).select();if(ar.length>0){P.data.canvasNeedsRedraw[E]=true;P.data.canvasRedrawReason[E].push("Selection")}},100)}if(!an.touches[1]){P.pinching=false}var ai=false;if(aa!=null){aa._private.active=false;ai=true;aa.trigger(new v.Event(an,{type:"unactivate"}))}if(an.touches[2]){P.data.bgActivePosistion=undefined}else{if(an.touches[1]){}else{if(an.touches[0]){}else{if(!an.touches[0]){P.data.bgActivePosistion=undefined;
if(aa!=null){if(aa._private.grabbed==true){aa._private.grabbed=false;aa.trigger(new v.Event(an,{type:"free"}));aa._private.rscratch.inDragLayer=false}var ad=aa._private.edges;for(var aj=0;aj<ad.length;aj++){ad[aj]._private.rscratch.inDragLayer=false}U(aa,false);if(aa.selected()){var ac=Z.$("node:selected");for(var ah=0;ah<ac.length;ah++){var al=ac[ah];al._private.rscratch.inDragLayer=false;var ad=al._private.edges;for(var aj=0;aj<ad.length;aj++){ad[aj]._private.rscratch.inDragLayer=false}U(al,false)
}}P.data.canvasNeedsRedraw[d]=true;P.data.canvasRedrawReason[d].push("touchdrag node end");P.data.canvasNeedsRedraw[E]=true;P.data.canvasRedrawReason[E].push("touchdrag node end");aa.trigger(new v.Event(an,{type:"touchend"})).trigger(new v.Event(an,{type:"tapend"})).trigger(new v.Event(an,{type:"vmouseup"}));P.touchData.start=null}else{var am=P.findNearestElement(X[0],X[1],true);if(am!=null){am.trigger(new v.Event(an,{type:"touchend"})).trigger(new v.Event(an,{type:"tapend"})).trigger(new v.Event(an,{type:"vmouseup"}))
}if(am==null){Z.trigger(new v.Event(an,{type:"touchend"})).trigger(new v.Event(an,{type:"tapend"})).trigger(new v.Event(an,{type:"vmouseup"}))}}if(aa!=null&&!P.dragData.didDrag&&aa._private.selectable&&(Math.sqrt(Math.pow(P.touchData.startPosition[0]-X[0],2)+Math.pow(P.touchData.startPosition[1]-X[1],2)))<6){if(aa.selected()){aa._private.selected=false;aa.trigger(new v.Event(an,{type:"unselect"}))}else{aa._private.selected=true;aa.trigger(new v.Event(an,{type:"select"}))}ai=true;P.data.canvasNeedsRedraw[E]=true;
P.data.canvasRedrawReason[E].push("sglslct")}if(P.touchData.singleTouchMoved==false){if(aa){aa.trigger(new v.Event(an,{type:"tap"})).trigger(new v.Event(an,{type:"vclick"}))}else{Z.trigger(new v.Event(an,{type:"tap"})).trigger(new v.Event(an,{type:"vclick"}))}}P.touchData.singleTouchMoved=true}}}}for(var aj=0;aj<X.length;aj++){ak[aj]=X[aj]}P.dragData.didDrag=false;if(ai&&aa){aa.updateStyle(false)}P.redraw()},false)};w.prototype.init=function(){};w.prototype.getCachedNodes=function(){var i=this.data;
var F=this.data.cy;if(i.cache==undefined){i.cache={}}if(i.cache.cachedNodes==undefined){i.cache.cachedNodes=F.nodes()}return i.cache.cachedNodes};w.prototype.updateNodesCache=function(){var i=this.data;var F=this.data.cy;if(i.cache==undefined){i.cache={}}i.cache.cachedNodes=F.nodes()};w.prototype.getCachedEdges=function(){var i=this.data;var F=this.data.cy;if(i.cache==undefined){i.cache={}}if(i.cache.cachedEdges==undefined){i.cache.cachedEdges=F.edges()}return i.cache.cachedEdges};w.prototype.updateEdgesCache=function(){var i=this.data;
var F=this.data.cy;if(i.cache==undefined){i.cache={}}i.cache.cachedEdges=F.edges()};w.prototype.projectIntoViewport=function(G,i){n=this.data.container;var J=false;var H=this.findContainerPageCoords();var I=H[0];var F=H[1];x=G-I;y=i-F;x-=this.data.cy.pan().x;y-=this.data.cy.pan().y;x/=this.data.cy.zoom();y/=this.data.cy.zoom();return[x,y]};w.prototype.findContainerPageCoords=function(){var i,K;var H=0;var G=0;var J;J=this.data.container;var I=false;while(J!=null){var F=window.getComputedStyle(J);
if(F.getPropertyValue("position").toLowerCase()==="fixed"){H=J.offsetLeft+window.scrollX;G=J.offsetTop+window.scrollY;J=null}else{if(typeof(J.offsetLeft)=="number"){H+=J.offsetLeft;G+=J.offsetTop;if(J==document.body||J==document.header){I=true}if(!I){H-=J.scrollLeft;G-=J.scrollTop}}}if(J){J=J.offsetParent}}return[H,G]};w.prototype.findNearestElement=function(T,S,H){var O=this.data;var I=this.getCachedNodes();var M=this.getCachedEdges();var P=[];var U=this.data.cy.zoom();var K=(u?256:32)/U;var R=(u?16:0)/U;
for(var N=0;N<I.length;N++){if(o[this.getNodeShape(I[N])].checkPointRough(T,S,I[N]._private.style["border-width"].value,this.getNodeWidth(I[N])+R,this.getNodeHeight(I[N])+R,I[N]._private.position.x,I[N]._private.position.y)&&o[this.getNodeShape(I[N])].checkPoint(T,S,I[N]._private.style["border-width"].value,(this.getNodeWidth(I[N])+R)/2,(this.getNodeHeight(I[N])+R)/2,I[N]._private.position.x,I[N]._private.position.y)){if(H){if(I[N]._private.style.opacity.value!=0&&I[N]._private.style.visibility.value=="visible"){P.push(I[N])
}}else{P.push(I[N])}}}var G;for(var N=0;N<M.length;N++){var J=M[N];var L=J._private.rscratch;G=false;if(L.edgeType=="self"){if((this.inBezierVicinity(T,S,L.startX,L.startY,L.cp2ax,L.cp2ay,L.selfEdgeMidX,L.selfEdgeMidY,Math.pow(J._private.style.width.value/2,2))&&(Math.pow(M[N]._private.style.width.value/2,2)+K>this.sqDistanceToQuadraticBezier(T,S,L.startX,L.startY,L.cp2ax,L.cp2ay,L.selfEdgeMidX,L.selfEdgeMidY)))||(this.inBezierVicinity(T,S,L.selfEdgeMidX,L.selfEdgeMidY,L.cp2cx,L.cp2cy,L.endX,L.endY,Math.pow(M[N]._private.style.width.value/2,2))&&(Math.pow(M[N]._private.style.width.value/2,2)+K>this.sqDistanceToQuadraticBezier(T,S,L.selfEdgeMidX,L.selfEdgeMidY,L.cp2cx,L.cp2cy,L.endX,L.endY)))){G=true
}}else{if(L.edgeType=="straight"){if(Math.pow(M[N]._private.style.width.value/2,2)+K>this.sqDistanceToFiniteLine(T,S,L.startX,L.startY,L.endX,L.endY)){G=true}}else{if(L.edgeType=="bezier"){if(this.inBezierVicinity(T,S,L.startX,L.startY,L.cp2x,L.cp2y,L.endX,L.endY,Math.pow(M[N]._private.style.width.value/2,2))&&(Math.pow(M[N]._private.style.width.value/2,2)+K>this.sqDistanceToQuadraticBezier(T,S,L.startX,L.startY,L.cp2x,L.cp2y,L.endX,L.endY))){G=true}}}}if(!P.length||P[P.length-1]!=M[N]){if((b[M[N]._private.style["source-arrow-shape"].value].roughCollide(T,S,M[N]._private.rscratch.arrowStartX,M[N]._private.rscratch.arrowStartY,this.getArrowWidth(M[N]._private.style.width.value),this.getArrowHeight(M[N]._private.style.width.value),[M[N]._private.rscratch.arrowStartX-M[N].source()[0]._private.position.x,M[N]._private.rscratch.arrowStartY-M[N].source()[0]._private.position.y],0)&&b[M[N]._private.style["source-arrow-shape"].value].collide(T,S,M[N]._private.rscratch.arrowStartX,M[N]._private.rscratch.arrowStartY,this.getArrowWidth(M[N]._private.style.width.value),this.getArrowHeight(M[N]._private.style.width.value),[M[N]._private.rscratch.arrowStartX-M[N].source()[0]._private.position.x,M[N]._private.rscratch.arrowStartY-M[N].source()[0]._private.position.y],0))||(b[M[N]._private.style["target-arrow-shape"].value].roughCollide(T,S,M[N]._private.rscratch.arrowEndX,M[N]._private.rscratch.arrowEndY,this.getArrowWidth(M[N]._private.style.width.value),this.getArrowHeight(M[N]._private.style.width.value),[M[N]._private.rscratch.arrowEndX-M[N].target()[0]._private.position.x,M[N]._private.rscratch.arrowEndY-M[N].target()[0]._private.position.y],0)&&b[M[N]._private.style["target-arrow-shape"].value].collide(T,S,M[N]._private.rscratch.arrowEndX,M[N]._private.rscratch.arrowEndY,this.getArrowWidth(M[N]._private.style.width.value),this.getArrowHeight(M[N]._private.style.width.value),[M[N]._private.rscratch.arrowEndX-M[N].target()[0]._private.position.x,M[N]._private.rscratch.arrowEndY-M[N].target()[0]._private.position.y],0))){G=true
}}if(G){if(H){var F=O.cy.getElementById(M[N]._private.data.source);var Q=O.cy.getElementById(M[N]._private.data.target);if(M[N]._private.style.opacity.value!=0&&M[N]._private.style.visibility.value=="visible"&&F._private.style.opacity.value!=0&&F._private.style.visibility.value=="visible"&&Q._private.style.opacity.value!=0&&Q._private.style.visibility.value=="visible"){P.push(M[N])}}else{P.push(M[N])}}}P.sort(function(V,i){var X=i._private.style["z-index"].value-V._private.style["z-index"].value;
var W=i._private.data.id.localeCompare(V._private.data.id);var Y=(function(aa,Z){if(aa.isEdge()&&Z.isNode()){return 1}else{if(aa.isNode()&&Z.isEdge()){return -1}}return 0})(V,i);return X||Y||W});if(P.length>0){return P[0]}else{return null}};w.prototype.getAllInBox=function(H,R,F,Q){var M=this.data;var G=this.getCachedNodes();var K=this.getCachedEdges();var N=[];var P=Math.min(H,F);var J=Math.max(H,F);var S=Math.min(R,Q);var O=Math.max(R,Q);H=P;F=J;R=S;Q=O;var I;for(var L=0;L<G.length;L++){if(o[this.getNodeShape(G[L])].intersectBox(H,R,F,Q,this.getNodeWidth(G[L]),this.getNodeHeight(G[L]),G[L]._private.position.x,G[L]._private.position.y,G[L]._private.style["border-width"].value/2)){N.push(G[L])
}}for(var L=0;L<K.length;L++){if(K[L]._private.rscratch.edgeType=="self"){if((I=this.boxInBezierVicinity(H,R,F,Q,K[L]._private.rscratch.startX,K[L]._private.rscratch.startY,K[L]._private.rscratch.cp2ax,K[L]._private.rscratch.cp2ay,K[L]._private.rscratch.endX,K[L]._private.rscratch.endY,K[L]._private.style.width.value))&&(I==2||(I==1&&this.checkBezierCrossesBox(H,R,F,Q,K[L]._private.rscratch.startX,K[L]._private.rscratch.startY,K[L]._private.rscratch.cp2ax,K[L]._private.rscratch.cp2ay,K[L]._private.rscratch.endX,K[L]._private.rscratch.endY,K[L]._private.style.width.value)))||(I=this.boxInBezierVicinity(H,R,F,Q,K[L]._private.rscratch.startX,K[L]._private.rscratch.startY,K[L]._private.rscratch.cp2cx,K[L]._private.rscratch.cp2cy,K[L]._private.rscratch.endX,K[L]._private.rscratch.endY,K[L]._private.style.width.value))&&(I==2||(I==1&&this.checkBezierCrossesBox(H,R,F,Q,K[L]._private.rscratch.startX,K[L]._private.rscratch.startY,K[L]._private.rscratch.cp2cx,K[L]._private.rscratch.cp2cy,K[L]._private.rscratch.endX,K[L]._private.rscratch.endY,K[L]._private.style.width.value)))){N.push(K[L])
}}if(K[L]._private.rscratch.edgeType=="bezier"&&(I=this.boxInBezierVicinity(H,R,F,Q,K[L]._private.rscratch.startX,K[L]._private.rscratch.startY,K[L]._private.rscratch.cp2x,K[L]._private.rscratch.cp2y,K[L]._private.rscratch.endX,K[L]._private.rscratch.endY,K[L]._private.style.width.value))&&(I==2||(I==1&&this.checkBezierCrossesBox(H,R,F,Q,K[L]._private.rscratch.startX,K[L]._private.rscratch.startY,K[L]._private.rscratch.cp2x,K[L]._private.rscratch.cp2y,K[L]._private.rscratch.endX,K[L]._private.rscratch.endY,K[L]._private.style.width.value)))){N.push(K[L])
}if(K[L]._private.rscratch.edgeType=="straight"&&(I=this.boxInBezierVicinity(H,R,F,Q,K[L]._private.rscratch.startX,K[L]._private.rscratch.startY,K[L]._private.rscratch.startX*0.5+K[L]._private.rscratch.endX*0.5,K[L]._private.rscratch.startY*0.5+K[L]._private.rscratch.endY*0.5,K[L]._private.rscratch.endX,K[L]._private.rscratch.endY,K[L]._private.style.width.value))&&(I==2||(I==1&&this.checkStraightEdgeCrossesBox(H,R,F,Q,K[L]._private.rscratch.startX,K[L]._private.rscratch.startY,K[L]._private.rscratch.endX,K[L]._private.rscratch.endY,K[L]._private.style.width.value)))){N.push(K[L])
}}return N};w.prototype.updateAllCompounds=function(I){for(var F=I.length-1;F>=0;F--){if(I[F].isNode()&&(I[F]._private.style.width.value=="auto"||I[F]._private.style.height.value=="auto")&&I[F].children().length>0){var H=I[F];var G=this.calcCompoundBounds(H);H._private.position.x=G.x;H._private.position.y=G.y;H._private.autoWidth=G.width;H._private.autoHeight=G.height}}};w.prototype.calcCompoundBounds=function(L){var K=L.children().not(":removed");var F={x:L._private.position.x,y:L._private.position.y,width:L._private.autoWidth,height:L._private.autoHeight};
if(L._private.style.visibility.value!="visible"){return F}var G=[];for(var N=0;N<K.size();N++){if(K[N]._private.style.visibility.value=="visible"){G.push(K[N])}}if(G.length==0){return F}K=G;var H=this.borderValue(K,"left");var O=this.borderValue(K,"right");var M=this.borderValue(K,"top");var J=this.borderValue(K,"bottom");var R=this.getNodePadding(L);var Q=(H-R.left+O+R.right)/2;var P=(M-R.top+J+R.bottom)/2;var I=(O-H)+R.left+R.right;var S=(J-M)+R.top+R.bottom;F={x:Q,y:P,width:I,height:S};return F
};w.prototype.borderValue=function(H,P){var L,G;var Q=1/0,N=-1/0;var i=this;var O=function(T){var S={};S.x=T._private.position.x;S.y=T._private.position.y;S.width=T.outerWidth();S.height=T.outerHeight();return S};var J=function(T){var U=String(T._private.style.content.value);var V=T._private.style["text-transform"].value;if(V=="none"){}else{if(V=="uppercase"){U=U.toUpperCase()}else{if(V=="lowercase"){U=U.toLowerCase()}}}var S=i.data.canvases[4].getContext("2d");return S.measureText(U).width};var R=function(W){var U={};
var S=i.getNodeWidth(W);var X=i.getNodeHeight(W);U.height=W._private.style["font-size"].value;U.width=U.height;var V=W._private.style["text-halign"].strValue;if(V=="left"){U.x=W._private.position.x-S/2;U.left=U.x-U.width;U.right=U.x}else{if(V=="right"){U.x=W._private.position.x+S/2;U.left=U.x;U.right=U.x+U.width}else{U.x=W._private.position.x;U.left=U.x-U.width/2;U.right=U.x+U.width/2}}var T=W._private.style["text-valign"].strValue;if(T=="top"){U.y=W._private.position.y-X/2;U.top=U.y-U.height;U.bottom=U.y
}else{if(T=="bottom"){U.y=W._private.position.y+X/2;U.top=U.y;U.bottom=U.y+U.height}else{U.y=W._private.position.y;U.top=U.y-U.height/2;U.bottom=U.y+U.height/2}}return U};for(C=0;C<H.length;C++){L=O(H[C]);G=R(H[C]);if(P=="left"){var F=Math.min(L.x-L.width/2,G.left);if(F<Q){Q=F}}else{if(P=="right"){var M=Math.max(L.x+L.width/2,G.right);if(M>N){N=M}}else{if(P=="top"){var K=Math.min(L.y-L.height/2,G.top);if(K<Q){Q=K}}else{if(P=="bottom"){var I=Math.max(L.y+L.height/2,G.bottom);if(I>N){N=I}}}}}}if((P=="left")||(P=="top")){return Q
}else{return N}};w.prototype.getNodeWidth=function(i){if(i._private.style.width.value=="auto"||i._private.style.height.value=="auto"){return i._private.autoWidth}else{return i._private.style.width.value}};w.prototype.getNodeHeight=function(i){if(i._private.style.width.value=="auto"||i._private.style.height.value=="auto"){return i._private.autoHeight}else{return i._private.style.height.value}};w.prototype.getNodeShape=function(i){return i._private.style.shape.value};w.prototype.getNodePadding=function(G){var I=G._private.style["padding-left"].value;
var F=G._private.style["padding-right"].value;var H=G._private.style["padding-top"].value;var i=G._private.style["padding-bottom"].value;if(isNaN(I)){I=0}if(isNaN(F)){F=0}if(isNaN(H)){H=0}if(isNaN(i)){i=0}return{left:I,right:F,top:H,bottom:i}};w.prototype.matchCanvasSize=function(G){var K=this.data;var J=G.clientWidth;var F=G.clientHeight;var H;for(var I=0;I<e;I++){H=K.canvases[I];if(H.width!==J||H.height!==F){H.width=J;H.height=F}}for(var I=0;I<a;I++){H=K.bufferCanvases[I];if(H.width!==J||H.height!==F){H.width=J;
H.height=F}}};w.prototype.redraw=function(ad,J){var aa=this;if(this.averageRedrawTime===undefined){this.averageRedrawTime=0}var H=1000/60;var U=1000/30;if(u){U=200}var ab=this.averageRedrawTime;ab=Math.max(H,ab);ab=Math.min(ab,U);if(this.lastDrawTime===undefined){this.lastDrawTime=0}var F=+new Date;var S=F-this.lastDrawTime;var af=S>=ab;if(!ad){if(!af){clearTimeout(this.redrawTimeout);this.redrawTimeout=setTimeout(function(){aa.redraw()},ab);return}this.lastDrawTime=F}var Z=F;var X=100;var O=this.data.cy;
var ah=this.data;var ac=this.getCachedNodes();var N=this.getCachedEdges();this.matchCanvasSize(ah.container);var Y=[];for(var ae=0;ae<ac.length;ae++){Y.push(ac[ae])}for(var ae=0;ae<N.length;ae++){Y.push(N[ae])}var Q=function(i){if(i._private.group=="nodes"){return i.parents().size()}else{if(i._private.group=="edges"){return Math.max(i.source()[0].parents().size(),i.target()[0].parents().size())}else{return 0}}};if(ah.canvasNeedsRedraw[d]||ah.canvasNeedsRedraw[E]||J){this.findEdgeControlPoints(N);
var M=false;for(var ae=0;ae<Y.length;ae++){if(Y[ae].isNode()&&Y[ae].isParent()){M=true;break}}var Y=[];for(var ae=0;ae<ac.length;ae++){Y.push(ac[ae])}for(var ae=0;ae<N.length;ae++){Y.push(N[ae])}Y.sort(function(aj,ai){var i=aj._private.style["z-index"].value-ai._private.style["z-index"].value;var al=0;var ak=0;if(M){al=Q(aj);ak=Q(ai)}if(i==0){if(al-ak==0){if(aj._private.group=="nodes"&&ai._private.group=="edges"){return 1}else{if(aj._private.group=="edges"&&ai._private.group=="nodes"){return -1}else{return 0
}}}else{return al-ak}}return 0});if(M){this.updateAllCompounds(Y)}}var T;var P;var K;if(ah.canvasNeedsRedraw[E]||J){if(!T||!P){T=[];P=[];for(var W=0;W<Y.length;W++){K=Y[W];if(K._private.rscratch.inDragLayer){T.push(K)}else{P.push(K)}}}var L=ad||ah.canvases[E].getContext("2d");L.setTransform(1,0,0,1,0,0);L.clearRect(0,0,L.canvas.width,L.canvas.height);if(!J){L.translate(O.pan().x,O.pan().y);L.scale(O.zoom(),O.zoom())}for(var W=0;W<P.length;W++){K=P[W];if(K._private.group=="nodes"){this.drawNode(L,K)
}else{if(K._private.group=="edges"){this.drawEdge(L,K)}}}for(var W=0;W<P.length;W++){K=P[W];if(K._private.group=="nodes"){this.drawNodeText(L,K)}else{if(K._private.group=="edges"){this.drawEdgeText(L,K)}}if(K._private.group=="nodes"){this.drawNode(L,K,true)}else{if(K._private.group=="edges"){this.drawEdge(L,K,true)}}}if(!J){ah.canvasNeedsRedraw[E]=false;ah.canvasRedrawReason[E]=[]}}if(ah.canvasNeedsRedraw[d]||J){if(!T||!P){T=[];P=[];for(var W=0;W<Y.length;W++){K=Y[W];if(K._private.rscratch.inDragLayer){T.push(K)
}else{P.push(K)}}}var L=ad||ah.canvases[d].getContext("2d");if(!J){L.setTransform(1,0,0,1,0,0);L.clearRect(0,0,L.canvas.width,L.canvas.height);L.translate(O.pan().x,O.pan().y);L.scale(O.zoom(),O.zoom())}var K;for(var W=0;W<T.length;W++){K=T[W];if(K._private.group=="nodes"){this.drawNode(L,K)}else{if(K._private.group=="edges"){this.drawEdge(L,K)}}}for(var W=0;W<T.length;W++){K=T[W];if(K._private.group=="nodes"){this.drawNodeText(L,K)}else{if(K._private.group=="edges"){this.drawEdgeText(L,K)}}if(K._private.group=="nodes"){this.drawNode(L,K,true)
}else{if(K._private.group=="edges"){this.drawEdge(L,K,true)}}}if(!J){ah.canvasNeedsRedraw[d]=false;ah.canvasRedrawReason[d]=[]}}if(ah.canvasNeedsRedraw[B]){var L=ad||ah.canvases[B].getContext("2d");if(!J){L.setTransform(1,0,0,1,0,0);L.clearRect(0,0,L.canvas.width,L.canvas.height);L.translate(O.pan().x,O.pan().y);L.scale(O.zoom(),O.zoom())}var I=O.style()._private.coreStyle;if(ah.select[4]==1){var G=ah.cy.zoom();var ag=I["selection-box-border-width"].value/G;L.lineWidth=ag;L.fillStyle="rgba("+I["selection-box-color"].value[0]+","+I["selection-box-color"].value[1]+","+I["selection-box-color"].value[2]+","+I["selection-box-opacity"].value+")";
L.fillRect(ah.select[0],ah.select[1],ah.select[2]-ah.select[0],ah.select[3]-ah.select[1]);if(ag>0){L.strokeStyle="rgba("+I["selection-box-border-color"].value[0]+","+I["selection-box-border-color"].value[1]+","+I["selection-box-border-color"].value[2]+","+I["selection-box-opacity"].value+")";L.strokeRect(ah.select[0],ah.select[1],ah.select[2]-ah.select[0],ah.select[3]-ah.select[1])}}if(ah.bgActivePosistion){var G=ah.cy.zoom();var V=ah.bgActivePosistion;L.fillStyle="rgba("+I["active-bg-color"].value[0]+","+I["active-bg-color"].value[1]+","+I["active-bg-color"].value[2]+","+I["active-bg-opacity"].value+")";
L.beginPath();L.arc(V.x,V.y,I["active-bg-size"].pxValue/G,0,2*Math.PI);L.fill()}if(!J){ah.canvasNeedsRedraw[B]=false;ah.canvasRedrawReason[B]=[]}}var L;var R=+new Date;if(this.averageRedrawTime===undefined){this.averageRedrawTime=R-Z}this.averageRedrawTime=this.averageRedrawTime/2+(R-Z)/2};var k={};var g=30*300;w.prototype.getCachedImage=function(F,G){if(k[F]&&k[F].image){k[F].keepTime=g;return k[F].image}var i=k[F];if(i==undefined){k[F]=new Object();k[F].image=new Image();k[F].image.onload=G;k[F].image.src=F;
k[F].keepTime=g;i=k[F]}return i.image};w.prototype.swapCachedImage=function(F){if(k[F]){if(k[F].image&&k[F].image.complete){var G=k[F].image;var i=document.createElement("canvas");i.width=G.width;i.height=G.height;i.getContext("2d").drawImage(G,0,0);k[F].image=i;k[F].swappedWithCanvas=true;return i}else{return null}}else{return null}};w.prototype.updateImageCaches=function(){for(var i in k){if(k[i].keepTime<=0){if(k[i].image!=undefined){k[i].image.src=undefined;k[i].image=undefined}k[i]=undefined
}else{k[i]-=1}}};w.prototype.drawImage=function(i,L,K,I,H,M,G){G.widthScale=0.5;G.heightScale=0.5;G.rotate=M;var J;var F;canvas.drawImage(G,L,K)};w.prototype.drawEdge=function(G,I,J){var K,O;K=I.source()[0];O=I.target()[0];if(I._private.style.visibility.value!="visible"||K._private.style.visibility.value!="visible"||O._private.style.visibility.value!="visible"){return}var S=I._private.style["overlay-padding"].value;var Q=I._private.style["overlay-opacity"].value;var R=I._private.style["overlay-color"].value;
if(J){G.strokeStyle="rgba( "+R[0]+", "+R[1]+", "+R[2]+", "+Q+" )";G.lineCap="round";if(I._private.rscratch.edgeType=="self"){G.lineCap="butt"}}else{G.strokeStyle="rgba("+I._private.style["line-color"].value[0]+","+I._private.style["line-color"].value[1]+","+I._private.style["line-color"].value[2]+","+I._private.style.opacity.value+")"}if(I._private.style.width.value<=0){return}var P=I._private.style.width.value+(J?2*S:0);var M=J?"solid":I._private.style["line-style"].value;G.lineWidth=P;this.findEndpoints(I);
if(I._private.rscratch.edgeType=="self"){var i=I._private.rscratch;this.drawStyledEdge(I,G,[i.startX,i.startY,i.cp2ax,i.cp2ay,i.selfEdgeMidX,i.selfEdgeMidY],M,P);this.drawStyledEdge(I,G,[i.selfEdgeMidX,i.selfEdgeMidY,i.cp2cx,i.cp2cy,i.endX,i.endY],M,P)}else{if(I._private.rscratch.edgeType=="straight"){var H=O._private.position.x-K._private.position.x;var F=O._private.position.y-K._private.position.y;var N=I._private.rscratch.endX-I._private.rscratch.startX;var L=I._private.rscratch.endY-I._private.rscratch.startY;
if(H*N+F*L<0){I._private.rscratch.straightEdgeTooShort=true}else{var i=I._private.rscratch;this.drawStyledEdge(I,G,[i.startX,i.startY,i.endX,i.endY],M,P);I._private.rscratch.straightEdgeTooShort=false}}else{var i=I._private.rscratch;this.drawStyledEdge(I,G,[i.startX,i.startY,i.cp2x,i.cp2y,i.endX,i.endY],M,P)}}if(I._private.rscratch.noArrowPlacement!==true&&I._private.rscratch.startX!==undefined){this.drawArrowheads(G,I,J)}};var t=function(N,J,H){var G=Math.sqrt(Math.pow(N[4]-N[0],2)+Math.pow(N[5]-N[1],2));
G+=Math.sqrt(Math.pow((N[4]+N[0])/2-N[2],2)+Math.pow((N[5]+N[1])/2-N[3],2));var M=Math.ceil(G/J);var F=G/J;var K;if(M>0){K=new Array(M*2)}else{return null}for(var I=0;I<M;I++){var L=I/M;K[I*2]=N[0]*(1-L)*(1-L)+2*(N[2])*(1-L)*L+N[4]*(L)*(L);K[I*2+1]=N[1]*(1-L)*(1-L)+2*(N[3])*(1-L)*L+N[5]*(L)*(L)}return K};var s=function(N,J,H){var G=Math.sqrt(Math.pow(N[2]-N[0],2)+Math.pow(N[3]-N[1],2));var M=Math.ceil(G/J);var K;if(M>0){K=new Array(M*2)}else{return null}var F=[N[2]-N[0],N[3]-N[1]];for(var I=0;I<M;
I++){var L=I/M;K[I*2]=F[0]*L+N[0];K[I*2+1]=F[1]*L+N[1]}return K};var p=function(G,i,F){pt1=_genpts(G,i);pt2=_genpts(G,F)};w.prototype.createBuffer=function(F,G){var i=document.createElement("canvas");i.width=F;i.height=G;return[i,i.getContext("2d")]};w.prototype.drawStyledEdge=function(L,H,Y,I,R){var F=this.data.cy.zoom();function X(ab,aa,Z,i){return(1-i)*(1-i)*ab+2*(1-i)*i*aa+i*i*Z}if(L._private.rstyle.bezierPts===undefined){L._private.rstyle.bezierPts=[]}var T=L._private.rstyle.bezierPts.length;
if(L.isLoop()){if(T>=12){L._private.rstyle.bezierPts=[]}else{}}else{L._private.rstyle.bezierPts=[]}var O=L._private.rstyle.bezierPts;if(Y.length===6){O.push({x:X(Y[0],Y[2],Y[4],0.05),y:X(Y[1],Y[3],Y[5],0.05)});O.push({x:X(Y[0],Y[2],Y[4],0.25),y:X(Y[1],Y[3],Y[5],0.25)});O.push({x:X(Y[0],Y[2],Y[4],0.35),y:X(Y[1],Y[3],Y[5],0.35)});O.push({x:X(Y[0],Y[2],Y[4],0.65),y:X(Y[1],Y[3],Y[5],0.65)});O.push({x:X(Y[0],Y[2],Y[4],0.75),y:X(Y[1],Y[3],Y[5],0.75)});O.push({x:X(Y[0],Y[2],Y[4],0.95),y:X(Y[1],Y[3],Y[5],0.95)})
}if(I=="solid"){H.beginPath();H.moveTo(Y[0],Y[1]);if(Y.length==3*2){H.quadraticCurveTo(Y[2],Y[3],Y[4],Y[5])}else{H.lineTo(Y[2],Y[3])}H.stroke()}else{if(I=="dotted"){var Q;if(Y.length==3*2){Q=t(Y,16,true)}else{Q=s(Y,16,true)}if(!Q){return}var K=Math.max(R*1.6,3.4)*F;var J=K*2,N=K*2;J=Math.max(J,1);N=Math.max(N,1);var U=this.createBuffer(J,N);var S=U[1];S.setTransform(1,0,0,1,0,0);S.clearRect(0,0,J,N);S.fillStyle=H.strokeStyle;S.beginPath();S.arc(J/2,N/2,K*0.5,0,Math.PI*2,false);S.fill();H.beginPath();
H.save();for(var V=0;V<Q.length/2;V++){H.drawImage(U[0],Q[V*2]-J/2/F,Q[V*2+1]-N/2/F,J/F,N/F)}H.restore()}else{if(I=="dashed"){var Q;if(Y.length==3*2){Q=t(Y,14,true)}else{Q=s(Y,14,true)}if(!Q){return}var J=R*2*F;var N=7.8*F;J=Math.max(J,1);N=Math.max(N,1);var U=this.createBuffer(J,N);var S=U[1];S.setTransform(1,0,0,1,0,0);S.clearRect(0,0,J,N);if(H.strokeStyle){S.strokeStyle=H.strokeStyle}S.lineWidth=R*cy.zoom();S.beginPath();S.moveTo(J/2,N*0.2);S.lineTo(J/2,N*0.8);S.stroke();H.save();var G=false;var M,W;
if(Y.length==2*2){M=[Y[2]-Y[0],Y[3]-Q[1]];W=Math.acos((M[0]*0+M[1]*-1)/Math.sqrt(M[0]*M[0]+M[1]*M[1]));if(M[0]<0){W=-W+2*Math.PI}}else{if(Y.length==3*2){G=true}}for(var V=0;V<Q.length/2;V++){var P=V/(Math.max(Q.length/2-1,1));if(G){M=[2*(1-P)*(Y[2]-Y[0])+2*P*(Y[4]-Y[2]),2*(1-P)*(Y[3]-Y[1])+2*P*(Y[5]-Y[3])];W=Math.acos((M[0]*0+M[1]*-1)/Math.sqrt(M[0]*M[0]+M[1]*M[1]));if(M[0]<0){W=-W+2*Math.PI}}H.translate(Q[V*2],Q[V*2+1]);H.rotate(W);H.translate(-J/2/F,-N/2/F);H.drawImage(U[0],0,0,J/F,N/F);H.translate(J/2/F,N/2/F);
H.rotate(-W);H.translate(-Q[V*2],-Q[V*2+1])}H.restore()}else{this.drawStyledEdge(L,H,Y,"solid",R)}}}};w.prototype.drawEdgeText=function(H,K){if(K._private.style.visibility.value!="visible"){return}H.textAlign="center";H.textBaseline="middle";var F,i;var J,I;if(K._private.rscratch.edgeType=="self"){J=K._private.rscratch.selfEdgeMidX;I=K._private.rscratch.selfEdgeMidY}else{if(K._private.rscratch.edgeType=="straight"){J=(K._private.rscratch.startX+K._private.rscratch.endX)/2;I=(K._private.rscratch.startY+K._private.rscratch.endY)/2
}else{if(K._private.rscratch.edgeType=="bezier"){J=0.25*K._private.rscratch.startX+2*0.5*0.5*K._private.rscratch.cp2x+(0.5*0.5)*K._private.rscratch.endX;I=Math.pow(1-0.5,2)*K._private.rscratch.startY+2*(1-0.5)*0.5*K._private.rscratch.cp2y+(0.5*0.5)*K._private.rscratch.endY}}}F=J;i=I;var G=K._private.rstyle;G.labelX=F;G.labelY=i;this.drawText(H,K,F,i)};w.prototype.drawNode=function(H,J,I){var N,G;if(J._private.style.visibility.value!="visible"){return}N=this.getNodeWidth(J);G=this.getNodeHeight(J);
if(I===undefined||!I){H.fillStyle="rgba("+J._private.style["background-color"].value[0]+","+J._private.style["background-color"].value[1]+","+J._private.style["background-color"].value[2]+","+(J._private.style["background-opacity"].value*J._private.style.opacity.value)+")";H.strokeStyle="rgba("+J._private.style["border-color"].value[0]+","+J._private.style["border-color"].value[1]+","+J._private.style["border-color"].value[2]+","+(J._private.style["border-opacity"].value*J._private.style.opacity.value)+")";
var F=J._private.style["background-image"].value[2]||J._private.style["background-image"].value[1];if(F!=undefined){var i=this;var K=this.getCachedImage(F,function(){i.data.canvasNeedsRedraw[E]=true;i.data.canvasRedrawReason[E].push("image finished load");i.data.canvasNeedsRedraw[d]=true;i.data.canvasRedrawReason[d].push("image finished load");i.swapCachedImage(F);i.redraw()});if(K.complete==false){o[i.getNodeShape(J)].drawPath(H,J._private.position.x,J._private.position.y,N,G);H.stroke();H.fillStyle="#555555";
H.fill()}else{this.drawInscribedImage(H,K,J)}}else{o[this.getNodeShape(J)].draw(H,J._private.position.x,J._private.position.y,N,G)}H.lineWidth=J._private.style["border-width"].pxValue;if(J._private.style["border-width"].value>0){H.stroke()}}else{var O=J._private.style["overlay-padding"].value;var L=J._private.style["overlay-opacity"].value;var M=J._private.style["overlay-color"].value;if(L>0){H.fillStyle="rgba( "+M[0]+", "+M[1]+", "+M[2]+", "+L+" )";o[this.getNodeShape(J)].draw(H,J._private.position.x,J._private.position.y,N+O*2,G+O*2)
}}};w.prototype.drawInscribedImage=function(F,K,I){var M=this.data.cy._private.zoom;var H=I._private.position.x;var G=I._private.position.y;var L=this.getNodeWidth(I);var i=this.getNodeHeight(I);F.save();o[r.getNodeShape(I)].drawPath(F,H,G,L,i);F.clip();var J=[K.width,K.height];F.drawImage(K,H-J[0]/2,G-J[1]/2,J[0],J[1]);F.restore();F.stroke()};w.prototype.drawNodeText=function(I,K){if(K._private.style.visibility.value!="visible"){return}var G,i;var F=this.getNodeWidth(K);var L=this.getNodeHeight(K);
var J=K._private.style["text-halign"].strValue;if(J=="left"){I.textAlign="right";G=K._private.position.x-F/2}else{if(J=="right"){I.textAlign="left";G=K._private.position.x+F/2}else{if(J=="center"){I.textAlign="center";G=K._private.position.x}else{I.textAlign="center";G=K._private.position.x}}}var H=K._private.style["text-valign"].strValue;if(H=="top"){I.textBaseline="bottom";i=K._private.position.y-L/2}else{if(H=="bottom"){I.textBaseline="top";i=K._private.position.y+L/2}else{if(H=="middle"||H=="center"){I.textBaseline="middle";
i=K._private.position.y}else{I.textBaseline="middle";i=K._private.position.y}}}this.drawText(I,K,G,i)};w.prototype.drawText=function(i,I,K,J){var N=I._private.style["font-style"].strValue;var H=I._private.style["font-size"].value+"px";var P=I._private.style["font-family"].strValue;var M=I._private.style["font-variant"].strValue;var F=I._private.style["font-weight"].strValue;i.font=N+" "+F+" "+H+" "+P;var O=String(I._private.style.content.value);var G=I._private.style["text-transform"].value;if(G=="none"){}else{if(G=="uppercase"){O=O.toUpperCase()
}else{if(G=="lowercase"){O=O.toLowerCase()}}}i.lineJoin="round";i.fillStyle="rgba("+I._private.style.color.value[0]+","+I._private.style.color.value[1]+","+I._private.style.color.value[2]+","+(I._private.style["text-opacity"].value*I._private.style.opacity.value)+")";i.strokeStyle="rgba("+I._private.style["text-outline-color"].value[0]+","+I._private.style["text-outline-color"].value[1]+","+I._private.style["text-outline-color"].value[2]+","+(I._private.style["text-opacity"].value*I._private.style.opacity.value)+")";
if(O!=undefined){var L=2*I._private.style["text-outline-width"].value;if(L>0){i.lineWidth=L;i.strokeText(O,K,J)}if(isNaN(K)){K=0}if(isNaN(J)){J=0}i.fillText(""+O,K,J);I._private.rstyle.labelWidth=i.measureText(O).width}};w.prototype.drawBackground=function(G,F,i,H,I){};w.prototype.findEdgeControlPoints=function(N){var T={};var L=this.data.cy;var U=[];var Q;for(var O=0;O<N.length;O++){Q=N[O]._private.data.source>N[O]._private.data.target?N[O]._private.data.target+"-"+N[O]._private.data.source:N[O]._private.data.source+"-"+N[O]._private.data.target;
if(T[Q]==undefined){T[Q]=[]}T[Q].push(N[O]);U.push(Q)}var G,M;for(var I=0;I<U.length;I++){Q=U[I];G=L.getElementById(T[Q][0]._private.data.source);M=L.getElementById(T[Q][0]._private.data.target);var S=(G._private.position.x+M._private.position.x)/2;var R=(G._private.position.y+M._private.position.y)/2;var H,F;if(T[Q].length>1){H=M._private.position.y-G._private.position.y;F=G._private.position.x-M._private.position.x;var P=Math.sqrt(H*H+F*F);H/=P;F/=P}var J;for(var O=0;O<T[Q].length;O++){J=T[Q][O];
if(G._private.data.id==M._private.data.id){var V=J._private.style["control-point-step-size"].pxValue;J._private.rscratch.edgeType="self";J._private.rscratch.cp2ax=G._private.position.x;J._private.rscratch.cp2ay=G._private.position.y-(1+Math.pow(this.getNodeHeight(G),1.12)/100)*V*(O/3+1);J._private.rscratch.cp2cx=G._private.position.x-(1+Math.pow(this.getNodeWidth(G),1.12)/100)*V*(O/3+1);J._private.rscratch.cp2cy=G._private.position.y;J._private.rscratch.selfEdgeMidX=(J._private.rscratch.cp2ax+J._private.rscratch.cp2cx)/2;
J._private.rscratch.selfEdgeMidY=(J._private.rscratch.cp2ay+J._private.rscratch.cp2cy)/2}else{if(T[Q].length%2==1&&O==Math.floor(T[Q].length/2)){J._private.rscratch.edgeType="straight"}else{var V=J._private.style["control-point-step-size"].value;var K=(0.5-T[Q].length/2+O)*V;J._private.rscratch.edgeType="bezier";J._private.rscratch.cp2x=S+H*K;J._private.rscratch.cp2y=R+F*K}}}}return T};w.prototype.findEndpoints=function(H){var J;var i=H.source()[0];var N=H.target()[0];var S=Math.max(this.getNodeWidth(i),this.getNodeHeight(i));
var I=Math.max(this.getNodeWidth(N),this.getNodeHeight(N));S=0;I/=2;var G=[H.source().position().x,H.source().position().y];var K=[H.target().position().x,H.target().position().y];if(H._private.rscratch.edgeType=="self"){var P=[H._private.rscratch.cp2cx,H._private.rscratch.cp2cy];J=o[this.getNodeShape(N)].intersectLine(N._private.position.x,N._private.position.y,this.getNodeWidth(N),this.getNodeHeight(N),P[0],P[1],N._private.style["border-width"].value/2);var F=this.shortenIntersection(J,P,b[H._private.style["target-arrow-shape"].value].spacing(H));
var O=this.shortenIntersection(J,P,b[H._private.style["target-arrow-shape"].value].gap(H));H._private.rscratch.endX=O[0];H._private.rscratch.endY=O[1];H._private.rscratch.arrowEndX=F[0];H._private.rscratch.arrowEndY=F[1];var P=[H._private.rscratch.cp2ax,H._private.rscratch.cp2ay];J=o[this.getNodeShape(i)].intersectLine(i._private.position.x,i._private.position.y,this.getNodeWidth(i),this.getNodeHeight(i),P[0],P[1],i._private.style["border-width"].value/2);var L=this.shortenIntersection(J,P,b[H._private.style["source-arrow-shape"].value].spacing(H));
var M=this.shortenIntersection(J,P,b[H._private.style["source-arrow-shape"].value].gap(H));H._private.rscratch.startX=M[0];H._private.rscratch.startY=M[1];H._private.rscratch.arrowStartX=L[0];H._private.rscratch.arrowStartY=L[1]}else{if(H._private.rscratch.edgeType=="straight"){J=o[this.getNodeShape(N)].intersectLine(N._private.position.x,N._private.position.y,this.getNodeWidth(N),this.getNodeHeight(N),i.position().x,i.position().y,N._private.style["border-width"].value/2);if(J.length==0){H._private.rscratch.noArrowPlacement=true
}else{H._private.rscratch.noArrowPlacement=false}var F=this.shortenIntersection(J,[i.position().x,i.position().y],b[H._private.style["target-arrow-shape"].value].spacing(H));var O=this.shortenIntersection(J,[i.position().x,i.position().y],b[H._private.style["target-arrow-shape"].value].gap(H));H._private.rscratch.endX=O[0];H._private.rscratch.endY=O[1];H._private.rscratch.arrowEndX=F[0];H._private.rscratch.arrowEndY=F[1];J=o[this.getNodeShape(i)].intersectLine(i._private.position.x,i._private.position.y,this.getNodeWidth(i),this.getNodeHeight(i),N.position().x,N.position().y,i._private.style["border-width"].value/2);
if(J.length==0){H._private.rscratch.noArrowPlacement=true}else{H._private.rscratch.noArrowPlacement=false}var L=this.shortenIntersection(J,[N.position().x,N.position().y],b[H._private.style["source-arrow-shape"].value].spacing(H));var M=this.shortenIntersection(J,[N.position().x,N.position().y],b[H._private.style["source-arrow-shape"].value].gap(H));H._private.rscratch.startX=M[0];H._private.rscratch.startY=M[1];H._private.rscratch.arrowStartX=L[0];H._private.rscratch.arrowStartY=L[1]}else{if(H._private.rscratch.edgeType=="bezier"){var P=[H._private.rscratch.cp2x,H._private.rscratch.cp2y];
var R=G[0]*0.25+K[0]*0.25+P[0]*0.5;var Q=G[1]*0.25+K[1]*0.25+P[1]*0.5;J=o[this.getNodeShape(N)].intersectLine(N._private.position.x,N._private.position.y,this.getNodeWidth(N),this.getNodeHeight(N),P[0],P[1],N._private.style["border-width"].value/2);var F=this.shortenIntersection(J,P,b[H._private.style["target-arrow-shape"].value].spacing(H));var O=this.shortenIntersection(J,P,b[H._private.style["target-arrow-shape"].value].gap(H));H._private.rscratch.endX=O[0];H._private.rscratch.endY=O[1];H._private.rscratch.arrowEndX=F[0];
H._private.rscratch.arrowEndY=F[1];J=o[this.getNodeShape(i)].intersectLine(i._private.position.x,i._private.position.y,this.getNodeWidth(i),this.getNodeHeight(i),P[0],P[1],i._private.style["border-width"].value/2);var L=this.shortenIntersection(J,P,b[H._private.style["source-arrow-shape"].value].spacing(H));var M=this.shortenIntersection(J,P,b[H._private.style["source-arrow-shape"].value].gap(H));H._private.rscratch.startX=M[0];H._private.rscratch.startY=M[1];H._private.rscratch.arrowStartX=L[0];
H._private.rscratch.arrowStartY=L[1]}else{if(H._private.rscratch.isArcEdge){return}}}}};w.prototype.findEdges=function(J){var G=this.getCachedEdges();var I={};var F=[];for(var H=0;H<J.length;H++){I[J[H]._private.data.id]=J[H]}for(var H=0;H<G.length;H++){if(I[G[H]._private.data.source]||I[G[H]._private.data.target]){F.push(G[H])}}return F};w.prototype.intersectLineEllipse=function(O,N,K,J,H,F){var G=K-O;var i=J-N;G/=H;i/=F;var L=Math.sqrt(G*G+i*i);var I=L-1;if(I<0){return[]}var M=I/L;return[(K-O)*M+O,(J-N)*M+N]
};w.prototype.findCircleNearPoint=function(I,H,J,N,M){var F=N-I;var i=M-H;var G=Math.sqrt(F*F+i*i);var L=F/G;var K=i/G;return[I+L*J,H+K*J]};w.prototype.findMaxSqDistanceToOrigin=function(I){var F=0.000001;var H;for(var G=0;G<I.length/2;G++){H=I[G*2]*I[G*2]+I[G*2+1]*I[G*2+1];if(H>F){F=H}}return F};w.prototype.finiteLinesIntersect=function(J,Q,H,O,F,N,R,L,P){var K=(R-F)*(Q-N)-(L-N)*(J-F);var M=(H-J)*(Q-N)-(O-Q)*(J-F);var I=(L-N)*(H-J)-(R-F)*(O-Q);if(I!=0){var G=K/I;var i=M/I;if(0<=G&&G<=1&&0<=i&&i<=1){return[J+G*(H-J),Q+G*(O-Q)]
}else{if(!P){return[]}else{return[J+G*(H-J),Q+G*(O-Q)]}}}else{if(K==0||M==0){if([J,H,R].sort()[1]==R){return[R,L]}if([J,H,F].sort()[1]==F){return[F,N]}if([F,R,H].sort()[1]==H){return[H,O]}return[]}else{return[]}}};w.prototype.boxIntersectEllipse=function(G,P,i,N,O,F,S,K,I){if(i<G){var H=G;G=i;i=H}if(N<P){var Q=P;P=N;N=Q}var M=[K-F/2-O,I];var R=[K+F/2+O,I];var J=[K,I+S/2+O];var L=[K,I-S/2-O];if(i<M[0]){return false}if(G>R[0]){return false}if(N<L[1]){return false}if(P>J[1]){return false}if(G<=M[0]&&M[0]<=i&&P<=M[1]&&M[1]<=N){return true
}if(G<=R[0]&&R[0]<=i&&P<=R[1]&&R[1]<=N){return true}if(G<=J[0]&&J[0]<=i&&P<=J[1]&&J[1]<=N){return true}if(G<=L[0]&&L[0]<=i&&P<=L[1]&&L[1]<=N){return true}G=(G-K)/(F+O);i=(i-K)/(F+O);P=(P-I)/(S+O);N=(N-I)/(S+O);if(G*G+P*P<=1){return true}if(i*i+P*P<=1){return true}if(i*i+N*N<=1){return true}if(G*G+N*N<=1){return true}return false};w.prototype.boxIntersectPolygon=function(V,J,T,H,K,Q,P,N,L,X,O){if(T<V){var U=V;V=T;T=U}if(H<J){var I=J;J=H;H=I}var M=new Array(K.length);var W=Math.asin(X[1]/(Math.sqrt(X[0]*X[0]+X[1]*X[1])));
if(X[0]<0){W=W+Math.PI/2}else{W=-W-Math.PI/2}var G=Math.cos(-W);var F=Math.sin(-W);for(var S=0;S<M.length/2;S++){M[S*2]=Q*(K[S*2]*G-K[S*2+1]*F);M[S*2+1]=P*(K[S*2+1]*G+K[S*2]*F);M[S*2]+=N;M[S*2+1]+=L}var R;if(O>0){var Y=A.expandPolygon(M,-O);R=A.joinLines(Y)}else{R=M}for(var S=0;S<M.length/2;S++){if(V<=M[S*2]&&M[S*2]<=T){if(J<=M[S*2+1]&&M[S*2+1]<=H){return true}}}if(A.pointInsidePolygon(V,J,R,0,0,1,1,0,X)){return true}else{if(A.pointInsidePolygon(V,H,R,0,0,1,1,0,X)){return true}else{if(A.pointInsidePolygon(T,H,R,0,0,1,1,0,X)){return true
}else{if(A.pointInsidePolygon(T,J,R,0,0,1,1,0,X)){return true}}}}return false};w.prototype.polygonIntersectLine=function(L,K,F,I,G,O,N,M){var W=[];var J;var H=new Array(F.length);for(var S=0;S<H.length/2;S++){H[S*2]=F[S*2]*O+I;H[S*2+1]=F[S*2+1]*N+G}var R;if(M>0){var V=A.expandPolygon(H,-M);R=A.joinLines(V)}else{R=H}var Q,P,U,T;for(var S=0;S<R.length/2;S++){Q=R[S*2];P=R[S*2+1];if(S<R.length/2-1){U=R[(S+1)*2];T=R[(S+1)*2+1]}else{U=R[0];T=R[1]}J=this.finiteLinesIntersect(L,K,I,G,Q,P,U,T);if(J.length!=0){W.push(J[0],J[1])
}}return W};w.prototype.shortenIntersection=function(J,I,G){var i=[J[0]-I[0],J[1]-I[1]];var H=Math.sqrt(i[0]*i[0]+i[1]*i[1]);var F=(H-G)/H;if(F<0){return[]}else{return[I[0]+F*i[0],I[1]+F*i[1]]}};b.arrow={_points:[-0.15,-0.3,0,0,0.15,-0.3],collide:function(K,H,G,F,i,M,I,J){var L=b.arrow._points;return c.pointInsidePolygon(K,H,L,G,F,i,M,I,J)},roughCollide:function(F,L,K,H,G,i,J,I){if(typeof(b.arrow._farthestPointSqDistance)=="undefined"){b.arrow._farthestPointSqDistance=c.findMaxSqDistanceToOrigin(b.arrow._points)
}return c.checkInBoundingCircle(F,L,b.arrow._farthestPointSqDistance,0,G,i,K,H)},draw:function(G){var H=b.arrow._points;for(var F=0;F<H.length/2;F++){G.lineTo(H[F*2],H[F*2+1])}},spacing:function(i){return 0},gap:function(i){return i._private.style.width.value*2}};b.triangle=b.arrow;b.none={collide:function(F,L,K,H,G,i,J,I){return false},roughCollide:function(F,L,K,H,G,i,J,I){return false},draw:function(i){},spacing:function(i){return 0},gap:function(i){return 0}};b.circle={_baseRadius:0.15,collide:function(N,K,H,G,i,Q,L,M){if(i!=Q){var F=Math.asin(L[1]/(Math.sqrt(L[0]*L[0]+L[1]*L[1])));
var P=Math.cos(-F);var J=Math.sin(-F);var I=[N*P-K*J,K*P+N*J];var O=(Q+M)/(i+M);K/=O;G/=O;return(Math.pow(H-N,2)+Math.pow(G-K,2)<=Math.pow((i+M)*b.circle._baseRadius,2))}else{return(Math.pow(H-N,2)+Math.pow(G-K,2)<=Math.pow((i+M)*b.circle._baseRadius,2))}},roughCollide:function(F,L,K,H,G,i,J,I){return true},draw:function(i){i.arc(0,0,b.circle._baseRadius,0,Math.PI*2,false)},spacing:function(i){return c.getArrowWidth(i._private.style.width.value)*b.circle._baseRadius},gap:function(i){return i._private.style.width.value*2
}};b.inhibitor={_points:[-0.25,0,-0.25,-0.1,0.25,-0.1,0.25,0],collide:function(K,H,G,F,i,M,I,J){var L=b.inhibitor._points;return c.pointInsidePolygon(K,H,L,G,F,i,M,I,J)},roughCollide:function(F,L,K,H,G,i,J,I){if(typeof(b.inhibitor._farthestPointSqDistance)=="undefined"){b.inhibitor._farthestPointSqDistance=c.findMaxSqDistanceToOrigin(b.inhibitor._points)}return c.checkInBoundingCircle(F,L,b.inhibitor._farthestPointSqDistance,0,G,i,K,H)},draw:function(G){var H=b.inhibitor._points;for(var F=0;F<H.length/2;
F++){G.lineTo(H[F*2],H[F*2+1])}},spacing:function(i){return 4},gap:function(i){return 4}};b.square={_points:[-0.12,0,0.12,0,0.12,-0.24,-0.12,-0.24],collide:function(K,H,G,F,i,M,I,J){var L=b.square._points;return c.pointInsidePolygon(K,H,L,G,F,i,M,I,J)},roughCollide:function(F,L,K,H,G,i,J,I){if(typeof(b.square._farthestPointSqDistance)=="undefined"){b.square._farthestPointSqDistance=c.findMaxSqDistanceToOrigin(b.square._points)}return c.checkInBoundingCircle(F,L,b.square._farthestPointSqDistance,0,G,i,K,H)
},draw:function(G){var H=b.square._points;for(var F=0;F<H.length/2;F++){G.lineTo(H[F*2],H[F*2+1])}},spacing:function(i){return 0},gap:function(i){return i._private.style.width.value*2}};b.diamond={_points:[-0.14,-0.14,0,-0.28,0.14,-0.14,0,0],collide:function(K,H,G,F,i,M,I,J){var L=b.diamond._points;return c.pointInsidePolygon(K,H,L,G,F,i,M,I,J)},roughCollide:function(F,L,K,H,G,i,J,I){if(typeof(b.diamond._farthestPointSqDistance)=="undefined"){b.diamond._farthestPointSqDistance=c.findMaxSqDistanceToOrigin(b.diamond._points)
}return c.checkInBoundingCircle(F,L,b.diamond._farthestPointSqDistance,0,G,i,K,H)},draw:function(i){i.lineTo(-0.14,-0.14);i.lineTo(0,-0.28);i.lineTo(0.14,-0.14);i.lineTo(0,0)},spacing:function(i){return 0},gap:function(i){return i._private.style.width.value*2}};b.tee=b.inhibitor;w.prototype.getArrowWidth=function(i){return Math.max(Math.pow(i*13.37,0.9),29)};w.prototype.getArrowHeight=function(i){return Math.max(Math.pow(i*13.37,0.9),29)};w.prototype.drawArrowheads=function(F,H,I){if(I){return}var G,i;
var K=H._private.rscratch.arrowStartX;var J=H._private.rscratch.arrowStartY;G=K-H.source().position().x;i=J-H.source().position().y;F.fillStyle="rgba("+H._private.style["source-arrow-color"].value[0]+","+H._private.style["source-arrow-color"].value[1]+","+H._private.style["source-arrow-color"].value[2]+","+H._private.style.opacity.value+")";F.lineWidth=H._private.style.width.value;this.drawArrowShape(F,H._private.style["source-arrow-shape"].value,K,J,G,i);var M=H._private.rscratch.arrowEndX;var L=H._private.rscratch.arrowEndY;
G=M-H.target().position().x;i=L-H.target().position().y;F.fillStyle="rgba("+H._private.style["target-arrow-color"].value[0]+","+H._private.style["target-arrow-color"].value[1]+","+H._private.style["target-arrow-color"].value[2]+","+H._private.style.opacity.value+")";F.lineWidth=H._private.style.width.value;this.drawArrowShape(F,H._private.style["target-arrow-shape"].value,M,L,G,i)};w.prototype.drawArrowShape=function(J,F,i,L,I,G){var K=Math.asin(G/(Math.sqrt(I*I+G*G)));if(I<0){K=K+Math.PI/2}else{K=-(Math.PI/2+K)
}J.save();J.translate(i,L);J.moveTo(0,0);J.rotate(-K);var H=this.getArrowWidth(J.lineWidth);J.scale(H,H);J.beginPath();b[F].draw(J);J.closePath();J.fill();J.restore()};var j=function(G,F){var L=1/G*2*Math.PI;var J=G%2==0?Math.PI/2+L/2:Math.PI/2;J+=F;var N=new Array(G*2);var H;for(var I=0;I<G;I++){H=I*L+J;N[2*I]=Math.cos(H);N[2*I+1]=Math.sin(-H)}var P=0;var O=0;for(var I=0;I<N.length/2;I++){if(Math.abs(N[2*I]>P)){P=Math.abs(N[2*I])}if(Math.abs(N[2*I+1]>O)){O=Math.abs(N[2*I+1])}}var K=0.0005;var M=Math.max(P,O);
for(var I=0;I<N.length/2;I++){if(M>K){N[2*I]*=(1/M);N[2*I+1]*=(1/M)}}return N};var A=c;o.ellipse={draw:function(F,I,H,G,i){o.ellipse.drawPath(F,I,H,G,i);F.fill()},drawPath:function(F,I,H,G,i){F.beginPath();F.save();F.translate(I,H);F.scale(G/2,i/2);F.arc(0,0,1,0,Math.PI*2,false);F.closePath();F.restore()},intersectLine:function(J,I,H,G,F,L,K){var i=c.intersectLineEllipse(F,L,J,I,H/2+K,G/2+K);return i},intersectBox:function(G,L,i,J,F,M,I,H,K){return w.prototype.boxIntersectEllipse(G,L,i,J,K,F,M,I,H)
},checkPointRough:function(F,K,J,G,i,I,H){return true},checkPoint:function(F,K,J,G,i,I,H){F-=I;K-=H;F/=(G+J);K/=(i+J);return(Math.pow(F,2)+Math.pow(K,2)<=1)}};o.triangle={points:j(3,0),draw:function(F,I,H,G,i){A.drawPolygon(F,I,H,G,i,o.triangle.points)},drawPath:function(F,I,H,G,i){A.drawPolygonPath(F,I,H,G,i,o.triangle.points)},intersectLine:function(I,H,G,F,i,K,J){return A.polygonIntersectLine(i,K,o.triangle.points,I,H,G/2,F/2,J)},intersectBox:function(G,L,i,J,F,N,I,H,K){var M=o.triangle.points;
return A.boxIntersectPolygon(G,L,i,J,M,F,N,I,H,[0,-1],K)},checkPointRough:function(F,K,J,G,i,I,H){return A.checkInBoundingBox(F,K,o.triangle.points,J,G,i,I,H)},checkPoint:function(F,K,J,G,i,I,H){return A.pointInsidePolygon(F,K,o.triangle.points,I,H,G,i,[0,-1],J)}};o.square={points:j(4,0),draw:function(F,I,H,G,i){A.drawPolygon(F,I,H,G,i,o.square.points)},drawPath:function(F,I,H,G,i){A.drawPolygonPath(F,I,H,G,i,o.square.points)},intersectLine:function(I,H,G,F,i,K,J){return A.polygonIntersectLine(i,K,o.square.points,I,H,G/2,F/2,J)
},intersectBox:function(G,L,i,J,F,N,I,H,K){var M=o.square.points;return A.boxIntersectPolygon(G,L,i,J,M,F,N,I,H,[0,-1],K)},checkPointRough:function(F,K,J,G,i,I,H){return A.checkInBoundingBox(F,K,o.square.points,J,G,i,I,H)},checkPoint:function(F,K,J,G,i,I,H){return A.pointInsidePolygon(F,K,o.square.points,I,H,G,i,[0,-1],J)}};o.rectangle=o.square;o.octogon={};o.roundrectangle=o.square;o.roundrectangle2={roundness:4.99,draw:function(G,F,i){if(F<=roundness*2){return}A.drawPolygon(G._private.position.x,G._private.position.y,F,i,nodeSapes.roundrectangle2.points)
},intersectLine:function(H,G,F,i,I){return A.findPolygonIntersection(H,G,F,i,I,o.square.points)},intersectBox:function(G,L,i,J,F,N,I,H,K){var M=o.square.points}};o.pentagon={points:j(5,0),draw:function(F,I,H,G,i){A.drawPolygon(F,I,H,G,i,o.pentagon.points)},drawPath:function(F,I,H,G,i){A.drawPolygonPath(F,I,H,G,i,o.pentagon.points)},intersectLine:function(I,H,G,F,i,K,J){return A.polygonIntersectLine(i,K,o.pentagon.points,I,H,G/2,F/2,J)},intersectBox:function(G,L,i,J,F,N,I,H,K){var M=o.pentagon.points;
return A.boxIntersectPolygon(G,L,i,J,M,F,N,I,H,[0,-1],K)},checkPointRough:function(F,K,J,G,i,I,H){return A.checkInBoundingBox(F,K,o.pentagon.points,J,G,i,I,H)},checkPoint:function(F,K,J,G,i,I,H){return A.pointInsidePolygon(F,K,o.pentagon.points,I,H,G,i,[0,-1],J)}};o.hexagon={points:j(6,0),draw:function(F,I,H,G,i){A.drawPolygon(F,I,H,G,i,o.hexagon.points)},drawPath:function(F,I,H,G,i){A.drawPolygonPath(F,I,H,G,i,o.hexagon.points)},intersectLine:function(I,H,G,F,i,K,J){return A.polygonIntersectLine(i,K,o.hexagon.points,I,H,G/2,F/2,J)
},intersectBox:function(G,L,i,J,F,N,I,H,K){var M=o.hexagon.points;return A.boxIntersectPolygon(G,L,i,J,M,F,N,I,H,[0,-1],K)},checkPointRough:function(F,K,J,G,i,I,H){return A.checkInBoundingBox(F,K,o.hexagon.points,J,G,i,I,H)},checkPoint:function(F,K,J,G,i,I,H){return A.pointInsidePolygon(F,K,o.hexagon.points,I,H,G,i,[0,-1],J)}};o.heptagon={points:j(7,0),draw:function(F,I,H,G,i){A.drawPolygon(F,I,H,G,i,o.heptagon.points)},drawPath:function(F,I,H,G,i){A.drawPolygonPath(F,I,H,G,i,o.heptagon.points)},intersectLine:function(I,H,G,F,i,K,J){return A.polygonIntersectLine(i,K,o.heptagon.points,I,H,G/2,F/2,J)
},intersectBox:function(G,L,i,J,F,N,I,H,K){var M=o.heptagon.points;return A.boxIntersectPolygon(G,L,i,J,M,F,N,I,H,[0,-1],K)},checkPointRough:function(F,K,J,G,i,I,H){return A.checkInBoundingBox(F,K,o.heptagon.points,J,G,i,I,H)},checkPoint:function(F,K,J,G,i,I,H){return A.pointInsidePolygon(F,K,o.heptagon.points,I,H,G,i,[0,-1],J)}};o.octagon={points:j(8,0),draw:function(F,I,H,G,i){A.drawPolygon(F,I,H,G,i,o.octagon.points)},drawPath:function(F,I,H,G,i){A.drawPolygonPath(F,I,H,G,i,o.octagon.points)},intersectLine:function(I,H,G,F,i,K,J){return A.polygonIntersectLine(i,K,o.octagon.points,I,H,G/2,F/2,J)
},intersectBox:function(G,L,i,J,F,N,I,H,K){var M=o.octagon.points;return A.boxIntersectPolygon(G,L,i,J,M,F,N,I,H,[0,-1],K)},checkPointRough:function(F,K,J,G,i,I,H){return A.checkInBoundingBox(F,K,o.octagon.points,J,G,i,I,H)},checkPoint:function(F,K,J,G,i,I,H){return A.pointInsidePolygon(F,K,o.octagon.points,I,H,G,i,[0,-1],J)}};var h=new Array(20);var f=j(5,0);var D=j(5,Math.PI/5);var m=0.5*(3-Math.sqrt(5));m*=1.57;for(var C=0;C<D.length/2;C++){D[C*2]*=m;D[C*2+1]*=m}for(var C=0;C<20/4;C++){h[C*4]=f[C*2];
h[C*4+1]=f[C*2+1];h[C*4+2]=D[C*2];h[C*4+3]=D[C*2+1]}o.star5={points:h,draw:function(F,I,H,G,i){A.drawPolygon(F,I,H,G,i,o.star5.points)},drawPath:function(F,I,H,G,i){A.drawPolygonPath(F,I,H,G,i,o.star5.points)},intersectLine:function(I,H,G,F,i,K,J){return A.polygonIntersectLine(i,K,o.star5.points,I,H,G/2,F/2,J)},intersectBox:function(G,L,i,J,F,N,I,H,K){var M=o.star5.points;return A.boxIntersectPolygon(G,L,i,J,M,F,N,I,H,[0,-1],K)},checkPointRough:function(F,K,J,G,i,I,H){return A.checkInBoundingBox(F,K,o.star5.points,J,G,i,I,H)
},checkPoint:function(F,K,J,G,i,I,H){return A.pointInsidePolygon(F,K,o.star5.points,I,H,G,i,[0,-1],J)}};w.prototype.expandPolygon=function(R,H){var I=new Array(R.length*2);var Q,P,O,N;for(var L=0;L<R.length/2;L++){Q=R[L*2];P=R[L*2+1];if(L<R.length/2-1){O=R[(L+1)*2];N=R[(L+1)*2+1]}else{O=R[0];N=R[1]}var M=(N-P);var K=-(O-Q);var J=Math.sqrt(M*M+K*K);var G=M/J;var F=K/J;I[L*4]=Q+G*H;I[L*4+1]=P+F*H;I[L*4+2]=O+G*H;I[L*4+3]=N+F*H}return I};w.prototype.joinLines=function(O){var N=new Array(O.length/2);var Q,P,M,L;
var J,I,H,G;for(var K=0;K<O.length/4;K++){Q=O[K*4];P=O[K*4+1];M=O[K*4+2];L=O[K*4+3];if(K<O.length/4-1){J=O[(K+1)*4];I=O[(K+1)*4+1];H=O[(K+1)*4+2];G=O[(K+1)*4+3]}else{J=O[0];I=O[1];H=O[2];G=O[3]}var F=this.finiteLinesIntersect(Q,P,M,L,J,I,H,G,true);N[K*2]=F[0];N[K*2+1]=F[1]}return N};w.prototype.pointInsidePolygon=function(Q,O,K,N,L,U,T,aa,S){var M=new Array(K.length);var Z=Math.asin(aa[1]/(Math.sqrt(aa[0]*aa[0]+aa[1]*aa[1])));if(aa[0]<0){Z=Z+Math.PI/2}else{Z=-Z-Math.PI/2}var H=Math.cos(-Z);var F=Math.sin(-Z);
for(var W=0;W<M.length/2;W++){M[W*2]=U*(K[W*2]*H-K[W*2+1]*F);M[W*2+1]=T*(K[W*2+1]*H+K[W*2]*F);M[W*2]+=N;M[W*2+1]+=L}var V;if(S>0){var ab=A.expandPolygon(M,-S);V=A.joinLines(ab)}else{V=M}var Y,J,X,I;var G;var P=0;var R=0;for(var W=0;W<V.length/2;W++){Y=V[W*2];J=V[W*2+1];if(W+1<V.length/2){X=V[(W+1)*2];I=V[(W+1)*2+1]}else{X=V[(W+1-V.length/2)*2];I=V[(W+1-V.length/2)*2+1]}if(Y==Q&&X==Q){}else{if((Y>=Q&&Q>=X)||(Y<=Q&&Q<=X)){G=(Q-Y)/(X-Y)*(I-J)+J;if(G>O){P++}if(G<O){R++}}else{continue}}}if(P%2==0){return false
}else{return true}};w.prototype.drawPolygonPath=function(I,G,L,K,F,J){I.save();I.translate(G,L);I.beginPath();I.scale(K/2,F/2);I.moveTo(J[0],J[1]);for(var H=1;H<J.length/2;H++){I.lineTo(J[H*2],J[H*2+1])}I.closePath();I.restore()};w.prototype.drawPolygon=function(G,F,J,I,i,H){this.drawPolygonPath(G,F,J,I,i,H);G.fill()};w.prototype.checkInBoundingCircle=function(F,L,G,K,H,i,J,I){F=(F-J)/(H+K);L=(L-I)/(i+K);return(F*F+L*L)<=G};w.prototype.checkInBoundingBox=function(P,N,Q,O,H,R,L,K){var J=Q[0],I=Q[1];
var G=Q[0],F=Q[1];for(var M=1;M<Q.length/2;M++){if(Q[M*2]<J){J=Q[M*2]}else{if(Q[M*2]>G){G=Q[M*2]}}if(Q[M*2+1]<I){I=Q[M*2+1]}else{if(Q[M*2+1]>F){F=Q[M*2+1]}}}P-=L;N-=K;P/=H;N/=R;if(P<J){return false}else{if(P>G){return false}}if(N<I){return false}else{if(N>F){return false}}return true};w.prototype.boxInBezierVicinity=function(P,L,V,M,R,K,Q,J,N,H,O){var W=Math.min(P,V)-O;var T=Math.min(L,M)-O;var U=Math.max(P,V)+O;var S=Math.max(L,M)+O;if(R>=W&&R<=U&&K>=T&&K<=S){return 2}else{if(N>=W&&N<=U&&H>=T&&H<=S){return 2
}else{if(Q>=W&&Q<=U&&J>=T&&J<=S){return 1}}}var I=Math.min(R,Q,N);var G=Math.min(K,J,H);var F=Math.max(R,Q,N);var i=Math.max(K,J,H);if(I>U||F<W||G>S||i<T){return 0}return 1};w.prototype.checkStraightEdgeCrossesBox=function(S,M,Z,P,U,I,T,F,R){var aa=Math.min(S,Z)-R;var X=Math.min(M,P)-R;var Y=Math.max(S,Z)+R;var W=Math.max(M,P)+R;var J=T-U;var O=U;var V;var G=F-I;var N=I;var Q;if(Math.abs(J)<0.0001){return(U>=aa&&U<=Y&&Math.min(I,F)<=X&&Math.max(I,F)>=W)}var i=(aa-O)/J;if(i>0&&i<=1){V=G*i+N;if(V>=X&&V<=W){return true
}}var L=(Y-O)/J;if(L>0&&L<=1){V=G*L+N;if(V>=X&&V<=W){return true}}var H=(X-N)/G;if(H>0&&H<=1){Q=J*H+O;if(Q>=aa&&Q<=Y){return true}}var K=(W-N)/G;if(K>0&&K<=1){Q=J*K+O;if(Q>=aa&&Q<=Y){return true}}return false};w.prototype.checkBezierCrossesBox=function(X,J,ak,Z,R,au,N,aq,L,an,F){var T=Math.min(X,ak)-F;var Q=Math.min(J,Z)-F;var at=Math.max(X,ak)+F;var ap=Math.max(J,Z)+F;if(R>=T&&R<=at&&au>=Q&&au<=ap){return true}else{if(L>=T&&L<=at&&an>=Q&&an<=ap){return true}}var W=R-2*N+L;var I=-2*R+2*N;var ag=R;
var aa=[];if(Math.abs(W)<0.0001){var al=(T-R)/I;var Y=(at-R)/I;aa.push(al,Y)}else{var ad=I*I-4*W*(ag-T);var U,S;if(ad>0){var ae=Math.sqrt(ad);U=(-I+ae)/(2*W);S=(-I-ae)/(2*W);aa.push(U,S)}var ac=I*I-4*W*(ag-at);var P,M;if(ac>0){var ae=Math.sqrt(ac);P=(-I+ae)/(2*W);M=(-I-ae)/(2*W);aa.push(P,M)}}aa.sort(function(aw,av){return aw-av});var V=au-2*aq+an;var G=-2*au+2*aq;var af=au;var aj=[];if(Math.abs(V)<0.0001){var H=(Q-au)/G;var ah=(ap-au)/G;aj.push(H,ah)}else{var O=G*G-4*V*(af-Q);var i,ar;if(O>0){var ae=Math.sqrt(O);
i=(-G+ae)/(2*V);ar=(-G-ae)/(2*V);aj.push(i,ar)}var K=G*G-4*V*(af-ap);var ao,am;if(K>0){var ae=Math.sqrt(K);ao=(-G+ae)/(2*V);am=(-G-ae)/(2*V);aj.push(ao,am)}}aj.sort(function(aw,av){return aw-av});for(var ab=0;ab<aa.length;ab+=2){for(var ai=1;ai<aj.length;ai+=2){if(aa[ab]<aj[ai]&&aj[ai]>=0&&aa[ab]<=1&&aa[ab+1]>aj[ai-1]&&aj[ai-1]<=1&&aa[ab+1]>=0){return true}}}return false};w.prototype.inBezierVicinity=function(K,J,U,G,T,F,S,i,Z){var M=0.25*U+0.5*T+0.25*S;var L=0.25*G+0.5*F+0.25*i;var P,O,X,V;var I,H,N;
var Y=function(ab,ah,ac,aa,ag,af,ad,ae){I=(af-aa)*(ab-ac)+(ac-ag)*(ah-aa);H=I*I;sideSquared=(af-aa)*(af-aa)+(ac-ag)*(ac-ag);if(ae){if(I>0){return false}}else{if(I<0){return false}}return(H/sideSquared>ad)};var R=(M+T)/2;var Q=(L+F)/2;var W=true;if(Y(R,Q,U,G,T,F,0,W)){W=!W}return(!Y(K,J,U,G,T,F,Z,W)&&!Y(K,J,T,F,S,i,Z,W)&&!Y(K,J,S,i,U,G,Z,W))};w.prototype.solveCubic=function(M,L,K,J,Q){L/=M;K/=M;J/=M;var O,F,i,H,P,N,G,I;F=(3*K-(L*L))/9;i=-(27*J)+L*(9*K-2*(L*L));i/=54;O=F*F*F+i*i;Q[1]=0;G=(L/3);if(O>0){P=i+Math.sqrt(O);
P=((P<0)?-Math.pow(-P,(1/3)):Math.pow(P,(1/3)));N=i-Math.sqrt(O);N=((N<0)?-Math.pow(-N,(1/3)):Math.pow(N,(1/3)));Q[0]=-G+P+N;G+=(P+N)/2;Q[4]=Q[2]=-G;G=Math.sqrt(3)*(-N+P)/2;Q[3]=G;Q[5]=-G;return}Q[5]=Q[3]=0;if(O==0){I=((i<0)?-Math.pow(-i,(1/3)):Math.pow(i,(1/3)));Q[0]=-G+2*I;Q[4]=Q[2]=-(I+G);return}F=-F;H=F*F*F;H=Math.acos(i/Math.sqrt(H));I=2*Math.sqrt(F);Q[0]=-G+I*Math.cos(H/3);Q[2]=-G+I*Math.cos((H+2*Math.PI)/3);Q[4]=-G+I*Math.cos((H+4*Math.PI)/3);return};w.prototype.sqDistanceToQuadraticBezier=function(P,O,U,J,T,I,S,H){var aa=1*U*U-4*U*T+2*U*S+4*T*T-4*T*S+S*S+J*J-4*J*I+2*J*H+4*I*I-4*I*H+H*H;
var Z=1*9*U*T-3*U*U-3*U*S-6*T*T+3*T*S+9*J*I-3*J*J-3*J*H-6*I*I+3*I*H;var X=1*3*U*U-6*U*T+U*S-U*P+2*T*T+2*T*P-S*P+3*J*J-6*J*I+J*H-J*O+2*I*I+2*I*O-H*O;var W=1*U*T-U*U+U*P-T*P+J*I-J*J+J*O-I*O;z("coefficients: "+aa/aa+", "+Z/aa+", "+X/aa+", "+W/aa);var K=[];this.solveCubic(aa,Z,X,W,K);var F=1e-7;var Y=[];for(var L=0;L<6;L+=2){if(Math.abs(K[L+1])<F&&K[L]>=0&&K[L]<=1){Y.push(K[L])}}Y.push(1);Y.push(0);var G=-1;var V;var N,M,Q;for(var R=0;R<Y.length;R++){N=Math.pow(1-Y[R],2)*U+2*(1-Y[R])*Y[R]*T+Y[R]*Y[R]*S;
M=Math.pow(1-Y[R],2)*J+2*(1-Y[R])*Y[R]*I+Y[R]*Y[R]*H;Q=Math.pow(N-P,2)+Math.pow(M-O,2);z("distance for param "+Y[R]+": "+Math.sqrt(Q));if(G>=0){if(Q<G){G=Q;V=Y[R]}}else{G=Q;V=Y[R]}}z("given: ( "+P+", "+O+"), ( "+U+", "+J+"), ( "+T+", "+I+"), ( "+S+", "+H+")");z("roots: "+K);z("params: "+Y);z("closest param: "+V);return G};w.prototype.sqDistanceToFiniteLine=function(M,J,F,L,i,K){var G=[M-F,J-L];var P=[i-F,K-L];var H=P[0]*P[0]+P[1]*P[1];var O=G[0]*G[0]+G[1]*G[1];var N=G[0]*P[0]+G[1]*P[1];var I=N*N/H;
if(N<0){return O}if(I>H){return(M-i)*(M-i)+(J-K)*(J-K)}return(O-I)};var z=function(){};v("renderer","canvas",w)})(cytoscape);(function(a){var b={ready:function(){},stop:function(){}};function c(d){this.options=a.util.extend(true,{},b,d)}c.prototype.run=function(){var d=this.options;var e=d.cy;e.nodes().positions(function(){return{x:0,y:0}});e.one("layoutready",d.ready);e.trigger("layoutready");e.one("layoutstop",d.stop);e.trigger("layoutstop")};c.prototype.stop=function(){var d=this.options;cy.one("layoutstop",d.stop);
