...
}return{x:e.x+ab*Math.cos(h),y:e.y+ab*Math.sin(h)}}else{return{x:(w+1)*ae,y:(j+1)*ad}}});if(g.fit){D.fit(G.padding)}D.one("layoutready",g.ready);D.trigger("layoutready");D.one("layoutstop",g.stop);D.trigger("layoutstop")};c.prototype.stop=function(){};a("layout","breadthfirst",c)})(cytoscape);(function(h){var f={ready:function(){},stop:function(){},numIter:100,refresh:0,fit:true,randomize:false,debug:false,nodeRepulsion:10000,nodeOverlap:10,idealEdgeLength:10,edgeElasticity:10000,nestingFactor:10,gravity:10,initialTemp:200,coolingFactor:0.95,minTemp:1};
function a(A){this.options=h.util.extend(true,{},f,A)}a.prototype.run=function(){var B=this.options;var F=B.cy;if(true==B.debug){DEBUG=true}else{DEBUG=false}var D=new Date();var E=o(F,B);if(DEBUG){i(E)}if(true==B.randomize){u(E,F);if(0<B.refresh){b(E,F,B)}}for(var C=0;C<B.numIter;C++){e(E,F,B,C);if(0<B.refresh&&0==(C%B.refresh)){b(E,F,B)}E.temperature=E.temperature*B.coolingFactor;r("New temperature: "+E.temperature);if(E.temperature<B.minTemp){r("Temperature drop below minimum threshold. Stopping computation in step "+C);
break}}b(E,F,B);if(true==B.fit){F.fit()}var A=new Date();console.info("Layout took "+(A-D)+" ms");F.one("layoutstop",B.stop);F.trigger("layoutstop")};a.prototype.stop=function(){var A=this.options;cy.one("layoutstop",A.stop);cy.trigger("layoutstop")};function o(C,D){var N={layoutNodes:[],idToIndex:{},nodeSize:C.nodes().size(),graphSet:[],indexToGraph:[],layoutEdges:[],edgeSize:C.edges().size(),temperature:D.initialTemp};var P=C.nodes();for(var W=0;W<N.nodeSize;W++){var X={};X.id=P[W].data("id");X.parentId=P[W].data("parent");
X.children=[];X.positionX=P[W].position("x");X.positionY=P[W].position("y");X.offsetX=0;X.offsetY=0;X.height=P[W].height();X.width=P[W].width();X.maxX=X.positionX+X.width/2;X.minX=X.positionX-X.width/2;X.maxY=X.positionY+X.height/2;X.minY=X.positionY-X.height/2;X.padLeft=P[W]._private.style["padding-left"].pxValue;X.padRight=P[W]._private.style["padding-right"].pxValue;X.padTop=P[W]._private.style["padding-top"].pxValue;X.padBottom=P[W]._private.style["padding-bottom"].pxValue;N.layoutNodes.push(X);
N.idToIndex[X.id]=W}var R=[];var G=0;var E=-1;var J=[];for(var W=0;W<N.nodeSize;W++){var O=N.layoutNodes[W];var Z=O.parentId;if(undefined!=Z){N.layoutNodes[N.idToIndex[Z]].children.push(O.id)}else{R[++E]=O.id;J.push(O.id)}}N.graphSet.push(J);while(G<=E){var ab=R[G++];var M=N.idToIndex[ab];var Q=N.layoutNodes[M];var F=Q.children;if(F.length>0){N.graphSet.push(F);for(var W=0;W<F.length;W++){R[++E]=F[W]}}}for(var W=0;W<N.graphSet.length;W++){var A=N.graphSet[W];for(var U=0;U<A.length;U++){var I=N.idToIndex[A[U]];
N.indexToGraph[I]=W}}var B=C.edges();for(var W=0;W<N.edgeSize;W++){var Y=B[W];var L={};L.id=Y.data("id");L.sourceId=Y.data("source");L.targetId=Y.data("target");var H=D.idealEdgeLength;var aa=N.idToIndex[L.sourceId];var T=N.idToIndex[L.targetId];var ad=N.indexToGraph[aa];var S=N.indexToGraph[T];if(ad!=S){var K=k(L.sourceId,L.targetId,N);var V=N.graphSet[K];var ac=0;var X=N.layoutNodes[aa];while(-1==$.inArray(X.id,V)){X=N.layoutNodes[N.idToIndex[X.parentId]];ac++}X=N.layoutNodes[T];while(-1==$.inArray(X.id,V)){X=N.layoutNodes[N.idToIndex[X.parentId]];
ac++}r("LCA of nodes "+L.sourceId+" and "+L.targetId+". Index: "+K+" Contents: "+V.toString()+". Depth: "+ac);H*=ac*D.nestingFactor}L.idealLength=H;N.layoutEdges.push(L)}return N}function k(B,A,D){var C=t(B,A,0,D);if(2>C.count){return 0}else{return C.graph}}function t(K,J,E,D){var I=D.graphSet[E];if(-1<$.inArray(K,I)&&-1<$.inArray(J,I)){return{count:2,graph:E}}var H=0;for(var G=0;G<I.length;G++){var C=I[G];var F=D.idToIndex[C];var B=D.layoutNodes[F].children;if(0==B.length){continue}var A=D.indexToGraph[D.idToIndex[B[0]]];
var L=t(K,J,A,D);if(0==L.count){continue}else{if(1==L.count){H++;if(2==H){break}}else{return L}}}return{count:H,graph:E}}function i(D){if(!DEBUG){return}console.debug("layoutNodes:");for(var A=0;A<D.nodeSize;A++){var F=D.layoutNodes[A];var B="\nindex: "+A+"\nId: "+F.id+"\nChildren: "+F.children.toString()+"\nparentId: "+F.parentId+"\npositionX: "+F.positionX+"\npositionY: "+F.positionY+"\nOffsetX: "+F.offsetX+"\nOffsetY: "+F.offsetY+"\npadLeft: "+F.padLeft+"\npadRight: "+F.padRight+"\npadTop: "+F.padTop+"\npadBottom: "+F.padBottom;
console.debug(B)}console.debug("idToIndex");for(var A in D.idToIndex){console.debug("Id: "+A+"\nIndex: "+D.idToIndex[A])}console.debug("Graph Set");var E=D.graphSet;for(var A=0;A<E.length;A++){console.debug("Set : "+A+": "+E[A].toString())}var B="IndexToGraph";for(var A=0;A<D.indexToGraph.length;A++){B+="\nIndex : "+A+" Graph: "+D.indexToGraph[A]}console.debug(B);B="Layout Edges";for(var A=0;A<D.layoutEdges.length;A++){var C=D.layoutEdges[A];B+="\nEdge Index: "+A+" ID: "+C.id+" SouceID: "+C.sourceId+" TargetId: "+C.targetId+" Ideal Length: "+C.idealLength
}console.debug(B);B="nodeSize: "+D.nodeSize;B+="\nedgeSize: "+D.edgeSize;B+="\ntemperature: "+D.temperature;console.debug(B);return}function u(E,G){var B=G.container();var D=B.clientWidth;var A=B.clientHeight;for(var C=0;C<E.nodeSize;C++){var F=E.layoutNodes[C];if(0==F.children.length){F.positionX=Math.random()*D;F.positionY=Math.random()*A}}}function b(F,G,C){var B=G.container();var E=B.clientWidth;var A=B.clientHeight;var D="Refreshing positions";r(D);G.nodes().positions(function(H,I){lnode=F.layoutNodes[F.idToIndex[I.data("id")]];
D="Node: "+lnode.id+". Refreshed position: ("+lnode.positionX+", "+lnode.positionY+").";r(D);return{x:lnode.positionX,y:lnode.positionY}});if(true!=b.ready){D="Triggering layoutready";r(D);b.ready=true;G.one("layoutready",C.ready);G.trigger("layoutready")}}function e(D,E,A,C){var B="\n\n###############################";B+="\nSTEP: "+C;B+="\n###############################\n";r(B);z(D,E,A);w(D,E,A);q(D,E,A);p(D,E,A);g(D,E,A)}function z(C,B,K){var J="calculateNodeForces";r(J);for(var F=0;F<C.graphSet.length;
F++){var H=C.graphSet[F];var A=H.length;J="Set: "+H.toString();r(J);for(var E=0;E<A;E++){var I=C.layoutNodes[C.idToIndex[H[E]]];for(var D=E+1;D<A;D++){var G=C.layoutNodes[C.idToIndex[H[D]]];j(I,G,C,B,K)}}}}function j(O,N,E,D,Q){var P="Node repulsion. Node1: "+O.id+" Node2: "+N.id;var L=N.positionX-O.positionX;var J=N.positionY-O.positionY;P+="\ndirectionX: "+L+", directionY: "+J;if(0==L&&0==J){P+="\nNodes have the same position.";return}overlap=c(O,N,L,J);if(overlap>0){P+="\nNodes DO overlap.";P+="\nOverlap: "+overlap;
var C=Q.nodeOverlap*overlap;var B=Math.sqrt(L*L+J*J);P+="\nDistance: "+B;var G=C*L/B;var F=C*J/B}else{P+="\nNodes do NOT overlap.";var I=d(O,L,J);var H=d(N,-1*L,-1*J);var M=H.x-I.x;var K=H.y-I.y;var A=M*M+K*K;var B=Math.sqrt(A);P+="\nDistance: "+B;var C=Q.nodeRepulsion/A;var G=C*M/B;var F=C*K/B}O.offsetX-=G;O.offsetY-=F;N.offsetX+=G;N.offsetY+=F;P+="\nForceX: "+G+" ForceY: "+F;r(P);return}function d(D,G,F){var B=D.positionX;var A=D.positionY;var L=D.height;var C=D.width;var J=F/G;var E=L/C;var I=C/L;
var M="Computing clipping point of node "+D.id+" . Height:  "+L+", Width: "+C+"\nDirection "+G+", "+F;var K={};do{if(0==G&&0<F){K.x=B;M+="\nUp direction";K.y=A+L/2;break}if(0==G&&0>F){K.x=B;K.y=A+L/2;M+="\nDown direction";break}if(0<G&&-1*E<=J&&J<=E){K.x=B+C/2;K.y=A+(C*F/2/G);M+="\nRightborder";break}if(0>G&&-1*E<=J&&J<=E){K.x=B-C/2;K.y=A-(C*F/2/G);M+="\nLeftborder";break}if(0<F&&(J<=-1*E||J>=E)){K.x=B+(L*G/2/F);K.y=A+L/2;M+="\nTop border";break}if(0>F&&(J<=-1*E||J>=E)){K.x=B-(L*G/2/F);K.y=A-L/2;
M+="\nBottom border";break}}while(false);M+="\nClipping point found at "+K.x+", "+K.y;r(M);return K}function c(E,C,B,A){if(B>0){var F=E.maxX-C.minX}else{var F=C.maxX-E.minX}if(A>0){var D=E.maxY-C.minY}else{var D=C.maxY-E.minY}if(F>=0&&D>=0){return Math.sqrt(F*F+D*D)}else{return 0}}function s(J,I,H,G,F,E){if(0!=F){var D=m(H.x,J.positionX,F);var C=m(G.x,J.positionX,F);var B=m(G.x,I.positionX,-1*F);var A=m(H.x,I.positionX,-1*F)}else{var D=m(H.y,J.positionY,E);var C=m(G.y,J.positionY,E);var B=m(G.y,I.positionY,-1*E);
var A=m(H.y,I.positionY,-1*E)}if(D>C){return true}if(B>A){return true}return false}function m(B,A,C){return((B-A)/C)}function w(N,D,H){for(var R=0;R<N.edgeSize;R++){var L=N.layoutEdges[R];var S=N.idToIndex[L.sourceId];var O=N.layoutNodes[S];var Q=N.idToIndex[L.targetId];var T=N.layoutNodes[Q];var B=T.positionX-O.positionX;var A=T.positionY-O.positionY;if(0==B&&0==A){return}var F=d(O,B,A);var E=d(T,-1*B,-1*A);var K=E.x-F.x;var I=E.y-F.y;var P=Math.sqrt(K*K+I*I);var C=Math.pow(L.idealLength-P,2)/H.edgeElasticity;
if(0!=P){var J=C*K/P;var G=C*I/P}else{var J=0;var G=0}O.offsetX+=J;O.offsetY+=G;T.offsetX-=J;T.offsetY-=G;var M="Edge force between nodes "+O.id+" and "+T.id;M+="\nDistance: "+P+" Force: ("+J+", "+G+")";r(M)}}function q(M,B,E){var L="calculateGravityForces";r(L);for(var Q=0;Q<M.graphSet.length;Q++){var A=M.graphSet[Q];var N=A.length;L="Set: "+A.toString();r(L);if(0==Q){var K=B.container();var G=K.clientHeight/2;var F=K.clientWidth/2}else{var R=M.layoutNodes[M.idToIndex[A[0]]];var H=M.layoutNodes[M.idToIndex[R.parentId]];
var G=H.positionX;var F=H.positionY}L="Center found at: "+G+", "+F;r(L);for(var P=0;P<N;P++){var O=M.layoutNodes[M.idToIndex[A[P]]];L="Node: "+O.id;var J=G-O.positionX;var I=F-O.positionY;var S=Math.sqrt(J*J+I*I);if(S>1){var D=E.gravity*J/S;var C=E.gravity*I/S;O.offsetX+=D;O.offsetY+=C;L+=": Applied force: "+D+", "+C}else{L+=": skypped since it's too close to center"}r(L)}}}function p(H,F,O){var K=[];var B=0;var I=-1;r("propagateForces");K.push.apply(K,H.graphSet[0]);I+=H.graphSet[0].length;while(B<=I){var D=K[B++];
var G=H.idToIndex[D];var E=H.layoutNodes[G];var C=E.children;if(0<C.length){var M=E.offsetX;var L=E.offsetY;var N="Propagating offset from parent node : "+E.id+". OffsetX: "+M+". OffsetY: "+L;N+="\n Children: "+C.toString();r(N);for(var J=0;J<C.length;J++){var A=H.layoutNodes[H.idToIndex[C[J]]];A.offsetX+=M;A.offsetY+=L;K[++I]=C[J]}E.offsetX=0;E.offsetY=0}}}function g(E,G,A){var D="Updating positions";r(D);for(var B=0;B<E.nodeSize;B++){var F=E.layoutNodes[B];if(0<F.children.length){r("Resetting boundaries of compound node: "+F.id);
F.maxX=undefined;F.minX=undefined;F.maxY=undefined;F.minY=undefined}}for(var B=0;B<E.nodeSize;B++){var F=E.layoutNodes[B];if(0<F.children.length){r("Skipping position update of node: "+F.id);continue}D="Node: "+F.id+" Previous position: ("+F.positionX+", "+F.positionY+").";var C=v(F.offsetX,F.offsetY,E.temperature);F.positionX+=C.x;F.positionY+=C.y;F.offsetX=0;F.offsetY=0;F.minX=F.positionX-F.width;F.maxX=F.positionX+F.width;F.minY=F.positionY-F.height;F.maxY=F.positionY+F.height;D+=" New Position: ("+F.positionX+", "+F.positionY+").";
r(D);l(F,E)}for(var B=0;B<E.nodeSize;B++){var F=E.layoutNodes[B];if(0<F.children.length){F.positionX=(F.maxX+F.minX)/2;F.positionY=(F.maxY+F.minY)/2;F.width=F.maxX-F.minX;F.height=F.maxY-F.minY;D="Updating position, size of compound node "+F.id;D+="\nPositionX: "+F.positionX+", PositionY: "+F.positionY;D+="\nWidth: "+F.width+", Height: "+F.height;r(D)}}}function v(F,D,A){var C="Limiting force: ("+F+", "+D+"). Max: "+A;var E=Math.sqrt(F*F+D*D);if(E>A){var B={x:A*F/E,y:A*D/E}}else{var B={x:F,y:D}}C+=".\nResult: ("+B.x+", "+B.y+")";
r(C);return B}function l(C,E){var B="Propagating new position/size of node "+C.id;var F=C.parentId;if(undefined==F){B+=". No parent node.";r(B);return}var D=E.layoutNodes[E.idToIndex[F]];var A=false;if(undefined==D.maxX||C.maxX+D.padRight>D.maxX){D.maxX=C.maxX+D.padRight;A=true;B+="\nNew maxX for parent node "+D.id+": "+D.maxX}if(undefined==D.minX||C.minX-D.padLeft<D.minX){D.minX=C.minX-D.padLeft;A=true;B+="\nNew minX for parent node "+D.id+": "+D.minX}if(undefined==D.maxY||C.maxY+D.padBottom>D.maxY){D.maxY=C.maxY+D.padBottom;
A=true;B+="\nNew maxY for parent node "+D.id+": "+D.maxY}if(undefined==D.minY||C.minY-D.padTop<D.minY){D.minY=C.minY-D.padTop;A=true;B+="\nNew minY for parent node "+D.id+": "+D.minY}if(A){r(B);return l(D,E)}B+=". No changes in boundaries/position of parent node "+D.id;r(B);return}function r(A){if(DEBUG){console.debug(A)}}h("layout","cose",a)})(cytoscape);