...
cytoscape(i)})}else{var c=k[0];var j=[];var d=[];for(var e=1;e<arguments.length;e++){d[e-1]=arguments[e]}k.each(function(){var l=a.getRegistrationForInstance(c);var o=l.cy;var m=h;if(o&&a.is.fn(o[m])){var i=o[m].apply(o,d);j.push(i)}});if(j.length===1){j=j[0]}else{if(j.length==0){j=b(this)}}return j}}}};b.cytoscape=cytoscape;if(b.fn.cy==null&&b.cy==null){b.fn.cy=b.fn.cytoscape;b.cy=b.cytoscape}b(function(){function c(h,d){var i=h.attr(d);var f;try{f=b.parseJSON(d)}catch(g){f=undefined}}b('[data-role="cytoscape"]').each(function(){var e=b(this);
var d={showOverlay:c(e,"data-show-overlay"),layout:c(e,"data-layout"),zoom:c(e,"data-zoom"),minZoom:c(e,"data-min-zoom"),maxZoom:c(e,"data-max-zoom"),pan:c(e,"data-pan"),renderer:c(e,"data-renderer"),style:c(e,"data-style"),elements:c(e,"data-elements")};e.cytoscape(d)})})})(typeof jQuery!=="undefined"?jQuery:null,cytoscape);(function(a){a.Event=function(e,d){if(!(this instanceof a.Event)){return new a.Event(e,d)}if(e&&e.type){this.originalEvent=e;this.type=e.type;this.isDefaultPrevented=(e.defaultPrevented||e.returnValue===false||e.getPreventDefault&&e.getPreventDefault())?c:b
}else{this.type=e}if(d){a.util.extend(this,d)}this.timeStamp=e&&e.timeStamp||+new Date};function b(){return false}function c(){return true}a.Event.prototype={preventDefault:function(){this.isDefaultPrevented=c;var d=this.originalEvent;if(!d){return}if(d.preventDefault){d.preventDefault()}else{d.returnValue=false}},stopPropagation:function(){this.isPropagationStopped=c;var d=this.originalEvent;if(!d){return}if(d.stopPropagation){d.stopPropagation()}d.cancelBubble=true},stopImmediatePropagation:function(){this.isImmediatePropagationStopped=c;
this.stopPropagation()},isDefaultPrevented:b,isPropagationStopped:b,isImmediatePropagationStopped:b}})(cytoscape);(function(a){a.define={data:function(c){var b={field:"data",bindingEvent:"data",allowBinding:false,allowSetting:false,allowGetting:false,settingEvent:"data",settingTriggersEvent:false,triggerFnName:"trigger",immutableKeys:{},updateMappers:false};c=a.util.extend({},b,c);return function(e,s){var g=c;var z=this;var f=z.length!==undefined;var t=f?z:[z];var w=f?z[0]:z;if(a.is.string(e)){if(g.allowGetting&&s===undefined){var q;
if(w){q=w._private[g.field][e]}return q}else{if(g.allowSetting&&s!==undefined){var d=!g.immutableKeys[e];if(d){for(var o=0,h=t.length;o<h;o++){t[o]._private[g.field][e]=s}if(g.updateMappers){z.updateMappers()}if(g.settingTriggersEvent){z[g.triggerFnName](g.settingEvent)}}}}}else{if(g.allowSetting&&a.is.plainObject(e)){var m=e;var j,u;for(j in m){u=m[j];var d=!g.immutableKeys[j];if(d){for(var o=0,h=t.length;o<h;o++){t[o]._private[g.field][j]=u}}}if(g.updateMappers){z.updateMappers()}if(g.settingTriggersEvent){z[g.triggerFnName](g.settingEvent)
}}else{if(g.allowBinding&&a.is.fn(e)){var r=e;z.bind(g.bindingEvent,r)}else{if(g.allowGetting&&e===undefined){var q;if(w){q=w._private[g.field]}return q}}}}return z}},batchData:function(d){var c={field:"data",event:"data",triggerFnName:"trigger",immutableKeys:{},updateMappers:false};var b=d=a.util.extend({},c,d);return function(f){var s=this;var h=s.length!==undefined;var u=h?s:s._private.elements;if(u.length===0){return s}var j=h?u[0]._private.cy:s;for(var o=0;o<u.length;o++){var t=u[o];var g=t._private.data.id;
var r=f[g];if(r!==undefined&&r!==null){var m=r;var l,p;for(l in m){p=m[l];var e=!b.immutableKeys[l];if(e){t._private[b.field][l]=p}}}}var q=new a.Collection(j,u);if(b.updateMappers){q.updateMappers()}q[b.triggerFnName](b.event);return s}},removeData:function(c){var b={field:"data",event:"data",triggerFnName:"trigger",triggerEvent:false,immutableKeys:{}};c=a.util.extend({},b,c);return function(q){var g=c;var v=this;var f=v.length!==undefined;var r=f?v:[v];var t=f?v[0]:v;if(a.is.string(q)){var u=q.split(/\s+/);
var j=u.length;for(var k=0;k<j;k++){var s=u[k];if(a.is.emptyString(s)){continue}var e=!g.immutableKeys[s];if(e){for(var o=0,h=r.length;o<h;o++){delete r[o]._private[g.field][s]}}}if(g.triggerEvent){v[g.triggerFnName](g.event)}}else{if(q===undefined){for(var o=0,h=r.length;o<h;o++){var m=r[o]._private[g.field];for(var s in m){var d=!g.immutableKeys[s];if(d){delete m[s]}}}if(g.triggerEvent){v[g.triggerFnName](g.event)}}}return v}},event:{regex:/(\w+)(\.\w+)?/,optionalTypeRegex:/(\w+)?(\.\w+)?/,props:"altKey bubbles button cancelable charCode clientX clientY ctrlKey currentTarget data detail eventPhase metaKey offsetX offsetY originalTarget pageX pageY prevValue relatedTarget screenX screenY shiftKey target view which".split(/\s+/),aliases:"mousedown mouseup click mouseover mouseout mousemove touchstart touchmove touchend grab drag free".split(/\s+/),aliasesOn:function(e){var b=a.define.event.aliases;
...
var px=parseFloat(pxAsStr);return px}else{return 1}};$$.styfn.containerCss=function(propName){var cy=this._private.cy;var domElement=cy.container();if(window&&domElement&&window.getComputedStyle){return window.getComputedStyle(domElement).getPropertyValue(propName)}};$$.styfn.containerProperty=function(propName){var propStr=this.containerCss(propName);var prop=this.parse(propName,propStr);return prop};$$.styfn.containerPropertyAsString=function(propName){var prop=this.containerProperty(propName);
...
if(e.initrender){i.on("initrender",e.initrender)}i.load(e.elements,function(){i.startAnimationLoop();i._private.ready=true;if(a.is.fn(e.ready)){i.bind("ready",e.ready)}for(var j=0;j<g.length;j++){var k=g[j];i.bind("ready",k)}f.readies=[];i.trigger("ready")},e.done)};a.corefn=a.Core.prototype;a.fn.core({ready:function(){return this._private.ready},registered:function(){if(this._private&&this._private.instanceId!=null){return true}else{return false}},registeredId:function(){return this._private.instanceId
},getElementById:function(e){var d=this._private.id2index[e];if(d!==undefined){return this._private.elements[d]}return new a.Collection(this)},hasCompoundNodes:function(){return this._private.hasCompoundNodes},addToPool:function(e){var j=this._private.elements;var l=this._private.id2index;for(var g=0;g<e.length;g++){var h=e[g];var k=h._private.data.id;var f=l[k];var d=f!==undefined;if(!d){f=j.length;j.push(h);l[k]=f;h._private.index=f}}return this},removeFromPool:function(p){var d=this._private.elements;
var g=this._private.id2index;for(var k=0;k<p.length;k++){var o=p[k];var e=o._private.data.id;var l=g[e];var m=l!==undefined;if(m){delete this._private.id2index[e];d.splice(l,1);for(var h=l;h<d.length;h++){var f=d[h]._private.data.id;g[f]--}}}},container:function(){return this._private.options.container},options:function(){return a.util.copy(this._private.options)},json:function(e){var d={};var f=this;d.elements={};f.elements().each(function(g,h){var j=h.group();if(!d.elements[j]){d.elements[j]=[]
}d.elements[j].push(h.json())});d.style=f.style();d.scratch=f.scratch();d.zoomEnabled=f._private.zoomEnabled;d.panEnabled=f._private.panEnabled;d.layout=f._private.options.layout;d.renderer=f._private.options.renderer;return d}})})(cytoscape);(function(a,b){a.fn.core({add:function(d){var c;var e=this;if(a.is.elementOrCollection(d)){var u=d;if(u._private.cy===e){c=u.restore()}else{var o=[];for(var h=0;h<u.length;h++){var t=u[h];o.push(t.json())}c=new a.Collection(e,o)}}else{if(a.is.array(d)){var o=d;
c=new a.Collection(e,o)}else{if(a.is.plainObject(d)&&(a.is.array(d.nodes)||a.is.array(d.edges))){var l=d;var o=[];var p=["nodes","edges"];for(var h=0,q=p.length;h<q;h++){var r=p[h];var m=l[r];if(a.is.array(m)){for(var f=0,g=m.length;f<g;f++){var s=m[f];var k=a.util.extend({},s,{group:r});o.push(k)}}}c=new a.Collection(e,o)}else{var s=d;c=(new a.Element(e,s)).collection()}}}return c},remove:function(d){if(a.is.elementOrCollection(d)){d=d}else{if(a.is.string(d)){var c=d;d=this.$(c)}}return d.remove()
},load:function(g,h,e){var j=this;var d=j.elements();if(d.length>0){d.remove()}j.notifications(false);var c=[];if(g!=null){if(a.is.plainObject(g)||a.is.array(g)){j.add(g)}}function i(){j.one("layoutready",function(k){j.notifications(true);j.trigger(k);j.notify({type:"load",collection:j.elements(),style:j._private.style});j.one("load",h);j.trigger("load")}).one("layoutstop",function(){j.one("done",e);j.trigger("done")});j.layout(j._private.options.layout)}if(b){function f(l){var k=(document&&(document.readyState==="interactive"||document.readyState==="complete"))?l:f;
setTimeout(k,9,l)}f(i)}else{i()}return this}})})(cytoscape,typeof window==="undefined"?null:window);(function(a){a.fn.core({addToAnimationPool:function(b){var h=this;var c=h._private.aniEles;var e=[];for(var d=0;d<c.length;d++){var g=c[d]._private.data.id;e[g]=true}for(var d=0;d<b.length;d++){var f=b[d];var g=f._private.data.id;if(!e[g]){c.push(f)}}},startAnimationLoop:function(){var g=this;var f=1000/60;var l=false;var i=true;g._private.aniEles=[];var j=typeof window==="undefined"?function(){}:(window.requestAnimationFrame||window.mozRequestAnimationFrame||window.webkitRequestAnimationFrame||window.msRequestAnimationFrame);
...
for(var J=0;J<a;J++){this.data.bufferCanvases[J]=document.createElement("canvas");this.data.bufferCanvases[J].style.position="absolute";this.data.bufferCanvases[J].setAttribute("data-id","buffer"+J);this.data.bufferCanvases[J].style.zIndex=String(-J-1);this.data.bufferCanvases[J].style.visibility="hidden";this.data.container.appendChild(this.data.bufferCanvases[J])}var H=document.createElement("div");this.data.container.appendChild(H);this.data.overlay=H;H.style.position="absolute";H.style.zIndex=1000;
...
}c.prototype.run=function(){var k=this.options;var j=k.cy;var w=j.nodes();var i=j.edges();var q=j.container();var v=q.clientWidth;var s=q.clientHeight;var t=k.simulationBounds;if(k.simulationBounds){v=t[2]-t[0];s=t[3]-t[1]}else{k.simulationBounds=[0,0,v,s]}var A=k.simulationBounds;A.x1=A[0];A.y1=A[1];A.x2=A[2];A.y2=A[3];if(j.nodes().size()<=1){if(k.fit){j.reset()}j.nodes().position({x:Math.round((A.x1+A.x2)/2),y:Math.round((A.y1+A.y2)/2)});j.one("layoutstop",k.stop);j.trigger("layoutstop");j.one("layoutstop",k.stop);
j.trigger("layoutstop");return}var m=this.system=arbor.ParticleSystem(k.repulsion,k.stiffness,k.friction,k.gravity,k.fps,k.dt,k.precision);this.system=m;if(k.liveUpdate&&k.fit){j.reset()}var u=250;var h;var p=false;var g=+new Date;var r={init:function(B){},redraw:function(){var C=m.energy();if(k.stableEnergy!=null&&C!=null&&C.n>0&&k.stableEnergy(C)){m.stop();return}clearTimeout(h);h=setTimeout(z,u);var D=[];m.eachNode(function(J,E){var I=J.name;var G=J.data;var F=G.element;if(F==null){return}var H=F._private.position;
if(!F.locked()&&!F.grabbed()){H.x=A.x1+E.x;H.y=A.y1+E.y;D.push(F)}});var B=(+new Date-g)>=16;if(k.liveUpdate&&D.length>0&&B){new a.Collection(j,D).rtrigger("position");g=+new Date}if(!p){p=true;j.one("layoutready",k.ready);j.trigger("layoutready")}}};m.renderer=r;m.screenSize(v,s);m.screenPadding(k.padding[0],k.padding[1],k.padding[2],k.padding[3]);m.screenStep(k.stepSize);function o(B,C){if(C==null){return undefined}else{if(typeof C==typeof function(){}){return C.apply(B,[B._private.data,{nodes:w.length,edges:i.length,element:B}])
}else{return C}}}function d(F){var I=F.x;var G=F.y;var J=v;var D=s;var C=-2;var K=2;var H=-2;var B=2;var E=4;return{x:I/J*E+C,y:G/D*E+K}}var l=function(C){grabbed=this;var D=m.fromScreen(this.position());var B=arbor.Point(D.x,D.y);this.scratch().arbor.p=B;switch(C.type){case"grab":this.scratch().arbor.fixed=true;break;case"dragstop":this.scratch().arbor.fixed=false;this.scratch().arbor.tempMass=1000;break}};w.bind("grab drag dragstop",l);w.each(function(D,E){var G=this._private.data.id;var C=o(this,k.nodeMass);
var B=this._private.locked;var F=d({x:E.position().x,y:E.position().y});if(E.locked()){return}this.scratch().arbor=m.addNode(G,{element:this,mass:C,fixed:B,x:B?F.x:undefined,y:B?F.y:undefined})});i.each(function(){var E=this.id();var C=this.source().id();var D=this.target().id();var B=o(this,k.edgeLength);this.scratch().arbor=m.addEdge(C,D,{length:B})});function f(B){if(k.fit){j.fit()}B()}var e=w.filter(":grabbable");if(k.ungrabifyWhileSimulating){e.ungrabify()}var z=function(){if(window.isIE){f(function(){B()
})}else{B()}function B(){if(!k.liveUpdate){if(k.fit){j.reset()}j.nodes().rtrigger("position")}w.unbind("grab drag dragstop",l);if(k.ungrabifyWhileSimulating){e.grabify()}j.one("layoutstop",k.stop);j.trigger("layoutstop")}};m.start();setTimeout(function(){m.stop()},k.maxSimulationTime)};c.prototype.stop=function(){if(this.system!=null){system.stop()}};a("layout","arbor",c)})(cytoscape);(function(a){var c={fit:true,ready:undefined,stop:undefined,rStepSize:10,padding:30,startAngle:3/2*Math.PI,counterclockwise:false};
...
var A=Math.sqrt((z.x-B.x)*(z.x-B.x)+(z.y-B.y)*(z.y-B.y));return A}while(k()<t&&!(f.length<2)){d+=w.rStepSize}var q=0;f.positions(function(){var i=this;var z=d*Math.cos(j);var r=d*Math.sin(j);var A={x:e.x+z,y:e.y+r};q++;j=w.counterclockwise?j-s:j+s;return A});if(o.fit){m.fit(w.padding)}m.one("layoutready",o.ready);m.trigger("layoutready");m.one("layoutstop",o.stop);m.trigger("layoutstop")};b.prototype.stop=function(){};a("layout","circle",b)})(cytoscape);(function(a){var b={fit:true,ready:undefined,stop:undefined,directed:true,padding:30,circle:false,roots:undefined,maximalAdjustments:0};
...
}for(var K=0;K<3;K++){for(var V=0;V<A.length;V++){var q=V;var X=[];A[V]=A[V].sort(function(i,h){var j=H(i);var w=H(h);return j-w})}L()}var e={x:d/2,y:l/2};S.positions(function(){var af=this[0];var i=af._private.scratch.BreadthFirstLayout;var j=i.depth;var w=i.index;var ae=Math.max(d/(A[j].length+1),o);var ad=Math.max(l/(A.length+1),o);var ac=Math.min(d/2/A.length,l/2/A.length);ac=Math.max(ac,o);if(G.circle){var ab=ac*j+ac-(A.length>0&&A[0].length<=3?ac/2:0);var h=2*Math.PI/A[j].length*w;if(j===0&&A[0].length===1){ab=1
}return{x:e.x+ab*Math.cos(h),y:e.y+ab*Math.sin(h)}}else{return{x:(w+1)*ae,y:(j+1)*ad}}});if(g.fit){D.fit(G.padding)}D.one("layoutready",g.ready);D.trigger("layoutready");D.one("layoutstop",g.stop);D.trigger("layoutstop")};c.prototype.stop=function(){};a("layout","breadthfirst",c)})(cytoscape);(function(h){var f={ready:function(){},stop:function(){},refresh:0,fit:true,randomize:true,debug:false,nodeRepulsion:10000,nodeOverlap:10,idealEdgeLength:10,edgeElasticity:10000,nestingFactor:10,gravity:10,numIter:100,initialTemp:200,coolingFactor:0.95,minTemp:1};
function a(w){this.options=h.util.extend(true,{},f,w)}a.prototype.run=function(){var z=this.options;var D=z.cy;if(true==z.debug){DEBUG=true}else{DEBUG=false}var B=new Date();var C=m(D,z);if(DEBUG){i(C)}if(true==z.randomize){s(C,D);if(0<z.refresh){b(C,D,z)}}for(var A=0;A<z.numIter;A++){e(C,D,z,A);if(0<z.refresh&&0==(A%z.refresh)){b(C,D,z)}C.temperature=C.temperature*z.coolingFactor;q("New temperature: "+C.temperature);if(C.temperature<z.minTemp){q("Temperature drop below minimum threshold. Stopping computation in step "+A);
break}}b(C,D,z);if(true==z.fit){D.fit()}var w=new Date();console.info("Layout took "+(w-B)+" ms");D.one("layoutstop",z.stop);D.trigger("layoutstop")};a.prototype.stop=function(){var w=this.options;cy.one("layoutstop",w.stop);cy.trigger("layoutstop")};function m(A,B){var L={layoutNodes:[],idToIndex:{},nodeSize:A.nodes().size(),graphSet:[],indexToGraph:[],layoutEdges:[],edgeSize:A.edges().size(),temperature:B.initialTemp};var N=A.nodes();for(var U=0;U<L.nodeSize;U++){var V={};V.id=N[U].data("id");V.parentId=N[U].data("parent");
V.children=[];V.positionX=N[U].position("x");V.positionY=N[U].position("y");V.offsetX=0;V.offsetY=0;V.height=N[U].height();V.width=N[U].width();V.maxX=V.positionX+V.width/2;V.minX=V.positionX-V.width/2;V.maxY=V.positionY+V.height/2;V.minY=V.positionY-V.height/2;V.padLeft=N[U]._private.style["padding-left"].pxValue;V.padRight=N[U]._private.style["padding-right"].pxValue;V.padTop=N[U]._private.style["padding-top"].pxValue;V.padBottom=N[U]._private.style["padding-bottom"].pxValue;L.layoutNodes.push(V);
L.idToIndex[V.id]=U}var P=[];var E=0;var C=-1;var H=[];for(var U=0;U<L.nodeSize;U++){var M=L.layoutNodes[U];var X=M.parentId;if(undefined!=X){L.layoutNodes[L.idToIndex[X]].children.push(M.id)}else{P[++C]=M.id;H.push(M.id)}}L.graphSet.push(H);while(E<=C){var Z=P[E++];var K=L.idToIndex[Z];var O=L.layoutNodes[K];var D=O.children;if(D.length>0){L.graphSet.push(D);for(var U=0;U<D.length;U++){P[++C]=D[U]}}}for(var U=0;U<L.graphSet.length;U++){var w=L.graphSet[U];for(var S=0;S<w.length;S++){var G=L.idToIndex[w[S]];
L.indexToGraph[G]=U}}var z=A.edges();for(var U=0;U<L.edgeSize;U++){var W=z[U];var J={};J.id=W.data("id");J.sourceId=W.data("source");J.targetId=W.data("target");var F=B.idealEdgeLength;var Y=L.idToIndex[J.sourceId];var R=L.idToIndex[J.targetId];var ab=L.indexToGraph[Y];var Q=L.indexToGraph[R];if(ab!=Q){var I=k(J.sourceId,J.targetId,L);var T=L.graphSet[I];var aa=0;var V=L.layoutNodes[Y];while(-1==$.inArray(V.id,T)){V=L.layoutNodes[L.idToIndex[V.parentId]];aa++}V=L.layoutNodes[R];while(-1==$.inArray(V.id,T)){V=L.layoutNodes[L.idToIndex[V.parentId]];
aa++}q("LCA of nodes "+J.sourceId+" and "+J.targetId+". Index: "+I+" Contents: "+T.toString()+". Depth: "+aa);F*=aa*B.nestingFactor}J.idealLength=F;L.layoutEdges.push(J)}return L}function k(z,w,B){var A=r(z,w,0,B);if(2>A.count){return 0}else{return A.graph}}function r(I,H,C,B){var G=B.graphSet[C];if(-1<$.inArray(I,G)&&-1<$.inArray(H,G)){return{count:2,graph:C}}var F=0;for(var E=0;E<G.length;E++){var A=G[E];var D=B.idToIndex[A];var z=B.layoutNodes[D].children;if(0==z.length){continue}var w=B.indexToGraph[B.idToIndex[z[0]]];
var J=r(I,H,w,B);if(0==J.count){continue}else{if(1==J.count){F++;if(2==F){break}}else{return J}}}return{count:F,graph:C}}function i(B){if(!DEBUG){return}console.debug("layoutNodes:");for(var w=0;w<B.nodeSize;w++){var D=B.layoutNodes[w];var z="\nindex: "+w+"\nId: "+D.id+"\nChildren: "+D.children.toString()+"\nparentId: "+D.parentId+"\npositionX: "+D.positionX+"\npositionY: "+D.positionY+"\nOffsetX: "+D.offsetX+"\nOffsetY: "+D.offsetY+"\npadLeft: "+D.padLeft+"\npadRight: "+D.padRight+"\npadTop: "+D.padTop+"\npadBottom: "+D.padBottom;
console.debug(z)}console.debug("idToIndex");for(var w in B.idToIndex){console.debug("Id: "+w+"\nIndex: "+B.idToIndex[w])}console.debug("Graph Set");var C=B.graphSet;for(var w=0;w<C.length;w++){console.debug("Set : "+w+": "+C[w].toString())}var z="IndexToGraph";for(var w=0;w<B.indexToGraph.length;w++){z+="\nIndex : "+w+" Graph: "+B.indexToGraph[w]}console.debug(z);z="Layout Edges";for(var w=0;w<B.layoutEdges.length;w++){var A=B.layoutEdges[w];z+="\nEdge Index: "+w+" ID: "+A.id+" SouceID: "+A.sourceId+" TargetId: "+A.targetId+" Ideal Length: "+A.idealLength
}console.debug(z);z="nodeSize: "+B.nodeSize;z+="\nedgeSize: "+B.edgeSize;z+="\ntemperature: "+B.temperature;console.debug(z);return}function s(C,E){var z=E.container();var B=z.clientWidth;var w=z.clientHeight;for(var A=0;A<C.nodeSize;A++){var D=C.layoutNodes[A];if(0==D.children.length){D.positionX=Math.random()*B;D.positionY=Math.random()*w}}}function b(D,E,A){var z=E.container();var C=z.clientWidth;var w=z.clientHeight;var B="Refreshing positions";q(B);E.nodes().positions(function(F,G){lnode=D.layoutNodes[D.idToIndex[G.data("id")]];
B="Node: "+lnode.id+". Refreshed position: ("+lnode.positionX+", "+lnode.positionY+").";q(B);return{x:lnode.positionX,y:lnode.positionY}});if(true!=b.ready){B="Triggering layoutready";q(B);b.ready=true;E.one("layoutready",A.ready);E.trigger("layoutready")}}function e(B,C,w,A){var z="\n\n###############################";z+="\nSTEP: "+A;z+="\n###############################\n";q(z);v(B,C,w);u(B,C,w);p(B,C,w);o(B,C,w);g(B,C,w)}function v(A,z,I){var H="calculateNodeForces";q(H);for(var D=0;D<A.graphSet.length;
D++){var F=A.graphSet[D];var w=F.length;H="Set: "+F.toString();q(H);for(var C=0;C<w;C++){var G=A.layoutNodes[A.idToIndex[F[C]]];for(var B=C+1;B<w;B++){var E=A.layoutNodes[A.idToIndex[F[B]]];j(G,E,A,z,I)}}}}function j(M,L,C,B,O){var N="Node repulsion. Node1: "+M.id+" Node2: "+L.id;var J=L.positionX-M.positionX;var H=L.positionY-M.positionY;N+="\ndirectionX: "+J+", directionY: "+H;if(0==J&&0==H){N+="\nNodes have the same position.";return}overlap=c(M,L,J,H);if(overlap>0){N+="\nNodes DO overlap.";N+="\nOverlap: "+overlap;
var A=O.nodeOverlap*overlap;var z=Math.sqrt(J*J+H*H);N+="\nDistance: "+z;var E=A*J/z;var D=A*H/z}else{N+="\nNodes do NOT overlap.";var G=d(M,J,H);var F=d(L,-1*J,-1*H);var K=F.x-G.x;var I=F.y-G.y;var w=K*K+I*I;var z=Math.sqrt(w);N+="\nDistance: "+z;var A=O.nodeRepulsion/w;var E=A*K/z;var D=A*I/z}M.offsetX-=E;M.offsetY-=D;L.offsetX+=E;L.offsetY+=D;N+="\nForceX: "+E+" ForceY: "+D;q(N);return}function d(B,E,D){var z=B.positionX;var w=B.positionY;var J=B.height;var A=B.width;var G=D/E;var C=J/A;var F=A/J;
var K="Computing clipping point of node "+B.id+" . Height:  "+J+", Width: "+A+"\nDirection "+E+", "+D;var I={};do{if(0==E&&0<D){I.x=z;K+="\nUp direction";I.y=w+J/2;break}if(0==E&&0>D){I.x=z;I.y=w+J/2;K+="\nDown direction";break}if(0<E&&-1*C<=G&&G<=C){I.x=z+A/2;I.y=w+(A*D/2/E);K+="\nRightborder";break}if(0>E&&-1*C<=G&&G<=C){I.x=z-A/2;I.y=w-(A*D/2/E);K+="\nLeftborder";break}if(0<D&&(G<=-1*C||G>=C)){I.x=z+(J*E/2/D);I.y=w+J/2;K+="\nTop border";break}if(0>D&&(G<=-1*C||G>=C)){I.x=z-(J*E/2/D);I.y=w-J/2;
K+="\nBottom border";break}}while(false);K+="\nClipping point found at "+I.x+", "+I.y;q(K);return I}function c(C,A,z,w){if(z>0){var D=C.maxX-A.minX}else{var D=A.maxX-C.minX}if(w>0){var B=C.maxY-A.minY}else{var B=A.maxY-C.minY}if(D>=0&&B>=0){return Math.sqrt(D*D+B*B)}else{return 0}}function u(L,B,F){for(var P=0;P<L.edgeSize;P++){var J=L.layoutEdges[P];var Q=L.idToIndex[J.sourceId];var M=L.layoutNodes[Q];var O=L.idToIndex[J.targetId];var R=L.layoutNodes[O];var z=R.positionX-M.positionX;var w=R.positionY-M.positionY;
if(0==z&&0==w){return}var D=d(M,z,w);var C=d(R,-1*z,-1*w);var I=C.x-D.x;var G=C.y-D.y;var N=Math.sqrt(I*I+G*G);var A=Math.pow(J.idealLength-N,2)/F.edgeElasticity;if(0!=N){var H=A*I/N;var E=A*G/N}else{var H=0;var E=0}M.offsetX+=H;M.offsetY+=E;R.offsetX-=H;R.offsetY-=E;var K="Edge force between nodes "+M.id+" and "+R.id;K+="\nDistance: "+N+" Force: ("+H+", "+E+")";q(K)}}function p(K,z,C){var J="calculateGravityForces";q(J);for(var O=0;O<K.graphSet.length;O++){var w=K.graphSet[O];var L=w.length;J="Set: "+w.toString();
q(J);if(0==O){var I=z.container();var E=I.clientHeight/2;var D=I.clientWidth/2}else{var P=K.layoutNodes[K.idToIndex[w[0]]];var F=K.layoutNodes[K.idToIndex[P.parentId]];var E=F.positionX;var D=F.positionY}J="Center found at: "+E+", "+D;q(J);for(var N=0;N<L;N++){var M=K.layoutNodes[K.idToIndex[w[N]]];J="Node: "+M.id;var H=E-M.positionX;var G=D-M.positionY;var Q=Math.sqrt(H*H+G*G);if(Q>1){var B=C.gravity*H/Q;var A=C.gravity*G/Q;M.offsetX+=B;M.offsetY+=A;J+=": Applied force: "+B+", "+A}else{J+=": skypped since it's too close to center"
}q(J)}}}function o(F,D,M){var I=[];var z=0;var G=-1;q("propagateForces");I.push.apply(I,F.graphSet[0]);G+=F.graphSet[0].length;while(z<=G){var B=I[z++];var E=F.idToIndex[B];var C=F.layoutNodes[E];var A=C.children;if(0<A.length){var K=C.offsetX;var J=C.offsetY;var L="Propagating offset from parent node : "+C.id+". OffsetX: "+K+". OffsetY: "+J;L+="\n Children: "+A.toString();q(L);for(var H=0;H<A.length;H++){var w=F.layoutNodes[F.idToIndex[A[H]]];w.offsetX+=K;w.offsetY+=J;I[++G]=A[H]}C.offsetX=0;C.offsetY=0
}}}function g(C,E,w){var B="Updating positions";q(B);for(var z=0;z<C.nodeSize;z++){var D=C.layoutNodes[z];if(0<D.children.length){q("Resetting boundaries of compound node: "+D.id);D.maxX=undefined;D.minX=undefined;D.maxY=undefined;D.minY=undefined}}for(var z=0;z<C.nodeSize;z++){var D=C.layoutNodes[z];if(0<D.children.length){q("Skipping position update of node: "+D.id);continue}B="Node: "+D.id+" Previous position: ("+D.positionX+", "+D.positionY+").";var A=t(D.offsetX,D.offsetY,C.temperature);D.positionX+=A.x;
D.positionY+=A.y;D.offsetX=0;D.offsetY=0;D.minX=D.positionX-D.width;D.maxX=D.positionX+D.width;D.minY=D.positionY-D.height;D.maxY=D.positionY+D.height;B+=" New Position: ("+D.positionX+", "+D.positionY+").";q(B);l(D,C)}for(var z=0;z<C.nodeSize;z++){var D=C.layoutNodes[z];if(0<D.children.length){D.positionX=(D.maxX+D.minX)/2;D.positionY=(D.maxY+D.minY)/2;D.width=D.maxX-D.minX;D.height=D.maxY-D.minY;B="Updating position, size of compound node "+D.id;B+="\nPositionX: "+D.positionX+", PositionY: "+D.positionY;
B+="\nWidth: "+D.width+", Height: "+D.height;q(B)}}}function t(D,B,w){var A="Limiting force: ("+D+", "+B+"). Max: "+w;var C=Math.sqrt(D*D+B*B);if(C>w){var z={x:w*D/C,y:w*B/C}}else{var z={x:D,y:B}}A+=".\nResult: ("+z.x+", "+z.y+")";q(A);return z}function l(A,C){var z="Propagating new position/size of node "+A.id;var D=A.parentId;if(undefined==D){z+=". No parent node.";q(z);return}var B=C.layoutNodes[C.idToIndex[D]];var w=false;if(undefined==B.maxX||A.maxX+B.padRight>B.maxX){B.maxX=A.maxX+B.padRight;
w=true;z+="\nNew maxX for parent node "+B.id+": "+B.maxX}if(undefined==B.minX||A.minX-B.padLeft<B.minX){B.minX=A.minX-B.padLeft;w=true;z+="\nNew minX for parent node "+B.id+": "+B.minX}if(undefined==B.maxY||A.maxY+B.padBottom>B.maxY){B.maxY=A.maxY+B.padBottom;w=true;z+="\nNew maxY for parent node "+B.id+": "+B.maxY}if(undefined==B.minY||A.minY-B.padTop<B.minY){B.minY=A.minY-B.padTop;w=true;z+="\nNew minY for parent node "+B.id+": "+B.minY}if(w){q(z);return l(B,C)}z+=". No changes in boundaries/position of parent node "+B.id;
q(z);return}function q(w){if(DEBUG){console.debug(w)}}h("layout","cose",a)})(cytoscape);