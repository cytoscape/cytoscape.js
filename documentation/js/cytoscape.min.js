...
e.splice(m,1);for(var k=m;k<e.length;k++){var g=e[k]._private.data.id;h[g]--}}}},container:function(){return this._private.options.container},options:function(){return b.util.copy(this._private.options)},json:function(f){var e={};var g=this;e.elements={};g.elements().each(function(h,j){var k=j.group();if(!e.elements[k]){e.elements[k]=[]}e.elements[k].push(j.json())});e.style=g.style();e.scratch=g.scratch();e.zoomingEnabled=g._private.zoomingEnabled;e.userZoomingEnabled=g._private.userZoomingEnabled;
...
if(f.width!==g||f.height!==k){f.width=g;f.height=k}}};a.prototype.renderTo=function(c,d,e){this.redraw({forcedContext:c,forcedZoom:d,forcedPan:e,drawAllLayers:true})};a.prototype.redraw=function(j){j=j||{};var z=j.forcedContext;var A=j.drawAllLayers;var F=j.forcedZoom;var p=j.forcedPan;var t=this;var f=this.getPixelRatio();if(this.averageRedrawTime===undefined){this.averageRedrawTime=0}var e=1000/60;var l=1000;var w=this.averageRedrawTime;w=Math.max(e,w);w=Math.min(w,l);if(this.lastDrawTime===undefined){this.lastDrawTime=0
}var c=+new Date;var k=c-this.lastDrawTime;var D=k>=w;if(!z){if(!D){clearTimeout(this.redrawTimeout);this.redrawTimeout=setTimeout(function(){t.redraw()},w);return}this.lastDrawTime=c}var s=c;var o=100;var h=t.data.cy;var G=t.data;var v=t.getCachedNodes();var g=t.getCachedEdges();if(!z){t.matchCanvasSize(G.container)}var d=h.zoom();var m=F!==undefined?F:d;var u=h.pan();var E={x:u.x,y:u.y};if(p){E=p}m*=f;E.x*=f;E.y*=f;var q=[];for(var C=0;C<v.length;C++){q.push(v[C])}for(var C=0;C<g.length;C++){q.push(g[C])
}function B(){function P(S){S.setTransform(1,0,0,1,0,0);!z&&S.clearRect(0,0,S.canvas.width,S.canvas.height);if(!A){S.translate(E.x,E.y);S.scale(m,m)}if(p){S.translate(p.x,p.y)}if(F){S.scale(F,F)}}if(G.canvasNeedsRedraw[a.DRAG]||G.canvasNeedsRedraw[a.NODE]||A){if(t.hideEdgesOnViewport&&(t.pinching||t.hoverData.dragging||t.data.wheel||t.swipePanning)){}else{t.findEdgeControlPoints(g)}var H=t.getCachedZSortedEles();if(h.hasCompoundNodes()){t.updateAllCompounds(H)}}var I;var K;var L;if(G.canvasNeedsRedraw[a.NODE]||A){if(!I||!K){I=[];
K=[];for(var N=0;N<H.length;N++){L=H[N];if(L._private.rscratch.inDragLayer){I.push(L)}else{K.push(L)}}}var J=z||G.canvases[a.NODE].getContext("2d");P(J);for(var N=0;N<K.length;N++){L=K[N];if(L._private.group=="nodes"){t.drawNode(J,L)}else{if(L._private.group=="edges"){t.drawEdge(J,L)}}}for(var N=0;N<K.length;N++){L=K[N];if(L._private.group=="nodes"){t.drawNodeText(J,L)}else{if(L._private.group=="edges"){t.drawEdgeText(J,L)}}if(L._private.group=="nodes"){t.drawNode(J,L,true)}else{if(L._private.group=="edges"){t.drawEdge(J,L,true)
}}}if(!A){G.canvasNeedsRedraw[a.NODE]=false}}if(G.canvasNeedsRedraw[a.DRAG]||A){if(!I||!K){I=[];K=[];for(var N=0;N<H.length;N++){L=H[N];if(L._private.rscratch.inDragLayer){I.push(L)}else{K.push(L)}}}var J=z||G.canvases[a.DRAG].getContext("2d");P(J);var L;for(var N=0;N<I.length;N++){L=I[N];if(L._private.group=="nodes"){t.drawNode(J,L)}else{if(L._private.group=="edges"){t.drawEdge(J,L)}}}for(var N=0;N<I.length;N++){L=I[N];if(L._private.group=="nodes"){t.drawNodeText(J,L)}else{if(L._private.group=="edges"){t.drawEdgeText(J,L)
}}if(L._private.group=="nodes"){t.drawNode(J,L,true)}else{if(L._private.group=="edges"){t.drawEdge(J,L,true)}}}if(!A){G.canvasNeedsRedraw[a.DRAG]=false}}if(G.canvasNeedsRedraw[a.SELECT_BOX]&&!A){var J=z||G.canvases[a.SELECT_BOX].getContext("2d");P(J);var O=h.style()._private.coreStyle;if(G.select[4]==1){var R=G.cy.zoom();var r=O["selection-box-border-width"].value/R;J.lineWidth=r;J.fillStyle="rgba("+O["selection-box-color"].value[0]+","+O["selection-box-color"].value[1]+","+O["selection-box-color"].value[2]+","+O["selection-box-opacity"].value+")";
J.fillRect(G.select[0],G.select[1],G.select[2]-G.select[0],G.select[3]-G.select[1]);if(r>0){J.strokeStyle="rgba("+O["selection-box-border-color"].value[0]+","+O["selection-box-border-color"].value[1]+","+O["selection-box-border-color"].value[2]+","+O["selection-box-opacity"].value+")";J.strokeRect(G.select[0],G.select[1],G.select[2]-G.select[0],G.select[3]-G.select[1])}}if(G.bgActivePosistion){var R=G.cy.zoom();var Q=G.bgActivePosistion;J.fillStyle="rgba("+O["active-bg-color"].value[0]+","+O["active-bg-color"].value[1]+","+O["active-bg-color"].value[2]+","+O["active-bg-opacity"].value+")";
J.beginPath();J.arc(Q.x,Q.y,O["active-bg-size"].pxValue/R,0,2*Math.PI);J.fill()}if(!A){G.canvasNeedsRedraw[a.SELECT_BOX]=false}}var M=+new Date;if(t.averageRedrawTime===undefined){t.averageRedrawTime=M-s}t.averageRedrawTime=t.averageRedrawTime/2+(M-s)/2}if(!z){setTimeout(B,0)}else{B()}if(!z&&!t.initrender){t.initrender=true;h.trigger("initrender")}}})(cytoscape);(function(b){var a=b("renderer","canvas");a.prototype.drawPolygonPath=function(f,d,j,h,c,g){f.translate(d,j);f.scale(h/2,c/2);f.beginPath();
...
}}else{if(B.which==1){if(A){A.activate()}if(A!=null&&p.nodeIsDraggable(A)){if(A._private.group=="nodes"&&A._private.selected==false){C=p.dragData.possibleDragElements=[];q(A,C);A.trigger(D);if(A._private.style.width.value=="auto"||A._private.style.height.value=="auto"){m(A,true,C)}v(A,true)}if(A._private.group=="nodes"&&A._private.selected==true){C=p.dragData.possibleDragElements=[];var I=false;var G=r.$("node:selected");for(var z=0;z<G.length;z++){if(p.nodeIsDraggable(G[z])){q(G[z],C);if(!I){A.trigger(D);
I=true}if(G[z]._private.style.width.value=="auto"||G[z]._private.style.height.value=="auto"){m(G[z],false,C)}v(G[z],true)}}}A.trigger(new b.Event(B,{type:"mousedown"})).trigger(new b.Event(B,{type:"tapstart"})).trigger(new b.Event(B,{type:"vmousedown"}))}else{if(A==null){r.trigger(new b.Event(B,{type:"mousedown"})).trigger(new b.Event(B,{type:"tapstart"})).trigger(new b.Event(B,{type:"vmousedown"}))}}p.hoverData.down=A;p.hoverData.downTime=(new Date()).getTime();if(A==null||A.isEdge()){F[4]=1;var J=Math.max(0,a.panOrBoxSelectDelay-(+new Date-p.hoverData.downTime));
clearTimeout(p.bgActiveTimeout);p.bgActiveTimeout=setTimeout(function(){if(A){A.unactivate()}p.data.bgActivePosistion={x:E[0],y:E[1]};p.data.canvasNeedsRedraw[a.SELECT_BOX]=true;p.redraw()},J)}}}F[0]=F[2]=E[0];F[1]=F[3]=E[1];p.redraw()},false);p.registerBinding(window,"mousemove",function(P){var F=false;var L=p.hoverData.capture;if(!L){var G=p.findContainerPageCoords();if(P.pageX>G[0]&&P.pageX<G[0]+p.data.container.clientWidth&&P.pageY>G[1]&&P.pageY<G[1]+p.data.container.clientHeight){}else{return
}}var A=p.data.cy;var E=p.projectIntoViewport(P.pageX,P.pageY);var J=p.data.select;var Q=p.findNearestElement(E[0],E[1],true);var H=p.hoverData.last;var I=p.hoverData.down;var M=[E[0]-J[2],E[1]-J[3]];var K=p.getCachedNodes();var z=p.getCachedEdges();var C=p.dragData.possibleDragElements;var B=P.shiftKey;F=true;var N=new b.Event(P,{type:"mousemove"});if(Q!=null){Q.trigger(N)}else{if(Q==null){A.trigger(N)}}if(p.hoverData.which===3){var D=new b.Event(P,{type:"cxtdrag"});if(I){I.trigger(D)}else{A.trigger(D)
}p.hoverData.cxtDragged=true}else{if(p.hoverData.dragging){F=true;if(A.panningEnabled()&&A.userPanningEnabled()){var r={x:M[0]*A.zoom(),y:M[1]*A.zoom()};A.panBy(r)}E=p.projectIntoViewport(P.pageX,P.pageY)}else{if(J[4]==1&&(I==null||I.isEdge())&&(!A.boxSelectionEnabled()||+new Date-p.hoverData.downTime>=a.panOrBoxSelectDelay)&&(Math.abs(J[3]-J[1])+Math.abs(J[2]-J[0])<4)&&A.panningEnabled()&&A.userPanningEnabled()){p.hoverData.dragging=true;J[4]=0}else{if(A.boxSelectionEnabled()&&Math.pow(J[2]-J[0],2)+Math.pow(J[3]-J[1],2)>7&&J[4]){clearTimeout(p.bgActiveTimeout);
p.data.bgActivePosistion=undefined}if(I&&I.isEdge()&&I.active()){I.unactivate()}if(Q!=H){if(H){H.trigger(new b.Event(P,{type:"mouseout"}))}if(Q){Q.trigger(new b.Event(P,{type:"mouseover"}))}p.hoverData.last=Q}if(I&&I.isNode()&&p.nodeIsDraggable(I)){p.dragData.didDrag=true;var R=[];for(var O=0;O<C.length;O++){if(C[O]._private.group=="nodes"&&p.nodeIsDraggable(C[O])){C[O]._private.position.x+=M[0];C[O]._private.position.y+=M[1];R.push(C[O])}}(new b.Collection(A,R)).trigger(new b.Event(P,{type:"drag"})).trigger(new b.Event(P,{type:"position"}));
if(J[2]==J[0]&&J[3]==J[1]){p.data.canvasNeedsRedraw[a.NODE]=true}p.data.canvasNeedsRedraw[a.DRAG]=true}if(A.boxSelectionEnabled()){p.data.canvasNeedsRedraw[a.SELECT_BOX]=true}F=true}}}J[2]=E[0];J[3]=E[1];p.redraw();if(F){if(P.stopPropagation){P.stopPropagation()}if(P.preventDefault){P.preventDefault()}return false}},false);p.registerBinding(window,"mouseup",function(T){var N=p.hoverData.capture;if(!N){return}p.hoverData.capture=false;var z=p.data.cy;var F=p.projectIntoViewport(T.pageX,T.pageY);var L=p.data.select;
var U=p.findNearestElement(F[0],F[1],true);var M=p.getCachedNodes();var r=p.getCachedEdges();var C=p.dragData.possibleDragElements;var H=p.hoverData.down;var B=T.shiftKey;p.data.bgActivePosistion=undefined;clearTimeout(p.bgActiveTimeout);if(H){H.unactivate()}if(p.hoverData.which===3){var D=new b.Event(T,{type:"cxttapend"});if(H){H.trigger(D)}else{z.trigger(D)}if(!p.hoverData.cxtDragged){var K=new b.Event(T,{type:"cxttap"});if(H){H.trigger(K)}else{z.trigger(K)}}p.hoverData.cxtDragged=false;p.hoverData.which=null
}else{if((H==null)&&!p.dragData.didDrag&&!(Math.pow(L[2]-L[0],2)+Math.pow(L[3]-L[1],2)>7&&L[4])&&!p.hoverData.dragging){z.$(":selected").unselect();if(C.length>0){p.data.canvasNeedsRedraw[a.NODE]=true}p.dragData.possibleDragElements=C=[]}if(Math.pow(L[2]-L[0],2)+Math.pow(L[3]-L[1],2)==0){if(U!=null){U.trigger(new b.Event(T,{type:"click"})).trigger(new b.Event(T,{type:"tap"})).trigger(new b.Event(T,{type:"vclick"}))}else{if(U==null){z.trigger(new b.Event(T,{type:"click"})).trigger(new b.Event(T,{type:"tap"})).trigger(new b.Event(T,{type:"vclick"}))
}}}if(U!=null){U.trigger(new b.Event(T,{type:"mouseup"})).trigger(new b.Event(T,{type:"tapend"})).trigger(new b.Event(T,{type:"vmouseup"}))}else{if(U==null){z.trigger(new b.Event(T,{type:"mouseup"})).trigger(new b.Event(T,{type:"tapend"})).trigger(new b.Event(T,{type:"vmouseup"}))}}if(U==H&&!p.dragData.didDrag){if(U!=null&&U._private.selectable){if(z.selectionType()==="additive"){if(U.selected()){U.unselect()}else{U.select()}}else{if(!B){z.$(":selected").unselect()}U.select()}v(U,false);p.data.canvasNeedsRedraw[a.NODE]=true
}}else{if(U==H){if(U!=null&&U._private.grabbed){var O=z.$(":grabbed");for(var R=0;R<O.length;R++){var I=O[R];I._private.grabbed=false;var J=I._private.edges;for(var Q=0;Q<J.length;Q++){J[Q]._private.rscratch.inDragLayer=false}v(I,false)}var S=new b.Event(T,{type:"free"});O.trigger(S)}}}if(z.boxSelectionEnabled()&&Math.pow(L[2]-L[0],2)+Math.pow(L[3]-L[1],2)>7&&L[4]){var A=[];var G=p.getAllInBox(L[0],L[1],L[2],L[3]);var P=new b.Event(T,{type:"select"});for(var R=0;R<G.length;R++){if(G[R]._private.selectable){C.push(G[R]);
A.push(G[R])}}var E=new b.Collection(z,A);if(z.selectionType()==="additive"){E.select()}else{if(!B){z.$(":selected").unselect()}E.select()}if(G.length>0){p.data.canvasNeedsRedraw[a.NODE]=true}}p.hoverData.dragging=false;if(!L[4]){var S=new b.Event(T,{type:"free"});for(var R=0;R<C.length;R++){if(C[R]._private.group=="nodes"){C[R]._private.rscratch.inDragLayer=false;var J=C[R]._private.edges;for(var Q=0;Q<J.length;Q++){J[Q]._private.rscratch.inDragLayer=false}v(C[R],false)}else{if(C[R]._private.group=="edges"){C[R]._private.rscratch.inDragLayer=false
}}}if(H){H.trigger(S)}p.data.canvasNeedsRedraw[a.DRAG]=true;p.data.canvasNeedsRedraw[a.NODE]=true}}L[4]=0;p.hoverData.down=null;p.data.canvasNeedsRedraw[a.SELECT_BOX]=true;p.dragData.didDrag=false;p.redraw()},false);var j=function(z){var C=p.data.cy;var B=p.projectIntoViewport(z.pageX,z.pageY);var A=[B[0]*C.zoom()+C.pan().x,B[1]*C.zoom()+C.pan().y];if(C.panningEnabled()&&C.userPanningEnabled()&&C.zoomingEnabled()&&C.userZoomingEnabled()){z.preventDefault();var r=z.wheelDeltaY/1000||z.wheelDelta/1000||z.detail/-32||-z.deltaY/500;
C.zoom({level:C.zoom()*Math.pow(10,r),renderedPosition:{x:A[0],y:A[1]}})}};p.registerBinding(p.data.container,"wheel",j,true);p.registerBinding(p.data.container,"mousewheel",j,true);p.registerBinding(p.data.container,"DOMMouseScroll",j,true);p.registerBinding(p.data.container,"MozMousePixelScroll",function(r){},false);p.registerBinding(p.data.container,"mouseout",function(r){p.data.cy.trigger(new b.Event(r,{type:"mouseout"}))},false);p.registerBinding(p.data.container,"mouseover",function(r){p.data.cy.trigger(new b.Event(r,{type:"mouseover"}))
},false);var d,k,s,w;var t;var g,f;var h,u;var l=p.data.container.clientWidth,o=p.data.container.clientHeight;var c;function e(z,B,r,A){return Math.sqrt((r-z)*(r-z)+(A-B)*(A-B))}p.registerBinding(p.data.container,"touchstart",function(T){clearTimeout(this.threeFingerSelectTimeout);if(T.target!==p.data.link){T.preventDefault()}p.touchData.capture=true;p.data.bgActivePosistion=undefined;var B=p.data.cy;var M=p.getCachedNodes();var A=p.getCachedEdges();var z=p.touchData.now;var Q=p.touchData.earlier;
if(T.touches[0]){var E=p.projectIntoViewport(T.touches[0].pageX,T.touches[0].pageY);z[0]=E[0];z[1]=E[1]}if(T.touches[1]){var E=p.projectIntoViewport(T.touches[1].pageX,T.touches[1].pageY);z[2]=E[0];z[3]=E[1]}if(T.touches[2]){var E=p.projectIntoViewport(T.touches[2].pageX,T.touches[2].pageY);z[4]=E[0];z[5]=E[1]}if(T.touches[1]){function S(V){for(var W=0;W<V.length;W++){V[W]._private.grabbed=false;V[W]._private.rscratch.inDragLayer=false;if(V[W].active()){V[W].unactivate()}}}S(M);S(A);var D=p.findContainerPageCoords();
u=D[1];h=D[0];d=T.touches[0].pageX-h;k=T.touches[0].pageY-u;s=T.touches[1].pageX-h;w=T.touches[1].pageY-u;c=0<=d&&d<=l&&0<=s&&s<=l&&0<=k&&k<=o&&0<=w&&w<=o;var L=B.pan();var r=B.zoom();t=e(d,k,s,w);g=[(d+s)/2,(k+w)/2];f=[(g[0]-L.x)/r,(g[1]-L.y)/r];if(t<100){var G=p.findNearestElement(z[0],z[1],true);var F=p.findNearestElement(z[2],z[3],true);var C=new b.Event(T,{type:"cxttapstart"});if(G&&G.isNode()){G.activate().trigger(C);p.touchData.start=G}else{if(F&&F.isNode()){F.activate().trigger(C);p.touchData.start=F
}else{B.trigger(C);p.touchData.start=null}}if(p.touchData.start){p.touchData.start._private.grabbed=false}p.touchData.cxt=true;p.touchData.cxtDragged=false;p.data.bgActivePosistion=undefined;p.redraw();return}}if(T.touches[2]){}else{if(T.touches[1]){}else{if(T.touches[0]){var U=p.findNearestElement(z[0],z[1],true);if(U!=null){U.activate();p.touchData.start=U;if(U._private.group=="nodes"&&p.nodeIsDraggable(U)){var H=p.dragData.touchDragEles=[];q(U,H);U.trigger(new b.Event(T,{type:"grab"}));if(U.selected()){H=p.dragData.touchDragEles=[];
var J=B.$("node:selected");for(var N=0;N<J.length;N++){var R=J[N];if(p.nodeIsDraggable(R)){H.push(R);R._private.rscratch.inDragLayer=true;var I=R._private.edges;for(var O=0;O<I.length;O++){I[O]._private.rscratch.inDragLayer=true}if(R._private.style.width.value=="auto"||R._private.style.height.value=="auto"){m(R,false,H)}v(R,true)}}}else{if(U._private.style.width.value=="auto"||U._private.style.height.value=="auto"){m(U,true,H)}v(U,true)}}U.trigger(new b.Event(T,{type:"touchstart"})).trigger(new b.Event(T,{type:"tapstart"})).trigger(new b.Event(T,{type:"vmousdown"}))
}if(U==null){B.trigger(new b.Event(T,{type:"touchstart"})).trigger(new b.Event(T,{type:"tapstart"})).trigger(new b.Event(T,{type:"vmousedown"}));p.data.bgActivePosistion={x:E[0],y:E[1]};p.data.canvasNeedsRedraw[a.SELECT_BOX]=true}for(var P=0;P<z.length;P++){Q[P]=z[P];p.touchData.startPosition[P]=z[P]}p.touchData.singleTouchMoved=false;p.touchData.singleTouchStartTime=+new Date;var K=setTimeout(function(){if(p.touchData.singleTouchMoved==false&&(+new Date)-p.touchData.singleTouchStartTime>250){if(p.touchData.start){p.touchData.start.trigger(new b.Event(T,{type:"taphold"}))
}else{p.data.cy.trigger(new b.Event(T,{type:"taphold"}));B.$(":selected").unselect()}}},1000)}}}p.redraw()},false);p.registerBinding(window,"touchmove",function(ag){var Z=p.data.select;var N=p.touchData.capture;N&&ag.preventDefault();var L=p.data.cy;var V=p.getCachedNodes();var F=p.getCachedEdges();var Y=p.touchData.now;var z=p.touchData.earlier;if(ag.touches[0]){var Q=p.projectIntoViewport(ag.touches[0].pageX,ag.touches[0].pageY);Y[0]=Q[0];Y[1]=Q[1]}if(ag.touches[1]){var Q=p.projectIntoViewport(ag.touches[1].pageX,ag.touches[1].pageY);
...
}if(ag.touches[1]){var Q=p.projectIntoViewport(ag.touches[1].pageX,ag.touches[1].pageY);Y[2]=Q[0];Y[3]=Q[1]}if(ag.touches[2]){var Q=p.projectIntoViewport(ag.touches[2].pageX,ag.touches[2].pageY);Y[4]=Q[0];Y[5]=Q[1]}}else{if(ag.touches[0]){var O=p.touchData.start;var I=p.touchData.last;if(O!=null&&O._private.group=="nodes"&&p.nodeIsDraggable(O)){var J=p.dragData.touchDragEles;for(var ab=0;ab<J.length;ab++){var M=J[ab];if(p.nodeIsDraggable(M)){p.dragData.didDrag=true;M._private.position.x+=B[0];M._private.position.y+=B[1]
}}(new b.Collection(L,J)).trigger(new b.Event(ag,{type:"drag"})).trigger(new b.Event(ag,{type:"position"}));p.data.canvasNeedsRedraw[a.DRAG]=true;if(p.touchData.startPosition[0]==z[0]&&p.touchData.startPosition[1]==z[1]){p.data.canvasNeedsRedraw[a.NODE]=true}}if(O!=null){O.trigger(new b.Event(ag,{type:"touchmove"}))}if(O==null){var R=p.findNearestElement(Y[0],Y[1],true);if(R!=null){R.trigger(new b.Event(ag,{type:"touchmove"}))}if(R==null){L.trigger(new b.Event(ag,{type:"touchmove"}))}}if(R!=I){if(I){I.trigger(new b.Event(ag,{type:"touchout"}))
}if(R){R.trigger(new b.Event(ag,{type:"touchover"}))}}p.touchData.last=R;for(var af=0;af<Y.length;af++){if(Y[af]&&p.touchData.startPosition[af]&&Math.abs(Y[af]-p.touchData.startPosition[af])>4){p.touchData.singleTouchMoved=true}}if(N&&(O==null||O.isEdge())&&L.panningEnabled()&&L.userPanningEnabled()){if(O){O.unactivate();if(!p.data.bgActivePosistion){p.data.bgActivePosistion={x:Y[0],y:Y[1]}}p.data.canvasNeedsRedraw[a.SELECT_BOX]=true}L.panBy({x:B[0]*L.zoom(),y:B[1]*L.zoom()});p.swipePanning=true;
var Q=p.projectIntoViewport(ag.touches[0].pageX,ag.touches[0].pageY);Y[0]=Q[0];Y[1]=Q[1]}}}}}for(var ad=0;ad<Y.length;ad++){z[ad]=Y[ad]}p.redraw()},false);p.registerBinding(window,"touchend",function(V){var M=p.touchData.capture;if(!M){return}p.touchData.capture=false;V.preventDefault();var J=p.data.select;p.swipePanning=false;var B=p.data.cy;var L=p.getCachedNodes();var A=p.getCachedEdges();var z=p.touchData.now;var S=p.touchData.earlier;var D=p.touchData.start;if(V.touches[0]){var E=p.projectIntoViewport(V.touches[0].pageX,V.touches[0].pageY);
z[0]=E[0];z[1]=E[1]}if(V.touches[1]){var E=p.projectIntoViewport(V.touches[1].pageX,V.touches[1].pageY);z[2]=E[0];z[3]=E[1]}if(V.touches[2]){var E=p.projectIntoViewport(V.touches[2].pageX,V.touches[2].pageY);z[4]=E[0];z[5]=E[1]}if(p.touchData.cxt){ctxTapend=new b.Event(V,{type:"cxttapend"});if(D){D.unactivate();D.trigger(ctxTapend)}else{B.trigger(ctxTapend)}if(!p.touchData.cxtDragged){var K=new b.Event(V,{type:"cxttap"});if(D){D.trigger(K)}else{B.trigger(K)}}if(p.touchData.start){p.touchData.start._private.grabbed=false
}p.touchData.cxt=false;p.touchData.start=null;p.redraw();return}var r=+new Date;if(!V.touches[2]&&B.boxSelectionEnabled()){clearTimeout(this.threeFingerSelectTimeout);var C=[];var G=p.getAllInBox(J[0],J[1],J[2],J[3]);J[0]=undefined;J[1]=undefined;J[2]=undefined;J[3]=undefined;J[4]=0;p.data.canvasNeedsRedraw[a.SELECT_BOX]=true;var Q=new b.Event(V,{type:"select"});for(var R=0;R<G.length;R++){if(G[R]._private.selectable){C.push(G[R])}}var F=(new b.Collection(B,C));if(B.selectionType()==="single"){B.$(":selected").unselect()
}F.select();if(G.length>0){p.data.canvasNeedsRedraw[a.NODE]=true}}if(!V.touches[1]){p.pinching=false}var P=false;if(D!=null){D._private.active=false;P=true;D.trigger(new b.Event(V,{type:"unactivate"}))}if(V.touches[2]){p.data.bgActivePosistion=undefined;p.data.canvasNeedsRedraw[a.SELECT_BOX]=true}else{if(V.touches[1]){}else{if(V.touches[0]){}else{if(!V.touches[0]){p.data.bgActivePosistion=undefined;p.data.canvasNeedsRedraw[a.SELECT_BOX]=true;if(D!=null){if(D._private.grabbed==true){D._private.grabbed=false;
D.trigger(new b.Event(V,{type:"free"}));D._private.rscratch.inDragLayer=false}var I=D._private.edges;for(var O=0;O<I.length;O++){I[O]._private.rscratch.inDragLayer=false}v(D,false);if(D.selected()){var H=B.$("node:selected");for(var N=0;N<H.length;N++){var T=H[N];T._private.rscratch.inDragLayer=false;var I=T._private.edges;for(var O=0;O<I.length;O++){I[O]._private.rscratch.inDragLayer=false}v(T,false)}}p.data.canvasNeedsRedraw[a.DRAG]=true;p.data.canvasNeedsRedraw[a.NODE]=true;D.trigger(new b.Event(V,{type:"touchend"})).trigger(new b.Event(V,{type:"tapend"})).trigger(new b.Event(V,{type:"vmouseup"}));
p.touchData.start=null}else{var U=p.findNearestElement(z[0],z[1],true);if(U!=null){U.trigger(new b.Event(V,{type:"touchend"})).trigger(new b.Event(V,{type:"tapend"})).trigger(new b.Event(V,{type:"vmouseup"}))}if(U==null){B.trigger(new b.Event(V,{type:"touchend"})).trigger(new b.Event(V,{type:"tapend"})).trigger(new b.Event(V,{type:"vmouseup"}))}}if(D!=null&&!p.dragData.didDrag&&D._private.selectable&&(Math.sqrt(Math.pow(p.touchData.startPosition[0]-z[0],2)+Math.pow(p.touchData.startPosition[1]-z[1],2)))<6){if(B.selectionType()==="single"){B.$(":selected").unselect();
D.select()}else{if(D.selected()){D.unselect()}else{D.select()}}P=true;p.data.canvasNeedsRedraw[a.NODE]=true}if(p.touchData.singleTouchMoved==false){if(D){D.trigger(new b.Event(V,{type:"tap"})).trigger(new b.Event(V,{type:"vclick"}))}else{B.trigger(new b.Event(V,{type:"tap"})).trigger(new b.Event(V,{type:"vclick"}))}}p.touchData.singleTouchMoved=true}}}}for(var O=0;O<z.length;O++){S[O]=z[O]}p.dragData.didDrag=false;if(P&&D){D.updateStyle(false)}p.redraw()},false)}})(cytoscape);(function(g){var e=g("renderer","canvas");
var c=e.prototype;var a=e.nodeShapes={};a.ellipse={draw:function(l,p,o,m,k){a.ellipse.drawPath(l,p,o,m,k);l.fill()},drawPath:function(l,p,o,m,k){l.beginPath();l.translate(p,o);l.scale(m/2,k/2);l.arc(0,0,1,0,Math.PI*2*0.999,false);l.closePath();l.scale(2/m,2/k);l.translate(-p,-o)},intersectLine:function(q,p,o,m,l,s,r){var k=g.math.intersectLineEllipse(l,s,q,p,o/2+r,m/2+r);return k},intersectBox:function(m,s,k,q,l,t,p,o,r){return g.math.boxIntersectEllipse(m,s,k,q,r,l,t,p,o)},checkPointRough:function(l,r,q,m,k,p,o){return true
},checkPoint:function(l,r,q,m,k,p,o){l-=p;r-=o;l/=(m/2+q);r/=(k/2+q);return(Math.pow(l,2)+Math.pow(r,2)<=1)}};a.triangle={points:g.math.generateUnitNgonPoints(3,0),draw:function(l,p,o,m,k){c.drawPolygon(l,p,o,m,k,a.triangle.points)},drawPath:function(l,p,o,m,k){c.drawPolygonPath(l,p,o,m,k,a.triangle.points)},intersectLine:function(p,o,m,l,k,r,q){return g.math.polygonIntersectLine(k,r,a.triangle.points,p,o,m/2,l/2,q)},intersectBox:function(m,s,k,q,l,u,p,o,r){var t=a.triangle.points;return g.math.boxIntersectPolygon(m,s,k,q,t,l,u,p,o,[0,-1],r)
},checkPointRough:function(l,r,q,m,k,p,o){return g.math.checkInBoundingBox(l,r,a.triangle.points,q,m,k,p,o)},checkPoint:function(l,r,q,m,k,p,o){return g.math.pointInsidePolygon(l,r,a.triangle.points,p,o,m,k,[0,-1],q)}};a.square={points:g.math.generateUnitNgonPoints(4,0),draw:function(l,p,o,m,k){c.drawPolygon(l,p,o,m,k,a.square.points)},drawPath:function(l,p,o,m,k){c.drawPolygonPath(l,p,o,m,k,a.square.points)},intersectLine:function(p,o,m,l,k,r,q){return g.math.polygonIntersectLine(k,r,a.square.points,p,o,m/2,l/2,q)
},intersectBox:function(m,s,k,q,l,u,p,o,r){var t=a.square.points;return g.math.boxIntersectPolygon(m,s,k,q,t,l,u,p,o,[0,-1],r)},checkPointRough:function(l,r,q,m,k,p,o){return g.math.checkInBoundingBox(l,r,a.square.points,q,m,k,p,o)},checkPoint:function(l,r,q,m,k,p,o){return g.math.pointInsidePolygon(l,r,a.square.points,p,o,m,k,[0,-1],q)}};a.rectangle=a.square;a.octogon={};a.roundrectangle={points:g.math.generateUnitNgonPoints(4,0),draw:function(l,p,o,m,k){c.drawRoundRectangle(l,p,o,m,k,10)},drawPath:function(l,p,o,m,k){c.drawRoundRectanglePath(l,p,o,m,k,10)
},intersectLine:function(p,o,m,l,k,r,q){return g.math.roundRectangleIntersectLine(k,r,p,o,m,l,q)},intersectBox:function(m,s,k,q,l,t,p,o,r){return g.math.roundRectangleIntersectBox(m,s,k,q,l,t,p,o,r)},checkPointRough:function(l,r,q,m,k,p,o){return g.math.checkInBoundingBox(l,r,a.roundrectangle.points,q,m,k,p,o)},checkPoint:function(s,q,r,k,t,o,l){var p=g.math.getRoundRectangleRadius(k,t);if(g.math.pointInsidePolygon(s,q,a.roundrectangle.points,o,l,k,t-2*p,[0,-1],r)){return true}if(g.math.pointInsidePolygon(s,q,a.roundrectangle.points,o,l,k-2*p,t,[0,-1],r)){return true
}var m=function(v,C,B,z,w,u,A){v-=B;C-=z;v/=(w/2+A);C/=(u/2+A);return(Math.pow(v,2)+Math.pow(C,2)<=1)};if(m(s,q,o-k/2+p,l-t/2+p,p*2,p*2,r)){return true}if(m(s,q,o+k/2-p,l-t/2+p,p*2,p*2,r)){return true}if(m(s,q,o+k/2-p,l+t/2-p,p*2,p*2,r)){return true}if(m(s,q,o-k/2+p,l+t/2-p,p*2,p*2,r)){return true}return false}};a.roundrectangle2={roundness:4.99,draw:function(m,l,k){if(l<=roundness*2){return}c.drawPolygon(m._private.position.x,m._private.position.y,l,k,nodeSapes.roundrectangle2.points)},intersectLine:function(o,m,l,k,p){return g.math.findPolygonIntersection(o,m,l,k,p,a.square.points)
},intersectBox:function(m,s,k,q,l,u,p,o,r){var t=a.square.points}};a.pentagon={points:g.math.generateUnitNgonPoints(5,0),draw:function(l,p,o,m,k){c.drawPolygon(l,p,o,m,k,a.pentagon.points)},drawPath:function(l,p,o,m,k){c.drawPolygonPath(l,p,o,m,k,a.pentagon.points)},intersectLine:function(p,o,m,l,k,r,q){return c.polygonIntersectLine(k,r,a.pentagon.points,p,o,m/2,l/2,q)},intersectBox:function(m,s,k,q,l,u,p,o,r){var t=a.pentagon.points;return g.math.boxIntersectPolygon(m,s,k,q,t,l,u,p,o,[0,-1],r)},checkPointRough:function(l,r,q,m,k,p,o){return g.math.checkInBoundingBox(l,r,a.pentagon.points,q,m,k,p,o)
