...
var O=i.data.select;var K=i.findNearestElement(N[0],N[1]);var P=i.hoverData.down;var M=i.dragData.possibleDragElements;if(L.button==0){if(K&&K._private.grabbable){if(K._private.group=="nodes"&&K._private.selected==false){K._private.grabbed=true;K._private.rscratch.inDragLayer=true;K.trigger(new s.Event(L,{type:"grab"}));var E=new s.Event(L,{type:"unselect"});var D=new s.Event(L,{type:"free"});for(var I=0;I<M.length;I++){var G=M[I];var Q=false;if(G._private.selected||G._private.grabbed){Q=true}if(G._private.selected){G._private.selected=false;
G.trigger(E)}if(G._private.grabbed){G._private.grabbed=false;G.trigger(D)}if(Q){G.updateStyle(false)}}i.dragData.possibleDragElements=M=[];i.dragData.possibleDragElements.push(K);for(var I=0;I<K._private.edges.length;I++){K._private.edges[I]._private.rscratch.inDragLayer=true}}if(K._private.group=="nodes"&&K._private.selected==true){var B=new s.Event(L,{type:"grab"});for(var I=0;I<M.length;I++){if(M[I]._private.group=="nodes"){M[I]._private.rscratch.inDragLayer=true;M[I]._private.grabbed=true;var C=M[I]._private.edges;
for(var H=0;H<C.length;H++){C[H]._private.rscratch.inDragLayer=true}M[I].trigger(B)}}}K.trigger(new s.Event(L,{type:"mousedown"}));i.data.canvasNeedsRedraw[d]=true;i.data.canvasRedrawReason[d].push("Single node moved to drag layer");i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("Single node moved to drag layer")}else{if(K==null){var J=new s.Event(L,{type:"mousedown"});F.trigger(J)}}i.hoverData.down=K;i.hoverData.downTime=(new Date()).getTime();if(K==null){O[4]=1}}else{if(L.button==1){i.hoverData.dragging=true;
i.hoverData.initialPan=[F.pan().x,F.pan().y]}}O[0]=O[2]=N[0];O[1]=O[3]=N[1];i.redraw()},false);window.addEventListener("mousemove",function(I){var D=i.data.cy;var K=i.projectIntoViewport(I.pageX,I.pageY);var L=i.data.select;var H=i.findNearestElement(K[0],K[1]);var N=i.hoverData.last;var M=i.hoverData.down;var O=[K[0]-L[2],K[1]-L[3]];var C=i.getCachedNodes();var E=i.getCachedEdges();var J=i.dragData.possibleDragElements;var Q=i.hoverData.capture;if(!Q){var P=i.findContainerPageCoords();if(I.pageX>P[0]&&I.pageX<P[0]+i.data.container.clientWidth&&I.pageY>P[1]&&I.pageY<P[1]+i.data.container.clientHeight){}else{return
}}var B=new s.Event(I,{type:"mousemove"});if(H!=null){H.trigger(B)}else{if(H==null){D.trigger(B)}}if(i.hoverData.dragging){D.panBy({x:O[0]*D.zoom(),y:O[1]*D.zoom()});K=i.projectIntoViewport(I.pageX,I.pageY)}else{if(L[4]==1&&M==null&&(new Date()).getTime()-i.hoverData.downTime>200&&(Math.abs(L[3]-L[1])+Math.abs(L[2]-L[0])<4)){i.hoverData.dragging=true;L[4]=0}else{if(H!=N){if(N){N.trigger(new s.Event(I,{type:"mouseout"}))}if(H){H.trigger(new s.Event(I,{type:"mouseover"}))}i.hoverData.last=H}if(M){var G=new s.Event(I,{type:"position"});
for(var F=0;F<J.length;F++){if(!J[F]._private.locked&&J[F]._private.group=="nodes"){J[F]._private.position.x+=O[0];J[F]._private.position.y+=O[1];J[F].trigger(G)}}i.data.canvasNeedsRedraw[d]=true;i.data.canvasRedrawReason[d].push("Nodes dragged")}i.data.canvasNeedsRedraw[w]=true;i.data.canvasRedrawReason[w].push("Mouse moved, redraw selection box")}}L[2]=K[0];L[3]=K[1];i.redraw()},false);window.addEventListener("mouseup",function(S){var N=i.hoverData.capture;if(!N){return}i.hoverData.capture=false;
var D=i.data.cy;var H=i.projectIntoViewport(S.pageX,S.pageY);var L=i.data.select;var T=i.findNearestElement(H[0],H[1]);var M=i.getCachedNodes();var C=i.getCachedEdges();var G=i.dragData.possibleDragElements;var J=i.hoverData.down;if(T==null||T!=J){var F=new s.Event(S,{type:"unselect"});for(var Q=0;Q<G.length;Q++){if(G[Q]._private.selected){G[Q]._private.selected=false;G[Q].trigger(F);G[Q].updateStyle(false)}}if(G.length>0){i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("De-select")
}i.dragData.possibleDragElements=G=[]}if(Math.pow(L[2]-L[0],2)+Math.pow(L[3]-L[1],2)==0){var B=new s.Event(S,{type:"click"});if(T!=null){T.trigger(B)}else{if(T==null){D.trigger(B)}}}var E=new s.Event(S,{type:"mouseup"});if(T!=null){T.trigger(E)}else{if(T==null){D.trigger(E)}}if(T==J&&(Math.pow(L[2]-L[0],2)+Math.pow(L[3]-L[1],2)<7)){if(T!=null&&T._private.selectable&&T._private.selected==false){T._private.selected=true;T.trigger(new s.Event(S,{type:"select"}));T.updateStyle(false);G.push(T);i.data.canvasNeedsRedraw[z]=true;
i.data.canvasRedrawReason[z].push("sglslct")}}else{if(T==J){if(T!=null&&T._private.grabbed){var R=new s.Event(S,{type:"free"});T._private.grabbed=false;T.trigger(R);var K=T._private.edges;for(var P=0;P<K.length;P++){K[P]._private.rscratch.inDragLayer=false}}}}if(Math.pow(L[2]-L[0],2)+Math.pow(L[3]-L[1],2)>7&&L[4]){var I=i.getAllInBox(L[0],L[1],L[2],L[3]);var O=new s.Event(S,{type:"select"});for(var Q=0;Q<I.length;Q++){if(I[Q]._private.selectable){I[Q]._private.selected=true;I[Q].trigger(O);I[Q].updateStyle(false);
G.push(I[Q])}}if(I.length>0){i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("Selection")}}i.hoverData.dragging=false;if(!L[4]){var R=new s.Event(S,{type:"free"});for(var Q=0;Q<G.length;Q++){if(G[Q]._private.group=="nodes"){G[Q]._private.rscratch.inDragLayer=false;var K=G[Q]._private.edges;for(var P=0;P<K.length;P++){K[P]._private.rscratch.inDragLayer=false}}else{if(G[Q]._private.group=="edges"){G[Q]._private.rscratch.inDragLayer=false}}G[Q].trigger(R)}i.data.canvasNeedsRedraw[d]=true;
