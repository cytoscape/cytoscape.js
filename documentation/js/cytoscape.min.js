...
this.redraws++;this.redraw()};t.prototype.load=function(){var i=this;i.data.container.addEventListener("mousedown",function(K){i.hoverData.capture=true;var F=i.data.cy;var M=i.projectIntoViewport(K.pageX,K.pageY);var N=i.data.select;var J=i.findNearestElement(M[0],M[1]);var O=i.hoverData.down;var L=i.dragData.possibleDragElements;if(K.button==0){if(J&&J._private.grabbable){if(J._private.group=="nodes"&&J._private.selected==false){J._private.grabbed=true;J.trigger(new s.Event(K,{type:"grab"}));var E=new s.Event(K,{type:"unselect"});
var D=new s.Event(K,{type:"free"});for(var I=0;I<L.length;I++){var G=L[I];var P=false;if(G._private.selected||G._private.grabbed){P=true}if(G._private.selected){G._private.selected=false;G.trigger(E)}if(G._private.grabbed){G._private.grabbed=false;G.trigger(D)}if(P){G.updateStyle(false)}}i.dragData.possibleDragElements=L=[];L.push(J);for(var I=0;I<J._private.edges.length;I++){J._private.edges[I]._private.grabbed=true}}if(J._private.group=="nodes"&&J._private.selected==true){var B=new s.Event(K,{type:"grab"});
for(var I=0;I<L.length;I++){if(L[I]._private.group=="nodes"){L[I]._private.grabbed=true;var C=L[I]._private.edges;for(var H=0;H<C.length;H++){C[H]._private.grabbed=true}L[I].trigger(B)}}}i.data.canvasNeedsRedraw[d]=true;i.data.canvasRedrawReason[d].push("Single node moved to drag layer");i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("Single node moved to drag layer")}if(J){J.trigger(new s.Event(K,{type:"mousedown"}))}i.hoverData.down=J;i.hoverData.downTime=(new Date()).getTime();
if(J==null){N[4]=1}}else{if(K.button==1){i.hoverData.dragging=true;i.hoverData.initialPan=[F.pan().x,F.pan().y]}}N[0]=N[2]=M[0];N[1]=N[3]=M[1];i.redraw()},false);window.addEventListener("mousemove",function(I){var D=i.data.cy;var K=i.projectIntoViewport(I.pageX,I.pageY);var L=i.data.select;var H=i.findNearestElement(K[0],K[1]);var N=i.hoverData.last;var M=i.hoverData.down;var O=[K[0]-L[2],K[1]-L[3]];var C=i.getCachedNodes();var E=i.getCachedEdges();var J=i.dragData.possibleDragElements;var Q=i.hoverData.capture;
if(!Q){var P=i.findContainerPageCoords();if(I.pageX>P[0]&&I.pageX<P[0]+i.data.container.clientWidth&&I.pageY>P[1]&&I.pageY<P[1]+i.data.container.clientHeight){}else{return}}if(i.hoverData.dragging){D.panBy({x:O[0]*D.zoom(),y:O[1]*D.zoom()});K=i.projectIntoViewport(I.pageX,I.pageY);var B=new s.Event(I,{type:"mousemove"});if(H!=null){H.trigger(B)}else{if(H==null){D.trigger(B)}}}else{if(L[4]==1&&M==null&&(new Date()).getTime()-i.hoverData.downTime>200&&(Math.abs(L[3]-L[1])+Math.abs(L[2]-L[0])<4)){i.hoverData.dragging=true;
L[4]=0}else{if(H!=N){if(N){N.trigger(new s.Event(I,{type:"mouseout"}))}if(H){H.trigger(new s.Event(I,{type:"mouseover"}))}i.hoverData.last=H}if(M){var G=new s.Event(I,{type:"position"});for(var F=0;F<J.length;F++){if(!J[F]._private.locked&&J[F]._private.group=="nodes"){J[F]._private.position.x+=O[0];J[F]._private.position.y+=O[1];J[F].trigger(G)}}i.data.canvasNeedsRedraw[d]=true;i.data.canvasRedrawReason[d].push("Nodes dragged")}if(H){H.trigger(new s.Event(I,{type:"mousemove"}))}i.data.canvasNeedsRedraw[w]=true;
i.data.canvasRedrawReason[w].push("Mouse moved, redraw selection box")}}L[2]=K[0];L[3]=K[1];i.redraw()},false);window.addEventListener("mouseup",function(S){var N=i.hoverData.capture;if(!N){return}i.hoverData.capture=false;var D=i.data.cy;var H=i.projectIntoViewport(S.pageX,S.pageY);var L=i.data.select;var T=i.findNearestElement(H[0],H[1]);var M=i.getCachedNodes();var C=i.getCachedEdges();var G=i.dragData.possibleDragElements;var J=i.hoverData.down;if(T==null||T!=J||!T.selected()){var F=new s.Event(S,{type:"unselect"});
for(var Q=0;Q<G.length;Q++){if(G[Q]._private.selected){G[Q]._private.selected=false;G[Q].trigger(F);G[Q].updateStyle(false)}}if(G.length>0){i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("De-select")}G=i.dragData.possibleDragElements=[]}if(Math.pow(L[2]-L[0],2)+Math.pow(L[3]-L[1],2)==0){var B=new s.Event(S,{type:"click"});if(T!=null){T.trigger(B)}else{if(T==null){D.trigger(B)}}}var E=new s.Event(S,{type:"mouseup"});if(T!=null){T.trigger(E)}else{if(T==null){D.trigger(E)}}if(T==J&&(Math.pow(L[2]-L[0],2)+Math.pow(L[3]-L[1],2)<7)){if(T!=null&&T._private.selectable&&T._private.selected==false){T._private.selected=true;
...
F.zoom({level:F.zoom()*(1+B),position:{x:C[0],y:C[1]}})}};i.data.container.addEventListener("mousewheel",A,false);i.data.container.addEventListener("DOMMouseScroll",A,false);i.data.container.addEventListener("mousemove",function(B){if(i.zoomData.lastPointerX&&i.zoomData.lastPointerX!=B.pageX&&!i.zoomData.freeToZoom){i.zoomData.freeToZoom=true}i.zoomData.lastPointerX=B.pageX},false);i.data.container.addEventListener("mouseout",function(B){i.zoomData.freeToZoom=false;i.zoomData.lastPointerX=null},false);
i.data.container.addEventListener("touchstart",function(K){K.preventDefault();i.touchData.capture=true;var D=i.data.cy;var B=i.getCachedNodes();var G=i.getCachedEdges();var C=i.touchData.now;var E=i.touchData.earlier;if(K.touches[0]){var M=i.projectIntoViewport(K.touches[0].pageX,K.touches[0].pageY);C[0]=M[0];C[1]=M[1]}if(K.touches[1]){var M=i.projectIntoViewport(K.touches[1].pageX,K.touches[1].pageY);C[2]=M[0];C[3]=M[1]}if(K.touches[2]){var M=i.projectIntoViewport(K.touches[2].pageX,K.touches[2].pageY);
C[4]=M[0];C[5]=M[1]}if(K.touches[2]){}else{if(K.touches[1]){}else{if(K.touches[0]){var J=i.findNearestElement(C[0],C[1]);if(J!=null){i.touchData.start=J;if(J._private.group=="nodes"){J._private.grabbed=true;J.trigger(new s.Event(K,{type:"grab"}));i.data.canvasNeedsRedraw[d]=true;i.data.canvasRedrawReason[d].push("touchdrag node start");i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("touchdrag node start");var I=J._private.edges;for(var F=0;F<I.length;F++){I[F]._private.grabbed=true
}}J.trigger(new s.Event(K,{type:"touchstart"}))}else{if(J==null){D.trigger(new s.Event(K,{type:"touchstart"}))}}for(var H=0;H<C.length;H++){E[H]=C[H];i.touchData.startPosition[H]=C[H]}i.touchData.singleTouchMoved=false;i.touchData.singleTouchStartTime=l();var L=setTimeout(function(){if(i.touchData.singleTouchMoved==false&&l()-i.touchData.singleTouchStartTime<1040&&l()-i.touchData.singleTouchStartTime>960){if(i.touchData.start){i.touchData.start.trigger(new s.Event(K,{type:"taphold"}))}else{i.data.cy.trigger(new s.Event(K,{type:"taphold"}))
}}},1000)}}}i.redraw()},true);window.addEventListener("touchmove",function(N){var R=i.touchData.capture;if(!R){return}N.preventDefault();var F=i.data.cy;var B=i.getCachedNodes();var I=i.getCachedEdges();var D=i.touchData.now;var G=i.touchData.earlier;if(N.touches[0]){var P=i.projectIntoViewport(N.touches[0].pageX,N.touches[0].pageY);D[0]=P[0];D[1]=P[1]}if(N.touches[1]){var P=i.projectIntoViewport(N.touches[1].pageX,N.touches[1].pageY);D[2]=P[0];D[3]=P[1]}if(N.touches[2]){var P=i.projectIntoViewport(N.touches[2].pageX,N.touches[2].pageY);
D[4]=P[0];D[5]=P[1]}var Q=[];for(var H=0;H<D.length;H++){Q[H]=D[H]-G[H]}if(N.touches[2]){}else{if(N.touches[1]){var M=[(Q[0]+Q[2])/2,(Q[1]+Q[3])/2];F.panBy({x:M[0]*F.zoom(),y:M[1]*F.zoom()});var K=Math.sqrt(Math.pow(G[2]-G[0],2)+Math.pow(G[3]-G[1],2));var E=Math.sqrt(Math.pow(D[2]-D[0],2)+Math.pow(D[3]-D[1],2));var O=E/K;if(O>1){O=(O-1)*1.5+1}else{O=1-(1-O)*1.5}F.zoom({level:F.zoom()*O,position:{x:(D[0]+D[2])/2,y:(D[1]+D[3])/2}});if(N.touches[0]){var P=i.projectIntoViewport(N.touches[0].pageX,N.touches[0].pageY);
D[0]=P[0];D[1]=P[1]}if(N.touches[1]){var P=i.projectIntoViewport(N.touches[1].pageX,N.touches[1].pageY);D[2]=P[0];D[3]=P[1]}if(N.touches[2]){var P=i.projectIntoViewport(N.touches[2].pageX,N.touches[2].pageY);D[4]=P[0];D[5]=P[1]}}else{if(N.touches[0]){var C=i.touchData.start;if(C!=null&&C._private.group=="nodes"){C._private.position.x+=Q[0];C._private.position.y+=Q[1];i.data.canvasNeedsRedraw[d]=true;i.data.canvasRedrawReason[d].push("touchdrag node");C.trigger(new s.Event(N,{type:"position"}))}if(C!=null){C.trigger(new s.Event(N,{type:"touchmove"}))
}if(C==null){var L=i.findNearestElement(D[0],D[1]);if(L!=null){L.trigger(new s.Event(N,{type:"touchmove"}))}if(L==null){F.trigger(new s.Event(N,{type:"touchmove"}))}}for(var J=0;J<D.length;J++){if(D[J]&&i.touchData.startPosition[J]&&Math.abs(D[J]-i.touchData.startPosition[J])>4){i.touchData.singleTouchMoved=true}}if(C==null){F.panBy({x:Q[0]*F.zoom(),y:Q[1]*F.zoom()});var P=i.projectIntoViewport(N.touches[0].pageX,N.touches[0].pageY);D[0]=P[0];D[1]=P[1]}}}}for(var H=0;H<D.length;H++){G[H]=D[H]}i.redraw()
},true);window.addEventListener("touchend",function(K){var M=i.touchData.capture;if(!M){return}i.touchData.capture=false;K.preventDefault();var E=i.data.cy;var B=i.getCachedNodes();var H=i.getCachedEdges();var D=i.touchData.now;var F=i.touchData.earlier;if(K.touches[0]){var L=i.projectIntoViewport(K.touches[0].pageX,K.touches[0].pageY);D[0]=L[0];D[1]=L[1]}if(K.touches[1]){var L=i.projectIntoViewport(K.touches[1].pageX,K.touches[1].pageY);D[2]=L[0];D[3]=L[1]}if(K.touches[2]){var L=i.projectIntoViewport(K.touches[2].pageX,K.touches[2].pageY);
D[4]=L[0];D[5]=L[1]}if(K.touches[2]){}else{if(K.touches[1]){}else{if(K.touches[0]){}else{if(!K.touches[0]){var C=i.touchData.start;if(C!=null){if(C._private.grabbed==true){C._private.grabbed=false;C.trigger(new s.Event(K,{type:"free"}))}var I=C._private.edges;for(var G=0;G<I.length;G++){I[G]._private.grabbed=false}i.data.canvasNeedsRedraw[d]=true;i.data.canvasRedrawReason[d].push("touchdrag node end");i.data.canvasNeedsRedraw[z]=true;i.data.canvasRedrawReason[z].push("touchdrag node end");C.trigger(new s.Event(K,{type:"touchend"}));
i.touchData.start=null}else{var J=i.findNearestElement(D[0],D[1]);if(J!=null){J.trigger(new s.Event(K,{type:"touchend"}))}if(J==null){E.trigger(new s.Event(K,{type:"touchend"}))}}if(i.touchData.singleTouchMoved==false){if(C){C.trigger(new s.Event(K,{type:"tap"}))}else{E.trigger(new s.Event(K,{type:"tap"}))}}i.touchData.singleTouchMoved=true}}}}for(var G=0;G<D.length;G++){F[G]=D[G]}i.redraw()},true)};t.prototype.init=function(){};t.prototype.getCachedNodes=function(){var i=this.data;var A=this.data.cy;
if(i.cache==undefined){i.cache={}}if(i.cache.cachedNodes==undefined){i.cache.cachedNodes=A.nodes()}return i.cache.cachedNodes};t.prototype.updateNodesCache=function(){var i=this.data;var A=this.data.cy;if(i.cache==undefined){i.cache={}}i.cache.cachedNodes=A.nodes()};t.prototype.getCachedEdges=function(){var i=this.data;var A=this.data.cy;if(i.cache==undefined){i.cache={}}if(i.cache.cachedEdges==undefined){i.cache.cachedEdges=A.edges()}return i.cache.cachedEdges};t.prototype.updateEdgesCache=function(){var i=this.data;
var A=this.data.cy;if(i.cache==undefined){i.cache={}}i.cache.cachedEdges=A.edges()};t.prototype.projectIntoViewport=function(C,A){var i,G;var D=0;var B=0;var F;F=this.data.container;var E=false;while(F!=null){if(typeof(F.offsetLeft)=="number"){D+=F.offsetLeft;B+=F.offsetTop;if(F==document.body||F==document.header){E=true}if(!E){D-=F.scrollLeft;B-=F.scrollTop}}F=F.parentNode}i=C-D;G=A-B;i-=this.data.cy.pan().x;G-=this.data.cy.pan().y;i/=this.data.cy.zoom();G/=this.data.cy.zoom();return[i,G]};t.prototype.findContainerPageCoords=function(){var i,E;
...
C.drawImage(F.bufferCanvases[1],0,0)}};var j={};var g=30*300;t.prototype.getCachedImage=function(A){if(j[A]&&j[A].image){j[A].keepTime=g;return j[A].image}var i=j[A];if(i==undefined){j[A]=new Object();j[A].image=new Image();j[A].image.src=A;j[A].keepTime=g;i=j[A]}return i.image};t.prototype.updateImageCaches=function(){for(var i in j){if(j[i].keepTime<=0){if(j[i].image!=undefined){j[i].image.src=undefined;j[i].image=undefined}j[i]=undefined}else{j[i]-=1}}};t.prototype.drawImage=function(i,G,F,D,C,H,B){B.widthScale=0.5;
B.heightScale=0.5;B.rotate=H;var E;var A;canvas.drawImage(B,G,F)};t.prototype.drawEdge=function(A,C){var D,H;D=C.source()[0];H=C.target()[0];if(C._private.style.visibility.value!="visible"||D._private.style.visibility.value!="visible"||H._private.style.visibility.value!="visible"){return}A.strokeStyle="rgba("+C._private.style["line-color"].value[0]+","+C._private.style["line-color"].value[1]+","+C._private.style["line-color"].value[2]+","+C._private.style.opacity.value+")";if(C._private.style.width.value<=0){return
}A.lineWidth=C._private.style.width.value;this.findEndpoints(C);if(C._private.rscratch.isSelfEdge){var i=C._private.rscratch;this.drawStyledEdge(A,[i.startX,i.startY,i.cp2ax,i.cp2ay,i.selfEdgeMidX,i.selfEdgeMidY],C._private.style["line-style"].value,C._private.style.width.value);this.drawStyledEdge(A,[i.selfEdgeMidX,i.selfEdgeMidY,i.cp2cx,i.cp2cy,i.endX,i.endY],C._private.style["line-style"].value,C._private.style.width.value)}else{if(C._private.rscratch.isStraightEdge){var E=H._private.position.x-D._private.position.x;
var B=H._private.position.y-D._private.position.y;var G=C._private.rscratch.endX-C._private.rscratch.startX;var F=C._private.rscratch.endY-C._private.rscratch.startY;if(E*G+B*F<0){C._private.rscratch.straightEdgeTooShort=true}else{var i=C._private.rscratch;this.drawStyledEdge(A,[i.startX,i.startY,i.endX,i.endY],C._private.style["line-style"].value,C._private.style.width.value);C._private.rscratch.straightEdgeTooShort=false}}else{var i=C._private.rscratch;this.drawStyledEdge(A,[i.startX,i.startY,i.cp2x,i.cp2y,i.endX,i.endY],C._private.style["line-style"].value,C._private.style.width.value)
}}if(C._private.rscratch.noArrowPlacement!==true&&C._private.rscratch.startX!==undefined){this.drawArrowheads(A,C)}};var r=function(I,E,C){var B=Math.sqrt(Math.pow(I[4]-I[0],2)+Math.pow(I[5]-I[1],2));B+=Math.sqrt(Math.pow((I[4]+I[0])/2-I[2],2)+Math.pow((I[5]+I[1])/2-I[3],2));var H=Math.ceil(B/E);var A=B/E;var F;if(H>0){F=new Array(H*2)}else{return null}for(var D=0;D<H;D++){var G=D/H;F[D*2]=I[0]*(1-G)*(1-G)+2*(I[2])*(1-G)*G+I[4]*(G)*(G);F[D*2+1]=I[1]*(1-G)*(1-G)+2*(I[3])*(1-G)*G+I[5]*(G)*(G)}return F
};var p=function(I,E,C){var B=Math.sqrt(Math.pow(I[2]-I[0],2)+Math.pow(I[3]-I[1],2));var H=Math.ceil(B/E);var F;if(H>0){F=new Array(H*2)}else{return null}var A=[I[2]-I[0],I[3]-I[1]];for(var D=0;D<H;D++){var G=D/H;F[D*2]=A[0]*G+I[0];F[D*2+1]=A[1]*G+I[1]}return F};var o=function(B,i,A){pt1=_genpts(B,i);pt2=_genpts(B,A)};t.prototype.createBuffer=function(A,B){var i=document.createElement("canvas");i.width=A;i.height=B;return[i,i.getContext("2d")]};t.prototype.drawStyledEdge=function(D,O,K,C){var N=this.data.cy.zoom();
if(K=="solid"){D.beginPath();D.moveTo(O[0],O[1]);if(O.length==3*2){D.quadraticCurveTo(O[2],O[3],O[4],O[5])}else{D.lineTo(O[2],O[3])}D.stroke()}else{if(K=="dotted"){var P;if(O.length==3*2){P=r(O,16,true)}else{P=p(O,16,true)}if(!P){return}var M=Math.max(C*1.6,3.4)*N;var J=M*2,F=M*2;var H=this.createBuffer(J,F);var E=H[1];E.setTransform(1,0,0,1,0,0);E.clearRect(0,0,J,F);E.fillStyle=D.strokeStyle;E.beginPath();E.arc(M,M,M*0.5,0,Math.PI*2,false);E.fill();D.beginPath();D.save();for(var I=0;I<P.length/2;
I++){D.drawImage(H[0],P[I*2]-J/2/N,P[I*2+1]-F/2/N,J/N,F/N)}D.restore()}else{if(K=="dashed"){var P;if(O.length==3*2){P=r(O,13,true)}else{P=p(O,13,true)}if(!P){return}var J=C*2*N,F=C*1.7*N;var H=this.createBuffer(J,F);var E=H[1];E.setTransform(1,0,0,1,0,0);E.clearRect(0,0,J,F);if(D.strokeStyle){E.strokeStyle=D.strokeStyle}E.beginPath();E.moveTo(J/2,F*0.2);E.lineTo(J/2,F*0.8);E.stroke();D.save();var B=false;var L,G;if(O.length==2*2){L=[O[2]-O[0],O[3]-P[1]];G=Math.acos((L[0]*0+L[1]*-1)/Math.sqrt(L[0]*L[0]+L[1]*L[1]));
if(L[0]<0){G=-G+2*Math.PI}}else{if(O.length==3*2){B=true}}for(var I=0;I<P.length/2;I++){var A=I/(Math.max(P.length/2-1,1));if(B){L=[2*(1-A)*(O[2]-O[0])+2*A*(O[4]-O[2]),2*(1-A)*(O[3]-O[1])+2*A*(O[5]-O[3])];G=Math.acos((L[0]*0+L[1]*-1)/Math.sqrt(L[0]*L[0]+L[1]*L[1]));if(L[0]<0){G=-G+2*Math.PI}}D.translate(P[I*2],P[I*2+1]);D.rotate(G);D.translate(-J/2/N,-F/2/N);D.drawImage(H[0],0,0,J/N,F/N);D.translate(J/2/N,F/2/N);D.rotate(-G);D.translate(-P[I*2],-P[I*2+1])}D.restore()}else{this.drawStyledEdge(D,O,"solid",C)
}}}};t.prototype.drawEdgeText=function(B,E){if(E._private.style.visibility.value!="visible"){return}B.textAlign="center";B.textBaseline="middle";var A,i;var D,C;if(E._private.rscratch.isSelfEdge){D=E._private.rscratch.selfEdgeMidX;C=E._private.rscratch.selfEdgeMidY}else{if(E._private.rscratch.isStraightEdge&&!E._private.rscratch.isBezierEdge){D=(E._private.rscratch.startX+E._private.rscratch.endX)/2;C=(E._private.rscratch.startY+E._private.rscratch.endY)/2}else{if(E._private.rscratch.isBezierEdge&&!E._private.rscratch.isStraightEdge){D=0.25*E._private.rscratch.startX+2*0.5*0.5*E._private.rscratch.cp2x+(0.5*0.5)*E._private.rscratch.endX;
C=Math.pow(1-0.5,2)*E._private.rscratch.startY+2*(1-0.5)*0.5*E._private.rscratch.cp2y+(0.5*0.5)*E._private.rscratch.endY}}}A=D;i=C;this.drawText(B,E,A,i)};t.prototype.drawNode=function(A,B){var i,C;if(B._private.style.visibility.value!="visible"){return}A.fillStyle="rgba("+B._private.style["background-color"].value[0]+","+B._private.style["background-color"].value[1]+","+B._private.style["background-color"].value[2]+","+(B._private.style["background-opacity"].value*B._private.style.opacity.value)+")";
A.strokeStyle="rgba("+B._private.style["border-color"].value[0]+","+B._private.style["border-color"].value[1]+","+B._private.style["border-color"].value[2]+","+(B._private.style["border-opacity"].value*B._private.style.opacity.value)+")";i=B._private.style.width.value;C=B._private.style.height.value;if(B._private.style["background-image"].value[2]||B._private.style["background-image"].value[1]){var D=this.getCachedImage(B._private.style["background-image"].value[2]||B._private.style["background-image"].value[1]);
this.drawInscribedImage(A,D,B)}else{n[B._private.style.shape.value].draw(A,B._private.position.x,B._private.position.y,i,C)}A.lineWidth=B._private.style["border-width"].value;if(B._private.style["border-width"].value>0){A.stroke()}};t.prototype.drawInscribedImage=function(A,F,D){console.log(this.data);var H=this.data.cy._private.zoom;var C=D._private.position.x;var B=D._private.position.y;var G=D._private.style.width.value;var i=D._private.style.height.value;n[D._private.style.shape.value].drawPath(A,C,B,G,i);
A.clip();var E=[F.width,F.height];A.drawImage(F,C-E[0]/2*H,B-E[1]/2*H,E[0]*H,E[1]*H);A.resetClip();A.stroke()};t.prototype.drawNodeText=function(D,F){if(F._private.style.visibility.value!="visible"){return}var B,i;var A=F._private.style.width.value;var G=F._private.style.height.value;var E=F._private.style["text-halign"].strValue;if(E=="left"){D.textAlign="right";B=F._private.position.x-A/2}else{if(E=="right"){D.textAlign="left";B=F._private.position.x+A/2}else{if(E=="center"){D.textAlign="center";
B=F._private.position.x}else{D.textAlign="center";B=F._private.position.x}}}var C=F._private.style["text-valign"].strValue;if(C=="top"){D.textBaseline="bottom";i=F._private.position.y-G/2}else{if(C=="bottom"){D.textBaseline="top";i=F._private.position.y+G/2}else{if(C=="middle"||C=="center"){D.textBaseline="middle";i=F._private.position.y}else{D.textBaseline="middle";i=F._private.position.y}}}this.drawText(D,F,B,i)};t.prototype.drawText=function(i,D,F,E){var I=D._private.style["font-style"].strValue;
var C=D._private.style["font-size"].strValue;var K=D._private.style["font-family"].strValue;var H=D._private.style["font-variant"].strValue;var A=D._private.style["font-weight"].strValue;i.font=I+" "+H+" "+A+" "+C+" "+K;var J=String(D._private.style.content.value);var B=D._private.style["text-transform"].value;if(B=="none"){}else{if(B=="uppercase"){J=J.toUpperCase()}else{if(B=="lowercase"){J=J.toLowerCase()}}}i.fillStyle="rgba("+D._private.style.color.value[0]+","+D._private.style.color.value[1]+","+D._private.style.color.value[2]+","+(D._private.style["text-opacity"].value*D._private.style.opacity.value)+")";
i.strokeStyle="rgba("+D._private.style["text-outline-color"].value[0]+","+D._private.style["text-outline-color"].value[1]+","+D._private.style["text-outline-color"].value[2]+","+(D._private.style["text-opacity"].value*D._private.style.opacity.value)+")";if(J!=undefined){if(isNaN(F)){F=0}if(isNaN(E)){E=0}i.fillText(""+J,F,E)}var G=D._private.style["text-outline-width"].value;if(G>0){i.lineWidth=G;i.strokeText(J,F,E)}};t.prototype.findEdgeControlPoints=function(H){var N={};var F=this.data.cy;var K;
for(var I=0;I<H.length;I++){K=H[I]._private.data.source>H[I]._private.data.target?H[I]._private.data.target+H[I]._private.data.source:H[I]._private.data.source+H[I]._private.data.target;if(N[K]==undefined){N[K]=[]}N[K].push(H[I])}var B,G;for(var K in N){B=F.getElementById(N[K][0]._private.data.source);G=F.getElementById(N[K][0]._private.data.target);var M=(B._private.position.x+G._private.position.x)/2;var L=(B._private.position.y+G._private.position.y)/2;var C,A;if(N[K].length>1){C=G._private.position.y-B._private.position.y;
