...
this._private.maxZoom=c.maxZoom}this._private.style=a.is.stylesheet(c.style)?c.style.generateStyle(this):new a.Style(g);g.initRenderer(c.renderer);g.load(c.elements,function(){g.startAnimationLoop();d.ready=true;for(var h=0;h<e.length;h++){var j=e[h];g.bind("ready",j)}d.readies=[];if(a.is.fn(c.ready)){g.bind("ready",c.ready)}g.trigger("ready")},c.done)};a.corefn=a.Core.prototype;a.fn.core({ready:function(){return this._private.ready},registered:function(){if(this._private&&this._private.instanceId!=null){return true
}else{return false}},registeredId:function(){return this._private.instanceId},getElementById:function(c){var b=this._private.id2index[c];if(b!==undefined){return this._private.elements[b]}return new a.Collection(this)},addToPool:function(c){var g=this._private.elements;var j=this._private.id2index;for(var e=0;e<c.length;e++){var f=c[e];var h=f._private.data.id;var d=j[h];var b=d!==undefined;if(!b){d=g.length;g.push(f);j[h]=d}}return this},removeFromPool:function(m){var b=this._private.elements;var e=this._private.id2index;
for(var g=0;g<m.length;g++){var l=m[g];var c=l._private.data.id;var h=e[c];var k=h!==undefined;if(k){delete this._private.id2index[c];b.splice(h,1);for(var f=h;f<b.length;f++){var d=b[f]._private.data.id;e[d]--}}}},container:function(){return this._private.options.container},options:function(){return a.util.copy(this._private.options)},json:function(c){var b={};var d=this;b.elements={};d.elements().each(function(e,f){var g=f.group();if(!b.elements[g]){b.elements[g]=[]}b.elements[g].push(f.json())
});b.style=d.style();b.scratch=d.scratch();b.zoomEnabled=d._private.zoomEnabled;b.panEnabled=d._private.panEnabled;b.layout=d._private.options.layout;b.renderer=d._private.options.renderer;return b}})})(cytoscape);(function(a,b){a.fn.core({add:function(d){var c;var e=this;if(a.is.elementOrCollection(d)){var u=d;var n=[];for(var h=0;h<u.length;h++){var t=u[h];n.push(t.json())}c=new a.Collection(e,n)}else{if(a.is.array(d)){var n=d;c=new a.Collection(e,n)}else{if(a.is.plainObject(d)&&(a.is.array(d.nodes)||a.is.array(d.edges))){var l=d;
var n=[];var o=["nodes","edges"];for(var h=0,p=o.length;h<p;h++){var r=o[h];var m=l[r];if(a.is.array(m)){for(var f=0,g=m.length;f<g;f++){var s=m[f];var k=a.util.extend({},s,{group:r});n.push(k)}}}c=new a.Collection(e,n)}else{var s=d;c=(new a.Element(e,s)).collection()}}}return c.filter(function(){return !this.removed()})},remove:function(d){if(!a.is.elementOrCollection(d)){d=d}else{if(a.is.string(d)){var c=d;d=this.$(c)}}return d.remove()},load:function(f,g,e){var i=this;var d=i.elements();if(d.length>0){d.remove()
}i.notifications(false);var c=[];if(f!=null){if(a.is.plainObject(f)||a.is.array(f)){i.add(f)}}function h(){i.one("layoutready",function(j){i.notifications(true);i.trigger(j);i.notify({type:"load",collection:i.elements(),style:i._private.style});i.one("load",g);i.trigger("load")}).one("layoutstop",function(){i.one("done",e);i.trigger("done")});i.layout(i._private.options.layout)}if(b&&b.chrome){setTimeout(function(){h()},30)}else{h()}return this}})})(cytoscape,typeof window==="undefined"?null:window);
(function(a){a.fn.core({addToAnimationPool:function(b){var h=this;var c=h._private.aniEles;var e=[];for(var d=0;d<c.length;d++){var g=c[d]._private.data.id;e[g]=true}for(var d=0;d<b.length;d++){var f=b[d];var g=f._private.data.id;if(!e[g]){c.push(f)}}},startAnimationLoop:function(){var g=this;var f=10;var l=false;var i=true;g._private.aniEles=[];var j=typeof window==="undefined"?function(){}:(window.requestAnimationFrame||window.mozRequestAnimationFrame||window.webkitRequestAnimationFrame||window.msRequestAnimationFrame);
if(j==null||!i){j=function(m){window.setTimeout(function(){m(+new Date)},f)}}var c=g.container();function k(){function m(){j(function(n){d(n);k()},c)}if(l){setTimeout(function(){m()},f)}else{m()}}k();function d(p){var z=g._private.aniEles;for(var v=0;v<z.length;v++){var y=z[v];var w=y._private.animation.current;var u=y._private.animation.queue;if(w.length===0){var n=u;var t=n.length>0?n.shift():null;if(t!=null){t.callTime=+new Date;w.push(t)}}var x=[];for(var s=0;s<w.length;s++){var r=w[s];e(y,r,p);
if(w[s].done){x.push(r);w.splice(s,1);s--}}for(var s=0;s<x.length;s++){var r=x[s];var o=r.params.complete;if(a.is.fn(o)){o.apply(y,[p])}}}if(z.length>0){g.notify({type:"draw",collection:z})}for(var s=0;s<z.length;s++){var y=z[s];var u=y._private.animation.queue;var w=y._private.animation.current;var m=w.length>0||u.length>0;if(!m){z.splice(s,1);s--}}}function e(D,w,s){var n=g.style();var A=w.properties;var v=w.params;var t=w.callTime;var z;if(w.duration===0){z=1}else{z=Math.min(1,(s-t)/w.duration)
}if(A.delay==null){var y=w.startPosition;var r=A.position;var C=D._private.position;if(r){if(b(y.x,r.x)){C.x=h(y.x,r.x)}if(b(y.y,r.y)){C.y=h(y.y,r.y)}}if(A.css){var B=a.style.properties;for(var x=0;x<B.length;x++){var m=B[x].name;var u=A.css[m];if(u!==undefined){var p=w.startStyle[m];var o=h(p,u,z);n.applyBypass(D,m,o)}}}}if(a.is.fn(v.step)){v.step.apply(D,[s])}if(z>=1){w.done=true}return z}function b(n,m){if(n==null||m==null){return false}if(a.is.number(n)&&a.is.number(m)){return true}else{if((n)&&(m)){return true
}}return false}function h(o,p,v){if(a.is.number(o)&&a.is.number(p)){return o+(p-o)*v}else{if(a.is.number(o[0])&&a.is.number(p[0])){var t=o;var s=p;function n(y,r){var z=r-y;var x=y;return Math.round(v*z+x)}var m=n(t[0],s[0]);var u=n(t[1],s[1]);var w=n(t[2],s[2]);return a.util.tuple2hex([m,u,w])}}return undefined}}})})(cytoscape);(function(a){a.fn.core({data:a.define.data({field:"data",bindingEvent:"data",allowBinding:true,allowSetting:true,settingEvent:"data",settingTriggersEvent:true,triggerFnName:"trigger",allowGetting:true}),removeData:a.define.removeData({field:"data",event:"data",triggerFnName:"trigger",triggerEvent:true}),scratch:a.define.data({field:"scratch",allowBinding:false,allowSetting:true,settingTriggersEvent:false,allowGetting:true}),removeScratch:a.define.removeData({field:"scratch",triggerEvent:false}),})
...
this.notify({type:"viewport"});return this},zoom:function(d){var j;var k;if(d===undefined){return this._private.zoom}else{if(a.is.number(d)){k=d;j={x:0,y:0}}else{if(a.is.plainObject(d)){k=d.level;if(d.renderedPosition){var l=d.renderedPosition;var b=this._private.pan;var i=this._private.zoom;j={x:(l.x-b.x)/i,y:(l.y-b.y)/i}}else{if(d.position){j=d.position}}if(j&&!this._private.panEnabled){return this}}}}if(!this._private.zoomEnabled){return this}if(!a.is.number(k)||!a.is.number(j.x)||!a.is.number(j.y)){return this
}k=k>this._private.maxZoom?this._private.maxZoom:k;k=k<this._private.minZoom?this._private.minZoom:k;var g=this._private.pan;var h=this._private.zoom;var f=k;var e={x:-f/h*(j.x-g.x)+j.x,y:-f/h*(j.y-g.y)+j.y};this._private.zoom=k;this._private.pan=e;var c=g.x!==e.x||g.y!==e.y;this.trigger("zoom"+(c?" pan":""));this.notify({type:"viewport"});return this},boundingBox:function(r){var p;if(!r||(a.is.elementOrCollection(r)&&r.length===0)){p=this.$()}else{if(a.is.string(r)){p=this.$(r)}else{if(a.is.elementOrCollection(r)){p=r
}}}var v=Infinity;var u=-Infinity;var d=Infinity;var c=-Infinity;for(var s=0;s<p.length;s++){var l=p[s];if(l.isNode()){var f=l._private.position;var j=f.x;var g=f.y;var k=l.outerWidth();var e=k/2;var t=l.outerHeight();var m=t/2;var o=j-e;var n=j+e;var b=g-m;var z=g+m;v=o<v?o:v;u=n>u?n:u;d=b<d?b:d;c=z>c?z:c}}return{x1:v,x2:u,y1:d,y2:c,w:u-v,h:c-d}},center:function(f){if(!this._private.panEnabled||!this._private.zoomEnabled){return this}var g=this.boundingBox(f);var d=this.style();var b=parseFloat(d.containerCss("width"));
var c=parseFloat(d.containerCss("height"));var e=this._private.zoom;this.pan({x:(b-e*(g.x1+g.x2))/2,y:(c-e*(g.y1+g.y2))/2});this.trigger("pan");this.notify({type:"viewport"});return this},reset:function(){if(!this._private.panEnabled||!this._private.zoomEnabled){return this}this.pan({x:0,y:0});if(this._private.maxZoom>1&&this._private.minZoom<1){this.zoom(1)}this.notify({type:"viewport"});return this}})})(cytoscape);(function(b){b.fn.collection=b.fn.eles=function(c,e){for(var d in c){var f=c[d];b.Collection.prototype[d]=f
}};var a={prefix:{nodes:"n",edges:"e"},id:{nodes:0,edges:0},generate:function(h,d,g){var c=b.is.element(d)?d._private:d;var e=c.group;var f=g!=null?g:this.prefix[e]+this.id[e];if(h.getElementById(f).empty()){this.id[e]++}else{while(!h.getElementById(f).empty()){f=this.prefix[e]+(++this.id[e])}}return f}};b.Element=function(d,f,h){if(!(this instanceof b.Element)){return new b.Element(d,f,h)}var n=this;h=(h===undefined||h?true:false);if(d===undefined||f===undefined||!b.is.core(d)){b.util.error("An element must have a core reference and parameters set");
return}if(f.group!=="nodes"&&f.group!=="edges"){b.util.error("An element must be of type `nodes` or `edges`; you specified `"+f.group+"`");return}this.length=1;this[0]=this;this._private={cy:d,single:true,data:f.data||{},position:f.position||{},listeners:[],group:f.group,style:{},removed:true,selected:f.selected?true:false,selectable:f.selectable===undefined?true:(f.selectable?true:false),locked:f.locked?true:false,grabbed:false,grabbable:f.grabbable===undefined?true:(f.grabbable?true:false),classes:{},animation:{current:[],queue:[]},rscratch:{},scratch:{},edges:[],children:[]};
...
})(cytoscape);(function(K){var x=function(P){if(false){console.log(P)}};var F={minZoom:0.001,maxZoom:1000,maxPan:-1>>>1,minPan:(-(-1>>>1)-1),selectionToPanDelay:500,dragToSelect:true,dragToPan:true,};var A={};var J=[0,0,0,0,0];var e;var d;var m;var N=false;var o=false;var a=false;var C;var I=999;var L;var r=999;var k={};var O={};var z={};var t={};var v={};var f={};var g=false;var y=false;var j;var b=[];var h=[];var u;var G;var n=new Array(2);var H=new Array(2);var s;var i=new Array(2);var D=new Array(2);
var l;var w=false;var E=0;var c=0;function M(S){this.options=$.extend(true,{},F,S);this.cy=S.cy;u=S.cy;this.init();this.nodePairEdgeData={};var U=5;this.canvases=new Array(U);this.canvasContexts=new Array(U);var R=2;this.bufferCanvases=new Array(R);this.bufferCanvasContexts=new Array(R);this.canvasNeedsRedraw=new Array(U);this.redrawReason=new Array(U);var P=this.options.cy.container();this.container=P;for(var T=0;T<U+R;T++){var Q=document.createElement("canvas");Q.width=P.clientHeight;Q.height=P.clientWidth;
Q.style.position="absolute";if(T<U){Q.id="layer"+T;Q.style.zIndex=String(-T-R);Q.style.visibility="hidden";this.canvases[T]=Q;this.canvasContexts[T]=Q.getContext("2d");this.canvasNeedsRedraw[T]=false;this.redrawReason[T]=new Array()}else{Q.id="buffer"+(T-U);Q.style.zIndex=-(T-U);this.bufferCanvases[T-U]=Q;this.bufferCanvasContexts[T-U]=Q.getContext("2d")}P.appendChild(Q)}this.bufferCanvases[0].style.visibility="visible";this.bufferCanvases[1].style.visibility="hidden";this.canvas=this.bufferCanvases[0];
this.context=this.bufferCanvasContexts[0];this.center=[P.clientWidth/2,P.clientHeight/2];this.scale=[1,1];this.zoomLevel=0;G=this}M.prototype.notify=function(Q){var P=["draw","viewport","add","style"];switch(Q.type){case"load":x("load call");this.load();this.canvasNeedsRedraw[2]=true;this.redrawReason[2].push("Load");this.canvasNeedsRedraw[4]=true;this.redrawReason[4].push("Load");this.redraw();break;case"draw":x("draw call");break;case"viewport":if(!w){this.canvasNeedsRedraw[2]=true;this.redrawReason[2].push("Viewport change");
this.canvasNeedsRedraw[4]=true;this.redrawReason[4].push("Viewport change")}else{w=false}break;case"style":p=true;this.canvasNeedsRedraw[2]=true;this.redrawReason[2].push("Style change");this.canvasNeedsRedraw[4]=true;this.redrawReason[4].push("Style change");break;case"add":this.canvasNeedsRedraw[2]=true;this.redrawReason[2].push("Elements added");this.canvasNeedsRedraw[4]=true;this.redrawReason[4].push("Elements added");break;default:console.log("event: "+Q.type)}if(P.indexOf(Q.type)!=-1){this.redraw()
}};M.prototype.projectMouse=function(Q){var P=Q.clientX-this.canvas.offsetParent.offsetLeft;var R=Q.clientY-this.canvas.offsetParent.offsetTop;P+=(Q.pageX-Q.clientX);R+=(Q.pageY-Q.clientY);P-=u.pan().x;R-=u.pan().y;P/=u.zoom();R/=u.zoom();return[P,R]};M.prototype.findEdgeMetrics=function(P){this.nodePairEdgeData={};var S,R;for(var Q=0;Q<P.length;Q++){S=P[Q];R=S._private.data.source<=S._private.data.target?S._private.data.source+S._private.data.target:S._private.data.target+S._private.data.source;
if(this.nodePairEdgeData[R]==undefined){this.nodePairEdgeData[R]=1}else{this.nodePairEdgeData[R]++}S._private.rscratch.nodePairId=R;S._private.rscratch.nodePairEdgeNum=this.nodePairEdgeData[R]}};M.prototype.findEdges=function(T){var Q=u.edges();var S={};var P=[];for(var R=0;R<T.length;R++){S[T[R]._private.data.id]=T[R]}for(var R=0;R<Q.length;R++){if(S[Q[R]._private.data.source]||S[Q[R]._private.data.target]){P.push(Q[R])}}return P};M.prototype.findEdgeControlPoints=function(V){var ab={};var Y;for(var W=0;
W<V.length;W++){Y=V[W]._private.data.source>V[W]._private.data.target?V[W]._private.data.target+V[W]._private.data.source:V[W]._private.data.source+V[W]._private.data.target;if(ab[Y]==undefined){ab[Y]=[]}ab[Y].push(V[W])}var Q,U;for(var Y in ab){Q=u.getElementById(ab[Y][0]._private.data.source);U=u.getElementById(ab[Y][0]._private.data.target);var aa=(Q._private.position.x+U._private.position.x)/2;var Z=(Q._private.position.y+U._private.position.y)/2;var R,P;if(ab[Y].length>1){R=U._private.position.y-Q._private.position.y;
P=Q._private.position.x-U._private.position.x;var X=Math.sqrt(R*R+P*P);R/=X;P/=X}var S;for(var W=0;W<ab[Y].length;W++){S=ab[Y][W];if(Q._private.data.id==U._private.data.id){var ac=S._private.style["control-point-step-size"].pxValue;S._private.rscratch.isSelfEdge=true;S._private.rscratch.cp2ax=Q._private.position.x;S._private.rscratch.cp2ay=Q._private.position.y-1.3*ac*(W/3+1);S._private.rscratch.cp2cx=Q._private.position.x-1.3*ac*(W/3+1);S._private.rscratch.cp2cy=Q._private.position.y;S._private.rscratch.selfEdgeMidX=(S._private.rscratch.cp2ax+S._private.rscratch.cp2cx)/2;
S._private.rscratch.selfEdgeMidY=(S._private.rscratch.cp2ay+S._private.rscratch.cp2cy)/2}else{if(ab[Y].length%2==1&&W==Math.floor(ab[Y].length/2)){S._private.rscratch.isStraightEdge=true}else{var ac=S._private.style["control-point-step-size"].value;var T=(0.5-ab[Y].length/2+W)*ac;S._private.rscratch.isBezierEdge=true;S._private.rscratch.cp2x=aa+R*T;S._private.rscratch.cp2y=Z+P*T}}}}return ab};M.prototype.findEdgeControlPoints2=function(Q){var T={};var P;for(var S=0;S<Q.length;S++){if(T[Q[S]._private.data.id]==undefined){P=Q[S].parallelEdges();
for(var R=0;R<Q.length;R++){T[Q[S]._private.data.id]=true}K.styfn.calculateControlPoints(P)}}};M.prototype.checkRecordPinchCoordinates=function(P){if(P.touches.length>=2){prevTouch1=P.touches[0];prevTouch2=P.touches[1];prevTouch1.offsetX=prevTouch1.clientX+G.canvas.parentElement.offsetLeft;prevTouch1.offsetY=prevTouch1.clientY+G.canvas.parentElement.offsetTop;prevTouch2.offsetX=prevTouch2.clientX+G.canvas.parentElement.offsetLeft;prevTouch2.offsetY=prevTouch2.clientY+G.canvas.parentElement.offsetTop
}else{prevTouch1=undefined;prevTouch2=undefined}};M.prototype.mouseDownHandler=function(V){var S=u.nodes();var R=u.edges();if(V.changedTouches){V.preventDefault();if(V.touches.length>=2){firstTouchFinger=V.touches[0];secondTouchFinger=V.touches[1];var P=[G.canvas.parentElement.offsetLeft,-G.canvas.parentElement.offsetTop];i[0]=V.touches[0].clientX+P[0];i[1]=V.touches[0].clientY+P[1];D[0]=V.touches[1].clientX+P[0];D[1]=V.touches[1].clientY+P[1];l=Math.sqrt(Math.pow(D[0]-i[0],2)+Math.pow(D[1]-i[1],2))
}V=V.changedTouches[0];V.button=0;V.touch=1;L=C=undefined;G.mouseMoveHelper.hoverHandler(S,R,V)}var Q=V;if((Q.button==1&&Q.target==u.renderer().canvas)||(V.touch&&L==undefined&&C==undefined)){e=Q.clientX;d=Q.clientY;N=true;if(u.renderer().canvas.style.cursor!=u.style()._private.coreStyle["panning-cursor"].value){u.renderer().canvas.style.cursor=u.style()._private.coreStyle["panning-cursor"].value}}var W=u.renderer().projectMouse(V);J[0]=W[0];J[1]=W[1];if(Q.button==0&&Q.target==u.renderer().canvas&&L==undefined&&C==undefined){J[4]=1
}if(Q.button==0&&Q.target==u.renderer().canvas){if(L!=undefined){g=true;b=[];if(L.selected()){y=true;for(var T=0;T<S.length;T++){if(S[T].selected()){S[T]._private.rscratch.dragStartX=S[T]._private.position.x;S[T]._private.rscratch.dragStartY=S[T]._private.position.y;b.push(S[T]);S[T]._private.rscratch.layer2=true}}}else{y=false;j=L;j._private.rscratch.dragStartX=j._private.position.x;j._private.rscratch.dragStartY=j._private.position.y;b.push(j);j._private.rscratch.layer2=true}h=G.findEdges(b);for(var U=0;
U<h.length;U++){h[U]._private.rscratch.layer2=true}G.canvasNeedsRedraw[4]=true;G.redrawReason[4].push("nodes being dragged, moved to drag layer");G.canvasNeedsRedraw[2]=true;G.redrawReason[2].push("nodes being dragged, moved to drag layer")}}u.renderer().redraw()};M.prototype.mouseMoveHandler=function(ab){E=setTimeout(function(){E=null},1000/100);var R=ab;if(ab.touches){ab.preventDefault();if(ab.touches.length>=2){var af=[G.canvas.parentElement.offsetLeft,-G.canvas.parentElement.offsetTop];n[0]=R.touches[0].clientX+af[0];
n[1]=R.touches[0].clientY+af[1];H[0]=R.touches[1].clientX+af[0];H[1]=R.touches[1].clientY+af[1];s=Math.sqrt(Math.pow(D[0]-i[0],2)+Math.pow(D[1]-i[1],2));var ad=[n[0]-i[0],n[1]-i[1]];var ac=[H[0]-D[0],H[1]-D[1]];var P=[(ad[0]+ac[0])/2,(ac[1]+ac[1])/2];var T=s/l;if(T>1){T=(T-1)*1.5+1}else{T=1-(1-T)*1.5}w=true;u.panBy({x:P[0],y:P[1]});u.zoom({level:u.zoom()*T,position:{x:(n[0]+H[0])/2,y:(n[1]+H[1])/2}});i[0]=n[0];i[1]=n[1];D[0]=H[0];D[1]=H[1];l=s;return}ab=ab.touches[0];ab.button=0}var X=R;var Z=G.mouseMoveHelper.dragHandler;
var ae=G.mouseMoveHelper.checkEdgeHover;var S=G.mouseMoveHelper.checkStraightEdgeHover;var W=G.mouseMoveHelper.checkNodeHover;var U=G.mouseMoveHelper.hoverHandler;var V=u.edges();var Q=u.nodes();if(N){Z(ab)}var aa=u.renderer().projectMouse(ab);J[2]=aa[0];J[3]=aa[1];if(!J[4]){U(Q,V,ab)}if(L!=undefined&&u.renderer().canvas.style.cursor!=L._private.style.cursor.value){u.renderer().canvas.style.cursor=L._private.style.cursor.value}else{if(C!=undefined&&u.renderer().canvas.style.cursor!=C._private.style.cursor.value){u.renderer().canvas.style.cursor=C._private.style.cursor.value
}else{if(!L&&!C&&u.renderer().canvas.style.cursor!="default"){u.renderer().canvas.style.cursor="default"}}}if(g){for(var Y=0;Y<Q.length;Y++){if((y&&Q[Y].selected())||(!y&&Q[Y]==j)){Q[Y]._private.position.x=Q[Y]._private.rscratch.dragStartX+(J[2]-J[0]);Q[Y]._private.position.y=Q[Y]._private.rscratch.dragStartY+(J[3]-J[1])}}G.canvasNeedsRedraw[2]=true;G.redrawReason[2].push("nodes being dragged")}if(J[4]){G.canvasNeedsRedraw[0]=true;G.redrawReason[0].push("selection boxed moved")}if(N||g||J[4]){u.renderer().redraw()
}};M.prototype.mouseUpHandler=function(ad){var Y;if(ad.changedTouches){ad.preventDefault();Y=ad;ad=ad.changedTouches[0];ad.button=0}var ac=ad;var R=u.edges();var aa=u.nodes();var X=g&&(Math.abs(J[2]-J[0])+Math.abs(J[3]-J[1])>1);for(var ae=0;ae<b.length;ae++){b[ae]._private.rscratch.layer2=false}for(var ae=0;ae<h.length;ae++){h[ae]._private.rscratch.layer2=false}if(!o&&!X){var V=u.collection();for(var T=0;T<aa.length;T++){aa[T]._private.rscratch.selected=false;if(aa[T]._private.selected){V=V.add(aa[T])
}}for(var T=0;T<R.length;T++){R[T]._private.rscratch.selected=false;if(R[T]._private.selected){V=V.add(R[T])}}if(V.length>0){V.unselect()}}if(J[4]==1&&!g&&Math.abs(J[2]-J[0])+Math.abs(J[3]-J[1])>2){console.log("performing box selection, from ("+J[0]+","+J[1]+") to ("+J[2]+","+J[3]+")");var W=2;var P;var Z;var ag=u.collection();for(var T=0;T<R.length;T++){P=R[T]._private.selected;var S;var Q=R[T]._private.rscratch;if(R[T]._private.rscratch.isStraightEdge){S=K.math.boxInBezierVicinity(J[0],J[1],J[2],J[3],R[T]._private.rscratch.startX,R[T]._private.rscratch.startY,(R[T]._private.rscratch.startX+R[T]._private.rscratch.endX)/2,(R[T]._private.rscratch.startY+R[T]._private.rscratch.endY)/2,R[T]._private.rscratch.endX,R[T]._private.rscratch.endY,W)
}else{if(R[T]._private.rscratch.isSelfEdge){S=K.math.boxInBezierVicinity(J[0],J[1],J[2],J[3],R[T]._private.rscratch.startX,R[T]._private.rscratch.startY,R[T]._private.rscratch.cp2ax,R[T]._private.rscratch.cp2ay,R[T]._private.rscratch.selfEdgeMidX,R[T]._private.rscratch.selfEdgeMidY,W);if(S==0){S=K.math.boxInBezierVicinity(J[0],J[1],J[2],J[3],R[T]._private.rscratch.selfEdgeMidX,R[T]._private.rscratch.selfEdgeMidY,R[T]._private.rscratch.cp2cx,R[T]._private.rscratch.cp2cy,R[T]._private.rscratch.endX,R[T]._private.rscratch.endY,W)
}}else{S=K.math.boxInBezierVicinity(J[0],J[1],J[2],J[3],R[T]._private.rscratch.startX,R[T]._private.rscratch.startY,R[T]._private.rscratch.cp2x,R[T]._private.rscratch.cp2y,R[T]._private.rscratch.endX,R[T]._private.rscratch.endY,W)}}if(S==2){Z=true}else{if(S==1){if(R[T]._private.rscratch.isSelfEdge){Z=K.math.checkBezierCrossesBox(J[0],J[1],J[2],J[3],R[T]._private.rscratch.startX,R[T]._private.rscratch.startY,R[T]._private.rscratch.cp2ax,R[T]._private.rscratch.cp2ay,R[T]._private.rscratch.selfEdgeMidX,R[T]._private.rscratch.selfEdgeMidY,W);
if(!Z){Z=K.math.checkBezierCrossesBox(J[0],J[1],J[2],J[3],R[T]._private.rscratch.selfEdgeMidX,R[T]._private.rscratch.selfEdgeMidY,R[T]._private.rscratch.cp2cx,R[T]._private.rscratch.cp2cy,R[T]._private.rscratch.endX,R[T]._private.rscratch.endY,W)}}else{if(R[T]._private.rscratch.isStraightEdge){Z=K.math.checkStraightEdgeCrossesBox(J[0],J[1],J[2],J[3],R[T]._private.rscratch.startX,R[T]._private.rscratch.startY,R[T]._private.rscratch.endX,R[T]._private.rscratch.endY,W)}else{Z=K.math.checkBezierCrossesBox(J[0],J[1],J[2],J[3],R[T]._private.rscratch.startX,R[T]._private.rscratch.startY,R[T]._private.rscratch.cp2x,R[T]._private.rscratch.cp2y,R[T]._private.rscratch.endX,R[T]._private.rscratch.endY,W)
}}}else{Z=false}}if(Z&&!P){ag=ag.add(R[T])}else{if(!Z&&P){}}}var ak=Math.min(J[0],J[2]);var aj=Math.min(J[1],J[3]);var ai=Math.max(J[0],J[2]);var ah=Math.max(J[1],J[3]);var af,Z;var ab,U;for(var T=0;T<aa.length;T++){af=aa[T]._private.selected;ab=aa[T].position();U=aa[T]._private.data.weight/5;if(ab.x>ak-U&&ab.x<ai+U&&ab.y>aj-U&&ab.y<ah+U){Z=true;aa[T]._private.rscratch.selected=true}else{Z=false;aa[T]._private.rscratch.selected=false}if(Z&&!af){ag=ag.add(aa[T])}else{if(!Z&&af){}}}if(ag.length>0){ag.select()
}}else{if(J[4]==0&&!X){if(L!=undefined){L._private.rscratch.hovered=false;L._private.rscratch.selected=true;if(!L._private.selected){L.select()}}else{if(C!=undefined){C._private.rscratch.hovered=false;C._private.rscratch.selected=true;if(!C._private.selected){C.select()}}}}}N=false;if(u.renderer().canvas.style.cursor!="default"){u.renderer().canvas.style.cursor="default"}J[4]=0;G.canvasNeedsRedraw[0]=true;G.redrawReason[0].push("Selection box gone");if(X){G.canvasNeedsRedraw[4]=true;G.redrawReason[4].push("Node drag completed")
}g=false;u.renderer().redraw();if(Y&&Y.touches.length==1){e=Y.touches[0].clientX;d=Y.touches[0].clientY;N=true;if(u.renderer().canvas.style.cursor!=u.style()._private.coreStyle["panning-cursor"].value){u.renderer().canvas.style.cursor=u.style()._private.coreStyle["panning-cursor"].value}}};M.prototype.mouseWheelHandler=function(Q){Q.preventDefault();var P=Q.wheelDeltaY;u.renderer().zoomLevel-=P/5/500;u.renderer().scale[0]=Math.pow(10,-u.renderer().zoomLevel);u.renderer().scale[1]=Math.pow(10,-u.renderer().zoomLevel);
var R=u.renderer().projectMouse(Q);var S=u.zoom()*Math.pow(10,Q.wheelDeltaY/500);S=Math.min(S,100);S=Math.max(S,0.01);u.zoom({level:S,position:{x:Q.offsetX,y:Q.offsetY}})};M.prototype.keyDownHandler=function(P){if(P.keyCode==16&&J[4]!=1){o=true}};M.prototype.keyUpHandler=function(P){if(P.keyCode==16){J[4]=0;o=false}};M.prototype.mouseMoveHelper=function(){var R=function(W){var V=W.clientX-e;var X=W.clientY-d;u.panBy({x:V,y:X});e=W.clientX;d=W.clientY};var U=function(Y,W,Z){var X=19;if(K.math.inBezierVicinity(Y,W,Z._private.rscratch.startX,Z._private.rscratch.startY,Z._private.rscratch.cp2x,Z._private.rscratch.cp2y,Z._private.rscratch.endX,Z._private.rscratch.endY,X)){var V=K.math.sqDistanceToQuadraticBezier(Y,W,Z._private.rscratch.startX,Z._private.rscratch.startY,Z._private.rscratch.cp2x,Z._private.rscratch.cp2y,Z._private.rscratch.endX,Z._private.rscratch.endY);
if(V<X){if(V<I){C=Z;I=V}}}};var Q=function(Z,X,aa){var Y=19;var V=false;if(K.math.inBezierVicinity(Z,X,aa._private.rscratch.startX,aa._private.rscratch.startY,aa._private.rscratch.cp2ax,aa._private.rscratch.cp2ay,aa._private.rscratch.selfEdgeMidX,aa._private.rscratch.selfEdgeMidY)){var W=K.math.sqDistanceToQuadraticBezier(Z,X,aa._private.rscratch.startX,aa._private.rscratch.startY,aa._private.rscratch.cp2ax,aa._private.rscratch.cp2ay,aa._private.rscratch.selfEdgeMidX,aa._private.rscratch.selfEdgeMidY);
if(W<Y){if(W<I){C=aa;I=W;V=true}}}if(!V&&K.math.inBezierVicinity(Z,X,aa._private.rscratch.selfEdgeMidX,aa._private.rscratch.selfEdgeMidY,aa._private.rscratch.cp2cx,aa._private.rscratch.cp2cy,aa._private.rscratch.endX,aa._private.rscratch.endY)){var W=K.math.sqDistanceToQuadraticBezier(Z,X,aa._private.rscratch.selfEdgeMidX,aa._private.rscratch.selfEdgeMidY,aa._private.rscratch.cp2cx,aa._private.rscratch.cp2cy,aa._private.rscratch.endX,aa._private.rscratch.endY);if(W<Y){if(W<I){C=aa;I=W;V=true}}}};
var T=function(ag,af,ad,aa,ai,Y,ah){var al=19;var ae=ag-aa;var ac=af-ai;var Z=ag-Y;var X=af-ah;var W=Y-aa;var V=ah-ai;var ab;if(ae*W+ac*V<=0){ab=(Math.pow(aa-ag,2)+Math.pow(ai-af,2))}else{if(Z*W+X*V>=0){ab=(Math.pow(Y-ag,2)+Math.pow(ah-af,2))}else{var ak=V;var aj=-W;ab=Math.pow(ae*ak+ac*aj,2)/(ak*ak+aj*aj)}}if(ab<=al){if(ab<I){C=ad;I=ab}}};var P=function(Z,Y,aa){var W=Z-aa.position().x;var V=Y-aa.position().y;var X=Math.pow(Math.max(aa._private.style.width.value,aa._private.style.height.value+aa._private.style["border-width"].value)/2,2);
var ab=W*W+V*V;if(X>ab){if(ab<r){L=aa;r=ab;a=true}}};var S=function(X,W,V){var aa=u.renderer().projectMouse(V);var ab=aa[0];var Z=aa[1];if(L!=undefined){L=undefined;r=99999}else{if(C!=undefined){C=undefined;I=99999}}a=false;for(var Y=0;Y<X.length;Y++){P(ab,Z,X[Y])}for(var Y=0;Y<W.length;Y++){if(a){break}else{if(W[Y]._private.rscratch.isStraightEdge){T(ab,Z,W[Y],W[Y]._private.rscratch.startX,W[Y]._private.rscratch.startY,W[Y]._private.rscratch.endX,W[Y]._private.rscratch.endY)}else{if(W[Y]._private.rscratch.isSelfEdge){Q(ab,Z,W[Y])
}else{U(ab,Z,W[Y])}}}}if(L!=undefined){L._private.rscratch.hovered=true}else{if(C!=undefined){C._private.rscratch.hovered=true}}};this.mouseMoveHelper.dragHandler=R;this.mouseMoveHelper.checkEdgeHover=U;this.mouseMoveHelper.checkStraightEdgeHover=T;this.mouseMoveHelper.checkNodeHover=P;this.mouseMoveHelper.hoverHandler=S};M.prototype.load=function(){var P=this;this.mouseMoveHelper();document.addEventListener("keydown",this.keyDownHandler,false);document.addEventListener("keyup",this.keyUpHandler,false);
this.bufferCanvases[0].addEventListener("mousedown",this.mouseDownHandler,false);this.bufferCanvases[0].addEventListener("mouseup",this.mouseUpHandler,false);this.bufferCanvases[0].addEventListener("mousemove",this.mouseMoveHandler,false);this.bufferCanvases[0].addEventListener("mousewheel",this.mouseWheelHandler,false);this.bufferCanvases[0].addEventListener("touchstart",this.mouseDownHandler,true);this.bufferCanvases[0].addEventListener("touchmove",this.mouseMoveHandler,true);this.bufferCanvases[0].addEventListener("touchend",this.mouseUpHandler,true)
};M.prototype.init=function(){};M.prototype.complexSqrt=function(U,Q,R){var T=Math.sqrt(U*U+Q*Q);var P=Math.sqrt(0.5*(U+T));var S=Math.sqrt(0.5*(T-U));if(Q<-R){S=-S}else{if(Q<R){S=0}}return[P,S]};M.prototype.initStyle=function(){var S=this.options.cy.nodes();var R=this.options.cy.edges();var V;for(var T=0;T<S.length;T++){V=S[T];V._private.rscratch.override={};var Q=Math.max(Math.random(),0.6);V._private.rscratch.override.regularColor="rgba("+String(Math.floor(Q*100+125))+","+String(Math.floor(Q*100+125))+","+String(Math.floor(Q*100+125))+","+255+")";
V._private.rscratch.override.regularBorderColor="rgba("+String(Math.floor(Q*70+160))+","+String(Math.floor(Q*70+160))+","+String(Math.floor(Q*70+160))+","+255+")";var P=Math.random();if(P<10.35){V._private.rscratch.override.shape="ellipse"}else{if(P<0.49){V._private.rscratch.override.shape="hexagon"}else{if(P<0.76){V._private.rscratch.override.shape="square"}else{if(P<0.91){V._private.rscratch.override.shape="pentagon"}else{V._private.rscratch.override.shape="octogon"}}}}V._private.rscratch.canvas=document.createElement("canvas")
}var U;for(var T=0;T<R.length;T++){U=R[T];U._private.rscratch.cp2x=Math.random()*this.options.cy.container().width();U._private.rscratch.cp2y=Math.random()*this.options.cy.container().height();U._private.rscratch.override={};if(Math.random()<0.45){U._private.rscratch.override.endShape="inhibitor"}U._private.rscratch.override.regularColor=U.source()[0]._private.rscratch.override.regularBorderColor||defaultNode.regularColor}};M.prototype.findBezierIntersection=function(X,am){var ak=X.source().position().x;
var ai=X.target().position().x;var U=X.source().position().y;var R=X.target().position().y;var ac;var W=X._private.rscratch.cp2x;var T=X._private.rscratch.cp2y;ac=0.5+(0.5-0.5*am/Math.sqrt(Math.pow(W-ai,2)+Math.pow(T-R,2)));var V=ak-2*W+ai;var ab=-2*ak+2*W;var al=ak;var S=U-2*T+R;var aa=-2*U+2*T;var aj=U;var aq=V*ac*ac+ab*ac+al;var ap=S*ac*ac+aa*ac+aj;var Q=W-ak;var Z=ak;var P=T-U;var Y=U;var ag=aq-ai;var ao=ai;var af=ap-R;var an=R;var ah;if(Math.abs(Q)>0.0001){ah=(P/Q*(ao-Z)+Y-an)/(af-(P/Q)*ag)}else{ah=(Z-ao)/(ag)
}var ae=ag*ah+ao;var ad=af*ah+an;X._private.rscratch.newCp2x=ae;X._private.rscratch.newCp2y=ad;X._private.rscratch.newEndPointX=aq;X._private.rscratch.newEndPointY=ap};M.prototype.findIntersection=function(S,X,Q,W,T){var R=Q-S;var P=W-X;var V=Math.sqrt(R*R+P*P);var U=V-T;if(U<0){U=0}return[(U/V)*R+S,(U/V)*P+X]};M.prototype.intersectLineEllipse=function(Z,Y,V,U,S,Q){var R=V-Z;var P=U-Y;R/=S;P/=Q;var W=Math.sqrt(R*R+P*P);var T=W-1;if(T<0){return[]}var X=T/W;return[(V-Z)*X+Z,(U-Y)*X+Y]};M.prototype.findCircleNearPoint=function(T,S,U,Y,X){var Q=Y-T;
var P=X-S;var R=Math.sqrt(Q*Q+P*P);var W=Q/R;var V=P/R;return[T+W*U,S+V*U]};M.prototype.findNewEndPoints=function(Q,P,X,V,Y,W,T,R){var S=this.findCircleNearPoint(Q,P,T,X,V);var U=this.findCircleNearPoint(Y,W,R,X,V);return[S[0],S[1],U[0],U[1]]};M.prototype.calculateNewEndPoints=function(){var Q=u.edges();var S,T;var P;for(var R=0;R<Q.length;R++){S=Q[R].source()[0];T=Q[R].target()[0];if(Q[R]._private.rscratch.isStraightEdge){continue}P=this.findNewEndPoints(S.position().x,S.position().y,Q[R]._private.rscratch.controlPointX,Q[R]._private.rscratch.controlPointY,T.position().x,T.position().y);
Q[R]._private.rscratch.updatedStartX=P[0];Q[R]._private.rscratch.updatedStartY=P[1];Q[R]._private.rscratch.updatedEndX=P[2];Q[R]._private.rscratch.updatedEndY=P[3]}};k.arrow=function(P){P.lineTo(-0.15,0.3);P.lineTo(0,0);P.lineTo(0.15,0.3)};O.arrow=0;z.arrow=4;k.triangle=k.arrow;O.triangle=O.arrow;z.triangle=z.arrow;k.none=function(P){};O.none=0;z.none=0;k.circle=function(P){P.translate(0,-0.15);P.arc(0,0,0.15,0,Math.PI*2,false)};O.circle=0;z.circle=0.1;k.inhibitor=function(P){P.lineTo(-0.25,0);P.lineTo(-0.25,-0.1);
P.lineTo(0.25,-0.1);P.lineTo(0.25,0)};O.inhibitor=4;z.inhibitor=4;k.tee=k.inhibitor;O.tee=O.inhibitor;z.tee=z.inhibitor;M.prototype.drawArrowShape=function(Q,P,V,T,R){var U=Math.asin(R/(Math.sqrt(T*T+R*R)));if(T<0){U=U+Math.PI/2}else{U=-(Math.PI/2+U)}var S=this.context;S.save();S.translate(P,V);S.moveTo(0,0);S.rotate(-U);S.scale(S.lineWidth*12,S.lineWidth*12.1);S.beginPath();k[Q](S);S.closePath();S.fill();S.restore()};M.prototype.findEndpoints=function(S){var U;var P=S.source()[0];var Y=S.target()[0];
var ad=Math.max(S.source()[0]._private.style.width.value,S.source()[0]._private.style.height.value);var T=Math.max(S.target()[0]._private.style.width.value,S.target()[0]._private.style.height.value);ad=0;T/=2;var R=[S.source().position().x,S.source().position().y];var V=[S.target().position().x,S.target().position().y];if(S._private.rscratch.isSelfEdge){var aa=[S._private.rscratch.cp2cx,S._private.rscratch.cp2cy];U=v[Y._private.style.shape.value](Y,Y._private.style.width.value,Y._private.style.height.value,aa[0],aa[1]);
var Q=this.shortenIntersection(U,aa,O[S._private.style["target-arrow-shape"].value]);var Z=this.shortenIntersection(U,aa,z[S._private.style["target-arrow-shape"].value]);S._private.rscratch.endX=Z[0];S._private.rscratch.endY=Z[1];S._private.rscratch.arrowEndX=Q[0];S._private.rscratch.arrowEndY=Q[1];var aa=[S._private.rscratch.cp2ax,S._private.rscratch.cp2ay];U=v[P._private.style.shape.value](P,P._private.style.width.value,P._private.style.height.value,aa[0],aa[1]);var W=this.shortenIntersection(U,aa,O[S._private.style["source-arrow-shape"].value]);
var X=this.shortenIntersection(U,aa,z[S._private.style["source-arrow-shape"].value]);S._private.rscratch.startX=X[0];S._private.rscratch.startY=X[1];S._private.rscratch.arrowStartX=W[0];S._private.rscratch.arrowStartY=W[1]}else{if(S._private.rscratch.isStraightEdge){U=v[Y._private.style.shape.value](Y,Y._private.style.width.value,Y._private.style.height.value,P.position().x,P.position().y);if(U.length==0){S._private.rscratch.noArrowPlacement=true;return}else{S._private.rscratch.noArrowPlacement=false
}var Q=this.shortenIntersection(U,[P.position().x,P.position().y],O[S._private.style["target-arrow-shape"].value]);var Z=this.shortenIntersection(U,[P.position().x,P.position().y],z[S._private.style["target-arrow-shape"].value]);S._private.rscratch.endX=Z[0];S._private.rscratch.endY=Z[1];S._private.rscratch.arrowEndX=Q[0];S._private.rscratch.arrowEndY=Q[1];U=v[P._private.style.shape.value](P,P._private.style.width.value,P._private.style.height.value,Y.position().x,Y.position().y);if(U.length==0){S._private.rscratch.noArrowPlacement=true;
return}else{S._private.rscratch.noArrowPlacement=false}var W=this.shortenIntersection(U,[Y.position().x,Y.position().y],O[S._private.style["source-arrow-shape"].value]);var X=this.shortenIntersection(U,[Y.position().x,Y.position().y],z[S._private.style["source-arrow-shape"].value]);S._private.rscratch.startX=X[0];S._private.rscratch.startY=X[1];S._private.rscratch.arrowStartX=W[0];S._private.rscratch.arrowStartY=W[1]}else{if(S._private.rscratch.isBezierEdge){var aa=[S._private.rscratch.cp2x,S._private.rscratch.cp2y];
var ac=R[0]*0.25+V[0]*0.25+aa[0]*0.5;var ab=R[1]*0.25+V[1]*0.25+aa[1]*0.5;U=v[Y._private.style.shape.value](Y,Y._private.style.width.value,Y._private.style.height.value,aa[0],aa[1]);var Q=this.shortenIntersection(U,aa,O[S._private.style["target-arrow-shape"].value]);var Z=this.shortenIntersection(U,aa,z[S._private.style["target-arrow-shape"].value]);S._private.rscratch.endX=Z[0];S._private.rscratch.endY=Z[1];S._private.rscratch.arrowEndX=Q[0];S._private.rscratch.arrowEndY=Q[1];U=v[P._private.style.shape.value](P,P._private.style.width.value,P._private.style.height.value,aa[0],aa[1]);
var W=this.shortenIntersection(U,aa,O[S._private.style["source-arrow-shape"].value]);var X=this.shortenIntersection(U,aa,z[S._private.style["source-arrow-shape"].value]);S._private.rscratch.startX=X[0];S._private.rscratch.startY=X[1];S._private.rscratch.arrowStartX=W[0];S._private.rscratch.arrowStartY=W[1]}else{if(S._private.rscratch.isArcEdge){return}}}}};M.prototype.drawArrowhead=function(S){var R=S._private.rscratch.override.endShape;R=R?R:defaultEdge.endShape;var Q=S.target().position().x-S._private.rscratch.newEndPointX;
var P=S.target().position().y-S._private.rscratch.newEndPointY;this.drawArrowShape(S,S._private.rscratch.newEndPointX,S._private.rscratch.newEndPointY,Q,P)};M.prototype.drawArrowheads=function(V){var S,R;var Q=V._private.rscratch.arrowStartX;var P=V._private.rscratch.arrowStartY;S=Q-V.source().position().x;R=P-V.source().position().y;this.context.fillStyle="rgba("+V._private.style["source-arrow-color"].value[0]+","+V._private.style["source-arrow-color"].value[1]+","+V._private.style["source-arrow-color"].value[2]+","+V._private.style.opacity.value+")";
this.context.lineWidth=V._private.style.width.value;this.drawArrowShape(V._private.style["source-arrow-shape"].value,Q,P,S,R);var U=V._private.rscratch.arrowEndX;var T=V._private.rscratch.arrowEndY;S=V.target().position().x-U;R=V.target().position().y-T;this.context.fillStyle="rgba("+V._private.style["target-arrow-color"].value[0]+","+V._private.style["target-arrow-color"].value[1]+","+V._private.style["target-arrow-color"].value[2]+","+V._private.style.opacity.value+")";this.context.lineWidth=V._private.style.width.value;
this.drawArrowShape(V._private.style["target-arrow-shape"].value,U,T,S,R)};M.prototype.drawStraightArrowhead=function(R){var Q=R.target().position().x-R._private.rscratch.newStraightEndX;var P=R.target().position().y-R._private.rscratch.newStraightEndY;this.drawArrowShape(R,R._private.rscratch.newStraightEndX,R._private.rscratch.newStraightEndY,Q,P)};M.prototype.calculateEdgeMetrics=function(S){if(S._private.data.source==S._private.data.target){S._private.rscratch.selfEdge=true;return}var T=S._private.data.source<S._private.data.target?S.source()[0]:S.target()[0];
var X=S._private.data.target<S._private.data.source?S.source()[0]:S.target()[0];var Z=0.5*(T._private.position.x+X._private.position.x);var Y=0.5*(T._private.position.y+X._private.position.y);if(this.nodePairEdgeData[S._private.rscratch.nodePairId]==1){S._private.rscratch.straightEdge=true;S._private.rscratch.cp2x=Z;S._private.rscratch.cp2y=Y;return}var Q=S._private.rscratch.nodePairEdgeNum-1;var U=this.nodePairEdgeData[S._private.rscratch.nodePairId]-1;var aa=(Q/U-0.5);if(Math.abs(aa)<0.0001){S._private.rscratch.straightEdge=true;
S._private.rscratch.cp2x=Z;S._private.rscratch.cp2y=Y;return}var R=X._private.position.x-T._private.position.x;var P=X._private.position.y-T._private.position.y;var W=P*aa;var V=-R*aa;S._private.rscratch.cp2x=Z+W;S._private.rscratch.cp2y=Y+V};t.ellipse=function(S,R,P){var Q=G.context;Q.beginPath();Q.save();Q.translate(S._private.position.x,S._private.position.y);Q.scale(R/2,P/2);Q.arc(0,0,1,0,Math.PI*2,false);Q.closePath();Q.restore();Q.fill()};v.ellipse=function(T,S,R,Q,U){var P=G.intersectLineEllipse(Q,U,T.position().x,T.position().y,S/2+T._private.style["border-width"].value/2,R/2+T._private.style["border-width"].value/2);
return P};var B=function(U,Q){var P=1/U*2*Math.PI;var S=U%2==0?Math.PI/2+P/2:Math.PI/2;S+=Q;var T=new Array(U*2);var V;for(var R=0;R<U;R++){V=R*P+S;T[2*R]=Math.cos(V);T[2*R+1]=Math.sin(-V)}return T};M.prototype.findPolygonIntersection=function(S,R,Q,P,W,V,T){if(f[V]==undefined){f[V]=B(T,0)}var U=G.polygonIntersectLine(P,W,f[V],S._private.position.x,S._private.position.y,R/2,Q/2,S._private.style["border-width"].value/2);return G.findNearestIntersection(U,P,W)};M.prototype.expandPolygon=function(ab,R){var S=new Array(ab.length*2);
var aa,Z,Y,X;for(var V=0;V<ab.length/2;V++){aa=ab[V*2];Z=ab[V*2+1];if(V<ab.length/2-1){Y=ab[(V+1)*2];X=ab[(V+1)*2+1]}else{Y=ab[0];X=ab[1]}var W=(X-Z);var U=-(Y-aa);var T=Math.sqrt(W*W+U*U);var Q=W/T;var P=U/T;S[V*4]=aa+Q*R;S[V*4+1]=Z+P*R;S[V*4+2]=Y+Q*R;S[V*4+3]=X+P*R}return S};M.prototype.joinLines=function(Y){var X=new Array(Y.length/2);var aa,Z,W,V;var T,S,R,Q;for(var U=0;U<Y.length/4;U++){aa=Y[U*4];Z=Y[U*4+1];W=Y[U*4+2];V=Y[U*4+3];if(U<Y.length/4-1){T=Y[(U+1)*4];S=Y[(U+1)*4+1];R=Y[(U+1)*4+2];Q=Y[(U+1)*4+3]
}else{T=Y[0];S=Y[1];R=Y[2];Q=Y[3]}var P=this.finiteLinesIntersect(aa,Z,W,V,T,S,R,Q,true);X[U*2]=P[0];X[U*2+1]=P[1]}return X};t.triangle=function(R,Q,P){u.renderer().drawPolygon(R._private.position.x,R._private.position.y,Q,P,"triangle",3)};v.triangle=function(S,R,Q,P,T){return G.findPolygonIntersection(S,R,Q,P,T,"triangle",3)};t.square=function(R,Q,P){u.renderer().drawPolygon(R._private.position.x,R._private.position.y,Q,P,"square",4)};v.square=function(S,R,Q,P,T){return G.findPolygonIntersection(S,R,Q,P,T,"square",4)
};t.rectangle=t.square;v.rectangle=v.square;t.pentagon=function(R,Q,P){u.renderer().drawNgon(R._private.position.x,R._private.position.y,Q,P,"pentagon",5)};v.pentagon=function(S,R,Q,P,T){return G.findPolygonIntersection(S,R,Q,P,T,"pentagon",5)};t.hexagon=function(R,Q,P){u.renderer().drawNgon(R._private.position.x,R._private.position.y,Q,P,"hexagon",6)};v.hexagon=function(S,R,Q,P,T){return G.findPolygonIntersection(S,R,Q,P,T,"hexagon",6)};t.heptagon=function(R,Q,P){u.renderer().drawNgon(R._private.position.x,R._private.position.y,Q,P,"heptagon",7)
};v.heptagon=function(S,R,Q,P,T){return G.findPolygonIntersection(S,R,Q,P,T,"heptagon",7)};t.octagon=function(R,Q,P){u.renderer().drawNgon(R._private.position.x,R._private.position.y,Q,P,"octagon",8)};v.octagon=function(S,R,Q,P,T){return G.findPolygonIntersection(S,R,Q,P,T,"octagon",8)};M.prototype.findNearestIntersection=function(V,P,W){var R;var U;var S;var Q;if(V.length==0){return[]}for(var T=0;T<V.length/2;T++){R=Math.pow(P-V[T*2],2)+Math.pow(W-V[T*2+1],2);if(U==undefined||U>R){U=R;S=V[T*2];Q=V[T*2+1]
}}return[S,Q]};M.prototype.shortenIntersection=function(U,T,R){var P=[U[0]-T[0],U[1]-T[1]];var S=Math.sqrt(P[0]*P[0]+P[1]*P[1]);var Q=(S-R)/S;if(Q<0){return[]}else{return[T[0]+Q*P[0],T[1]+Q*P[1]]}};M.prototype.drawPolygon=function(V,U,P,X,S,R){if(f[S]==undefined){f[S]=B(R,0)}var W=f[S];var Q=u.renderer().context;Q.save();Q.translate(V,U);Q.beginPath();Q.scale(P/2,X/2);Q.moveTo(W[0],W[1]);for(var T=1;T<W.length/2;T++){Q.lineTo(W[T*2],W[T*2+1])}Q.closePath();Q.fill();Q.restore()};M.prototype.polygonIntersectLine=function(V,U,P,S,Q,Y,X,W){var ag=[];
var T;var R=new Array(P.length);for(var ac=0;ac<R.length/2;ac++){R[ac*2]=P[ac*2]*Y+S;R[ac*2+1]=P[ac*2+1]*X+Q}var af=this.expandPolygon(R,-W);var ab=this.joinLines(af);var aa,Z,ae,ad;for(var ac=0;ac<ab.length/2;ac++){aa=ab[ac*2];Z=ab[ac*2+1];if(ac<ab.length/2-1){ae=ab[(ac+1)*2];ad=ab[(ac+1)*2+1]}else{ae=ab[0];ad=ab[1]}T=this.finiteLinesIntersect(V,U,S,Q,aa,Z,ae,ad);if(T.length!=0){ag.push(T[0],T[1])}}return ag};M.prototype.finiteLinesIntersect=function(U,ab,S,Z,Q,Y,ac,W,aa){var V=(ac-Q)*(ab-Y)-(W-Y)*(U-Q);
var X=(S-U)*(ab-Y)-(Z-ab)*(U-Q);var T=(W-Y)*(S-U)-(ac-Q)*(Z-ab);if(T!=0){var R=V/T;var P=X/T;if(0<=R&&R<=1&&0<=P&&P<=1){return[U+R*(S-U),ab+R*(Z-ab)]}else{if(!aa){return[]}else{return[U+R*(S-U),ab+R*(Z-ab)]}}}else{if(V==0||X==0){if([U,S,ac].sort()[1]==ac){return[ac,W]}if([U,S,Q].sort()[1]==Q){return[Q,Y]}if([Q,ac,S].sort()[1]==S){return[S,Z]}return[]}else{return[]}}};M.prototype.drawNgon=function(V,U,R,P,X){var Q=u.renderer().context;Q.save();Q.translate(V,U);Q.beginPath();var W=1/R*2*Math.PI;var T=R%2==0?Math.PI/2+W/2:Math.PI/2;
Q.scale(P/2,X/2);Q.moveTo(Math.cos(T),-Math.sin(T));for(var S=T;S<T+2*Math.PI;S+=W){Q.lineTo(Math.cos(S),-Math.sin(S))}Q.closePath();Q.fill();Q.restore()};var p=false;M.prototype.redraw=function(T){if(c){}c=setTimeout(function(){c=null;if(p&&!T){G.redraw(true);p=false}},1000/80);var Q=this.context;var R=this.canvasContexts;var P=this.options.cy.elements().toArray();var X=[];var V=[];if(this.canvasNeedsRedraw[2]||this.canvasNeedsRedraw[4]){this.findEdgeControlPoints(this.options.cy.edges());P.sort(function(aa,Z){var Y=aa._private.style["z-index"].value-Z._private.style["z-index"].value;
if(Y==0){if(aa._private.group=="nodes"&&Z._private.group=="edges"){return 1}else{if(aa._private.group=="edges"&&Z._private.group=="nodes"){return -1}}}return 0})}if(this.canvasNeedsRedraw[2]){Q=this.canvasContexts[2];this.context=Q;Q.setTransform(1,0,0,1,0,0);Q.clearRect(0,0,this.options.cy.container().clientWidth,this.options.cy.container().clientHeight);Q.translate(this.cy.pan().x,this.cy.pan().y);Q.scale(this.cy.zoom(),this.cy.zoom());var S;for(var U=0;U<P.length;U++){S=P[U];if(S._private.rscratch.layer2){if(S._private.group=="nodes"){this.drawNode(S)
}else{if(S._private.group=="edges"){this.drawEdge(S)}}}}this.canvasNeedsRedraw[2]=false;this.redrawReason[2]=[]}if(this.canvasNeedsRedraw[4]){Q=this.canvasContexts[4];this.context=Q;Q.setTransform(1,0,0,1,0,0);Q.clearRect(0,0,this.options.cy.container().clientWidth,this.options.cy.container().clientHeight);Q.translate(this.cy.pan().x,this.cy.pan().y);Q.scale(this.cy.zoom(),this.cy.zoom());var S;for(var U=0;U<P.length;U++){S=P[U];if(!S._private.rscratch.layer2){if(S._private.group=="nodes"){this.drawNode(S)
}else{if(S._private.group=="edges"){this.drawEdge(S)}}}}this.canvasNeedsRedraw[4]=false;this.redrawReason[4]=[]}if(this.canvasNeedsRedraw[0]){Q=this.canvasContexts[0];Q.setTransform(1,0,0,1,0,0);Q.clearRect(0,0,this.canvases[0].width,this.canvases[0].height);Q.translate(this.cy.pan().x,this.cy.pan().y);Q.scale(this.cy.zoom(),this.cy.zoom());if(J[4]==1){var W=u.style()._private.coreStyle;Q.lineWidth=W["selection-box-border-width"].value;Q.fillStyle="rgba("+W["selection-box-color"].value[0]+","+W["selection-box-color"].value[1]+","+W["selection-box-color"].value[2]+","+W["selection-box-opacity"].value+")";
Q.fillRect(J[0],J[1],J[2]-J[0],J[3]-J[1]);Q.strokeStyle="rgba("+W["selection-box-border-color"].value[0]+","+W["selection-box-border-color"].value[1]+","+W["selection-box-border-color"].value[2]+","+W["selection-box-opacity"].value+")";Q.strokeRect(J[0],J[1],J[2]-J[0],J[3]-J[1])}this.canvasNeedsRedraw[0]=false;this.redrawReason[0]=[]}this.bufferCanvasContexts[1].globalCompositeOperation="copy";this.bufferCanvasContexts[1].drawImage(this.canvases[4],0,0);this.bufferCanvasContexts[1].globalCompositeOperation="source-over";
this.bufferCanvasContexts[1].drawImage(this.canvases[2],0,0);this.bufferCanvasContexts[1].drawImage(this.canvases[0],0,0);this.bufferCanvasContexts[0].globalCompositeOperation="copy";this.bufferCanvasContexts[0].drawImage(this.bufferCanvases[1],0,0)};M.prototype.drawEdge=function(Q){var P=G.context;var R,W;var Y,T,Z,X,S;if(Q._private.style.visibility.value!="visible"){return}R=Q.source()[0];W=Q.target()[0];P.strokeStyle="rgba("+Q._private.style["line-color"].value[0]+","+Q._private.style["line-color"].value[1]+","+Q._private.style["line-color"].value[2]+","+Q._private.style.opacity.value+")";
P.lineWidth=Q._private.style.width.value;if(P.lineWidth<=0){return}this.findEndpoints(Q);if(Q._private.rscratch.isSelfEdge){P.beginPath();P.moveTo(Q._private.rscratch.startX,Q._private.rscratch.startY);P.quadraticCurveTo(Q._private.rscratch.cp2ax,Q._private.rscratch.cp2ay,Q._private.rscratch.selfEdgeMidX,Q._private.rscratch.selfEdgeMidY);P.moveTo(Q._private.rscratch.selfEdgeMidX,Q._private.rscratch.selfEdgeMidY);P.quadraticCurveTo(Q._private.rscratch.cp2cx,Q._private.rscratch.cp2cy,Q._private.rscratch.endX,Q._private.rscratch.endY);
P.stroke()}else{if(Q._private.rscratch.isStraightEdge){P.beginPath();P.moveTo(Q._private.rscratch.startX,Q._private.rscratch.startY);P.lineTo(Q._private.rscratch.endX,Q._private.rscratch.endY);P.stroke()}else{P.beginPath();P.moveTo(Q._private.rscratch.startX,Q._private.rscratch.startY);P.quadraticCurveTo(Q._private.rscratch.cp2x,Q._private.rscratch.cp2y,Q._private.rscratch.endX,Q._private.rscratch.endY);P.stroke()}}if(!Q._private.rscratch.noArrowPlacement){this.drawArrowheads(Q)}P.textAlign="center";
P.textBaseline="middle";var V,U;var ab,aa;if(Q._private.rscratch.isSelfEdge){ab=Q._private.rscratch.selfEdgeMidX;aa=Q._private.rscratch.selfEdgeMidY}else{if(Q._private.rscratch.isStraightEdge){ab=(Q._private.rscratch.startX+Q._private.rscratch.endX)/2;aa=(Q._private.rscratch.startY+Q._private.rscratch.endY)/2}else{if(Q._private.rscratch.isBezierEdge){ab=Math.pow(1-0.5,2)*Q._private.rscratch.startX+2*(1-0.5)*0.5*Q._private.rscratch.cp2x+(0.5*0.5)*Q._private.rscratch.endX;aa=Math.pow(1-0.5,2)*Q._private.rscratch.startY+2*(1-0.5)*0.5*Q._private.rscratch.cp2y+(0.5*0.5)*Q._private.rscratch.endY
}}}V=ab;U=aa;this.drawText(Q,V,U)};M.prototype.drawNode=function(S){var Q=G.context;var Z,U,ab,X,R;var W,V;var aa,P;if(S._private.style.visibility.value!="visible"){return}Q.fillStyle="rgba("+S._private.style["background-color"].value[0]+","+S._private.style["background-color"].value[1]+","+S._private.style["background-color"].value[2]+","+(S._private.style["background-opacity"].value*S._private.style.opacity.value)+")";Q.strokeStyle="rgba("+S._private.style["border-color"].value[0]+","+S._private.style["border-color"].value[1]+","+S._private.style["border-color"].value[2]+","+(S._private.style["border-opacity"].value*S._private.style.opacity.value)+")";
aa=S._private.style.width.value;P=S._private.style.height.value;t[S._private.style.shape.value](S,aa,P);Q.lineWidth=S._private.style["border-width"].value;if(Q.lineWidth>0){Q.stroke()}var T=S._private.style["text-halign"].strValue;if(T=="left"){Q.textAlign="right";W=S._private.position.x-aa/2}else{if(T=="right"){Q.textAlign="left";W=S._private.position.x+aa/2}else{if(T=="center"){Q.textAlign="center";W=S._private.position.x}else{Q.textAlign="center";W=S._private.position.x}}}var Y=S._private.style["text-valign"].strValue;
if(Y=="top"){Q.textBaseline="bottom";V=S._private.position.y-P/2}else{if(Y=="bottom"){Q.textBaseline="top";V=S._private.position.y+P/2}else{if(Y=="middle"||styleValue=="center"){Q.textBaseline="middle";V=S._private.position.y}else{Q.textBaseline="middle";V=S._private.position.y}}}this.drawText(S,W,V)};M.prototype.drawText=function(T,R,Q){var S=G.context;labelStyle=T._private.style["font-style"].strValue;labelSize=T._private.style["font-size"].strValue;labelFamily=T._private.style["font-family"].strValue;
labelVariant=T._private.style["font-variant"].strValue;labelWeight=T._private.style["font-weight"].strValue;S.font=labelStyle+" "+labelVariant+" "+labelWeight+" "+labelSize+" "+labelFamily;var U=String(T._private.style.content.value);var V=T._private.style["text-transform"].value;if(V=="none"){}else{if(V=="uppercase"){U=U.toUpperCase()}else{if(V=="lowercase"){U=U.toLowerCase()}}}S.fillStyle="rgba("+T._private.style.color.value[0]+","+T._private.style.color.value[1]+","+T._private.style.color.value[2]+","+(T._private.style["text-opacity"].value*T._private.style.opacity.value)+")";
S.strokeStyle="rgba("+T._private.style["text-outline-color"].value[0]+","+T._private.style["text-outline-color"].value[1]+","+T._private.style["text-outline-color"].value[2]+","+(T._private.style["text-opacity"].value*T._private.style.opacity.value)+")";S.fillText(U,R,Q);var P=T._private.style["text-outline-width"].value;if(P>0){S.lineWidth=P;S.strokeText(U,R,Q)}};M.prototype.zoom=function(P){if(P!=undefined&&P.level!=undefined){this.scale[0]=P.level;this.scale[1]=P.level}console.log("zoom call");
console.log(P)};M.prototype.fit=function(P){console.log("fit call");console.log(P)};M.prototype.pan=function(P){console.log("pan call");console.log(P);if(this.context!=undefined){}};M.prototype.panBy=function(P){this.center[0]-=P.x;this.center[1]-=P.y;this.redraw();console.log("panBy call");console.log(P)};K("renderer","canvas",M)})(cytoscape);(function(a){var b={ready:function(){},stop:function(){}};function c(d){this.options=a.util.extend(true,{},b,d)}c.prototype.run=function(){var d=this.options;
