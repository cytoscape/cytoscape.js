...
* **`loop-direction`** : Determines the direction that loops extend from the node in cases when the source and target node of an edge is the same. Possible values are `north`, `south`, `west`, `east`, `northwest`, `southwest`, `northeast`, and `southeast`. Default is `northwest`.
