...
* **`background-clip`** : How background image clipping is handled; may be `node` for clipped to node shape or `none` for no clipping. Supports multiple values, so some `background-image`s can be clipped and others not (useful when you want to draw a background image outside the node).
