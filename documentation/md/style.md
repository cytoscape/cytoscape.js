...
* **`padding-relative-to`** : Determines how padding is calculated if and only if the percentage unit is used. If `width` is specified, the padding will be calculated as a percentage of the node width. If `height` is specified, the padding will be calculated as a percentage of the node height. If `whAverage` is specified, the padding will be calculated as a percentage of the average of the node height and width. If `whMin` is specified, the padding will be calculated as a percentage of the smallest number between width and height. If `whMax` is specified, the padding will be calculated as a percentage of the largest number between width and height.
