...
( function(){
  ...
  function toPoints( raw ){
    ...
    if( typeof raw[ 0 ] === 'number' ){
      for( i = 0; i + 1 < raw.length; i += 2 ){ out.push( { x: raw[ i ], y: raw[ i + 1 ] } ); }
  ...
  function finiteBox( bb ){
    if( bb == null || bb.__error ){ return false; }
  ...
  window.probeRoutes = function( cy, ids ){
    ...
    for( var k = 0; k < ids.length; k++ ){
      ...
      if( e.length === 0 ){ out[ id ] = { id: id, missing: true }; continue; }

      var ctrl = callOrNull( function(){ return e.controlPoints(); } );
      var seg = callOrNull( function(){ return e.segmentPoints(); } );
      ...
      if( interior.length > 0 ){ sig = 'ctrl:' + interior.length; }
      else if( segPts.length > 0 ){ sig = 'seg:' + segPts.length; interior = segPts; }
      else { sig = 'none'; }

      var rec = {
        id: id,
        sig: sig,
        src: pt( callOrNull( function(){ return e.sourceEndpoint(); } ) ),
        tgt: pt( callOrNull( function(){ return e.targetEndpoint(); } ) ),
        mid: pt( callOrNull( function(){ return e.midpoint(); } ) ),
      ...
      rec.finite = {
        src: finite2( rec.src ), tgt: finite2( rec.tgt ), mid: finite2( rec.mid ),
        interior: rec.interior.every( finite2 ),
        bb: finiteBox( callOrNull( function(){ return e.boundingBox(); } ) )
      };
  ...
  window.probeScene = async function( scene ){
    if( window.cy3 ){ window.cy3.destroy(); window.cy3 = null; }
    if( window.cy4 ){ window.cy4.destroy(); window.cy4 = null; }

    var clone = function(){ return JSON.parse( JSON.stringify( scene.elements ) ); };
    ...
    var cy3 = window.makeV3( {
      elements: clone(), style: scene.v3Style, layout: { name: 'preset', fit: false }
    } );
    var cy4 = window.makeV4( { elements: clone(), style: scene.v4Style } );

    await cy4.ready;

    return { v3: window.probeRoutes( cy3, scene.edges ), v4: window.probeRoutes( cy4, scene.edges ) };
  };
} )();
