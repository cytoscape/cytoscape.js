...
const clipPixels = async (page, x, y, width) => {
  ...
  if (y < 0 || y >= view.height || w <= 0) {
    return null;
  }

  return decodePng(
    await page.screenshot({ clip: { x: x0, y, width: w, height: 1 } }),
  ).data;
};
...
test.describe('WebGPU renderer', () => {
  ...
  test('renders a red node on white (premultiplied compositing)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('pick() resolves the node under the point and background elsewhere', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('slot compaction is a visual no-op and keeps picking + labels working (19.4)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    await page.evaluate(() => {
      ...
      for (let i = 0; i < 30; i++) {
        window.cy.getElementById(`x${i}`).remove();
      }
    });
  ...
  test('a mid-flight animation survives a compaction and completes (19.4)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('mouse drag moves the node in the model and on screen', async ({
    ...
    await expect
      .poll(() => page.evaluate(() => window.__hovered), { timeout: 5000 })
      .toBe(true);
  ...
  test('unmount() goes headless; mount() rebuilds from the CPU-canonical model (round 10)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('device loss auto-recovers: devicelost, rebuild, devicerestored (round 10)', async ({
    ...
    await page.evaluate(() => {
      ...
      window.cy.on('devicelost', () => {
        window.__lost = true;
      });
      window.cy.on('devicerestored', () => {
    ...
    await expect
      .poll(() => page.evaluate(() => window.__lost), { timeout: 5000 })
      .toBe(true);
    await expect
      .poll(() => page.evaluate(() => window.__restored), { timeout: 10000 })
      .toBe(true);
  ...
  */
  test('mouseout fires when the hover leaves the node (the mouseover sibling)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    expect(await page.evaluate(() => window.__events.join(','))).not.toContain(
      'mouseout',
    );
  ...
  test('pointercancel aborts the gesture: free without dragfree (17.1/17.2)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('the drag-state family: grab/drag/free with companions (round 17.2)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('tapselect/tapunselect + hover-during-drag events (round 17.3)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    await page.evaluate(() => {
      window.cy.userPanningEnabled(false);
      window.__events = [];
    });
  ...
  test('gesture parity: cxttap family, dbltap, taphold (round 10)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('interaction options: wheelSensitivity, desktopTapThreshold, tapholdDuration (round 20.1)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    const zoomAfterWheel = async () => {
      ...
      await expect
        .poll(() => page.evaluate(() => window.cy.zoom()))
        .not.toBe(before);
  ...
  test('dragging a selected node drags the whole selection (round 10)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('a locked node does not drag; the gesture pans instead', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('hide() removes a node from both rendering and picking', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test.describe('SDF labels', () => {
    ...
    test('renders below the node', async ({ page }) => {
      ...
      const inLabelRow = await darkPixelsInBand(
        page,
        center.x - 120,
        240,
        center.y + LABEL_ROW_OFFSET,
      );
      const aboveNode = await darkPixelsInBand(
        page,
        center.x - 120,
        240,
        center.y - 100,
      ...
      expect(
        await page.evaluate(() => window.cy._renderer.stats().glyphs),
      ).toBe(5);
    });

    test('follows a node move on-GPU without a glyph rebuild', async ({
      page,
    }) => {
      test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
      ...
      const atNewSpot = await darkPixelsInBand(
        ...
        center.x - 150 - 120,
        240,
        center.y - 100 + LABEL_ROW_OFFSET,
      );
      const atOldSpot = await darkPixelsInBand(
        page,
        center.x - 120,
        240,
        center.y + LABEL_ROW_OFFSET,
    ...
    test('fades out below the LOD threshold', async ({ page }) => {
      ...
      await page.evaluate(() =>
        window.cy.zoom({ level: 0.05, renderedPosition: window.cy.pan() }),
      );
      await waitFrames(page);
  ...
  test('viewport gesture events: dragpan, scrollzoom, pinchzoom (round 17.4)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('the GPU force integrator holds the lease and settles (round 18.3)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('two-finger pinch zooms about the midpoint', async ({ page }) => {
    ...
    const result = await page.evaluate(async (center) => {
      ...
      return {
        ...
        deadTouchPanned:
          panAfterDrag.x !== panBeforeDrag.x ||
          panAfterDrag.y !== panBeforeDrag.y,
      };
    }, center);
  ...
  test('two-finger cxt gesture: cxttap family on touch (round 20.4)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    const run = await page.evaluate(async (center) => {
      ...
      for (let step = 1; step <= 4; step++) {
        // 80 -> 240 apart (past 1.5x and 150 px)
  ...
  test('three-finger box selection on touch (round 20.5)', async ({ page }) => {
    ...
    const run = await page.evaluate(async (center) => {
      ...
      window.cy.on('boxselect', (e) =>
        window.__events.push('boxselect:' + e.target.id()),
      );
      ...
      return {
        ...
        panUnchanged:
          window.cy.pan().x === before.pan.x &&
          window.cy.pan().y === before.pan.y,
      };
    }, center);
  ...
  test('pixelRatio pins the backing-store resolution; picking stays in css px (round 20.6)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    await makeReadyCy(
      page,
      Object.assign({}, RED_NODE_GRAPH, { pixelRatio: 2 }),
    );
  ...
  test('tap selects and background tap clears', async ({ page }) => {
    ...
    await expect
      .poll(
        () =>
          page.evaluate(() =>
            window.cy.pick(10, 10).then((ele) => ele == null),
          ),
        { timeout: 5000 },
      )
      .toBe(true);
    ...
    expect(await page.evaluate(() => window.cy.$id('a').selected())).toBe(
      false,
    );
  });
  ...
  test('shift-drag box-selects the contained elements', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      window.cy.on('boxselect', (e) =>
        window.__boxEvents.push('boxselect:' + e.target.id()),
      );
    });
    ...
    expect(
      await page.evaluate(() => window.cy.elements({ selected: true }).length),
    ).toBe(0);
  });

  test('events carry the DOM event as originalEvent (round 41.4)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    for (const rec of seen) {
      ...
      expect(
        rec.clientX,
        `${rec.name}'s originalEvent has no coordinates`,
  ...
  test('boxSelectionMode overlap catches what the band touches (round 39.1)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test("events: 'no' elements are pointer-transparent but still render (round 20.2)", async ({
    page,
  }) => {
    await page.goto(PAGE);
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    await makeReadyCy(page, {
      ...
      style: {
        nodes: {
          ...
          shape: 'rectangle',
          'background-color': {
            case: [{ when: { data: 'kind', eq: 'ui' }, then: 'blue' }],
            else: 'red',
          },
          events: {
            case: [{ when: { data: 'kind', eq: 'ui' }, then: 'no' }],
            else: 'yes',
          },
        },
    ...
    await expect
      .poll(() => page.evaluate(() => window.__hovers))
      .toContain('under');
    ...
    expect(await page.evaluate(() => window.__boxed.sort())).toEqual([
      'under',
      'x1',
      'x2',
    ]);
  ...
  test("text-events: 'yes' makes the label box tap the node (round 20.3)", async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    await expect
      .poll(() => page.evaluate(() => window.cy.$id('a').selected()))
      .toBe(true);
  });

  test("visibility: 'hidden' blanks pixels but keeps space (round 22)", async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    const style = (vis) => ({
      nodes: {
        ...
        'font-size': 16,
        visibility: {
          case: [{ when: { data: 'kind', eq: 'ghosty' }, then: vis }],
          else: 'visible',
        },
      },
    ...
    const labelDark = await darkPixelsInBand(
      page,
      center.x - 60,
      120,
      center.y + 45,
  ...
  test('mapped opacity evaluates on the GPU: a data write repaints without a CPU restyle', async ({
    page,
  }) => {
    await page.goto(PAGE);
  ...
  test('animate() tweens a node position to the target over time', async ({
    page,
  }) => {
    await page.goto(PAGE);
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter');
    ...
    await page.evaluate(async () => {
      await window.cy
        .$id('a')
        .animation({ position: { x: 0, y: 0 }, duration: 200 })
        .play();
    });
    await waitFrames(page);
    ...
    expect(await page.evaluate(() => window.cy.$id('a').animated())).toBe(
      false,
    );
  });
  ...
  test('GPU position tween holds the lease: CPU position stays stale while the node moves', async ({
    ...
    await page.evaluate(() =>
      window.cy
        .$id('a')
        .animation({ position: { x: 120, y: 0 }, duration: 1 })
        .play(),
    );
    await waitFrames(page);

    expect(
      await page.evaluate(() => window.cy.$id('a').position().x),
    ).toBeCloseTo(120, 0);
    expect(await page.evaluate(() => window.cy.$id('a').animated())).toBe(
      false,
    );
  ...
  test('GPU paint tween holds the lease: pixels fade while the CPU style stays stale', async ({
    page,
  }) => {
    await page.goto(PAGE);
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter');
    ...
    expect((await pixelAt(page, center.x, center.y)).slice(0, 3)).toEqual([
      0, 0, 255,
    ]);
    ...
    const mid = await untilMidFlight(
      page,
    ...
    expect(
      await page.evaluate(() => window.cy.$id('a').style('background-color')),
    ).toBe('rgb(0,0,255)');
  ...
  test('a style transition rides the GPU tween path and lands on the resolved end state (24.2)', async ({
    page,
  }) => {
    await page.goto(PAGE);
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter');
    ...
    await expect
      .poll(
        ...
        {
          timeout: 5000,
          message: 'the transition settles on the resolved end state',
        },
      )
  ...
  test('a mapped-channel transition works under the renderer: listing a prop demotes its kernel eval (24.2)', async ({
    page,
  }) => {
    await page.goto(PAGE);
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter');
    ...
    await expect
      .poll(
        ...
        {
          timeout: 5000,
          message: "the transition settles on the mapper's value at w=1",
        },
      )
      .toBe('rgb(255,255,0)');
  ...
  test('a size transition tweens pixels mid-flight with the label anchor riding (25.6)', async ({
    page,
  }) => {
    await page.goto(PAGE);
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter');
    ...
    expect((await pixelAt(page, center.x + 40, center.y))[1]).toBeGreaterThan(
      200,
    );
    ...
    const midState = await page.evaluate(() => ({
      ...
      anchorX: window.cy._store.labelAt(
        window.cy._store.lookup('a').slot,
        'nodes',
      ).anchorX,
    }));
    ...
    await expect
      .poll(() => page.evaluate(() => window.cy.$id('a').width()), {
        timeout: 5000,
        message: 'the width transition settles on 200',
      })
      .toBe(200);
  ...
  test('an edge-width transition thickens the casing in step (25.6)', async ({
    page,
  }) => {
    await page.goto(PAGE);
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter');
    ...
    expect((await pixelAt(page, center.x, center.y + 8))[1]).toBeGreaterThan(
      200,
    );
  ...
  test('spring() overshoots on the device: the node passes the target, then settles on it', async ({
    page,
  }) => {
    await page.goto(PAGE);
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter');
    ...
    */
    ...
    await page.evaluate(
      () =>
        new Promise((resolve) => {
          const wait = () => {
            if (performance.now() - window.__springStart >= 2200) {
              resolve();
            } else {
    ...
    expect((await pixelAt(page, center.x + 35, center.y))[1]).toBeGreaterThan(
      200,
    );
    expect((await pixelAt(page, center.x, center.y))[1]).toBeLessThan(80);
  });

  test('a steep named easing is evaluated on the device, not treated as linear', async ({
    page,
  }) => {
    await page.goto(PAGE);
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter');
    ...
    expect((await pixelAt(page, center.x - 95, center.y))[1]).toBeLessThan(80);
    expect((await pixelAt(page, center.x - 20, center.y))[1]).toBeGreaterThan(
      200,
    );
  ...
  test('mapped colors render the OKLab interpolation the getters report', async ({
    page,
  }) => {
    await page.goto(PAGE);
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter');
    ...
    const readRgb = async () =>
      await page.evaluate(() => {
        const match = /rgb\((\d+),(\d+),(\d+)\)/.exec(
          window.cy.$id('a').style('background-color'),
        );
  ...
  test('png() exports the viewport: dimensions, node pixels, transparent background', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    const { width, height, pixels, prefix } = await pngAndSample(page, {}, [
      [center.x, center.y],
      [5, 5],
    ]);
  ...
  test('png() full export sizes to the graph bounds; scale and maxWidth apply', async ({
    ...
    const full = await pngAndSample(page, { full: true }, [
      [50, 50],
      [250, 150],
    ]);
  ...
  test('png() bg option fills the background; jpg() defaults to white', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  */
  ...
  test('png() rejects on bad options, naming the option at fault', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('png() rejects a viewport export of a zero-sized container', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    const hidden = await exportErrors(page, {
      viewport: {},
      full: { full: true },
    });
  ...
  */
  ...
  test('ready rejects when the canvas gives no webgpu context', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('ready rejects when glyph rasterization has no 2d context', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('a background image that 404s warns once and renders imageless', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('export WYSIWYG: a viewport export at scale 1 pixel-matches the screen', async ({
    page,
  }, testInfo) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('the shaping memo shares laid blocks across same-text labels (round 16.5)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    await makeReadyCy(page, {
      elements: Array.from({ length: 120 }, (_, i) => ({
        data: { id: 'n' + i },
        position: { x: (i % 12) * 40, y: Math.floor(i / 12) * 40 },
      })),
      style: {
        nodes: {
          width: 20,
  ...
  test('font-family change re-rasters the atlas and re-lays-out labels live', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('glyphs re-raster when a web font finishes loading after label build', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('edge labels render at the midpoint and follow endpoint moves on-GPU', async ({
    ...
    const uploadedBefore = await page.evaluate(
      () => window.cy.renderer().stats().uploadedBytes,
    );
    ...
    const uploadedAfter = await page.evaluate(
      () => window.cy.renderer().stats().uploadedBytes,
    );
  ...
  test('edge label autorotate rotates the run and follows the angle on-GPU', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('png() export mid-animation snapshots the GPU-owned position', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    await untilMidFlight(
      ...
      {
        timeout: 8000,
        what: 'the export showing the node off the start and over the midpoint',
      },
    );
  ...
  test('bezier bundle: curves render off the chord and follow a drag on-GPU', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    const movedMid = await page.evaluate(() =>
      window.cy.$id('e0').renderedMidpoint(),
    );
  ...
  test('pick() hits a curved edge on its bulge, background on the chord', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('curved-edge labels anchor at the curve midpoint and follow drags', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    await makeReadyCy(page, {
      ...
      style: {
        ...
        edges: {
          ...
          width: 2,
          'line-color': '#dfe6e9', // light, so only glyphs read dark
          label: { data: 'lbl' },
          'font-size': 16,
          color: '#000000',
        },
    ...
    expect(await darkPixelsInBand(page, mid.x - 40, 80, mid.y)).toBeGreaterThan(
      5,
    );
    ...
    expect(uploadedAfter - uploadedBefore).toBeLessThanOrEqual(64);
    expect(
      await darkPixelsInBand(page, movedMid.x - 40, 80, movedMid.y),
    ).toBeGreaterThan(5);
  });
  ...
  test('self-loops render as loops (not degenerate points)', async ({
    ...
    const mid = await page.evaluate(() =>
      window.cy.$id('loop').renderedMidpoint(),
    );
  ...
  test('segments edges render the polyline and follow drags on-GPU (12b)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('taxi edges route axis-aligned and pick on their legs (12b)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    const segpts = await page.evaluate(() =>
      window.cy.$id('e0').renderedSegmentPoints(),
    );
  ...
  test('round-segments corners are rounded to the arc (12b)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    const cornerPt = await page.evaluate(
      () => window.cy.$id('e0').renderedSegmentPoints()[0],
    );
  ...
  test('unbundled bezier splines through its control points (12b)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('taxi arrows ride the final leg into the node (12b)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('route labels anchor at the route midpoint and follow drags (12b)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    expect(await darkPixelsInBand(page, center.x - 40, 80, center.y - 20)).toBe(
      0,
    );
  ...
  test('haystack edges draw between their offset points and stay pickable (12c)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('straight-triangle edges taper from a wide base to the target apex (12c)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('manual endpoints move the drawn edge and follow drags on-GPU (12c)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    const src = await page.evaluate(() =>
      window.cy.$id('e0').renderedSourceEndpoint(),
    );
    ...
    const movedSrc = await page.evaluate(() =>
      window.cy.$id('e0').renderedSourceEndpoint(),
    );
  ...
  test('arrows sit at manual endpoints along the chord tangent (12c)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    const tgt = await page.evaluate(() =>
      window.cy.$id('e0').renderedTargetEndpoint(),
    );
  ...
  test('ghosts duplicate the node body at the offset and follow drags (A1)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('overlay washes over the body; underlay peeks out under it (A2)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('core theming styles the selection box and active-bg circle (A2)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    await makeReadyCy(page, {
      ...
      style: {
        ...
        core: {
          ...
          'selection-box-border-color': '#00ff00',
          'selection-box-border-width': 2,
          'active-bg-color': '#0000ff',
    ...
    const boxStyle = await page.evaluate(() => {
      const el = [...document.querySelectorAll('div')].find(
        (d) => d.style.display === 'block' && d.style.border.includes('solid'),
      );

      return el == null
        ? null
        : { background: el.style.background, border: el.style.border };
    });
  ...
  test('the active-bg circle follows the background drag (round 43.7)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('the active-bg circle stays put when panning is disabled (round 43.7)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('arrow scalars: hollow rings and scaled heads (B7)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('mid arrows sit at straight and curved midpoints (C1)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    await makeReadyCy(page, {
      ...
      style: {
        ...
        edges: {
          ...
          'line-color': '#bdc3c7',
          'curve-style': {
            case: [{ when: { data: 'curved', eq: 1 }, then: 'bezier' }],
            else: 'straight',
          },
          'mid-target-arrow-shape': 'triangle',
    ...
    const straightMid = await page.evaluate(() =>
      window.cy.$id('straight').renderedMidpoint(),
    );
    ...
    const curveMid = await page.evaluate(() =>
      window.cy.$id('q1').renderedMidpoint(),
    );
  ...
  test('custom polygons draw and pick by their point list (C3)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('font-weight thickens label ink and font-style reshapes it (D1)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    const boldUri = await page.evaluate(
      async () => await window.cy.png({ bg: '#fff' }),
    );
    ...
    const italicUri = await page.evaluate(
      async () => await window.cy.png({ bg: '#fff' }),
    );
  ...
  test('min-zoomed-font-size hides only the floored label on zoom-out (D2)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    const labelInk = async (id) => {
      const p = await page.evaluate(
        (id) => window.cy.$id(id).renderedPosition(),
        id,
      );
  ...
  test('text-valign/-halign move the label around the node (D3)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('source/target labels sit at their arc offsets and follow restyles (D4)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('compound parents draw under children and edges, and pick around them (round 14.9)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  test('dragging a parent moves its subtree; a selected parent+child pair moves once (round 14.11)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
  ...
  */
  ...
  test('device loss mid-animation settles the lease and completes (round 48.5)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    await expect
      .poll(() => page.evaluate(() => window.__done), { timeout: 15000 })
      .toBe(true);
    await expect
      .poll(() => page.evaluate(() => window.__restored), { timeout: 15000 })
      .toBe(true);
  ...
  test('device loss mid-export rejects rather than hanging (round 48.5)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    if (outcome.settled === 'rejected') {
      expect(
        outcome.error,
        `export rejected with a non-Error: ${outcome.message}`,
      ).toBe(true);
    }

    // and the instance recovers regardless of which way the export went
    await expect
      .poll(() => page.evaluate(() => window.cy.renderer() != null), {
        timeout: 15000,
      })
      .toBe(true);
  ...
  test('device loss mid-force-run leaves readable positions (round 48.5)', async ({
    page,
  }) => {
    test.skip(!(await hasAdapter(page)), 'no WebGPU adapter available');
    ...
    await makeReadyCy(page, {
      elements: (() => {
        ...
        for (let i = 0; i < 60; i++) {
          elements.push({
            data: { id: 'n' + i },
            position: { x: (i % 10) * 20, y: ((i / 10) | 0) * 20 },
          });
        }
    ...
    await expect
      .poll(() => page.evaluate(() => window.__restored), { timeout: 20000 })
      .toBe(true);
