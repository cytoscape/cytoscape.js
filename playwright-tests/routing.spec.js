...
test.describe( 'v3-vs-v4 routing parity (numeric)', () => {
  test.beforeAll( () => {
    const bundle = path.resolve( process.cwd(), 'v3', 'build', 'cytoscape.umd.js' );
  ...
  test.beforeEach( async ( { page } ) => {
    ...
    await page.waitForFunction( () => window.probeScene != null && window.makeV3 != null );
  } );
  ...
  for( const scene of SCENES ){
    test( `routing: ${scene.name} — ${scene.note}`, async ( { page } ) => {
      ...
      // records.
      if( scene.expectFail != null ){ test.fail( true, scene.expectFail ); }

      const { v3, v4 } = await page.evaluate( s => window.probeScene( s ), scene );
      const result = compareRoutes( v3, v4, {
        scene: scene.name, tolClass: scene.tolClass, ledger: DIVERGENCES
      } );
      ...
      expect( result.rows.length, `${scene.name} compared no fields` ).toBeGreaterThan( 0 );

      expect( result.staleLedger, `stale ledger entries for ${scene.name}` ).toEqual( [] );
      expect( result.structural, `structural mismatches in ${scene.name}` ).toEqual( [] );
      expect( result.diverged.map( r => `${r.key} by ${r.delta.toFixed( 4 )}` ),
      ...
      expect( result.maxDelta,
        `${scene.name}: matching fields have crept to within 4x of the ${result.tol} tolerance; ` +
        're-derive the bound rather than raising it' ).toBeLessThan( result.tol / 4 );
    } );
  }
   ...
   */
  test( 'control: a 0.01 px perturbation fails at every tolerance class', async ( { page } ) => {
    ...
    for( const rec of Object.values( nudged ) ){
      if( rec.src != null && typeof rec.src.x === 'number' ){ rec.src.x += 0.01; }
    }

    for( const tolClass of Object.keys( TOL ) ){
      const result = compareRoutes( v3, nudged, { scene: scene.name, tolClass, ledger: [] } );

      expect( result.diverged.length,
   ...
   */
  test( 'control: a non-finite value is structural, not a passing delta', async ( { page } ) => {
    const scene = SCENES[ 0 ];
    const { v3, v4 } = await page.evaluate( s => window.probeScene( s ), scene );

    const before = compareRoutes( v3, v4, { scene: scene.name, tolClass: 'exact', ledger: [] } );

    const broken = JSON.parse( JSON.stringify( v4 ) );
    let nanned = 0;

    for( const rec of Object.values( broken ) ){
      if( rec.mid != null ){ rec.mid.x = 'NaN'; nanned++; }
    }

    const after = compareRoutes( v3, broken, { scene: scene.name, tolClass: 'exact', ledger: [] } );
    ...
    expect( nanned, 'the control must actually corrupt something' ).toBeGreaterThan( 0 );
    expect( after.structural.length - before.structural.length,
      'every NaN must be reported structurally' ).toBe( nanned );
    expect( after.diverged.length,
      'a NaN must not be counted as a numeric divergence' ).toBeLessThanOrEqual( before.diverged.length );
  } );
  ...
  for( const scene of SCENES ){
    test( `finite geometry: ${scene.name}`, async ( { page } ) => {
      ...
      expect( Object.keys( v4 ).length, `${scene.name} produced no records` )
        .toBe( scene.edges.length );
      ...
      for( const rec of Object.values( v4 ) ){
        for( const [ field, value ] of Object.entries( rec.finite ?? {} ) ){
          ...
          const allowed = NON_FINITE_ALLOWED.find(
            a => a.scene === scene.name && a.id === rec.id && a.field === field );

          if( allowed != null ){ allowedHit.add( `${allowed.id}/${allowed.field}` ); continue; }
      ...
      const expectedHits = NON_FINITE_ALLOWED
        .filter( a => a.scene === scene.name ).map( a => `${a.id}/${a.field}` );

      expect( [ ...allowedHit ].sort(),
   ...
   */
  test( 'control: consecutive scenes do not read each other\'s numbers', async ( { page } ) => {
  ...
  test( 'the divergence ledger names only live scenes', () => {
    ...
    const orphans = DIVERGENCES
      .filter( e => !names.has( e.key.split( '/' )[ 0 ] ) )
      .map( e => e.key );

    expect( orphans, 'ledger entries naming a scene the matrix no longer has' ).toEqual( [] );

    for( const scene of SCENES ){
      for( const entry of ledgerFor( scene.name ) ){
        const id = entry.key.split( '/' )[ 1 ];

        expect( scene.edges, `ledger entry ${entry.key} names an edge ${scene.name} does not probe` )
          .toContain( id );
      }
    }
  } );
