...
export function summarize( results, { file, note = null } ){
  ...
  return {
    ...
    adapter: meta.adapter != null
      ? [ meta.adapter.vendor, meta.adapter.architecture || meta.adapter.description ]
...
export function loadPublished( dir = PUBLISHED_DIR ){
  ...
  return sortRuns( runs )
    .map( run => {
      ...
      try {
        return { ...run, results: JSON.parse( readFileSync( join( dir, run.file ), 'utf8' ) ) };
      } catch{
        return null;
      }
    } )
...
function main( argv ){
  ...
  const source = positional.length > 0 ? resolve( positional[ 0 ] ) : newestResults();

  if( source == null || !existsSync( source ) || !statSync( source ).isFile() ){
    console.error( source == null
      ? 'no results file to publish.  Run `npm run benchmark:report` first,'
      : `not a results file: ${source}` );
    console.error( 'pass a path explicitly: node scripts/benchmark-publish.mjs <results.json>' );
    process.exit( 1 );
  }
  ...
  if( results.meta?.dirty === true && !allowDirty ){
    console.error( `refusing to publish: ${basename( source )} was measured on a dirty tree,` );
    console.error( 'so its numbers are not attributable to its commit.  Re-run on a clean' );
  ...
  const { keep, drop } = pruneTo != null && pruneTo > 0
    ? pruneRuns( runs, pruneTo )
    : { keep: runs, drop: [] };

  console.log( `publishing ${file}` );
  console.log( `  ${entry.profile} profile · ${entry.jobs} job(s) · ${entry.commit ?? '?'} (${entry.branch ?? '?'})` );
  console.log( `  ${entry.machine ?? 'machine unknown'}` );

  if( entry.adapter != null ){ console.log( `  adapter ${entry.adapter}` ); }

  for( const r of drop ){ console.log( `  prune ${r.file} (${r.profile}, ${r.date})` ); }
  ...
  for( const r of drop ){ rmSync( join( PUBLISHED_DIR, r.file ), { force: true } ); }

  writeFileSync( join( PUBLISHED_DIR, INDEX_FILE ), `${JSON.stringify( { runs: keep }, null, 2 )}\n` );
