...
//
//   node scripts/gpu-jsdoc-coverage.mjs   — which members are documented
//   node scripts/gpu-throw-coverage.mjs   — which throws the suite runs
//   node scripts/gpu-bench-coverage.mjs   — which members a benchmark calls
//
//   node scripts/gpu-bench-coverage.mjs [--verbose]
//
...
// mentions* of a member's name in `benchmark/gpu/*.mjs`.  That means:
//
...
// contradict it later.
//
// The member scan is `auditFile()` from gpu-jsdoc-coverage.mjs — the
...
import { auditFile, PUBLIC_API } from './gpu-jsdoc-coverage.mjs';

const ROOT = fileURLToPath( new URL( '..', import.meta.url ) );
const BENCH_DIR = join( ROOT, 'benchmark/gpu' );
