...
export function parseOsRelease( text ){
  ...
  return name != null ? [ name, version ].filter( v => v != null ).join( ' ' ) : null;
}
...
export function parseLspciMm( text ){
  ...
  for( const line of text.split( '\n' ) ){
    ...
    if( !/VGA compatible controller|3D controller|Display controller/i.test( deviceClass ) ){ continue; }
...
export function parseSystemProfilerDisplays( text ){
  ...
  return cards.map( c => ( {
    ...
    vramBytes: parseAppleVram( c.spdisplays_vram ?? c._spdisplays_vram ?? c.spdisplays_vram_shared )
  } ) );
}
...
export function parseWindowsGpu( text ){
  ...
  return cards.map( c => ( {
    ...
    vramBytes: c.AdapterRAM > 0 && c.AdapterRAM < 0xffffffff ? c.AdapterRAM : null,
...
function linuxProbe( io, errors ){
  ...
  const maxKHz = Number( io.read( '/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_max_freq' ) );

  if( cpu.model == null ){ errors.push( '/proc/cpuinfo unreadable' ); }

  const gpus = parseLspciMm( io.run( 'lspci', [ '-mm' ] ) );

  if( gpus.length === 0 ){ errors.push( 'lspci reported no display device' ); }

  // join the driver's VRAM figure onto the PCI inventory by slot
  for( const card of io.drmCards() ){
    const uevent = parseDrmUevent( io.read( `/sys/class/drm/${card}/device/uevent` ) );

    if( uevent?.slot == null ){ continue; }
    ...
    const vram = Number( io.read( `/sys/class/drm/${card}/device/mem_info_vram_total` ) );
  ...
  return {
    cpu: {
      ...
      maxGHz: Number.isFinite( maxKHz ) && maxKHz > 0 ? Math.round( maxKHz / 1000 ) / 1000 : null
    },
...
function darwinProbe( io, errors ){
  ...
  if( model == null ){ errors.push( 'sysctl machdep.cpu.brand_string unavailable' ); }
  ...
  if( displays == null ){ errors.push( 'system_profiler SPDisplaysDataType unavailable' ); }
  ...
  return {
    cpu: {
      ...
      baseGHz: brandGHz != null ? Number( brandGHz[ 1 ] )
        : Number.isFinite( hz ) && hz > 0 ? Math.round( hz / 1e7 ) / 100 : null,
      maxGHz: null
    },
...
function win32Probe( io, errors ){
  const ps = query => io.run( 'powershell', [
    '-NoProfile', '-Command', `Get-CimInstance ${query} | ConvertTo-Json`
  ] );

  const cpu = parseWindowsCpu( ps( 'Win32_Processor' ) );

  if( cpu == null ){ errors.push( 'Win32_Processor unavailable' ); }
  ...
  return {
    cpu: { maxGHz: null, ...( cpu ?? { model: null, physicalCores: null, logicalCores: null, baseGHz: null } ) },
    memory: { totalBytes: Number( memory ) || null },
    distro: ( () => {
      const v = io.run( 'powershell', [ '-NoProfile', '-Command', '(Get-CimInstance Win32_OperatingSystem).Caption' ] );
 ...
 */
export function describeMachine( { platform = process.platform, io = defaultIo() } = {} ){
  const errors = [];
  let probed;

  try {
    probed = platform === 'linux' ? linuxProbe( io, errors )
      : platform === 'darwin' ? darwinProbe( io, errors )
        : platform === 'win32' ? win32Probe( io, errors )
          : null;
  } catch( err ){
  ...
  } catch{ /* os is not expected to fail; if it does, null is the answer */ }
...
export function fingerprint( machine ){
  const parts = [
    machine.cpu.model, machine.cpu.physicalCores, machine.cpu.logicalCores,
    machine.memory.totalBytes, machine.os.platform, machine.os.arch,
...
export function machineLine( machine ){
  ...
  const cpu = machine.cpu.model != null
    ? machine.cpu.model.replace( /\(R\)|\(TM\)|CPU|Processor|@.*$/g, '' ).replace( /\s+/g, ' ' ).trim()
    : null;
  const cores = machine.cpu.physicalCores != null && machine.cpu.logicalCores != null
    ? `${machine.cpu.physicalCores}c/${machine.cpu.logicalCores}t` : null;
  const gpus = machine.gpus
    .map( g => {
      const vram = fmtBytes( g.vramBytes );

      return g.model != null ? `${g.model}${vram != null ? ` (${vram})` : ''}` : null;
    } )
    .filter( v => v != null );
...
if( import.meta.filename === process.argv[ 1 ] ){
  ...
  if( process.argv.includes( '--json' ) ){
    ...
    console.log( `  os      ${machine.os.distro ?? machine.os.platform} · ${machine.os.kernel} · ${machine.os.arch}` );
    console.log( `  node    ${machine.node.version} (v8 ${machine.node.v8})` );
    console.log( `  id      ${machine.fingerprint}` );

    for( const err of machine.probe.errors ){ console.log( `  probe   ${err}` ); }
  }
}
