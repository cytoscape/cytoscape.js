...
function memberHtml(fn, prefix, ctx, seen, linkRef) {
  ...
  const sig =
    prefix == null || fn.name === prefix || fn.name.startsWith(`${prefix}.`)
      ? fn.name
      : `${prefix}.${fn.name}`;

  const formats = (fn.formats ?? [])
    .map((format) => {
      ...
      const argList =
        args.length > 0
          ? `<dl class="args">${args
              .map(
                (a) =>
                  `<dt><code>${esc(a.name)}</code></dt><dd>${prose(a.descr, ctx)}</dd>`,
              )
              .join('')}</dl>`
          : '';
  ...
  const tag = (label, value) => {
    ...
    return body === ''
      ? ''
      : `<div class="tag"><span class="tag-label">${esc(label)}</span>${body}</div>`;
  };

  const aliases =
    (fn.pureAliases ?? []).length > 0
...
export function apiPage(model, ctx) {
  ...
  for (const ns of model.sections ?? []) {
    for (const group of ns.sections ?? []) {
      for (const fn of group.fns ?? []) {
        ...
        if (!refs.has(`${ns.name}#${fn.name}`)) {
          refs.set(`${ns.name}#${fn.name}`, slug);
        }
      }
    }
  }
  ...
  const linkRef = (html) =>
    html.replace(/<code>([A-Z]\w+)#(\w+)<\/code>/g, (whole, nsName, name) => {
      const slug = refs.get(`${nsName}#${name}`);

      return slug != null
        ? `<a href="#${esc(slug)}"><code>${esc(nsName)}#${esc(name)}</code></a>`
        : whole;
    });

  const namespaces = (model.sections ?? [])
    .map((ns) => {
      const nsSlug = slugify(`ns-${ns.name}`, tocSeen);

      toc.push({ depth: 2, text: ns.name, slug: nsSlug });

      const groups = (ns.sections ?? [])
        .map((group) => {
