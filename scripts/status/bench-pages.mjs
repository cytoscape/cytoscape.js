...
export function geoSpeedup( results ){
  ...
  return Math.exp( speedups.reduce( ( s, x ) => s + Math.log( x ), 0 ) / speedups.length );
}
...
export function byMachine( runs ){
  ...
  for( const run of runs ){
    ...
    const group = groups.get( key ) ?? { fingerprint: run.fingerprint ?? null, machine: run.machine ?? null, runs: [] };
...
function trendTable( group, now ){
  ...
  return `<section class="doc-section">
    ...
    <p class="note">${group.fingerprint != null
    ? `Machine id <code>${esc( group.fingerprint )}</code>. Only runs sharing this id are comparable.`
    : 'These runs predate machine fingerprinting, so nothing can be said about whether they share hardware.'}</p>
...
export function planBenchmarks( { runs, now = Date.now() } ){
  if( runs.length === 0 ){
    return {
      ops: [], count: 0, available: false,
      reason: 'benchmark/published/ is empty — run `npm run benchmark:report` then `npm run benchmark:publish`',
...
npm run benchmark:publish</code></pre>`
  ...
  const ops = runs.map( run => write( pageNameFor( run.file ), renderReport( run.results ) ) );
  const newest = runs[ 0 ];
  const age = newest.date != null ? fmtAge( now - Date.parse( newest.date ) ) : null;
