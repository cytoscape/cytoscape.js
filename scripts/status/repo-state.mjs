...
export function repoState( root, { now = Date.now(), gzip = true } = {} ){
  ...
  const dirtyFiles = dirtyOut == null || dirtyOut === '' ? [] : dirtyOut.split( '\n' );
  const authorDate = git( root, 'log', '-1', '--format=%aI' );

  const bundles = bundleSizes( root, { gzip } );
  const umd = bundles.find( b => b.file === 'cytoscape.umd.js' ) ?? bundles[ 0 ] ?? null;
  ...
  return {
    ...
    staleness: {
      ...
      staleByMs: umd != null && newestSource > 0 ? Math.max( 0, newestSource - umd.mtime ) : null
    },
