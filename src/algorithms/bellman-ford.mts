...
export const bellmanFord = (
  ...
  if (rootSlot == null || !index.has(rootSlot)) {
    throw new TypeError(
      'bellmanFord requires a `root` node in the calling collection',
    );
  }
  ...
  for (const e of view.edgeSlots) {
    ...
    if (s !== t && index.has(s) && index.has(t)) {
      edges.push(e);
    }
  }
  ...
  for (let i = 1; i < n; i++) {
    ...
    for (let e = 0; e < m; e++) {
      ...
      if (!directed) {
        relax(tgtIdx[e], srcIdx[e], edges[e], weights[e]);
      }
    }
  ...
  if (replacedEdge) {
    ...
    for (let e = 0; e < m; e++) {
      ...
      if (forward) {
        negativeNodes.push(u);
      }
      if (backward) {
        negativeNodes.push(v);
      }
      ...
      for (const start of negativeNodes) {
        ...
        nodes = nodes
          .slice(smallestIndex)
          .concat(nodes.slice(0, smallestIndex));
        cycleEdges = cycleEdges
          .slice(smallestIndex)
          .concat(cycleEdges.slice(0, smallestIndex));
  ...
  return {
    ...
    pathTo(node: Collection, thisStart?: Collection): Collection {
      ...
      const start =
        thisStart == null
          ? (index.get(rootSlot) as number)
          : denseOf(thisStart, 'thisStart');
      const refs: Ref[] = [];
      let u = end;
