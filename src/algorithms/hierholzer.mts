...
export const hierholzer = (
  ...
  if (dir) {
    ...
    for (let i = 0; i < nodeSlots.length; i++) {
      ...
      for (const e of store.adj.outEdges(slot)) {
        if (view.edgeIn.has(e) && index.has(endpoints[e * 2 + 1])) {
          outEdges.push(e);
        }
      }
    ...
    for (let i = 0; i < nodeSlots.length; i++) {
      ...
      nodesAdj.set(
        slot,
        incidentEdgesInView(view, slot).filter(
          (e) => index.has(endpoints[e * 2]) && index.has(endpoints[e * 2 + 1]),
        ),
      );
    }
  }
  ...
  if (oddOut != null && oddIn != null) {
    if (dir) {
      if (startVertex != null && oddOut !== startVertex) {
        return result;
      }
  ...
  const walk = (v: number): number[] => {
    ...
    for (;;) {
      ...
      if (current !== head) {
        nodesAdj.set(
          head,
          (nodesAdj.get(head) as number[]).filter((e) => e !== adj),
        );
        current = head;
      } else if (!dir && current !== tail) {
        nodesAdj.set(
          tail,
          (nodesAdj.get(tail) as number[]).filter((e) => e !== adj),
        );
        current = tail;
      }
