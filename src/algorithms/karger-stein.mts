...
const collapse = ( edgeIndex: number, nodeMap: Int32Array, remainingEdges: EdgeIndex[] ): EdgeIndex[] => {
  ...
  for( let i = newEdges.length - 1; i >= 0; i-- ){
    ...
    if( ( src === partition1 && tgt === partition2 ) || ( src === partition2 && tgt === partition1 ) ){
      newEdges.splice( i, 1 );
    }
  }
  ...
  for( let i = 0; i < nodeMap.length; i++ ){
    if( nodeMap[ i ] === partition2 ){ nodeMap[ i ] = partition1; }
  }
...
const contractUntil = (
  metaNodeMap: Int32Array, remainingEdges: EdgeIndex[], size: number, sizeLimit: number
): EdgeIndex[] => {
...
export const kargerStein = ( coll: GpuCollection ): KargerSteinResult => {
  ...
  const cutEdgeSlots = new Set<number>( minCutEdgeIndexes.map( e => edgeSlots[ e[ 0 ] ] ) );
  ...
  const constructComponent = ( wantPartition1: boolean ): GpuCollection => {
    ...
    for( let i = 0; i < numNodes; i++ ){
      if( ( inPartition1[ i ] === 1 ) !== wantPartition1 ){ continue; }
