;(function($, $$){
	...
	$$.elesfn.animated = function(){
		var ele = this[0];

		return ele && ele._private.animation.current.length > 0
	};

	$$.elesfn.clearQueue = function(){
	...
	$$.elesfn.delay = function( time, complete ){
		return this.animate({
			delay: time
		}, {
			duration: time,
	...
	$$.elesfn.animate = function( properties, params ){
	...
	$$.elesfn.stop = function(clearQueue, jumpToEnd){
		this.each(function(){
			var self = this;
