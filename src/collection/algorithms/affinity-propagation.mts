...
let defaults = util.defaults({
  distance: 'euclidean' as DistanceMetric, // distance metric to compare attributes between two nodes
  preference: 'median' as AffinityPreference, // suitability of a data point to serve as an exemplar
  ...
  ] as AffinityAttributeFn[]
});

let setOptions = function( options?: AffinityPropagationOptions ) {
  let dmp = ( options as AffinityPropagationOptions | undefined )?.damping;
  let pref = ( options as AffinityPropagationOptions | undefined )?.preference;

  if( !(dmp != null && 0.5 <= dmp && dmp < 1) ){
...
let getPreference = function( S: number[], preference: AffinityPreference ): number { // larger preference = greater # of clusters
  let p: number;
...
let findExemplars = function( n: number, R: number[], A: number[] ): number[] {
  let indices: number[] = [];
  for ( let i = 0; i < n; i++ ) {
...
let assignClusters = function( n: number, S: number[], exemplars: number[] ): number[] {
  let clusters: number[] = [];
...
let assign = function( n: number, S: number[], exemplars: number[] ): number[] {
...
let affinityPropagation = function( this: SharedCollection, options?: AffinityPropagationOptions ): Collection[] {
  ...
  let id2position: Record<string, number> = {};
  ...
  let S: number[];  // similarity matrix (1D array)
  let p;  // preference/suitability of a data point to serve as an exemplar
  let R: number[];  // responsibility matrix (1D array)
  let A: number[];  // availability matrix (1D array)
  ...
  let old: number[] = new Array(n);
  let Rp: number[]  = new Array(n);
  let se: number[]  = new Array(n);
  ...
  let e: number[] = new Array(n * opts.minIterations);
  ...
  let clusterIndices = assign( n, S, exemplarsIndices );

  let clusters: Record<number, Element[]> = {};
  ...
  let retClusters: Collection[] = new Array(exemplarsIndices.length);
  for ( let c = 0; c < exemplarsIndices.length; c++ ) {
...
export default { affinityPropagation, ap: affinityPropagation } as AlgorithmsAffinityPropagation;
