...
const collapse = function( edgeIndex: number, nodeMap: number[], remainingEdges: EdgeIndex[] ): EdgeIndex[] {
  ...
  for( let i = 0; i < newEdges.length; i++ ){
    ...
    if( edge[1] === partition2 ){ // Check source
      newEdges[i] = edge.slice() as EdgeIndex; // copy
...
const contractUntil = function( metaNodeMap: number[], remainingEdges: EdgeIndex[], size: number, sizeLimit: number ): EdgeIndex[] {
...
const elesfn = ({
  ...
  kargerStein: function( this: Collection ): KargerSteinResult | undefined {
    ...
    let minCutNodeMap: number[] = new Array(numNodes);

    // Initial meta node partition
    let metaNodeMap: number[] = new Array(numNodes);
    let metaNodeMap2: number[] = new Array(numNodes);

    let copyNodesMap = ( from: number[], to: number[] ) => {
    ...
    // Construct result
    let cut = this.spawn( minCutEdgeIndexes.map(( e: EdgeIndex ) => edges[e[0]]) );
    ...
    const constructComponent = ( subset: Collection ): Collection => {
      const component = this.spawn();

      subset.forEach(( node: Element ) => {
        component.merge(node);

        node.connectedEdges().forEach(( edge: Element ) => {
...
}) satisfies AlgorithmsKargerStein; // elesfn


export default elesfn as AlgorithmsKargerStein;
