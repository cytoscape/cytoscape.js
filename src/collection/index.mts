...
let Collection = function( this: CollectionType, cy: CoreAccess, elements?: ElementsInput, unique = false, removed = false ){
  ...
  let map = new Map<string, ElesEntry>();
  ...
  if( !elements ){
    ...
    let eles: ElementType[] = [];
    let elesIds = new Set<string>();

    for( let i = 0, l = elements.length; i < l; i++ ){
      let json = elements[ i ] as ElementDefinition;
  ...
  for( let i = 0, l = elements.length; i < l; i++ ){
    let element = ( elements[i] as ElementType )[0]; // [0] in case elements is an array of collections, rather than array of elements
  ...
  this._private = {
    ...
    set map(m: CollectionPrivate['map']){
      this.lazyMap = m;
    },
    rebuildMap(){
      const m = this.lazyMap = new Map<string, ElesEntry>();
...
let elesfn: CollectionType = Element.prototype = Collection.prototype = Object.create(Array.prototype);

elesfn.instanceString = function(){
  return 'collection';
};

elesfn.spawn = function( eles?: ElementsInput, unique?: boolean ){
...
elesfn.collection = function(){
  if( is.collection( this ) ){
    return this as unknown as CollectionType;
  } else { // an element
...
elesfn.hasElementWithId = function( id: string | number ){
  id = '' + id; // id must be string

  return this._private.map.has( id );
};

elesfn.getElementById = function( id: string | number ){
  id = '' + id; // id must be string
...
elesfn.indexOf = function( ele: CollectionType | ElementType ){
  let id = ele[0]._private.data.id!;

  return this._private.map.get( id )!.index;
};

elesfn.indexOfId = function( id: string | number ){
  id = '' + id; // id must be string

  return this._private.map.get( id )!.index;
};

elesfn.json = function( obj?: Partial<ElementJson> ){
  ...
  if( is.plainObject( obj ) ){ // set
    ...
    if( obj.data ){
      ...
      if( ele.isEdge() ){ // source and target are immutable via data()
        let move = false;
        let spec: { source?: string; target?: string } = {};
        ...
        let parent: string | null | undefined = obj.data.parent;
    ...
    let checkSwitch = function( k: keyof ElementJson, trueFnName: string, falseFnName: string ){
      let obj_k = obj![ k ];
      let eleAny = ele as unknown as Record<string, () => void>; // dynamic switch-method dispatch

      if( obj_k != null && obj_k !== ( p as unknown as Record<string, unknown> )[ k ] ){
        if( obj_k ){
          eleAny[ trueFnName ]();
        } else {
          eleAny[ falseFnName ]();
    ...
    let i = 0;
    p.classes.forEach( cls => json.classes = json.classes! + ( i++ === 0 ? cls : ' ' + cls ) );
...
elesfn.jsons = function(){
  let jsons: ( ElementJson | undefined )[] = [];

  for( let i = 0; i < this.length; i++ ){
    let ele = this[ i ];
    let json = ele.json();

    jsons.push( json as ElementJson | undefined ); // no-arg json() never returns the collection
  }
...
elesfn.clone = function(){
  let cy = this.cy();
  let elesArr: ElementType[] = [];

  for( let i = 0; i < this.length; i++ ){
    let ele = this[ i ];
    let json = ele.json();
    let clone = new Element( cy, json as unknown as ElementDefinition, false ); // NB no restore
...
elesfn.restore = function( notifyRenderer = true, addToPool = true ){
  ...
  let nodes: ElementType[] = [];
  let edges: ElementType[] = [];
  ...
  for( i = 0; i < elements.length; i++ ){
    ...
    if( ele.isEdge() ){ // extra checks for edges
      ...
      let edge = ele;
      let fields: ( keyof ElementData )[] = [ 'source', 'target' ];
      ...
      for( let j = 0; j < fieldsLength; j++ ){
        ...
        } else if( !cy.hasElementWithId( val as string ) ){
      ...
      let src = cy.getElementById( data.source! ) as ElementType; // existence verified above
      let tgt = cy.getElementById( data.target! ) as ElementType;
    ...
    _private.map = new Map<string, ElesEntry>();
  ...
  if( elements.length > 0 ){
    ...
    let toUpdateStyle: CollectionType;
...
elesfn.remove = function( notifyRenderer = true, removeFromPool = true ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias
  let elesToRemove: ElementType[] = [];
  let elesToRemoveIds: Record<string, boolean> = {};
  let cy = self._private.cy;

  // add connected edges
  function addConnectedEdges( node: ElementType ){
  ...
  function addChildren( node: ElementType ){
  ...
  function removeEdgeRef( node: ElementType, edge: ElementType ){
  ...
  function removeParallelRef( pllEdge: ElementType ){
  ...
  function removeChildRef( parent: SharedCollection, ele: SharedCollection ){
    ...
    let children = parentEle._private.children;
    let pid = parentEle.id()!;

    util.removeFromArray( children, eleEle ); // remove parent => child ref

    eleEle._private.parent = null; // remove child => parent ref

    if( !alteredParents.ids[ pid ] ){
      alteredParents.ids[ pid ] = true;
      alteredParents.push( parentEle );
    }
  }
...
elesfn.move = function( struct ){
  ...
  let toString = ( id?: string | number | null ) => id == null ? id : '' + id; // id must be string
