...
*/
export const BEZIER_EASINGS: Record<
  string,
...
const tForX = (
  ...
  for (; i !== SPLINE_SIZE - 1 && table[i] <= x; i++) {
    start += SPLINE_STEP;
  }
  ...
  if (slope >= NEWTON_MIN_SLOPE) {
    return newtonRefine(x, guess, x1, x2);
  }
  if (slope === 0) {
    return guess;
  }
...
export const bezierEasing = (
  ...
  for (let i = 0; i < SPLINE_SIZE; i++) {
    table[i] = calcBezier(i * SPLINE_STEP, x1, x2);
  }
...
export const pointsEasing =
  (points: Float32Array): Easing =>
  (t) =>
    samplePoints(points, t);
...
export const parseLinearPoints = (args: string, src: string): Float32Array => {
  ...
  for (const entry of args.split(',')) {
    ...
    if (parts.length === 0 || parts.length > 3) {
      throw new Error(
        `'${entry.trim()}' in '${src}' needs a value and at most two stops`,
      );
    }
  ...
  for (let i = 1; i < n; i++) {
    ...
    while (stops[j] == null) {
      j++;
    } // stops[n-1] is set, so this terminates
...
const springAt = (zeta: number, w0: number, t: number): number => {
  ...
  return (
    1 +
    (-r2 / (r2 - r1)) * Math.exp(r1 * t) +
    (r1 / (r2 - r1)) * Math.exp(r2 * t)
  );
};
...
const settleBound = (zeta: number, w0: number): number => {
  if (zeta < 1) {
    ...
    return (
      Math.log(Math.hypot(1, (zeta * w0) / wd) / SPRING_EPSILON) / (zeta * w0)
    );
  }
  ...
  if (zeta === 1) {
    ...
    for (let i = 0; i < 32; i++) {
      t = Math.log((1 + w0 * t) / SPRING_EPSILON) / w0;
    }
 ...
 */
export const springPoints = (
  bounce: number,
): { points: Float32Array; durationScale: number } => {
  ...
  */
  const cycles =
    zeta < 1 ? (w0 * Math.sqrt(1 - zeta * zeta) * settle) / (2 * Math.PI) : 0;
  const n = clamp(
...
const bezierProgram = (
  ...
  kind: EASING_KIND.bezier,
  bezier: p,
  points: null,
  durationScale: 1,
  fn: bezierEasing(p[0], p[1], p[2], p[3]),
});
...
const pointsProgram = (
  ...
  kind: EASING_KIND.points,
  bezier: null,
  points,
  durationScale,
  fn: pointsEasing(points),
});
...
const NAMED: Record<string, EasingProgram> = Object.fromEntries([
  ...
  ...Object.entries(BEZIER_EASINGS).map(([name, p]) => [
    name,
    bezierProgram(p),
  ]),
]);

const numbers = (args: string, src: string): number[] =>
  args.split(',').map((a) => {
...
export const compileEasing = (easing: string | undefined): EasingProgram => {
  if (easing != null && typeof easing !== 'string') {
    throw new Error(
      'A custom easing function is unsupported: the GPU kernel cannot run a ' +
  ...
  if (call == null) {
    throw new Error(
      `'${src}' is not a known easing; expected one of ${EASING_NAMES.join(
        ', ',
      )}, cubic-bezier(), linear() or spring()`,
    );
  }
  ...
  if (name === 'spring') {
    ...
    if (p.length !== 1) {
      throw new Error(
        `'${src}' takes one bounce factor in [-1, 1] (v3's spring(tension, friction) is gone)`,
      );
    }
...
export const resolveEasing = (easing: string | undefined): Easing =>
  compileEasing(easing).fn;
