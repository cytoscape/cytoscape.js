...
// pairInfo is an internal, incrementally-populated accumulator shared across
// the edge control-point routines; rscratch is the per-edge geometry bag.
// Both legitimately carry dozens of loosely-typed numeric/coord fields, so we
// model them as open bags. Per-routine locals are still typed honestly.
/* eslint-disable @typescript-eslint/no-explicit-any -- pairInfo/rscratch geometry bags */
type PairInfo = any;
type RScratch = any;
/* eslint-enable @typescript-eslint/no-explicit-any */

// pstyle() returns ParsedStyleProperty | null | undefined, with value/pfValue
// being dynamically-shaped (scalar or per-value array depending on the prop).
// These props are always present at runtime for the relevant ele; we read into
// their loosely-typed pfValue/value bags, so cast the result to a loose shape.
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- dynamic parsed style shape
type LoosePstyle = ParsedStyleProperty & { value: any; pfValue: any };
const pstyle = ( ele: Element, prop: string ): LoosePstyle =>
  ele.pstyle( prop ) as unknown as LoosePstyle;

let BRp: Record<string, unknown> = {};

BRp.findMidptPtsEtc = function( this: Renderer, edge: Element, pairInfo: PairInfo ) {
  ...
  let srcManEndpt = pstyle( edge, 'source-endpoint' );
  let tgtManEndpt = pstyle( edge, 'target-endpoint' );
  const haveManualEndPts = srcManEndpt.units != null && tgtManEndpt.units != null;

  const recalcVectorNormInverse = ( x1: number, y1: number, x2: number, y2: number ) => {
    let dy = ( y2 - y1 );
    let dx = ( x2 - x1 );
  ...
  const edgeDistances = pstyle( edge, 'edge-distances' ).value;
...
BRp.findHaystackPoints = function( this: Renderer, edges: Collection ){
  for( let i = 0; i < edges.length; i++ ){
    let edge = edges[i];
    let _p = edge._private;
    let rs = _p.rscratch as RScratch;
    ...
    let srcW = src.width() as number;
    let tgtW = tgt.width() as number;
    let srcH = src.height() as number;
    let tgtH = tgt.height() as number;
    let radius = pstyle( edge, 'haystack-radius' ).value as number;
...
BRp.findSegmentsPoints = function( this: Renderer, edge: Element, pairInfo: PairInfo ){
  // Segments (multiple straight lines)

  const rs = edge._private.rscratch as RScratch;
  const segmentWs = pstyle( edge, 'segment-weights' );
  const segmentDs = pstyle( edge, 'segment-distances' );
  const segmentRs = pstyle( edge, 'segment-radii' );
  const segmentTs = pstyle( edge, 'radius-type' );
...
BRp.findLoopPoints = function( edge: Element, pairInfo: PairInfo, _i: number, edgeIsUnbundled: boolean ){
  ...
  const ctrlptDists = pstyle( edge, 'control-point-distances' );
  const ctrlptDist = ctrlptDists ? ctrlptDists.pfValue[0] : undefined;
  const loopDir = pstyle( edge, 'loop-direction' ).pfValue;
  const loopSwp = pstyle( edge, 'loop-sweep' ).pfValue;
  const stepSize = pstyle( edge, 'control-point-step-size' ).pfValue;
...
BRp.findCompoundLoopPoints = function( edge: Element, pairInfo: PairInfo, i: number, edgeIsUnbundled: boolean ){
  // Compound edge
...
BRp.findStraightEdgePoints = function( edge: Element ){
  // Straight edge within bundle

  ( edge._private.rscratch as RScratch ).edgeType = 'straight';
};

BRp.findBezierPoints = function( this: Renderer, edge: Element, pairInfo: PairInfo, i: number, edgeIsUnbundled: boolean, edgeIsSwapped: boolean ){
  const rs = edge._private.rscratch as RScratch;
  const stepSize = pstyle( edge, 'control-point-step-size' ).pfValue;
  const ctrlptDists = pstyle( edge, 'control-point-distances' );
  const ctrlptWs = pstyle( edge, 'control-point-weights' );
...
BRp.findTaxiPoints = function( edge: Element, pairInfo: PairInfo ){
  ...
  let taxiDir = pstyle( edge, 'taxi-direction' ).value;
  let rawTaxiDir = taxiDir; // unprocessed value
  const taxiTurn = pstyle( edge, 'taxi-turn' );
  ...
  let minD = pstyle( edge, 'taxi-turn-min-distance' ).pfValue;
  ...
  const subDWH = ( dxy: number, dwh: number ) => {
    if( dxy > 0 ){
  ...
  const getIsTooClose = ( d: number ) => Math.abs(d) < minD || Math.abs(d) >= Math.abs(l);
  ...
  if (rs.isRound) {
    const radius = pstyle( edge, 'taxi-radius' ).value;
    const isArcRadius = pstyle( edge, 'radius-type' ).value[0] === 'arc-radius';
    rs.radii = new Array( rs.segpts.length / 2 ).fill( radius );
    rs.isArcRadius = new Array( rs.segpts.length / 2 ).fill( isArcRadius );
  }
};

BRp.tryToCorrectInvalidPoints = function( this: Renderer, edge: Element, pairInfo: PairInfo ){
  const rs = edge._private.rscratch as RScratch;
  ...
  if( rs.edgeType === 'bezier' ){
    ...
    let arrowW = this.getArrowWidth( pstyle( edge, 'width' ).pfValue, pstyle( edge, 'arrow-scale' ).value )
...
BRp.storeAllpts = function( edge: Element ){
  let rs = edge._private.rscratch as RScratch;
  ...
  if( rs.edgeType === 'multibezier' || rs.edgeType === 'bezier' || rs.edgeType === 'self' || rs.edgeType === 'compound' ){
    ...
    rs.allpts = [];
    rs.allpts.push( rs.startX, rs.startY );
    rs.allpts.push.apply( rs.allpts, rs.segpts ); // eslint-disable-line prefer-spread
    rs.allpts.push( rs.endX, rs.endY );
...
BRp.checkForInvalidEdgeWarning = function( edge: Element ){
  let rs = edge[0]._private.rscratch as RScratch;
...
BRp.findEdgeControlPoints = function( this: Renderer, edges: Collection ){
  ...
  let hashTable = new Map<string, PairInfo>();
  let getKey = ( pairId: number[], edgeIsUnbundled: boolean ) => [
    ...pairId,
    edgeIsUnbundled ? 1 : 0
  ].join('-');

  let pairIds: { pairId: number[]; edgeIsUnbundled: boolean }[] = [];
  let haystackEdges: Element[] = [];

  // create a table of edge (src, tgt) => list of edges between them
  for( let i = 0; i < edges.length; i++ ){
    let edge = edges[i];
    let _p = edge._private;
    let curveStyle = pstyle( edge, 'curve-style' ).value;
  ...
  for( let p = 0; p < pairIds.length; p++ ){
    ...
    if( !pairInfo.hasUnbundled ){
      let pllEdges = pairInfo.eles[0].parallelEdges().filter(( e: Element ) => e.isBundledBezier());

      util.clearArray( pairInfo.eles );

      pllEdges.forEach( ( edge: Element ) => pairInfo.eles.push(edge) );

      // for each pair id, the edges should be sorted by index
      pairInfo.eles.sort( ( edge1: Element, edge2: Element ) => edge1.poolIndex() - edge2.poolIndex() );
    }
    ...
    let srcCornerRadius = pairInfo.srcCornerRadius = pstyle( src as Element, 'corner-radius' ).value === 'auto' ? 'auto' : pstyle( src as Element, 'corner-radius' ).pfValue;
    let tgtCornerRadius = pairInfo.tgtCornerRadius = pstyle( tgt as Element, 'corner-radius' ).value === 'auto' ? 'auto' : pstyle( tgt as Element, 'corner-radius' ).pfValue;
...
function getPts( pts: number[] | null | undefined ){
  let retPts: Position[] = [];
...
BRp.getSegmentPoints = function( this: Renderer, edge: Element ){
  let rs = edge[0]._private.rscratch as RScratch;
...
BRp.getControlPoints = function( this: Renderer, edge: Element ){
  let rs = edge[0]._private.rscratch as RScratch;
...
BRp.getEdgeMidpoint = function( this: Renderer, edge: Element ){
  let rs = edge[0]._private.rscratch as RScratch;
