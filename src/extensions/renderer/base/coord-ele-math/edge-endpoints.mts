...
// a resolved style property (e.g. an endpoint), with the fields used here.
// value/pfValue are polymorphic across props (number | string | number[] | ...)
/* eslint-disable @typescript-eslint/no-explicit-any -- polymorphic style values */
interface StyleProp {
  value: any;
  pfValue: any;
  units?: string[];
...
// loosely typed (pstyle() may be null with an `unknown` value; outerWidth()
// etc. may be undefined). In renderer code these always resolve, so we view
// elements through a narrowed local contract. Internal only.
type REle = Omit<Element, 'pstyle' | 'outerWidth' | 'outerHeight' | 'position'> & {
  pstyle( prop: string ): StyleProp;
  outerWidth(): number;
  outerHeight(): number;
  position(): Position;
};
...
BRp.manualEndptToPx = function( this: Renderer, node: REle, prop: StyleProp ){
  ...
  if( prop.value.length === 2 ){
    ...
    if( prop.units![0] === '%' ){ // units present for a 2-value endpoint
      p[0] = p[0] * w;
    }
...
BRp.findEndpoints = function( this: Renderer, edge: REle ){
  let r = this; // eslint-disable-line @typescript-eslint/no-this-alias
  let intersect;

  let source = edge.source()[0] as REle;
  let target = edge.target()[0] as REle;
  ...
  let p1: number[] | undefined; // last known point of edge on target side
  let p2: number[] | undefined; // last known point of edge on source side

  let p1_i: number[] | undefined; // point to intersect with target shape
  let p2_i: number[] | undefined; // point to intersect with source shape
  ...
  if( bezier ){
    let ctrlpts = rs.ctrlpts as number[];
    let cpStart = [ ctrlpts[0], ctrlpts[1] ];
    let cpEnd = multi ? [ ctrlpts[ ctrlpts.length - 2], ctrlpts[ ctrlpts.length - 1] ] : cpStart;

    p1 = cpEnd;
    p2 = cpStart;
  } else if( lines ){
    let segpts = rs.segpts as number[];
  ...
  if( tgtManEndptVal === 'inside-to-node' ){
    ...
    if( tgtManEndptVal === 'outside-to-node-or-label' || tgtManEndptVal === 'outside-to-line-or-label' ){
      ...
      let lw = trs.labelWidth as number;
      let lh = trs.labelHeight as number;
      let lx = trs.labelX as number;
      let ly = trs.labelY as number;
      let lw2 = lw/2;
      let lh2 = lh/2;
  ...
  if( srcManEndptVal === 'inside-to-node' ){
    ...
    if( srcManEndptVal === 'outside-to-node-or-label' || srcManEndptVal === 'outside-to-line-or-label' ){
      ...
      let lw = srs.labelWidth as number;
      let lh = srs.labelHeight as number;
      let lx = srs.labelX as number;
      let ly = srs.labelY as number;
      let lw2 = lw/2;
      let lh2 = lh/2;
...
BRp.getSourceEndpoint = function( this: Renderer, edge: Element ){
  ...
  switch( rs.edgeType ){
    case 'haystack':
      return {
        x: ( rs.haystackPts as number[] )[0],
        y: ( rs.haystackPts as number[] )[1]
      };
    default:
      return {
        x: rs.arrowStartX,
...
BRp.getTargetEndpoint = function( this: Renderer, edge: Element ){
  let rs = edge[0]._private.rscratch;
  ...
  switch( rs.edgeType ){
    case 'haystack':
      return {
        x: ( rs.haystackPts as number[] )[2],
        y: ( rs.haystackPts as number[] )[3]
      };
    default:
      return {
        x: rs.arrowEndX,
