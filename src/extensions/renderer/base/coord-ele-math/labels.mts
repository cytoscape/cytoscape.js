...
type REle = Omit<Element, 'pstyle' | 'width' | 'height' | 'outerWidth' | 'outerHeight' | 'position' | 'padding'> & {
...
BRp.recalculateNodeLabelProjection = function( this: Renderer, node: REle ){
...
let lineAngleFromDelta = function( dx: number, dy: number ){
...
let lineAngle = function( p0: Pt, p1: Pt ){
  let dx = p1.x - p0.x;
  let dy = p1.y - p0.y;
...
BRp.recalculateEdgeLabelProjections = function( this: Renderer, edge: REle ){
  let p: Pt = { x: 0, y: 0 };
  ...
  let setRs = function( propName: string, prefix: string | null, value: unknown ){
  ...
  let midAngle = lineAngleFromDelta(rs.midDispX as number, rs.midDispY as number);
  setRs( 'labelAutoAngle', null, midAngle );

  interface Segment { p0: Pt; p1: Pt; t0: number; t1: number; startDist: number; length: number }
  interface ControlPoint { p0: Pt; p1: Pt; p2: Pt; startDist: number; length: number; segments: Segment[] }
  type CtrlPtInfoFn = { (): ControlPoint[]; cache?: ControlPoint[] };

  let createControlPointInfo: CtrlPtInfoFn = function(): ControlPoint[] {
    if( createControlPointInfo.cache ){ return createControlPointInfo.cache; } // use cache so only 1x per edge

    let ctrlpts: ControlPoint[] = [];
    let allpts = rs.allpts as number[];

    // store each ctrlpt info init
    for( let i = 0; i + 5 < allpts.length; i += 4 ){
      let p0 = { x: allpts[i], y: allpts[i+1] };
      let p1 = { x: allpts[i+2], y: allpts[i+3] }; // ctrlpt
      let p2 = { x: allpts[i+4], y: allpts[i+5] };
    ...
    let bpts = _p.rstyle.bezierPts as Pt[];
    let nProjs = r.bezierProjPcts.length;

    function addSegment( cp: ControlPoint, p0: Pt, p1: Pt, t0: number, t1: number ){
  ...
  let calculateEndProjection = function( prefix: string ){
    let angle;
    let isSrc = prefix === 'source';

    if( !content[ prefix as keyof typeof content ] ){ return; }
    ...
    switch( rs.edgeType ){
      ...
      case 'multibezier': {
        ...
        let selected: { cp: ControlPoint; segment: Segment } | undefined;
        let startDist = 0;
        let totalDist = 0;
        ...
        let cp = selected!.cp; // selected is always assigned by the loop above (last segment is a guaranteed fallback)
      ...
      case 'haystack': {
        let d = 0, di = 0, d0 = 0;
        let p0: Pt = { x: 0, y: 0 }, p1: Pt = { x: 0, y: 0 };
        let allpts = rs.allpts as number[];
        let l = allpts.length;

        for( let i = 0; i + 3 < l; i += 2 ){
          if( isSrc ){
            p0 = { x: allpts[i],     y: allpts[i+1] };
            p1 = { x: allpts[i+2],   y: allpts[i+3] };
          } else {
            p0 = { x: allpts[l-2-i], y: allpts[l-1-i] };
            p1 = { x: allpts[l-4-i], y: allpts[l-3-i] };
          }
...
BRp.applyLabelDimensions = function( this: Renderer, ele: REle ){
...
BRp.applyPrefixedLabelDimensions = function( this: Renderer, ele: REle, prefix?: string ){
  let _p = ele._private;

  let text = this.getLabelText( ele, prefix );

  let cacheKey = util.hashString( text, ele._private.labelDimsKey as number | undefined );
  ...
  let lines = ( util.getPrefixedProperty( _p.rscratch, 'labelWrapCachedLines', prefix ) || [] ) as string[];
...
BRp.getLabelText = function( this: Renderer, ele: REle, prefix?: string ){
  let _p = ele._private;
  let pfd = prefix ? prefix + '-' : '';
  let text = ele.pstyle( pfd + 'label' ).strValue;
  let textTransform = ele.pstyle( 'text-transform' ).value;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- prefixed rscratch values are polymorphic (string lines, keys, ...)
  let rscratch = function( propName: string, value?: unknown ): any {
    if( value ){
      util.setPrefixedProperty( _p.rscratch, propName, prefix, value );
      return value;
    } else {
...
BRp.getLabelJustification = function( this: Renderer, ele: REle ){
...
BRp.calculateLabelDimensions = function( this: Renderer, ele: REle, text: string ){
  let r = this; // eslint-disable-line @typescript-eslint/no-this-alias
...
BRp.calculateLabelAngle = function( this: Renderer, ele: REle, prefix?: string ){
  let _p = ele._private;
  let rs = _p.rscratch;
...
BRp.calculateLabelAngles = function( this: Renderer, ele: REle ){
  let r = this; // eslint-disable-line @typescript-eslint/no-this-alias
  let isEdge = ele.isEdge();
  let _p = ele._private;
  let rs = _p.rscratch;
