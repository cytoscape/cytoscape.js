...
export class ElementDrawingWebGL {
  ...
  maybeSetPlaceholderFlagForRestOfFrame(atlasInfo) {
    const { texFrameInfo } = this;
    const { drawn } = atlasInfo;
  ...
  drawTexture(ele, eleIndex, type) {
    ...
    const atlasInfo = atlasManager.getOrCreateAtlas(ele, type); // draws to the atlas if needed
    const { atlasID, tex1, tex2 } = atlasInfo;

    this.maybeSetPlaceholderFlagForRestOfFrame(atlasInfo);
    let first = true;
  ...
  _drawTexturePlacholder(ele, eleIndex, type) {
    ...
    const matrixView = this.transformBuffer.getMatrixView(instance);
    atlasManager.setPlaceholderTransformMatrix(matrixView, ele, type);
