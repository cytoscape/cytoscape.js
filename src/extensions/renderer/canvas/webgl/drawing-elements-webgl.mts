...
/* eslint-disable @typescript-eslint/no-explicit-any --
   This file manages dynamic WebGL vertex packing: buffer wrappers, augmented
   shader programs, atlas managers and per-vertex style/geometry reads are all
   open-ended runtime structures coming from untyped .mjs helpers. */
...
export class ElementDrawingWebGL {
  ...
  atlasManager: any;
  batchManager: any;
  simpleShapeOptions: Map<string, any>;
  program: any;
  pickingProgram: any;
  ...
  // dynamic-draw buffer wrappers created by webgl-util (untyped)
  transformBuffer: any;
  indexBuffer: any;
  vertTypeBuffer: any;
  atlasIdBuffer: any;
  texBuffer: any;
  pointAPointBBuffer: any;
  pointCPointDBuffer: any;
  lineWidthBuffer: any;
  colorBuffer: any;
  cornerRadiusBuffer: any;
  borderColorBuffer: any;
  _buffers: any[] | undefined;

  /**
   * @param {WebGLRenderingContext} gl
   */
  constructor( r: Renderer, gl: WebGL2RenderingContext, opts: any ) {
   ...
   */
  addAtlasCollection( collectionName: string, opts: any ) {
   ...
   */
  addTextureAtlasRenderType( typeName: string, opts: any ) {
  ...
  */
   ...
   */
  addSimpleShapeRenderType( typeName: string, opts: any ) {
   ...
   */
  invalidate( eles: any, { type }: { type?: string } = {} ) {
    const { atlasManager } = this;
    if(type) {
      return atlasManager.invalidate(eles, {
        filterType: ( t: string ) => t === type,
        forceRedraw: true
      });
    } else {
      return atlasManager.invalidate(eles);
  ...
  _createShaderProgram( renderTarget: any ) {
    const { gl } = this;
    ...
    const idxs: number[] = this.batchManager.getIndexArray();
    ...
    // the program is augmented with attribute/uniform locations below
    const program: any = util.createProgram(gl, vertexShaderSource, fragmentShaderSource);
  ...
  get buffers() {
    if(!this._buffers) {
      this._buffers = Object.keys(this).filter(k => endsWith(k, 'Buffer')).map(k => (this as any)[k]);
    }
    return this._buffers;
  }


  startFrame( panZoomMatrix: any, renderTarget: any = RENDER_TARGET.SCREEN ) {
  ...
  _isVisible( ele: Element, opts: any ) {
    if(ele.visible()) {
   ...
   */
  drawTexture( ele: Element, eleIndex: number, type: string ) {
    ...
    for(const atlasInfo of atlasInfoArray) {
      ...
      for(const [tex, first] of [[tex1, true], [tex2, false]] as [any, boolean][]) {
        if(tex.w != 0) {
   ...
   */
  setTransformMatrix( ele: Element, matrix: any, opts: any, atlasInfo?: any, first: boolean = true ) {
    let padding = 0;
    if(opts.shapeProps && opts.shapeProps.padding) {
      padding = (ele.pstyle(opts.shapeProps.padding) as any).pfValue;
    }
  ...
  _applyTransformMatrix( matrix: any, adjBB: any, opts: any, ele: Element ) {
   ...
   */
  _getAdjustedBB( bb: any, padding: number, first: boolean, ratio: number ) {
   ...
   */
  drawPickingRectangle( ele: Element, eleIndex: number, type: string ) {
   ...
   */
  drawNode( node: Element, eleIndex: number, type: string ) {
    ...
    let opacity: number = this.renderTarget.picking ? 1 : (type === 'node-body' ? (node.effectiveOpacity() as number) : 1);
    let bgOpacity = this.renderTarget.picking ? 1 : ((node.pstyle(props.opacity) as any).value * opacity);

    const color = (node.pstyle(props.color) as any).value;
    const colorView = this.colorBuffer.getView(instance);
    ...
    if(props.border) {
      const borderWidth = (node.pstyle('border-width') as any).value;
      if(borderWidth > 0) {
        const borderColor = (node.pstyle('border-color') as any).value;
        const borderOpacity = opacity * (node.pstyle('border-opacity') as any).value;
        ...
        const borderPos = (node.pstyle('border-position') as any).value;
        if(borderPos === 'inside') {
          lineWidthView[0] = 0;
  ...
  _getVertTypeForShape( node: Element, shapeProp: string ) {
    const shape = (node.pstyle(shapeProp) as any).value
    switch(shape) {
      case 'rectangle':
        return RECTANGLE;
      case 'ellipse':
  ...
  _getCornerRadius( node: Element, radiusProp: string, { w, h }: { w: number; h: number } ) { // see CRp.drawRoundRectanglePath
    if((node.pstyle(radiusProp) as any).value === 'auto') {
      return math.getRoundRectangleRadius(w, h);
    } else {
      const radius = (node.pstyle(radiusProp) as any).pfValue;
      const halfWidth  = w / 2;
  ...
  drawEdgeArrow( edge: Element, eleIndex: number, prefix: string ) {
    ...
    const arrowShape = (edge.pstyle(prefix + '-arrow-shape') as any).value;
    if(arrowShape === 'none') {
      return;
    }

    const color = (edge.pstyle(prefix + '-arrow-color') as any).value;
    const baseOpacity = (edge.pstyle('opacity') as any).value;
    const lineOpacity = (edge.pstyle('line-opacity') as any).value;
    const opacity = baseOpacity * lineOpacity;
    const lineWidth = (edge.pstyle('width') as any).pfValue;
    const scale = (edge.pstyle('arrow-scale') as any).value;
   ...
   */
  drawEdgeLine( edge: Element, eleIndex: number ) {
    if(!edge.visible()) {
      return;
    }
    ...
    const baseOpacity = (edge.pstyle('opacity') as any).value;
    const lineOpacity = (edge.pstyle('line-opacity') as any).value;
    const width = (edge.pstyle('width') as any).pfValue;
    const color = (edge.pstyle('line-color') as any).value;
    const opacity = baseOpacity * lineOpacity;
  ...
  _getEdgePoints( edge: Element ): number[] | undefined {
    const rs = edge._private.rscratch as any;
  ...
  _getCurveSegmentPoints( controlPoints: number[], segments: number ): number[] {
    if(controlPoints.length == 4) {
  ...
  _setCurvePoint( points: number[], t: number, curvePoints: number[], cpi: number ): void {
    if(points.length <= 2) {
  ...
  getDebugInfo() {
    ...
    const totalAtlases = atlasInfo.reduce(( count: number, info: any ) => count + info.atlasCount, 0);

    const batchInfo = this.batchDebugInfo;
    const totalInstances = batchInfo.reduce(( count: number, info: any ) => count + info.count, 0);
