...
// GLSL type names used by the buffer helpers below
type GlslType = 'float' | 'vec2' | 'vec3' | 'vec4' | 'int' | 'ivec2';

// The buffer/texture helpers below attach extra methods/fields onto the raw
// WebGL objects, so we describe those augmented shapes with local interfaces.
export interface AtlasTexture extends WebGLTexture {
  buffer(offscreenCanvas: HTMLCanvasElement): void;
  deleteTexture(): void;
}

export interface DynamicBuffer extends WebGLBuffer {
  dataArray: Float32Array | Int32Array;
  stride: number;
  size: number;
  getView(i: number): Float32Array | Int32Array;
  setPoint(i: number, x: number, y: number): void;
  bufferSubData(count?: number): void;
}

export interface MatrixBuffer extends WebGLBuffer {
  getMatrixView(i: number): Float32Array;
  setData(matrix: ArrayLike<number>, i: number): void;
  bufferSubData(): void;
}

export interface PickingFrameBuffer extends WebGLFramebuffer {
...
export function createProgram(gl: WebGL2RenderingContext, vertexSource: string, fragementSource: string): WebGLProgram {
 ...
 */
export function createTextureCanvas(r: Renderer, width: number, height?: number) {
  if(height === undefined) {
    height = width;
  }
 ...
 */
export function getEffectivePanZoom(r: Renderer) {
  const { pixelRatio } = r;
 ...
 */
export function getEffectiveZoom(r: Renderer) {
  const { pixelRatio } = r;
  const zoom = r.cy.zoom();
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- node is a cytoscape element, renderTarget is a renderer state bag
export function isSimpleShape(node: any, renderTarget: any) {
...
export function arrayEqual(a1: ArrayLike<unknown>, a2: ArrayLike<unknown>): boolean {
 ...
 */
export function toWebGLColor(color: ArrayLike<number>, opacity: number, outArray?: number[]) {
  const r = color[0] / 255;
  const g = color[1] / 255;
  const b = color[2] / 255;
  const a = opacity;
...
export function zeroColor(color: number[]) {
  color[0] = 0;
  color[1] = 0;
  color[2] = 0;
  color[3] = 0;
...
export function indexToVec4(index: number, outArray?: number[]) {
  const arr = outArray || new Array(4);
...
export function vec4ToIndex(vec4: ArrayLike<number>) {
  return (
     vec4[0] + 
    (vec4[1] << 8) + 
    (vec4[2] << 16) + 
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars -- debugID kept for call-site compatibility / debugging
export function createTexture(gl: WebGL2RenderingContext, debugID?: number): AtlasTexture {
  const texture = gl.createTexture()! as AtlasTexture;

  texture.buffer = (offscreenCanvas: HTMLCanvasElement) => {
...
function getTypeInfo(gl: WebGL2RenderingContext, glslType: GlslType): [number, number, number] {
  switch(glslType) {
...
function createTypedArray(
  ...
  switch(glType) {
    case gl.FLOAT: return new Float32Array(dataOrSize as number);
    case gl.INT  : return new Int32Array(dataOrSize as number);
  }
  throw new Error('unsupported gl type');
}
...
export function createBufferStaticDraw(
  gl: WebGL2RenderingContext,
  type: GlslType,
  attributeLoc: number,
  dataArray: number[]
 ...
 */
export function createBufferDynamicDraw(
  gl: WebGL2RenderingContext,
  instances: number,
  type: GlslType,
  attributeLoc: number
  ...
  const buffer = gl.createBuffer()! as DynamicBuffer;
  gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
  ...
  const views: (Float32Array | Int32Array)[] = new Array(instances);
  ...
  buffer.getView = (i: number) => {
    return views[i];
  };

  buffer.setPoint = (i: number, x: number, y: number) => {
    const view = views[i];
    view[0] = x;
    view[1] = y;
  };

  buffer.bufferSubData = (count?: number) => {
    gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
    if(count) {
 ...
 */
export function create3x3MatrixBufferDynamicDraw(
  gl: WebGL2RenderingContext,
  instances: number,
  attributeLoc: number
): MatrixBuffer {
  ...
  const matrixViews: Float32Array[] = new Array(instances);
  for(let i = 0; i < instances; i++) {
  ...
  const buffer = gl.createBuffer()! as MatrixBuffer;
  gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
 ...
 */
export function createPickingFrameBuffer(gl: WebGL2RenderingContext): PickingFrameBuffer {
  // Create and bind the framebuffer
  const fb = gl.createFramebuffer()! as PickingFrameBuffer;
  ...
  fb.setFramebufferAttachmentSizes = (width: number, height: number) => {
    gl.bindTexture(gl.TEXTURE_2D, targetTexture);
