...
const maxAbsDiff: Visit = ( currentMax, p, q ) => Math.max( currentMax, absDiff( p, q ) );

const getDistance = (
  length: number, getP: DimGetter, getQ: DimGetter,
  init: number, visit: Visit, post: ( ret: number ) => number = x => x
): number => {
  let ret = init;
...
const distances: Record<string, ( length: number, getP: DimGetter, getQ: DimGetter ) => number> = {
...
export const clusteringDistance = (
  ...
  const impl: CustomDistanceFn = typeof method === 'function'
    ? method
