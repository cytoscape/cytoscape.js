...
const CHART_COLUMNS: { id: ColumnId; visibility: number }[] = [
  ...
  { id: COL.NODE_CHART_REF, visibility: V | F },
];
...
export class ChartPipeline {
  ...
  constructor(
    ...
    this.bindLayout = device.createBindGroupLayout({
      ...
      entries: [
        ...
        {
          binding: CHART_COLUMNS.length + 1,
          visibility: V | F,
          buffer: { type: 'read-only-storage' as GPUBufferBindingType },
        }, // chart blob
        {
          binding: CHART_COLUMNS.length + 2,
          visibility: F,
          buffer: { type: 'read-only-storage' as GPUBufferBindingType },
        }, // poly blob
      ],
  ...
  private ensureBindGroup(
    ...
    if (cached != null && cached.mv === mirror.version) {
      return cached.group;
    }
    ...
    const group = device.createBindGroup({
      ...
      entries: [
        ...
        {
          binding: CHART_COLUMNS.length + 1,
          resource: { buffer: mirror.chartBlobBuffer() },
        },
        {
          binding: CHART_COLUMNS.length + 2,
