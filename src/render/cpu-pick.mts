...
export function pickNodeAt(
  ...
  const hits = (slot: number): boolean => {
    ...
    if ((flags[slot] & FLAG_NO_EVENTS) !== 0) {
      return false;
    }
    ...
    if (Math.abs(dx) > hmax || Math.abs(dy) > hmax) {
      return false;
    } // quick reject
...
function insideShape(
  ...
  switch (shape) {
    ...
    case SHAPE_ROUND_RECTANGLE: {
      // the resolved corner radius, as in nodeSD (B2)
      ...
      return (
        Math.min(Math.max(qx, qy), 0) + Math.sqrt(mx * mx + my * my) - r <= 0
      );
    }
    case SHAPE_BARREL: // 27.5: the same sampled outline the shader builds
      return insideBarrel(dx, dy, hw, hh, zoomDpr);
    case SHAPE_BOTTOM_ROUND_RECTANGLE: {
      // 27.4: only the bottom corners round
    ...
    case SHAPE_CUT_RECTANGLE: {
      // 27.2: the box intersected with the corner chamfers
...
function insideBarrel(
  ...
  const hOff = Math.min(
    BARREL_HEIGHT_OFFSET_MAX * zoomDpr,
    BARREL_HEIGHT_OFFSET_PCT * hh * 2,
  );
  const wOff = Math.min(
    BARREL_WIDTH_OFFSET_MAX * zoomDpr,
    BARREL_WIDTH_OFFSET_PCT * hw * 2,
  );
