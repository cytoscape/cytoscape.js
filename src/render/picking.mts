...
export class Picking {
  ...
  cachedIdAt(xPx: number, yPx: number): number | null {
    ...
    if (tx < 0 || tx >= PICK_TILE || ty < 0 || ty >= PICK_TILE) {
      return null;
    }
  ...
  encodeCopy(encoder: GPUCommandEncoder): InFlightCopy | null {
    ...
    if (slot == null) {
      // ring full: defer to a frame with a free slot
      this.deferrals++;
  ...
  async finish(inFlight: InFlightCopy): Promise<void> {
    ...
    for (const resolve of inFlight.resolvers) {
      resolve(id);
    }
  }
  ...
  destroy(): void {
    ...
    if (this.pending != null) {
      for (const resolve of this.pending.resolvers) {
        resolve(null);
      }
