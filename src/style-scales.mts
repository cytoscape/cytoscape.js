...
export const isMapperSpec = (value: unknown): value is MapperSpec => {
  ...
  return (
    typeof (value as Mapper).data === 'string' ||
...
const parseOutput = (opts: CompileOpts, value: unknown): number | RGBA => {
  ...
  if (out == null) {
    throw err(
      opts.prop,
      `'${String(value)}' is not a valid ${opts.kind} value`,
    );
  }
...
const transformOf = (spec: Mapper, prop: string): Transform => {
  switch (spec.scale ?? 'linear') {
    case 'log': {
      ...
      if (!(base > 0) || base === 1) {
        throw err(prop, `log base must be positive and ≠ 1`);
      }

      return { kind: 'log', base };
    }
    case 'sqrt':
      return { kind: 'pow', exponent: 0.5 };
    case 'pow':
      return { kind: 'pow', exponent: spec.exponent ?? 2 };
    case 'symlog': {
      const constant = spec.constant ?? 1;

      if (!(constant > 0)) {
        throw err(prop, `symlog constant must be positive`);
      }
  ...
  if (typeof range === 'string') {
    ...
    if (count != null) {
      if (count > stops.length) {
        throw err(
          opts.prop,
          `scheme '${range}' has only ${stops.length} entries; ${count} needed`,
        );
      }
      ...
      stops =
        scheme.kind === 'categorical'
          ...
          : Array.from(
              ...
              (_, i) =>
                stops[
                  Math.round((i * (stops.length - 1)) / Math.max(1, count - 1))
                ],
            );
    }
...
const compileContinuous = (opts: CompileOpts, spec: Mapper): Program => {
  if (opts.kind === 'enum') {
    throw err(
      opts.prop,
      `'${opts.prop}' cannot be continuously interpolated; ` +
  ...
  if (
    ...
    throw err(
      opts.prop,
      `a log domain cannot span or touch 0; use scale 'symlog'`,
    );
  }
  ...
  if (scheme?.kind === 'categorical') {
    throw err(
      opts.prop,
      `categorical scheme ranges cannot be interpolated; ` +
  ...
  if (pairwise && domain.length !== outputs.length) {
    throw err(
      opts.prop,
      `a multi-stop domain must pair 1:1 with the range ` +
...
const compileDiscrete = (opts: CompileOpts, spec: Mapper): Program => {
  if (spec.scale === 'threshold') {
    ...
    if (cuts == null) {
      throw err(
        opts.prop,
        `a threshold scale needs an explicit domain of cut points`,
      );
    }
    ...
    if (outputs.length !== cuts.length + 1) {
      throw err(
        opts.prop,
        `a threshold range needs domain.length + 1 outputs ` +
...
const compileOrdinal = (opts: CompileOpts, spec: Mapper): Program => {
  ...
  if (
    ...
    throw err(
      opts.prop,
      `an ordinal scale needs an explicit domain of categories ` +
  ...
  if (outputs.length !== domain.length) {
    throw err(
      opts.prop,
      `an ordinal range must pair 1:1 with the domain ` +
...
const compileCondition = (
  ...
  if (cond != null && named.length > 0) {
    ...
    if (
      cond.data != null ||
      Object.keys(cond).some((k) => CONDITION_OPS.has(k))
    ) {
      throw err(
        opts.prop,
        `a structural condition stands alone ` +
  ...
  const ops = (Object.keys(cond) as (keyof Condition)[]).filter((k) =>
    CONDITION_OPS.has(k),
  );
  ...
  if (ops.length !== 1) {
    throw err(
      opts.prop,
      `each case condition needs exactly one comparison ` +
...
const compileCase = (
  ...
  const clauses = spec.case.map((clause) => {
    ...
    const conds = (
      Array.isArray(clause.when) ? clause.when : [clause.when]
    ).map((c) => {
  ...
  const elseValue =
    spec.else !== undefined
      ? parseOutput(opts, spec.else)
      : spec.fallback !== undefined
        ? parseOutput(opts, spec.fallback)
...
export const compileMapper = (
  ...
  if (spec.data === '') {
    throw err(opts.prop, `the data key must be a non-empty string`);
  }

  const scale = spec.scale;
  const passthrough =
    scale == null && spec.domain == null && spec.range == null;
  ...
  const program: Program = passthrough
    ? { kind: 'passthrough' }
    : scale === 'ordinal'
      ? compileOrdinal(opts, spec)
      : scale === 'threshold' || scale === 'quantize'
        ? compileDiscrete(opts, spec)
...
export const autoExtentFor = (
  ...
  if (col?.kind === 'number') {
    ...
    for (const v of col.values) {
      if (typeof v !== 'number' || !isFinite(v) || (positiveOnly && v <= 0)) {
        continue;
      }
...
const lerpColor = (
  outStops: Extract<OutputStops, { kind: 'color' }>,
  i: number,
  u: number,
): RGBA => {
...
const continuousEval = (program: Extract<Program, { kind: 'continuous' }>) => {
  ...
  return (x: number): Evaluated | null => {
    ...
    if (transform.kind === 'log' && (x === 0 || x > 0 !== lo > 0)) {
      if (!clamp) {
        return null;
      } // sign mismatch is unmappable without clamping
...
export const testCondition = (cond: CompiledCondition, v: unknown): boolean => {
  ...
  switch (cond.op) {
    ...
    case 'ne':
      return v !== cond.value;
    case 'in':
      return (cond.value as (string | number)[]).includes(v as string | number);
    case 'lt':
 ...
 */
const bindCase = (
  program: Extract<Program, { kind: 'case' }>,
  read: ValueReader,
  group: GroupName,
...
export const bindEvaluator = (
  ...
  if (program.kind === 'case') {
    ...
    return bindCase(
      program,
      read ?? ((g, slot, key) => data.get(g, slot, key)),
      group,
    );
  }
  ...
  if (program.kind === 'ordinal') {
    ...
    if (col.kind === 'number') {
      ...
      return (slot) =>
        present[slot] ? (map.get(values[slot]) ?? missing) : missing;
    }
  ...
  if (program.kind === 'passthrough') {
    ...
    if (col.kind === 'number') {
      ...
      return (slot) =>
        present[slot] ? (coerce(values[slot]) ?? missing) : missing;
    }
