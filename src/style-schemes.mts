...
const linearToByte = ( c: number ): number => {
  const nonlin = c <= 0.0031308 ? 12.92 * c : 1.055 * Math.pow( c, 1 / 2.4 ) - 0.055;
...
export const oklabToSrgb = ( L: number, a: number, b: number ): [ number, number, number ] => {
  ...
  const s_ = fr( L - 0.0894841775 * a - 1.2914855480 * b );
  ...
  return [
    ...
    linearToByte( fr( -0.0041960863 * l - 0.7034186147 * m + 1.7076147010 * s ) )
  ];
};
