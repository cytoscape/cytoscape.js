...
describe('gpu/style: arrows', function () {
  it('defaults to no arrows (alpha 0 in both columns)', function () {
    ...
    expect(cy._styleEngine.arrowEnds).to.deep.equal({
      source: false,
      target: false,
    });
  });
  ...
  it('triangle shape writes the arrow color; v3-like default #999', function () {
    ...
    expect(arrowColor(cy, 'edge.targetArrow')).to.deep.equal([
      153, 153, 153, 255,
    ]);
    expect(arrowColor(cy, 'edge.sourceArrow')[3]).to.equal(0);
    expect(cy._styleEngine.arrowEnds).to.deep.equal({
  ...
  it('shape none suppresses the arrow even with a color set', function () {
    const cy = cytoscape({
      elements: GRAPH,
      style: {
        edges: { 'target-arrow-shape': 'none', 'target-arrow-color': '#f00' },
      },
    });
  ...
  it('rejects an unknown arrow-shape keyword', function () {
    ...
    expect(() =>
      cy.style({ edges: { 'target-arrow-shape': 'fishtail' } }),
  ...
  it("supports v3's compound arrow shapes (round 27.6)", function () {
    for (const shape of [
      ...
      const cy = cytoscape({
        elements: GRAPH,
        style: {
          edges: { 'target-arrow-shape': shape, 'target-arrow-color': '#f00' },
        },
      });
    ...
    for (const shape of shapes) {
      cy.style({
        edges: {
          ...
          'source-arrow-color': '#123',
          'target-arrow-shape': 'triangle',
          'target-arrow-color': '#456',
        },
      });
...
describe('gpu/store: the edge.width mirror lane (round 56)', function () {
  ...
  it('carries the shape word, and nothing else when both heads are opaque', function () {
    const cy = cytoscape({
      elements: GRAPH,
      style: {
        edges: { 'target-arrow-shape': 'vee', 'source-arrow-shape': 'tee' },
      },
    });
  ...
  describe('the SHOWS_LINE flags', function () {
    ...
    it('are set by a translucent head — the case a hollow-only rule missed', function () {
      ...
      expect(bitsFor({ 'source-arrow-color': 'rgba(0,0,0,0.5)' })).to.equal(
        SRC_SHOWS,
      );
      expect(
        bitsFor({ 'line-opacity': 0.5 }),
        'folds through line-opacity',
      ).to.equal(SHOWS);
      expect(bitsFor({ opacity: 0.4 }), 'folds through edge opacity').to.equal(
        SHOWS,
      );
    });
    ...
    it('follow a later arrow-colour write without a restyle of the shapes', function () {
      ...
      cy.style({
        edges: {
          'target-arrow-shape': 'triangle',
          'target-arrow-color': 'rgba(1,2,3,0.25)',
        },
      });
  ...
  it('survives table growth, which reallocates the column', function () {
    const cy = cytoscape({
      elements: GRAPH,
      style: {
        edges: { 'target-arrow-shape': 'vee', 'target-arrow-fill': 'hollow' },
      },
    });

    const bits = mirrorBits(cy);

    expect(bits & TGT_SHOWS, 'the flag is set to begin with').to.equal(
      TGT_SHOWS,
    );
