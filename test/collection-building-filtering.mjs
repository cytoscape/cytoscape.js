...
describe('gpu/collection: building and filtering', function(){
  ...
  it('takes differences', function(){
    ...
    expect( ids( cy.nodes().subtract( cy.$id('a').union( cy.$id('c') ) ) ) ).to.deep.equal(['b']);
  });

  it('takes intersections', function(){
    expect( ids( cy.nodes().intersection( cy.filter({ selected: true }) ) ) ).to.deep.equal(['c']);
    expect( ids( cy.elements().intersect( cy.edges() ) ) ).to.deep.equal(['ab', 'bc']);
    expect( ids( cy.nodes().and( cy.$id('a').union( cy.$id('b') ) ) ) ).to.deep.equal(['a', 'b']);
  });
  ...
  it('takes symmetric differences', function(){
    ...
    expect( ids( cy.$id('a').union( cy.$id('b') ).symmetricDifference( bc ) ) ).to.deep.equal(['a', 'c']);
    expect( ids( cy.$id('a').union( cy.$id('b') ).xor( bc ) ) ).to.deep.equal(['a', 'c']);
    expect( ids( cy.$id('a').union( cy.$id('b') ).symdiff( bc ) ) ).to.deep.equal(['a', 'c']);
  });

  it('filters with a query', function(){
    expect( ids( cy.elements().filter({ group: 'nodes' }) ) ).to.deep.equal(['a', 'b', 'c']);
    expect( ids( cy.elements().filter({ group: 'nodes', selected: true }) ) ).to.deep.equal(['c']);
  });
