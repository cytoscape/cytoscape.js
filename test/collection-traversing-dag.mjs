...
describe('gpu/collection: DAG traversal, edge relations, components', function () {
  ...
  it('successors() = transitive outgoing closure (nodes + edges)', function () {
    expect(ids(cy.$id('n1').successors().nodes())).to.deep.equal([
      'n1',
      'n2',
      'n3',
    ]);
    expect(ids(cy.$id('n1').successors({ group: 'nodes' }))).to.deep.equal([
      'n1',
      'n2',
      'n3',
    ]);
  ...
  it('predecessors() = transitive incoming closure', function () {
    expect(ids(cy.$id('n3').predecessors({ group: 'nodes' }))).to.deep.equal([
      'n1',
      'n2',
    ]);
  });
  ...
  it('parallelEdges() includes the edge itself + same-direction + opposite', function () {
    ...
    expect(ids(cy.$id('e12').parallelEdges())).to.deep.equal([
      'e12',
      'e12b',
      'e21',
    ]);
  ...
  it('edgesWith() = edges connecting this to others, either direction', function () {
    expect(ids(cy.$id('n1').edgesWith(cy.$id('n2')))).to.deep.equal([
      'e12',
      'e12b',
      'e21',
    ]);
  ...
  it('edgesTo() = directed edges from this to others', function () {
    expect(ids(cy.$id('n1').edgesTo(cy.$id('n2')))).to.deep.equal([
      'e12',
      'e12b',
    ]);
  ...
  it('components() includes internal edges', function () {
    var big = cy
      .elements()
      .components()
      .find((c) => c.nodes().length === 3);

    expect(ids(big.edges())).to.deep.equal([
      'e12',
      'e12b',
      'e13',
      'e21',
  ...
  it('component() = whole-graph component containing the element', function () {
    expect(ids(cy.$id('n2').component().nodes())).to.deep.equal([
      'n1',
      'n2',
      'n3',
    ]);
