...
describe('gpu/collection: traversing', function(){
  ...
  it('edges.sources() and edges.targets()', function(){
    expect( ids( cy.$id('n1n2').union( cy.$id('n2n3') ).sources() ) ).to.deep.equal(['n1', 'n2']);
    expect( ids( cy.$id('n1n2').union( cy.$id('n2n3') ).targets() ) ).to.deep.equal(['n2', 'n3']);
  });
  ...
  it('node.outgoers()', function(){
    ...
    expect( ids( cy.$id('n1').outgoers({ group: 'nodes' }) ) ).to.deep.equal(['n2']);
    expect( ids( cy.$id('n1').outgoers({ group: 'edges' }) ) ).to.deep.equal(['n1n2']);
  });

  it('node.incomers()', function(){
    expect( ids( cy.$id('n1').incomers() ) ).to.deep.equal(['n3', 'n3n1']);
    expect( ids( cy.$id('n1').incomers({ group: 'nodes' }) ) ).to.deep.equal(['n3']);
  });

  it('node.neighborhood()', function(){
    expect( ids( cy.$id('n2').neighborhood() ) ).to.deep.equal(['n1', 'n1n2', 'n2n3', 'n3']);
    expect( ids( cy.$id('n2').neighborhood({ group: 'nodes' }) ) ).to.deep.equal(['n1', 'n3']);
    expect( ids( cy.$id('n2').openNeighborhood() ) ).to.deep.equal(['n1', 'n1n2', 'n2n3', 'n3']);
  });

  it('node.closedNeighborhood()', function(){
    expect( ids( cy.$id('n2').closedNeighborhood() ) ).to.deep.equal(['n1', 'n1n2', 'n2', 'n2n3', 'n3']);
  });
  ...
  describe('edgesWith / edgesTo across collections', function(){
    it('edgesWith() with identical node collections on both sides', function(){
      expect( ids( cy.$id('n1').union( cy.$id('n2') ).edgesWith( cy.$id('n1').union( cy.$id('n2') ) ) ) ).to.deep.equal(['n1n2']);
    });

    it('edgesWith() with intersecting collections', function(){
      expect( ids( cy.$id('n1').union( cy.$id('n2') ).edgesWith( cy.$id('n1') ) ) ).to.deep.equal(['n1n2']);
    });
    ...
    it('edgesTo() with identical node collections respects direction', function(){
      expect( ids( cy.$id('n1').union( cy.$id('n2') ).edgesTo( cy.$id('n1').union( cy.$id('n2') ) ) ) ).to.deep.equal(['n1n2']);
    });

    it('edgesTo() with intersecting collections respects direction', function(){
      // n1->n2 counts; the reverse (n2->n1) does not exist
      expect( ids( cy.$id('n1').union( cy.$id('n2') ).edgesTo( cy.$id('n2') ) ) ).to.deep.equal(['n1n2']);
    });
  ...
  describe('components restricted to the collection', function(){
    it('splits a sub-collection into its internal components', function(){
      ...
      var comps = cy.$id('n1').union( cy.$id('n2') ).union( cy.$id('n1n2') ).union( cy.$id('n3') ).components();
