...
import { isColumnarElements, toColumnarElements } from '../src/columnar.mjs';
import { GraphStore } from '../src/store/graph-store.mjs';
import { FLAG_SELECTABLE } from '../src/contract.mjs';
...
describe('gpu/columnar', function () {
  describe('toColumnarElements', function () {
    ...
    it('throws on an edge without a source and target', function () {
      expect(() =>
        toColumnarElements([{ group: 'edges', data: { id: 'e' } }]),
      ).to.throw(/source and target/);
    });
  });
  ...
  describe('store ingest', function () {
    it('matches the definition path column for column', function () {
      ...
      const viaColumnar = cytoscape({ elements: toColumnarElements(FIXTURE) });
      ...
      for (const id of [
        ...
        expect(Array.from(viaColumnar._store.column(id)), id).to.deep.equal(
          Array.from(viaDefs._store.column(id)),
        );
      }

      expect(viaColumnar._store.slotsOrdered('nodes')).to.deep.equal(
        viaDefs._store.slotsOrdered('nodes'),
      );
      expect(viaColumnar._store.slotsOrdered('edges')).to.deep.equal(
        viaDefs._store.slotsOrdered('edges'),
      );
      expect(viaColumnar._store.boundingBox()).to.deep.equal(
        viaDefs._store.boundingBox(),
      );
    });
    ...
    it('allocates contiguously on a fresh store and reuses freed slots later', function () {
      ...
      const slots = store.addNodesColumnar(
        { count: 3, positions: new Float32Array([1, 1, 2, 2, 3, 3]) },
        newId,
      );
      ...
      const more = store.addNodesColumnar(
        { count: 2, positions: new Float32Array([8, 8, 9, 9]) },
        newId,
      );
    ...
    it('applies selection flags', function () {
      ...
      expect(
        cy._store.hasFlag('nodes', cy._store.lookup('c').slot, FLAG_SELECTABLE),
      ).to.be.false;
    ...
    it('builds adjacency', function () {
      ...
      expect(
        cy.getElementById('b').outgoers({ group: 'nodes' })[0].id(),
      ).to.equal('c');
    ...
    it('throws on duplicate ids and short endpoint arrays', function () {
      ...
      expect(() =>
        store.addNodesColumnar({ count: 1, ids: ['a'] }, newId),
      ).to.throw(/second element/);
      expect(() =>
        store.addEdgesColumnar(
          {
            count: 1,
  ...
  describe('core API', function () {
    it('auto-generates ids for holes', function () {
      const cy = cytoscape({
        elements: toColumnarElements([{ data: {} }, { data: { id: 'x' } }]),
      });
