...
describe('gpu/dash-props (round 13 B3)', function(){
  ...
  it('parses and reads back the dash props', function(){
    cy = makeCy({
      'line-style': 'dashed', 'line-dash-pattern': [ 10, 4 ],
      'line-dash-offset': 7, 'line-cap': 'round'
    });
  ...
  it('stores the pattern and meta columns', function(){
    cy = makeCy({ 'line-dash-pattern': [ 9, 2 ], 'line-dash-offset': 3, 'line-cap': 'square' });

    var slot = cy._store.lookup('e').slot;
    var pat = cy._store.column('edge.dashPattern');
    var meta = cy._store.column('edge.dashMeta');

    expect( Array.from( pat.subarray( slot * 4, slot * 4 + 4 ) ) ).to.deep.equal( [ 9, 2, 9, 2 ] );
  ...
  it('line-cap and line-dash-offset take mappers', function(){
    cy = cytoscape({
      ...
      style: { edges: {
        'line-cap': { case: [ { when: { data: 'kind', eq: 1 }, then: 'round' } ], else: 'butt' },
        'line-dash-offset': { data: 'kind', scale: 'linear', domain: [ 0, 1 ], range: [ 0, 12 ] }
      } }
    });
