...
Each spec below cites the ledger entry it pins (src/README.md's
...
describe('gpu/decided drops (29.3)', function () {
  ...
  describe('no selector strings, anywhere', function () {
    ...
    it('the same twelve reject a collection from another instance (round 48.4)', function () {
      ...
      try {
        ...
        expect(() => a.intersection(other.nodes())).to.throw(
    ...
    it('event delegation rejects one at registration, not at emit', function () {
      expect(() => cy.on('tap', 'node', () => {})).to.throw(
        /predicate function/,
      );
      expect(() => cy.one('tap', 'node', () => {})).to.throw(
        /predicate function/,
      );
      expect(() => cy.off('tap', 'node', () => {})).to.throw(
        /predicate function/,
      );
    });
    ...
    it('the breadthfirst layout rejects one as its roots', function () {
      ...
      expect(() =>
        cy.layout({ name: 'breadthfirst', roots: cy.$id('a') }).run(),
      ).to.not.throw();
      expect(() =>
        cy.layout({ name: 'breadthfirst', roots: ['a'] }).run(),
      ).to.not.throw();
  ...
  describe('the sheet rejects the dropped properties', function () {
    const sheetThrows = (props) => () =>
      cytoscape({ elements: [], style: { nodes: props } });
    ...
    it('the no-dash spelling in every enum that took it, not only `shape`', function () {
      ...
      expect(sheetThrows({ 'overlay-shape': 'roundrectangle' })).to.throw(
        /invalid/,
      );
      expect(
        sheetThrows({ 'text-background-shape': 'roundrectangle' }),
      ).to.throw(/invalid/);

      expect(
        sheetThrows({ 'overlay-shape': 'round-rectangle' }),
      ).to.not.throw();
      expect(
        sheetThrows({ 'text-background-shape': 'round-rectangle' }),
      ).to.not.throw();
    });
    ...
    it('the shape error stops advertising the dropped spelling', function () {
      ...
      expect(sheetThrows({ shape: 'nonsense' })).to.not.throw(
        /[^-]roundrectangle/,
      );
    });
  ...
  describe('dropped v3 event names register and never fire (round 17)', function () {
    ...
    */
    ...
    it('never fires a namespaced listener for an event v4 itself raises', function () {
      ...
      expect(
        plain,
        'the unqualified listener should see the library event',
      ).to.equal(1);
  ...
  */
  describe('the constructor is permissive on purpose', function () {
    it('ignores the dropped canvas-era options rather than throwing', function () {
      for (const opt of [
        ...
        const permissive = cytoscape({ elements: [], [opt]: true });
    ...
    it('ignores a plain typo, and round-trips it through options()', function () {
      const permissive = cytoscape({ elements: [], totallyUnknownOption: 1 });
    ...
    it('still throws on the unknown names that are v4 contract', function () {
      ...
      expect(() =>
        cytoscape({ elements: [], style: { nodes: { notAProp: 1 } } }),
      ).to.throw(/unsupported/);
      expect(() =>
        cytoscape({ elements: [], style: { notAGroup: {} } }),
      ).to.throw();
  ...
  describe('the other decided removals', function () {
    ...
    it('the animation queue and step callback (round 21)', function () {
      expect(() =>
        cy.$id('a').animate({ position: { x: 1, y: 1 }, queue: false }),
      ).to.throw(/queue/);
      expect(() =>
        cy.$id('a').animate({ position: { x: 1, y: 1 }, step: () => {} }),
      ).to.throw(/step/);
    });
    ...
    it('cytoscape.warnings is still absent, pending the error policy', function () {
      ...
      expect(cytoscape.warnings).to.equal(undefined);
