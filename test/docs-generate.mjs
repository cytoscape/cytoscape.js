...
import { generate, NAMESPACES, NOT_PUBLISHED } from '../scripts/docs-generate.mjs';
import { auditStrandedComments, PUBLIC_API } from '../scripts/jsdoc-coverage.mjs';
...
function dtsMembers( className ){
  ...
  expect( start, `no declare class ${className} in the shipped d.ts` ).to.be.greaterThan( -1 );
...
function dtsFactoryStatics(){
  ...
  expect( block, 'no cytoscape namespace block in the shipped d.ts' ).to.not.equal( null );

  return new Set( [ ...block[1].matchAll( /export \{\s*([A-Za-z_$][\w$]*)\s*\}/g ) ].map( m => m[1] ) );
}
...
function membersUnder( prefix ){
  ...
  expect( section, `the model publishes no ${prefix} namespace` ).to.not.equal( undefined );
  ...
  for( const child of section.sections ){
    for( const fn of child.fns ){
      ...
      const bare = fn.name.includes( '.' ) ? fn.name.slice( fn.name.indexOf( '.' ) + 1 ) : null;
...
describe( 'the docs generator', () => {
  describe( 'the model it builds', () => {
    it( 'covers every published namespace at a non-trivial size', () => {
      ...
      expect( model.sections.length ).to.equal( Object.keys( NAMESPACES ).length + 1 ); // + the factory
      expect( allFns.length ).to.be.greaterThan( 300 );

      const sections = model.sections.reduce( ( n, s ) => n + s.sections.length, 0 );
    ...
    it( 'documents each member exactly once', () => {
      ...
      for( const fn of allFns ){
        expect( seen.has( fn.name ), `${fn.name} documented twice (${seen.get( fn.name )} and ${fn.src})` )
          .to.equal( false );
    ...
    it( 'gives every entry a description and every argument a description', () => {
      for( const fn of allFns ){
        expect( fn.descr, `${fn.name} has no description` ).to.be.a( 'string' ).and.not.equal( '' );

        expect( fn.formats.length, `${fn.name} has no formats` ).to.be.greaterThan( 0 );

        for( const format of fn.formats ){
          for( const arg of format.args ){
            expect( arg.name, `${fn.name} has an unnamed argument` ).to.be.a( 'string' ).and.not.equal( '' );
    ...
    it( 'gives an overloaded member one format per signature', () => {
      ...
      expect( on.formats[1].args.map( a => a.name ) ).to.deep.equal( [ 'events', 'predicate', 'callback' ] );
  ...
  describe( 'against the shipped declaration', () => {
    it( 'documents no member that does not ship (no phantom entries)', () => {
      for( const [ className, prefix ] of Object.entries( NAMESPACES ) ){
        ...
        for( const name of membersUnder( prefix ) ){
          expect( shipped.has( name ), `${prefix}.${name} is documented but not in ${className}'s declaration` )
            .to.equal( true );
        }
      }
    } );
    ...
    it( 'documents every member that ships (nothing silently dropped)', () => {
      for( const [ className, prefix ] of Object.entries( NAMESPACES ) ){
        ...
        for( const name of dtsMembers( className ) ){
          ...
          expect( documented.has( name ), `${className}.${name} ships but the reference does not document it` )
            .to.equal( true );
        }
      }
    } );
    ...
    it( 'documents the factory and exactly the statics it hangs on itself', () => {
      ...
      const documented = new Set( names.filter( n => n !== 'cytoscape' ).map( n => n.slice( 'cytoscape.'.length ) ) );
    ...
    it( 'leaves the unpublished classes unpublished, each with a reason', () => {
      ...
      for( const [ className, reason ] of Object.entries( NOT_PUBLISHED ) ){
        expect( published.has( className ), `${className} is in both tables` ).to.equal( false );
        expect( reason, `${className} is excluded without a reason` ).to.be.a( 'string' );
        expect( reason.length, `${className}'s reason is too short to be one` ).to.be.greaterThan( 30 );
      }
    } );
  } );
  ...
  describe( 'preconditions', () => {
    it( 'has no stranded doc block in a published file', () => {
      const stranded = PUBLIC_API.flatMap( rel => auditStrandedComments( join( ROOT, rel ) ).stranded );

      expect( stranded, `a displaced block would be published twice: ${stranded.join( ', ' )}` )
        .to.have.lengthOf( 0 );
    } );
  } );
} );
