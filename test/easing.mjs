...
import v3Easings from '../v3/src/core/animation/easings.mjs';
import {
  BEZIER_EASINGS,
  EASING_KIND,
  EASING_NAMES,
...
describe('gpu/easing', function () {
  describe('the v3 enum', function () {
    it('names exactly what v3 names (minus the parameterized forms)', function () {
      const v3Names = Object.keys(v3Easings).filter(
        (n) => n !== 'spring' && n !== 'cubic-bezier',
      );
  ...
  describe('cubic-bezier()', function () {
    ...
    it('rejects the wrong arity and non-numbers', function () {
      expect(() => compileEasing('cubic-bezier(0, 0, 1)')).to.throw(
        /four control points/,
      );
      expect(() => compileEasing('cubic-bezier(0, 0, 1, 1, 1)')).to.throw(
        /four control points/,
      );
      expect(() => compileEasing('cubic-bezier(0, 0, 1, x)')).to.throw(
        /not a number/,
      );
    });
  });
  ...
  describe('linear()', function () {
    ...
    it('rejects too few points, bad stops, and junk', function () {
      expect(() => compileEasing('linear(0.5)')).to.throw(
        /at least two points/,
      );
      expect(() => compileEasing('linear(0, 0.5 half, 1)')).to.throw(
        /not a percentage/,
      );
      expect(() => compileEasing('linear(0, 0.5 10% 20% 30%, 1)')).to.throw(
        /at most two stops/,
      );
      expect(() => compileEasing('linear(0, nope, 1)')).to.throw(
        /not a number/,
      );
    });
  });
  ...
  describe('spring(bounce)', function () {
    ...
    it('reads bounce as the damping ratio: 0 is critical, positive overshoots', function () {
      const peak = (bounce) =>
        Math.max(...ts.map(compileEasing(`spring(${bounce})`).fn));
    ...
    it('reports the settling window as durationScale, so duration stays perceptual', function () {
      ...
      const scale = (bounce) =>
        compileEasing(`spring(${bounce})`).durationScale;
    ...
    it('tracks the closed form everywhere, to within the settling epsilon', function () {
      ...
      for (const bounce of [0, 0.3, 0.6, 0.9]) {
        ...
        for (let i = 0; i <= 1000; i++) {
          ...
          expect(fn(t), `${bounce} at ${t}`).to.be.closeTo(
            step(zeta, t * durationScale),
            2.5e-3,
          );
        }
      }
    ...
    it('clamps the bounce factor rather than diverging', function () {
      expect(compileEasing('spring(5)').durationScale).to.equal(
        compileEasing('spring(0.9)').durationScale,
      );
      expect(compileEasing('spring(-5)').durationScale).to.equal(
        compileEasing('spring(-0.9)').durationScale,
      );
    ...
    it("rejects v3's two-argument form", function () {
      expect(() => compileEasing('spring(500, 20)')).to.throw(
        /one bounce factor/,
      );
    });
  });
  ...
  describe('rejections', function () {
    it('throws on an unknown name, listing the enum', function () {
      expect(() => compileEasing('ease-in-out-bounce')).to.throw(
        /not a known easing.*ease-in-out-circ.*cubic-bezier\(\), linear\(\) or spring\(\)/,
      );
    });

    it('throws on an unknown function form', function () {
      expect(() => compileEasing('steps(4, end)')).to.throw(
        /not a known easing/,
      );
    });

    it('throws on a custom function, since it cannot cross to the device', function () {
      expect(() => compileEasing((t) => t * t)).to.throw(
