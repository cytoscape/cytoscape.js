...
describe('gpu/flag-scan', function () {
  describe('GraphStore.scanRefsInto', function () {
    ...
    it('applies the flags mask', function () {
      var sel = scan('nodes', FLAG_SELECTED, FLAG_SELECTED).map((r) =>
        store.idAt(r.group, r.slot),
      );
      var unsel = scan('nodes', FLAG_SELECTED, 0).map((r) =>
        store.idAt(r.group, r.slot),
      );
  ...
  describe('core query routing', function () {
    ...
    it('resolves structured queries on the core', function () {
      ...
      expect(
        ids(cy.elements({ group: 'nodes', selected: false })),
      ).to.deep.equal(['a']);
    ...
    it('resolves predicate functions per element', function () {
      expect(
        ids(cy.elements((ele) => ele.id() === 'ab' || ele.selected())),
      ).to.deep.equal(['b', 'ab']);
    ...
    it('follows selection changes and removals', function () {
      ...
      expect(ids(cy.filter({ group: 'nodes', selected: true }))).to.deep.equal([
        'a',
        'b',
      ]);
