...
// simulation (the compound precedent) — on a worker under 'auto'
// since 129.3 as revisited in 131, bit-identical to in-thread — so a
// headless run is deterministic under a seed, read at promise():
// what these specs pin.  The projection maths itself is pinned at the sim level in
...
describe('gpu/layout: force constraints (round 85.2)', function () {
  ...
  it('horizontal alignment settles the group onto one y', async function () {
    mk(ring(10));
    await settle({ alignment: { horizontal: [['n0', 'n3', 'n6']] } });
  ...
  it('vertical alignment settles the group onto one x', async function () {
    mk(ring(10));
    await settle({ alignment: { vertical: [['n1', 'n5', 'n8']] } });

    expect(spreadOf(['n1', 'n5', 'n8'], 'x')).to.be.below(1e-3);
  });

  it('groups sharing a node merge transitively', async function () {
    mk(ring(10));
    await settle({
      alignment: {
        horizontal: [
  ...
  it('a locked member pins its group, and never moves', async function () {
    mk(ring(10));
  ...
  it('relative placement holds its gaps at settle', async function () {
    mk(ring(10));
    await settle({
      relativePlacement: [
  ...
  it('gap defaults to the run mean ideal edge length', async function () {
    mk(ring(6));
    await settle({
      edgeLength: 80,
  ...
  it('alignment and relative placement compose', async function () {
    mk(ring(10));
    await settle({
  ...
  it('throws on an unknown id, before anything moves', async function () {
    mk(ring(4));
  ...
  it('throws on a placement cycle', async function () {
    mk(ring(4));
  ...
  it('throws on two locked members of one group at different coordinates', async function () {
    mk(ring(6));
  ...
  it('throws on a malformed relativePlacement entry', async function () {
    mk(ring(4));
