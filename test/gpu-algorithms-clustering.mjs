...
describe('gpu/algorithms: clustering', function(){
  ...
  describe('eles.kMedoids()', function(){
    ...
    it('allows a custom 2-arg distance function', function(){
      var clusters = cy.elements().kMedoids( {
        ...
        distance: ( nodeP, nodeQ ) => {
          ...
          return Math.abs( nodeP.data('attrA') - nodeQ.data('attrA') )
    ...
    it('throws when k exceeds the number of nodes', function(){
      expect( () => cy.elements().kMedoids( { ...options, testMode: false, k: 11 } ) )
        .to.throw(/cannot exceed the number of nodes/);
    });

    it('throws for an empty collection instead of hanging', function(){
      var emptyCy = cytoscapeGpu({ elements: [] });

      expect( () => emptyCy.elements().kMedoids({ k: 2 }) )
        .to.throw(/cannot exceed the number of nodes/);
    });
  });
  ...
  describe('eles.fuzzyCMeans()', function(){
    ...
    it('returns the numerically correct clusters (random init, separated data)', function(){
      ...
      expect( ids( clusters[ start ] ) ).to.deep.equal([ '1', '2', '3', '4', '5', '6', '7', '8' ]);
      expect( ids( clusters[ ( start + 1 ) % 2 ] ) ).to.deep.equal([ '9', '10', '11', '12', '13', '14', '15', '16' ]);
    });
  ...
  describe('eles.hierarchicalClustering()', function(){
    ...
    it('dendrogram level 0: one cluster with all nodes', function(){
      var clusters = cy.elements().hierarchicalClustering( { ...options, dendrogramDepth: 0 } );
    ...
    it('dendrogram level 1: two clusters', function(){
      var clusters = cy.elements().hierarchicalClustering( { ...options, dendrogramDepth: 1 } );
    ...
    it('dendrogram level 2: four clusters', function(){
      var clusters = cy.elements().hierarchicalClustering( { ...options, dendrogramDepth: 2 } );
    ...
    it('dendrogram level 3: five clusters', function(){
      var clusters = cy.elements().hierarchicalClustering( { ...options, dendrogramDepth: 3 } );
    ...
    it('dendrogram levels 4+: one node per cluster (fails safely)', function(){
      for( var depth of [ 4, 5, 10 ] ){
        var clusters = cy.elements().hierarchicalClustering( { ...options, dendrogramDepth: depth } );
    ...
    it('threshold mode with default Infinity merges everything', function(){
      var clusters = cy.elements().hierarchicalClustering( {
        distance: 'euclidean', linkage: 'min', attributes: options.attributes
      } );
    ...
    it('allows a custom 2-arg distance function', function(){
      var clusters = cy.elements().hierarchicalClustering( {
        ...
        // no attributes: the custom fn receives the operands directly, as v3
        distance: ( nodeP, nodeQ ) =>
          Math.abs( nodeP.data('X1') - nodeQ.data('X1') ) + Math.abs( nodeP.data('X2') - nodeQ.data('X2') )
      } );
    ...
    it('addDendrogram adds intermediary nodes and edges', function(){
      ...
      cy.elements().hierarchicalClustering( { ...options, dendrogramDepth: 0, addDendrogram: true } );
    ...
    it('hca alias resolves', function(){
      expect( cy.elements().hca( { ...options, dendrogramDepth: 0 } ).length ).to.equal( 1 );
    });
  });
  ...
  describe('eles.markovClustering()', function(){
    ...
    beforeEach(function(){
      ...
      cy = cytoscapeGpu({
        elements: [
          { data: { id: '11' } }, { data: { id: '22' } }, { data: { id: '33' } },
          { data: { id: '44' } }, { data: { id: '55' } }, { data: { id: '66' } },
          { data: { id: '77' } }, { data: { id: '88' } }, { data: { id: '99' } },
          { data: { id: '123' } }, { data: { id: '456' } }, { data: { id: '2147483647' } },
          edge('11', '22', 2), edge('11', '66', 3.4), edge('11', '77', 3),
          edge('11', '123', 8), edge('22', '11', 2), edge('22', '33', 3.8),
          edge('22', '55', 8.1), edge('33', '22', 3.8), edge('33', '44', 7),
          edge('33', '55', 6.2), edge('44', '33', 7), edge('44', '88', 5.7),
          edge('44', '99', 7.0), edge('44', '456', 3), edge('55', '22', 8.1),
          edge('55', '33', 6.2), edge('55', '77', 2.9), edge('55', '88', 3.0),
          edge('66', '11', 3.4), edge('66', '123', 5.1), edge('77', '11', 3),
          edge('77', '55', 2.9), edge('77', '123', 1.5), edge('88', '44', 5.7),
          edge('88', '55', 3.0), edge('88', '99', 3.0), edge('88', '456', 4.2),
          edge('99', '44', 7.0), edge('99', '88', 3.0), edge('99', '456', 1.8),
          edge('99', '2147483647', 3.9), edge('123', '11', 8), edge('123', '66', 5.1),
          edge('123', '77', 1.5), edge('456', '44', 3), edge('456', '88', 4.2),
          edge('456', '99', 1.8), edge('456', '2147483647', 6.3),
          edge('2147483647', '99', 3.9), edge('2147483647', '456', 6.3)
        ]
      });
    });
    ...
    it('returns the numerically correct clusters', function(){
      ...
      expect( ids( clusters[0] ) ).to.deep.equal([ '44', '88', '99', '456', '2147483647' ]);
  ...
  describe('eles.affinityPropagation()', function(){
    ...
    it('validates damping and preference like v3', function(){
      expect( () => cy.elements().affinityPropagation( { ...opts(), damping: 1.5 } ) )
        .to.throw(/Damping/);
      expect( () => cy.elements().affinityPropagation( { ...opts(), preference: 'bogus' } ) )
        .to.throw(/Preference/);
    });
