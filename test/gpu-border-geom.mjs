...
describe('gpu/border-geom (round 13 B2)', function(){
  ...
  it('parses and reads back corner-radius (auto default) and border-position', function(){
    ...
    cy.style({ nodes: { 'shape': 'round-rectangle', 'corner-radius': 5, 'border-position': 'outside' } });
  ...
  it('throws on bad keywords', function(){
    expect( () => makeCy({ 'border-position': 'middle' }) ).to.throw( /border-position/ );
  ...
  it('the CPU pick honors the per-node corner radius', function(){
    ...
    cy = makeCy({ 'shape': 'round-rectangle', 'width': 60, 'height': 60, 'corner-radius': 28 });
  ...
  it('corner-radius takes mappers; border-position takes case mappers', function(){
    cy = cytoscapeGpu({
      ...
      style: { nodes: {
        ...
        'corner-radius': { data: 'big', scale: 'linear', domain: [ 0, 1 ], range: [ 2, 12 ] },
        'border-position': { case: [ { when: { data: 'big', eq: 1 }, then: 'inside' } ], else: 'outside' }
      } }
    });
