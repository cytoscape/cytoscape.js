...
describe('gpu/style: the chart family (round 23)', function(){
  ...
  var makeCy = function( nodeStyle, data ){
    return cytoscapeGpu({
      elements: [ { data: Object.assign( { id: 'a' }, data ), position: { x: 0, y: 0 } } ],
  ...
  it('colors default to the category10 scheme and cycle past its length', function(){
    ...
    var named = makeCy( { chart: 'pie', 'chart-values': [ 0.5 ], 'chart-colors': 'viridis' } );

    expect( named._store.chartAt( 0 ).colors[ 0 ].slice( 0, 3 ) ).to.deep.equal( [ 0x44, 0x01, 0x54 ] );
  });

  it('the data passthrough reads per-element arrays and refreshes on writes', function(){
    var cy = makeCy( { chart: 'pie', 'chart-values': { data: 'parts' } }, { parts: [ 0.6, 0.4 ] } );
  ...
  it('chart-opacity folds into the stored slice alphas', function(){
    var cy = makeCy( {
      chart: 'pie', 'chart-values': [ 1 ], 'chart-colors': [ 'red' ], 'chart-opacity': 0.5
    } );
  ...
  it('the kind takes a case mapper; chart: none clears the record', function(){
    var cy = cytoscapeGpu({
      ...
      style: { nodes: {
        chart: { case: [ { when: { data: 'kind', eq: 'charty' }, then: 'pie' } ], else: 'none' },
  ...
  it('rejects bad values and the edges group', function(){
    ...
    expect( () => makeCy( { chart: 'pie', 'chart-values': [ 0.5 ], 'chart-hole': 2 } ) ).to.throw();
    expect( () => makeCy( { chart: 'pie', 'chart-direction': 'diagonal' } ) ).to.throw();
    expect( () => makeCy( { chart: 'pie', 'chart-colors': 'no-such-scheme' } ) ).to.throw();
    expect( () => cytoscapeGpu( { style: { edges: { chart: 'pie' } } } ) )
  ...
  it('removal frees the record; charts are paint-only (no bb growth)', function(){
    ...
    expect( cy.$id( 'a' ).boundingBox().w ).to.equal( plain.$id( 'a' ).boundingBox().w );
