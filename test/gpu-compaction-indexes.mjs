...
import { CURVE_STYLE_BEZIER } from '../src/gpu/store/curve-index.mjs';
...
function addFiller( store, count ){
  for( let i = 0; i < count; i++ ){ store.addNode( `x${i}`, -1000 - i, 0 ); }
}

function removeFiller( store, count ){
  for( let i = 0; i < count; i++ ){ removeNodeCascade( store, `x${i}` ); }
}

const label = ( text ) => ( {
  text, fontSize: 12, color: 0xff000000, anchorY: 0, marginX: 0, marginY: 0
} );
...
describe( 'gpu/store: compaction of dependent indexes (19.2)', function(){
  ...
  it( 'remaps the compound hierarchy: links, depth, order, styles', function(){
    ...
    expect( Array.from( store.childrenOf( pRef.slot ) ).sort() )
      .to.eql( [ aRef.slot, store.lookup( 'b' ).slot, qRef.slot ].sort() );
    ...
    expect( store.column( 'node.size' )[ pRef.slot * 2 ] ).to.be.greaterThan( pWBefore );
  } );
  ...
  it( 'keeps derived curve params byte-identical (monotone bundles + loops)', function(){
    ...
    for( const id of [ 'e1', 'e2', 'e3', 'loop' ] ){
      ...
      expect( store.curveStyleAt( store.lookup( id ).slot ).style ).to.equal( CURVE_STYLE_BEZIER );
    }
