...
import { deserializeElements, serializeElements } from '../src/gpu/wire.mjs';
...
describe('gpu/wire: compound parent sections (round 14.8)', function(){
  ...
  it('toColumnarElements lifts def parents into the parent column', function(){
    ...
    expect( Array.from( columnar.nodes.parent ) ).to.deep.equal( [ NO_PARENT, 0, 0, NO_PARENT ] );
  ...
  it('throws on an out-of-range parent index', function(){
    expect( () => cytoscapeGpu( { elements: {
      columnar: true,
      nodes: { count: 2, ids: [ 'x', 'y' ], parent: new Uint32Array( [ NO_PARENT, 7 ] ) }
    } } ) ).to.throw( /parent/ );
  });
  ...
  it('round-trips parents through the wire format', function(){
    ...
    expect( Array.from( decoded.nodes.parent ) ).to.deep.equal( [ NO_PARENT, 0, 0, NO_PARENT ] );

    const cy = cytoscapeGpu( { elements: buffer } );
  ...
  it('round-trips a live compound graph through cy.serialize()', function(){
    const cy = cytoscapeGpu( { elements: DEFS } );

    cy.$id( 'c' ).move( { parent: 'p' } );
    cy.$id( 'a' ).select();

    const cy2 = cytoscapeGpu( { elements: cy.serialize() } );
  ...
  it('keeps compound-free buffers unchanged and readable as version 2', function(){
    const flat = { nodes: [ { data: { id: 'x' }, position: { x: 5, y: 6 } } ], edges: [] };
