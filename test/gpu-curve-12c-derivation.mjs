...
import {
  CURVE_BEZIER, CURVE_HAS_ENDPT, CURVE_HAYSTACK, CURVE_MULTI, CURVE_TAXI, CURVE_TRIANGLE,
  FLAG_CURVED, FLAG_CURVED_BOX
} from '../src/gpu/contract.mjs';
import {
  EDGE_DIST_INTERSECTION, EDGE_DIST_ENDPOINTS,
  ENDPT_ANGLE, ENDPT_DEFAULT, ENDPT_POINT
...
describe('gpu/curve-12c: haystack, triangle and endpoint derivation', function(){
  ...
  var makeCy = ( elements, edgeStyle ) => cytoscapeGpu({
    elements,
  ...
  describe('haystack', function(){
    ...
    it('suppresses arrows (stored-truth reads none — a recorded deviation)', function(){
      cy = pairWith({
        ...
        'target-arrow-shape': 'triangle', 'target-arrow-color': '#f00'
      });
    ...
    it('throws on out-of-range haystack-radius', function(){
      expect( () => pairWith({ 'curve-style': 'haystack', 'haystack-radius': 1.5 }) ).to.throw();
    });
    ...
    it('box selection tests the haystack offset points', function(){
      ...
      var inBox = cy.elementsInBox(
        Math.min( pts.sx, pts.tx ) - 1, Math.min( pts.sy, pts.ty ) - 1,
        Math.max( pts.sx, pts.tx ) + 1, Math.max( pts.sy, pts.ty ) + 1 );
      ...
      var containsBoth = ( x, y ) => x >= -half && x <= 100 + half && y >= -half && y <= half;
      var expected = containsBoth( pts.sx, pts.sy ) && containsBoth( pts.tx, pts.ty ) ? 1 : 0;
  ...
  describe('manual endpoints', function(){
    it('a straight edge with endpoints derives the MULTI n=0 chord record', function(){
      cy = pairWith({ 'source-endpoint': '10 -5', 'target-distance-from-node': 4 });
    ...
    it('taxi keeps distances but drops manual endpoint modes (v3 override)', function(){
      cy = pairWith({
        'curve-style': 'taxi',
        'source-endpoint': '10 10', 'source-distance-from-node': 6
      });
    ...
    it('edge-distances: endpoints holds only with both ends manual', function(){
      cy = pairWith({
        'curve-style': 'unbundled-bezier', 'control-point-distances': [ 20 ],
        'edge-distances': 'endpoints',
        'source-endpoint': '0 10', 'target-endpoint': '90deg'
      });
