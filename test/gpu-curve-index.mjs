...
import { CURVE_BEZIER, CURVE_LOOP, CURVE_STRAIGHT, FLAG_CURVED } from '../src/gpu/contract.mjs';
...
describe('gpu/curve-index', function(){
  ...
  describe('bundled bezier derivation', function(){
    ...
    it('uses the per-edge step size', function(){
      cy = makeCy( pairGraph(2), { 'curve-style': 'bezier', 'control-point-step-size': 60 } );
  ...
  describe('self-loops', function(){
    ...
    it('parses loop-direction/loop-sweep deg strings', function(){
      cy = makeCy( loopGraph(1), { 'loop-direction': '90deg', 'loop-sweep': '-60deg' } );
  ...
  describe('style prop surface', function(){
    it('throws on unsupported curve-style keywords', function(){
      expect( () => makeCy( pairGraph(1), { 'curve-style': 'unbundled-bezier' } ) )
        .to.throw(/curve-style 'unbundled-bezier' is unsupported/);
    });

    it('throws on invalid angles', function(){
      expect( () => makeCy( pairGraph(1), { 'loop-direction': 'sideways' } ) )
        .to.throw(/not a valid angle/);
    });

    it('throws for curve props on the nodes group', function(){
      expect( () => cytoscapeGpu({ style: { nodes: { 'curve-style': 'bezier' } } }) )
        .to.throw(/'curve-style' is an edge style property/);
      expect( () => cytoscapeGpu({ style: { nodes: { 'loop-sweep': 1 } } }) )
    ...
    it('supports numeric mappers for step size', function(){
      ...
      cy = makeCy( elements, {
        'curve-style': 'bezier',
        'control-point-step-size': { data: 'w', domain: [ 0, 10 ], range: [ 0, 80 ] }
      } );
