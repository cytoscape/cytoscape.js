...
import {
  CURVE_SEGS, MAX_CURVE_PTS,
  EDGE_DIST_INTERSECTION, EDGE_DIST_NODE_POSITION,
  TAXI_AUTO, TAXI_VERTICAL, TAXI_DOWNWARD,
  emptyCurveRoute, evalRoute, routeVertex, flattenRoute, routeMidpoint,
  routePieceCount, quadPiece, computeCorner, emptyRouteCorner, arcSweep
} from '../src/gpu/curve-geometry.mjs';
import { CURVE_MULTI, CURVE_SEGMENTS, CURVE_TAXI, SHAPE_CIRCLE } from '../src/gpu/contract.mjs';
import { getRoundCorner } from '../src/round.mjs';
...
const taxiBlob = ( over = {} ) => {
  const rec = {
    dir: TAXI_AUTO, turn: 0.5, pct: 1, minD: 10,
    body: EDGE_DIST_INTERSECTION, round: 0, radius: 0, arc: 1,
    ...over
  };

  return [ rec.dir, rec.turn, rec.pct, rec.minD, rec.body, rec.round, rec.radius, rec.arc ];
};
...
describe('gpu/curve-routes: 12b route geometry', function(){
  ...
  describe('computeCorner (v3 getRoundCorner twin)', function(){
    const triples = [
      ...
      // acute
      [ { x: 0, y: 0 }, { x: 100, y: 0 }, { x: 20, y: 30 } ],
      // obtuse, other winding
      [ { x: 0, y: 50 }, { x: 80, y: 0 }, { x: 160, y: 60 } ],
      // short legs (limit clamp engages)
      [ { x: 0, y: 0 }, { x: 12, y: 0 }, { x: 12, y: 14 } ]
    ];
        ...
        it(`matches v3 exactly (${isArc ? 'arc' : 'influence'}-radius, r=${radius})`, function(){
          for( const [ p, c, n ] of triples ){
            ...
            const out = computeCorner(
              emptyRouteCorner(), p.x, p.y, c.x, c.y, n.x, n.y, radius, isArc );
    ...
    it('collapses collinear corners to the point, radius 0', function(){
      const v3 = getRoundCorner( { x: 0, y: 0 }, { x: 50, y: 0 }, { x: 100, y: 0 }, 10, true );
      const out = computeCorner( emptyRouteCorner(), 0, 0, 50, 0, 100, 0, 10, true );
    ...
    it('lands every piece boundary exactly on a subdivision index', function(){
      ...
      for( let P = 1; P <= 23; P++ ){
        ...
        for( let idx = 0; idx < CURVE_SEGS; idx++ ){
          ...
          if( out.t === 0 ){ starts.add( out.piece ); }
          lastPiece = out.piece;
        }
  ...
  describe('unbundled bezier (CURVE_MULTI)', function(){
    ...
    it('edge-distances node-position measures between centers with the intersection normal', function(){
      ...
      const inter = evalWith( CURVE_MULTI, [ EDGE_DIST_INTERSECTION, 40, 0.5 ], 1, nodes );
      const posMode = evalWith( CURVE_MULTI, [ EDGE_DIST_NODE_POSITION, 40, 0.5 ], 1, nodes );
    ...
    it('n=2 splines through the inserted midpoint (C1), boundary lands on an index', function(){
      const route = evalWith(
        CURVE_MULTI, [ EDGE_DIST_INTERSECTION, 40, 0.25, -40, 0.75 ], 2 );
  ...
  describe('segments (CURVE_SEGMENTS)', function(){
    ...
    it('midpoint: odd sharp counts take the middle point, tangent into it', function(){
      const route = evalWith(
        CURVE_SEGMENTS, [ EDGE_DIST_INTERSECTION, 0, 30, 0.5, 0, 0 ], 1 );
      const m = { x: 0, y: 0, tx: 0, ty: 0 };
    ...
    describe('round segments', function(){
      ...
      it('arc pieces sample the corner circle exactly', function(){
        ...
        const corner = computeCorner(
          emptyRouteCorner(),
          route.qx[ 0 ], route.qy[ 0 ], route.qx[ 1 ], route.qy[ 1 ],
          route.qx[ 2 ], route.qy[ 2 ], 8, true );

        for( let idx = 0; idx <= CURVE_SEGS; idx++ ){
          quadPiece( routePieceCount( route ), CURVE_SEGS, idx, out );

          if( out.piece === 1 ){ // the first corner's arc
      ...
      it('odd round midpoints sit on the arc apex with the arc tangent', function(){
        const route = evalWith(
          CURVE_SEGMENTS, [ EDGE_DIST_INTERSECTION, 1, 30, 0.5, 8, 1 ], 1 );
        const m = { x: 0, y: 0, tx: 0, ty: 0 };
        const corner = computeCorner(
          emptyRouteCorner(),
          route.qx[ 0 ], route.qy[ 0 ], route.qx[ 1 ], route.qy[ 1 ],
  ...
  describe('taxi (CURVE_TAXI)', function(){
    ...
    const nodes = ( sx, sy, tx, ty ) => [ sx, sy, 15, 15, SHAPE_CIRCLE, tx, ty, 15, 15, SHAPE_CIRCLE ];
    ...
    it('pixel turns offset from the source side', function(){
      const route = evalWith(
        CURVE_TAXI, taxiBlob( { turn: 20, pct: 0 } ), 0, nodes( 0, 0, 10, 200 ) );
    ...
    it('far-off fallbacks take the L-shape', function(){
      const route = evalWith(
        CURVE_TAXI, taxiBlob( { dir: TAXI_VERTICAL, turn: 5, pct: 0 } ),
        0, nodes( 0, 0, 100, 200 ) );
    ...
    it('explicit downward against the delta forces the direction (v3 growth)', function(){
      const route = evalWith(
        CURVE_TAXI, taxiBlob( { dir: TAXI_DOWNWARD, turn: 20, pct: 0 } ),
        0, nodes( 0, 0, 0, -200 ) );
    ...
    it('round-taxi fills radius and arc mode per point', function(){
      const route = evalWith(
        CURVE_TAXI, taxiBlob( { round: 1, radius: 12 } ), 0, nodes( 0, 0, 10, 200 ) );
  ...
  describe('flattenRoute', function(){
    it('starts and ends on the boundary endpoints', function(){
      const route = evalWith(
        CURVE_SEGMENTS,
        [ EDGE_DIST_INTERSECTION, 0, 30, 0.25, 0, 0, 30, 0.75, 0, 0 ], 2 );
    ...
    it('arcSweep follows the canvas arc direction', function(){
      ...
      expect( arcSweep( 0, Math.PI / 2, true ) ).to.be.closeTo( -3 * Math.PI / 2, 1e-12 );
