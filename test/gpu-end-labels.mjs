...
describe('gpu/style: source/target labels (round 13 D4)', function(){
  ...
  it('parses and reads back the ten props', function(){
    cy = makeCy({
      'source-label': 'from', 'source-text-offset': 20,
      'source-text-margin-x': 2, 'source-text-margin-y': -3,
      'source-text-rotation': 'autorotate',
      'target-label': 'to', 'target-text-offset': 30,
      'target-text-margin-x': -1, 'target-text-margin-y': 4,
  ...
  it('writes the end streams: entries with endOffset, main stream untouched', function(){
    cy = makeCy({ 'source-label': 'S', 'source-text-offset': 25, 'target-label': 'T' });
  ...
  it('shares the text style channels with the main label', function(){
    cy = makeCy({
      'source-label': 'S', 'font-size': 20,
      'text-background-color': '#fe0', 'text-background-opacity': 1,
      'text-transform': 'uppercase', 'source-text-margin-y': 6
    });
  ...
  it('throws on the nodes group', function(){
    expect( () => cytoscapeGpu({ style: { nodes: { 'source-label': 'x' } } }) )
      .to.throw( /edge style property/ );
    expect( () => cytoscapeGpu({ style: { nodes: { 'target-text-offset': 5 } } }) )
      .to.throw( /edge style property/ );
  });
  ...
  it('offset takes mappers; edge removal clears the streams', function(){
    cy = cytoscapeGpu({
      ...
      style: { edges: {
        'source-label': 'S',
        'source-text-offset': { data: 'off', scale: 'linear', domain: [ 0, 40 ], range: [ 0, 40 ] }
      } }
    });
