...
import { ForceSim, seedPositions, defaultForceParams } from '../src/gpu/layout/force-sim.mjs';
...
const mkSim = ( n, edges, over = {} ) => {
  ...
  const edgeLength = new Float32Array( edges.length / 2 ).fill( over.edgeLength ?? 60 );

  return new ForceSim( {
    n, edges, edgeLength, positions,
    ...defaultForceParams(),
    ...over
  } );
};
...
describe('gpu/layout: the force reference sim (round 18.1)', function(){
  ...
  it('seeds deterministic scatter', function(){
    ...
    for( let i = 0; i < 10; i++ ){ seen.add( `${a[ i * 2 ]},${a[ i * 2 + 1 ]}` ); }
  ...
  it('relaxes springs toward the ideal edge length', function(){
    ...
    const d = Math.hypot( positions[ 2 ] - positions[ 0 ], positions[ 3 ] - positions[ 1 ] );
  ...
  it('keeps disconnected components in frame via gravity', function(){
    ...
    for( let i = 0; i < 20; i++ ){
      expect( Math.hypot( sim.positions[ i * 2 ], sim.positions[ i * 2 + 1 ] ) )
        .to.be.lessThan( 3000 );
    }
  });
