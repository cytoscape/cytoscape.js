...
describe('gpu/ghost: ghost props (round 13 A1)', function(){
  ...
  var makeCy = ( nodeStyle, elements ) => cytoscapeGpu({
  ...
  it('parses and reads back the four ghost props', function(){
    cy = makeCy({
      'ghost': 'yes', 'ghost-offset-x': 12, 'ghost-offset-y': -8, 'ghost-opacity': 0.4
    });
  ...
  it('throws for ghost props on the edges group', function(){
    expect( () => cytoscapeGpu({ style: { edges: { 'ghost': 'yes' } } }) )
  ...
  it('an enabled ghost grows the store and collection bounding boxes', function(){
    ...
    cy.style({ nodes: { 'ghost': 'yes', 'ghost-offset-x': 40, 'ghost-opacity': 0.5 } });
  ...
  it('ghost props take mappers (case on data)', function(){
    cy = cytoscapeGpu({
      ...
      style: { nodes: {
        'ghost': { case: [ { when: { data: 'haunted', eq: 1 }, then: 'yes' } ], else: 'no' },
        'ghost-offset-x': 20,
        'ghost-opacity': { data: 'haunted', scale: 'linear', domain: [ 0, 1 ], range: [ 0, 0.6 ] }
      } }
    });
