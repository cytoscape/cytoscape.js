...
import { TierAllocator, packImageTableRow, IMAGE_MAX_LAYERS } from '../src/gpu/render/image-arrays.mjs';
import { IMAGE_READY } from '../src/gpu/image-registry.mjs';
...
describe('gpu/render: image tier arrays (round 15.3)', function(){
  ...
  it('caps at the WebGPU base layer limit with a warning', function(){
    ...
    try {
      ...
      for( let i = 0; i < IMAGE_MAX_LAYERS + 8; i++ ){
        if( alloc.alloc( i, 0 ) == null ){ failed++; }
      }
  ...
  it('packs image table rows', function(){
    const row = packImageTableRow(
      { status: IMAGE_READY, width: 300, height: 200 }, 1, 7, 300, 200 );
