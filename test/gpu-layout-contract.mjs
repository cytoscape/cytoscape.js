...
const mk = ( elements, opts = {} ) => cytoscapeGpu( {
  elements: elements ?? [
...
describe('gpu/layout: the extension contract (round 17.5)', function(){
  ...
  it('runs an object impl over the columnar context', async function(){
    ...
    const layout = cy.layout( { impl: {
      run( ctx ){
        ...
        ctx.nodeSlots().forEach( ( slot, i ) => { xy.push( i * 100, 42 ); } );
  ...
  it('drives the discrete finisher with one lifecycle', async function(){
    ...
    const layout = cy.layout( {
      impl: { run( ctx ){ ctx.layoutPositions( ( node, i ) => ( { x: i * 10, y: 5 } ) ); } },
  ...
  it('supports async run (the GPU-layout shape)', async function(){
    ...
    const layout = cy.layout( { impl: {
      async run( ctx ){
        ...
        ctx.setPositions( ctx.nodeSlots(), ctx.nodeSlots().flatMap( () => [ 1, 2 ] ) );
      }
    } } ).run();
  ...
  it('scopes to the collection via eles.layout', async function(){
    ...
    const layout = scope.layout( { impl: {
      run( ctx ){
        slots = [ ...ctx.nodeSlots() ];
