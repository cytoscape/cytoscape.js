...
describe('gpu/opacity-split (round 13 B1)', function(){
  ...
  var makeCy = ( style, extraEles ) => cytoscapeGpu({
    elements: [
  ...
  it('background-opacity folds into the stored fill alpha', function(){
    cy = makeCy({ nodes: { 'background-color': '#ff0000', 'background-opacity': 0.5 } });
  ...
  it('border-opacity folds into the stored border alpha', function(){
    cy = makeCy({ nodes: { 'border-width': 4, 'border-color': '#00f', 'border-opacity': 0.25 } });
  ...
  it('line-opacity folds into the line and arrow alphas (v3 effective opacities)', function(){
    cy = makeCy({ edges: {
      'line-opacity': 0.5, 'target-arrow-shape': 'triangle', 'target-arrow-color': '#0f0'
    } });
    ...
    var cy2 = makeCy({ edges: { 'line-opacity': 0, 'target-arrow-shape': 'triangle' } });
    var keep = cy;
  ...
  it('text-opacity folds into the label text/outline/background alphas', function(){
    cy = makeCy({ nodes: {
      'label': 'hi', 'color': '#000', 'text-opacity': 0.5,
      'text-outline-width': 2, 'text-outline-color': '#f00', 'text-outline-opacity': 1,
      'text-background-color': '#0f0', 'text-background-opacity': 1
    } });
  ...
  it('the channel opacities take (CPU) mappers', function(){
    cy = cytoscapeGpu({
      ...
      style: { nodes: {
        'background-opacity': { data: 'dim', scale: 'linear', domain: [ 0, 1 ], range: [ 1, 0.2 ] }
      } }
    });
  ...
  it('a non-1 channel opacity demotes that color channel off the GPU kernel', function(){
    cy = makeCy({ nodes: {
      'background-color': { data: 'x', scale: 'linear', domain: [ 0, 1 ], range: [ '#000', '#fff' ] },
      'background-opacity': 0.5
    } });

    expect( cy._styleEngine.paintInputs('nodes').length ).to.equal( 0 );

    var cy2 = makeCy({ nodes: {
      'background-color': { data: 'x', scale: 'linear', domain: [ 0, 1 ], range: [ '#000', '#fff' ] }
    } });
    var keep = cy;
  ...
  it('validates the [0, 1] range', function(){
    expect( () => makeCy({ nodes: { 'background-opacity': 1.5 } }) ).to.throw( /within \[0, 1\]/ );
    expect( () => makeCy({ edges: { 'line-opacity': -0.1 } }) ).to.throw( /within \[0, 1\]/ );
  });
