...
const make = ( style ) => cytoscapeGpu( { elements: ELEMENTS, ...( style != null ? { style } : {} ) } );
...
describe('gpu/style: parents sheet group (round 14.6)', function(){
  ...
  it('applies sheet compound props: padding, % padding, min sizes', function(){
    ...
    const cy2 = make( { parents: { padding: '10%', 'padding-relative-to': 'height' } } );
    ...
    const cy3 = make( { parents: { padding: 0, minWidth: 300, 'min-height': 100 } } );
    const p3 = cy3.$id( 'p' );
  ...
  it('accepts compound-sizing-wrt-labels: exclude; include throws (labels are not in bb)', function(){
    ...
    expect( () => make( { parents: { 'compound-sizing-wrt-labels': 'include' } } ) )
      .to.throw( /include/ );
    expect( () => make( { parents: { 'compound-sizing-wrt-labels': 'nope' } } ) )
      .to.throw();
  });
  ...
  it('rejects compound props outside the parents group, and mappers on them', function(){
    ...
    expect( () => make( { parents: { padding: { data: 'pad' } } } ) ).to.throw( /constant/i );
  });
