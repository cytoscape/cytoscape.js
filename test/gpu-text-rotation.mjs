...
const makeCy = ( nodes = {}, edges = {} ) => cytoscapeGpu( {
  elements: GRAPH,
  style: {
...
describe('gpu/style: numeric text-rotation (round 27.7)', function(){
  ...
  describe('numeric rotations', function(){
    ...
    it('are accepted on node labels — the mode v4 had no path for', function(){
      ...
      expect( cy.$id( 'a' ).style( 'text-rotation' ) ).to.be.closeTo( Math.PI / 4, 1e-6 );
      cy.destroy();
    });
  ...
  describe('the end-label streams', function(){
    ...
    it('take numeric rotations too', function(){
      const cy = cytoscapeGpu( {
        elements: GRAPH,
        style: { edges: {
          'source-label': 'S', 'source-text-rotation': 0.4,
          'target-label': 'T', 'target-text-rotation': 'autorotate',
          'font-size': 12
        } }
      } );
      const e = cy.$id( 'e' );
  ...
  describe('validation', function(){
    ...
    it('rejects a non-numeric, non-keyword value', function(){
      expect( () => makeCy( {}, { 'text-rotation': 'sideways' } ) )
        .to.throw( /text-rotation/ );
    });

    it('rejects a non-finite number', function(){
      expect( () => makeCy( {}, { 'text-rotation': Infinity } ) )
        .to.throw( /text-rotation/ );
    });
  ...
  describe('mappers', function(){
    ...
    it('drive the rotation from data', function(){
      const cy = cytoscapeGpu( {
        ...
        style: { nodes: { label: 'x', 'font-size': 12, 'text-rotation': { data: 'turn' } } }
      } );
