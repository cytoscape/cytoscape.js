...
describe('gpu/style: the visibility prop (round 22)', function(){
  ...
  it('parses, reads back and defaults to visible on both groups', function(){
    ...
    expect( () => cytoscapeGpu( { style: { nodes: { visibility: 'translucent' } } } ) ).to.throw();
  });

  it('visible()/hidden() fold visibility; takesUpSpace() stays the display tier', function(){
    var cy = cytoscapeGpu({
      elements: [ { data: { id: 'a', kind: 'x' } }, { data: { id: 'b', kind: 'y' } } ],
      style: {
        nodes: { visibility: { case: [ { when: { data: 'kind', eq: 'x' }, then: 'hidden' } ], else: 'visible' } }
      }
    });
  ...
  it('gates by ancestor: an invisible parent blanks its subtree', function(){
    var cy = cytoscapeGpu({
      ...
      style: {
        nodes: { visibility: { case: [ { when: { data: 'kind', eq: 'parent' }, then: 'hidden' } ], else: 'visible' } }
      }
    });
  ...
  it('an edge is invisible with an invisible endpoint (v3 fold)', function(){
    var cy = cytoscapeGpu({
      elements: [
        { data: { id: 'a', kind: 'ghosty' } }, { data: { id: 'b' } },
        { data: { id: 'e', source: 'a', target: 'b' } }
      ],
      style: {
        nodes: { visibility: { case: [ { when: { data: 'kind', eq: 'ghosty' }, then: 'hidden' } ], else: 'visible' } }
      }
    });
  ...
  it('invisible elements keep their space: bb/fit and compound auto-bounds', function(){
    var cy = cytoscapeGpu({
      ...
      style: {
        nodes: {
          width: 30, height: 30,
          visibility: { case: [ { when: { data: 'kind', eq: 'ghosty' }, then: 'hidden' } ], else: 'visible' }
        }
      }
    });
    ...
    var cy2 = cytoscapeGpu({
      elements: [
        ...
        { data: { id: 'c2', parent: 'p', kind: 'ghosty' }, position: { x: 200, y: 0 } }
      ],
      style: {
        nodes: {
          width: 30, height: 30,
  ...
  it('the CPU pick skips invisible nodes', function(){
    var cy = cytoscapeGpu({
      elements: [ { data: { id: 'a', kind: 'ghosty' }, position: { x: 0, y: 0 } } ],
      style: {
        nodes: {
  ...
  it('elementsInBox stays geometric: invisible elements are still inside', function(){
    var cy = cytoscapeGpu({
      ...
      style: { nodes: { width: 20, height: 20, visibility: 'hidden' }, edges: { visibility: 'hidden' } }
    });
...
describe('gpu/curves: bundles under display vs visibility (round 22.3)', function(){
  ...
  it('an invisible bundle member keeps every rank stable', function(){
    var cy = makeBundle( {
      edges: {
        'curve-style': 'bezier',
        visibility: { case: [ { when: { data: 'kind', eq: 'mid' }, then: 'hidden' } ], else: 'visible' }
      }
    } );
    ...
    expect( cy.$id( 'e1' ).controlPoints()[ 0 ].y )
      .to.be.closeTo( cyRef.$id( 'e1' ).controlPoints()[ 0 ].y, 1e-4 );
    expect( cy.$id( 'e2' ).controlPoints() ).to.equal( undefined );
  });
