...
describe('gpu/gradients (round 13 C2)', function () {
  ...
  it('stores a linear background gradient and reads it back', function () {
    ...
    expect(n.style('background-gradient-stop-colors')).to.equal(
  ...
  it('explicit positions clamp monotone and read back as percents', function () {
    ...
    expect(cy.$id('a').style('background-gradient-stop-positions')).to.equal(
      '10% 10% 90%',
    );
  });
  ...
  it('validates keywords', function () {
    expect(() => makeCy({ 'background-fill': 'conic-gradient' })).to.throw(
      /background-fill/,
    );
    expect(() =>
      makeCy({ 'background-gradient-direction': 'sideways' }),
    ).to.throw(/direction/);
  });
  ...
  it('rejects a non-numeric stop position on either fill', function () {
    expect(() =>
      ...
      /'half' is not a valid percent for 'background-gradient-stop-positions'/,
    );
