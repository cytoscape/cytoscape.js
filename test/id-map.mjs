...
describe('gpu/store: id map', function () {
  ...
  it('survives growth and rehash with removals mixed in', function () {
    ...
    for (let i = 0; i < N; i++) {
      ids.set('id-' + i, i % 2 === 0 ? 'nodes' : 'edges', i);
    }
    for (let i = 0; i < N; i += 3) {
      ids.remove('id-' + i);
    }
    for (let i = 0; i < N; i += 3) {
      ids.set('id-' + i, 'nodes', N + i);
    } // tombstone reuse
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (i % 3 === 0) {
        ...
        expect(entry, 'id-' + i).to.deep.equal({
          group: i % 2 === 0 ? 'nodes' : 'edges',
          slot: i,
        });
      }
    }
  ...
  describe('blob compaction', function () {
    it('keeps the blob bounded under add/remove churn', function () {
      ...
      expect(ids.blobBytes < 4096 || ids.wasteBytes * 2 <= ids.blobBytes).to.be
        .true;
    });
    ...
    it('keeps live ids resolvable and decodable across a compaction', function () {
      ...
      for (let i = 0; i < N; i++) {
        ids.set('element-' + i, i % 2 === 0 ? 'nodes' : 'edges', i);
      }
      ...
      for (let i = 0; i < N; i++) {
        if (i % 5 === 0) {
          expect(ids.get('element-' + i), 'element-' + i).to.deep.equal({
            group: i % 2 === 0 ? 'nodes' : 'edges',
            slot: i,
          });
          expect(ids.idAt(i % 2 === 0 ? 'nodes' : 'edges', i)).to.equal(
            'element-' + i,
          );
        } else {
      ...
      expect(ids.get('element-1')).to.deep.equal({
        group: 'nodes',
        slot: 2000,
      });
    });
    ...
    it('carries non-ASCII ids through a compaction', function () {
      ...
      for (let i = 0; i < 600; i += 4) {
        expect(ids.get('nöde-🚀-' + i)).to.deep.equal({
          group: 'nodes',
          slot: i,
        });
