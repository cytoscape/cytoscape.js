...
describe('gpu/collection: label bounding boxes (round 16.4)', function () {
  ...
  it('grows the label term by the text-background padding', function () {
    const plain = mk({
      nodes: { width: 30, height: 30, label: 'x', 'font-size': 16 },
    });
    const padded = mk({
      nodes: {
        width: 30,
    ...
    expect(padded.$id('a').boundingBox().y2).to.be.greaterThan(
      plain.$id('a').boundingBox().y2 + 5,
    );
  });
  ...
  it('throws on unknown bb option keys', function () {
    ...
    expect(() => cy.$id('a').boundingBox({ includeLabls: true })).to.throw(
      /includeLabls/,
    );
  });
  ...
  it('multiline labels deepen the box', function () {
    const single = mk({
      nodes: { width: 30, height: 30, label: 'aa bb cc dd', 'font-size': 16 },
    });
    const wrapped = mk({
      nodes: {
        width: 30,
    ...
    expect(wrapped.$id('a').boundingBox().y2).to.be.greaterThan(
      single.$id('a').boundingBox().y2 + 16,
    );
  });
    ...
    const cy = mk(
      {
        ...
        edges: { label: 'edge label', 'font-size': 14 },
      },
      elements,
    );
  ...
  it('drives the whole-graph fit box (the store scan)', function () {
    const cy = mk({
      nodes: {
        width: 30,
        height: 30,
        label: 'a rather long label text',
        'font-size': 16,
      },
    });
  ...
  it('measures labels exactly via labelBoundingBox()', function () {
    const cy = mk({
      nodes: { width: 30, height: 30, label: 'measured', 'font-size': 16 },
    });
  ...
  it('gates box selection on label containment when opted in (16.5)', function () {
    ...
    const off = cytoscape({ elements, style });
    ...
    const on = cytoscape({ elements, style, boxSelectionIncludesLabels: true });
