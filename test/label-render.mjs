...
import { layoutLabel } from '../src/render/label-layout.mjs';
import { computeSdf, SDF_RADIUS } from '../src/render/glyph-atlas.mjs';
import {
  GlyphBuffer,
  GLYPH_WORDS,
  DEAD_GLYPH,
...
describe('gpu/labels: render pieces', function () {
  ...
  describe('GlyphBuffer', function () {
    ...
    it('rewrites a same-count replacement in place (25.5: no growth under a tween)', function () {
      ...
      mock.device.queue.writeBuffer = function (
        buf,
        offset,
        data,
        dataOffset,
    ...
    it('reallocates the GPU buffer on growth with a version bump', async function () {
      ...
      var full = mock.writes.find(
        (w) => w.buf === buffer.buffer() && w.offset === 0,
      );
    ...
    it('rejects glyph data that is not a whole number of glyphs', function () {
      expect(() => buffer.set(0, new Uint32Array(GLYPH_WORDS + 1))).to.throw(
        /whole number of glyphs/,
      );
