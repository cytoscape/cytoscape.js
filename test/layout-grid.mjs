...
describe('gpu/layout: grid', function () {
  ...
  it('respects the bounding box', function () {
    cy.layout({
      name: 'grid',
      fit: false,
      boundingBox: { x1: 100, y1: 200, w: 100, h: 100 },
    }).run();
  ...
  it('supports sorting', function () {
    ...
    var byX = cy
      .nodes()
      .toArray()
      .sort((m, n) => m.position().x - n.position().x)
      .map((n) => n.id());
  ...
  it('writes positions as one coalesced dirty span', function () {
    ...
    var spans = cy._store
      .takeDelta()
      .spans.filter((s) => s.column === 'node.position');
  ...
  it('slot path matches the handle path position for position', function () {
    var mk = () =>
      cytoscape({
        ...
        style: {
          nodes: {
            width: {
              case: [{ when: { data: 'id', eq: 'c' }, then: 90 }],
              else: 30,
            },
            height: {
              case: [{ when: { data: 'id', eq: 'c' }, then: 70 }],
              else: 30,
            },
            'border-width': {
              case: [{ when: { data: 'id', eq: 'c' }, then: 4 }],
              else: 0,
            },
          },
        },
    ...
    for (var id of ['a', 'b', 'c', 'd', 'e']) {
      expect(bySlot.$id(id).position(), id).to.deep.equal(
        byHandle.$id(id).position(),
      );
    }
  });
