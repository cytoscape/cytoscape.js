...
const input = (spec, opts, fallback = 1) => ({
  m: compileMapper(spec, opts),
  fallback,
});
...
describe('gpu/mapper-pack', function () {
  it('packs a linear scalar program with exact word offsets', function () {
    const packed = packPrograms(
      ...
      [
        input(
          { data: 'w', domain: [2, 10], range: [0.2, 1] },
          NUM_OPACITY,
          0.5,
        ),
      ],
  ...
  it('packs OKLab color stops (and sRGB under interpolate: srgb)', function () {
    ...
    expect(new Uint32Array(srgb.programData)[2]).to.equal(
      FLAG.COLOR | FLAG.CLAMP | FLAG.SRGB,
    );
  ...
  it('orders opacity programs first and dedupes data regions by key', function () {
    const packed = packPrograms(
      ...
      [
        input(
          { data: 'w', domain: [0, 1], range: ['#000000', '#ffffff'] },
          COLOR_BG,
          [0, 0, 0, 255],
        ),
        input({ data: 'w', domain: [0, 1], range: [0, 1] }, NUM_OPACITY),
        input(
          { data: 'z', domain: [0, 1], range: ['#000000', '#ffffff'] },
          { kind: 'color', prop: 'border-color' },
          [0, 0, 0, 255],
        ),
      ],
      dataWith([0.5]),
  ...
  it('skips non-paint props, string/mixed columns and ordinal programs', function () {
    ...
    expect(packed.programCount).to.equal(1);
    expect(
      packed.skipped.map((m) => m.prop + ':' + m.program.kind),
    ).to.deep.equal([
      'width:continuous',
      'opacity:continuous',
      'opacity:continuous',
  ...
  it('flags discrete color outputs as sRGB', function () {
    ...
    expect(new Uint32Array(packed.programData)[2]).to.equal(
      FLAG.COLOR | FLAG.SRGB,
    );
  });
  ...
  it('synthesizes constant arrow programs when edge opacity is mapped', function () {
    const ctx = {
      ...
      source: {
        enabled: false,
        colorMapped: false,
        constColor: [0, 0, 0, 255],
      },
      target: {
        enabled: true,
        colorMapped: false,
        constColor: [255, 0, 0, 255],
      },
    };
    const packed = packPrograms(
      'edges',
      [
        input(
          { data: 'o', domain: [0, 1], range: [0, 1] },
          { kind: 'number', prop: 'opacity' },
        ),
      ],
      dataWith([0.5], 'o'),
      4,
    ...
    expect(packed.ownedColumns).to.deep.equal([
      'edge.opacity',
      'edge.targetArrow',
    ]);
  ...
  it('folds a constant edge opacity into mapped arrow colors via alphaMul', function () {
    ...
    const packed = packPrograms(
      ...
      [
        input(
          { data: 'o', domain: [0, 1], range: ['#000000', '#ffffff'] },
          { kind: 'color', prop: 'source-arrow-color' },
          [0, 0, 0, 255],
        ),
      ],
      dataWith([0.5], 'o'),
  ...
  it('packs string ordinals as dict-index LUTs with fallback holes', function () {
    ...
    expect([...packed.stopData.slice(0, 8)]).to.deep.equal([
      1, 0, 0, 1, 0, 0, 1, 1,
    ]);
  });
