...
import { MapperRuntime } from '../src/render/mapper-runtime.mjs';
...
describe('gpu/mapper-runtime', function () {
  it('configures on the engine paint version: owns columns and evaluates in full', function () {
    ...
    expect(mock.writes.some((w) => w.label === 'cy-gpu:nodes-mapper-programs'))
      .to.be.true;
    expect(mock.writes.some((w) => w.label === 'cy-gpu:nodes-mapper-values')).to
      .be.true;
    ...
    expect(pass.dispatches).to.deep.equal([
      Math.ceil(store.capacity('nodes') / 256),
    ]);
  ...
  it('turns data writes into byte uploads + a coalesced dispatch', function () {
    ...
    const valueWrite = mock.writes.find(
      (w) => w.label === 'cy-gpu:nodes-mapper-values',
    );
    ...
    const infoWrite = mock.writes.find(
      (w) => w.label === 'cy-gpu:nodes-mapper-info',
    );
  ...
  it('owns color channels and answers arrow getters with the folded IR', function () {
    ...
    engine.setSheet({
      nodes: {
        'background-color': { data: 'w', domain: [0, 1], range: 'viridis' },
      },
      edges: {
    ...
    expect(engine.readProp(edge, 'target-arrow-color')).to.equal(
      'rgba(255,0,0,0.502)',
    );
  ...
  it('repacks the ordinal LUT when a dict compaction remaps indices', function () {
    ...
    } // setup made slots 0, 1
    for (let i = 0; i < 9; i++) {
      store.setData('nodes', i, 'cat', 'c' + i);
    } // dict c0..c8
    ...
    for (let i = 0; i < 5; i++) {
      store.setData('nodes', i, 'cat', 'c5');
    }
    for (let i = 0; i < 5; i++) {
      store.setData('nodes', i, 'cat', 'x' + i);
    }
