...
describe('gpu/mappers', function () {
  ...
  describe('compile validation', function () {
    const bad = (spec, opts, re) => {
      expect(() => compileMapper(spec, opts), JSON.stringify(spec)).to.throw(
        re,
      );
    };
    ...
    it('throws on continuous scales over enum channels', function () {
      bad(
        { data: 'w', domain: [0, 1], range: ['circle', 'square'] },
        ENUM,
        /cannot be continuously interpolated/,
      );
    });

    it('throws on log domains touching zero', function () {
      bad(
        { data: 'w', scale: 'log', domain: [0, 10], range: [0, 1] },
        NUM,
        /symlog/,
      );
      bad(
        { data: 'w', scale: 'log', domain: [-1, 1], range: [0, 1] },
        NUM,
        /symlog/,
      );
    });
    ...
    it('throws on malformed domains', function () {
      ...
      bad(
        { data: 'w', domain: [0, 'x'], range: [0, 1] },
        NUM,
        /finite numbers/,
      );
      bad(
        { data: 'w', scale: 'diverging', domain: [0, 1], range: [0, 1, 2] },
        NUM,
        /min, mid, max/,
      );
    });
    ...
    it('throws on pairing violations', function () {
      ...
      bad(
        { data: 'w', scale: 'ordinal', domain: ['a', 'b'], range: [1] },
        NUM,
        /pair 1:1/,
      );
      bad(
        { data: 'w', scale: 'threshold', domain: [10], range: [1, 2, 3] },
        NUM,
        /domain.length \+ 1/,
      );
    });
    ...
    it('throws on scheme misuse', function () {
      ...
      bad(
        { data: 'w', scale: 'quantize', domain: [0, 1], range: 'viridis' },
        COLOR,
        /bins/,
      );
    });
    ...
    it('throws on unparsable range entries and fallbacks', function () {
      bad(
        { data: 'w', domain: [0, 1], range: [0, 'wide'] },
        NUM,
        /not a valid number/,
      );
      bad(
        { data: 'w', domain: [0, 1], range: ['#000', 'nocolor'] },
        COLOR,
        /not a valid color/,
      );
      bad(
        { data: 'w', scale: 'ordinal', domain: ['a'], range: ['hexagon'] },
        ENUM,
        /not a valid enum/,
      );
  ...
  describe('continuous scales', function () {
    it('linear: endpoints, midpoint, default clamp', function () {
      const ev = evaluator(
        { data: 'w', domain: [0, 10], range: [0, 100] },
        NUM,
        [0, 5, 10, 20, -3],
      );
    ...
    it('clamp: false extrapolates the end segments', function () {
      const ev = evaluator(
        { data: 'w', domain: [0, 10], range: [0, 100], clamp: false },
        NUM,
        [20, -5],
      );
    ...
    it('multi-stop: pairwise and even-spread', function () {
      const pairwise = evaluator(
        { data: 'w', domain: [0, 10, 100], range: [0, 1, 10] },
        NUM,
        [55],
      );
      const spread = evaluator(
        { data: 'w', domain: [0, 10], range: [0, 1, 10] },
        NUM,
        [7.5],
      );
    ...
    it('sqrt and pow', function () {
      const sqrt = evaluator(
        { data: 'w', scale: 'sqrt', domain: [0, 100], range: [0, 10] },
        NUM,
        [25],
      );
      const pow = evaluator(
    ...
    it('symlog crosses zero', function () {
      const ev = evaluator(
        { data: 'w', scale: 'symlog', domain: [-10, 10], range: [0, 10] },
        NUM,
        [0, -10, 10],
      );
  ...
  describe('discrete scales', function () {
    ...
    it('quantize: uniform bins from the range length', function () {
      const ev = evaluator(
        { data: 'w', scale: 'quantize', domain: [0, 10], range: [1, 2] },
        NUM,
        [4.9, 5],
      );
  ...
  describe('colors', function () {
    it('scheme endpoints are exact', function () {
      const ev = evaluator(
        { data: 'w', domain: [0, 1], range: 'viridis' },
        COLOR,
        [0, 1],
      );
    ...
    it('OKLab midpoints avoid the gray sRGB midpoint', function () {
      const oklab = evaluator(
        { data: 'w', domain: [0, 1], range: ['blue', 'yellow'] },
        COLOR,
        [0.5],
      );
      ...
      expect(
        spread,
        `oklab mid rgb(${r},${g},${b}) should not be gray`,
      ).to.be.above(30);
    });
  ...
  describe('passthrough & fallback', function () {
    ...
    it('treats non-numeric data as missing under numeric scales', function () {
      const ev = evaluator(
        { data: 'w', domain: [0, 1], range: [0, 100], fallback: -5 },
        NUM,
        ['oops'],
      );
  ...
  describe('schemes', function () {
    it('tables are well-formed', function () {
      for (const [name, scheme] of Object.entries(SCHEMES)) {
        expect(['sequential', 'diverging', 'categorical'], name).to.include(
          scheme.kind,
        );
  ...
  describe('case (conditional) mapper', function () {
    ...
    it('maps typed edges to colors and enums (the biological case)', function () {
      const color = evalCase(
        ...
        COLOR,
        [{ type: 'activation' }, { type: 'inhibition' }, { type: 'other' }],
      );
    ...
    it('falls back to the channel default when no else is given', function () {
      const ev = evalCase(
        { case: [{ when: { data: 'w', gt: 100 }, then: 5 }] },
        NUM,
        [{ w: 1 }],
      );
    ...
    it('validates clause and condition shapes', function () {
      const bad = (spec, re) =>
        expect(() => compileMapper(spec, NUM), JSON.stringify(spec)).to.throw(
          re,
        );
      ...
      bad(
        { case: [{ when: { data: 'w' }, then: 1 }] },
        /exactly one comparison/,
      );
      bad(
        { case: [{ when: { data: 'w', gt: 1, lt: 5 }, then: 1 }] },
        /exactly one comparison/,
      );
      bad(
        { case: [{ when: { data: 'w', gt: 'x' }, then: 1 }] },
        /numeric value/,
      );
      bad(
        { case: [{ when: { data: '', eq: 1 }, then: 1 }] },
        /non-empty 'data'/,
      );
      bad(
        { case: [{ when: { data: 'w', in: [] }, then: 1 }] },
        /non-empty array/,
      );
      bad(
        { case: [{ when: { data: 'w', eq: 1 }, then: 'notanumber' }] },
        /not a valid number/,
      );
    });
    ...
    it('is rejected on label (passthrough only)', function () {
      ...
      expect(() =>
        cy.style({
          nodes: {
            label: { case: [{ when: { data: 't', eq: 'x' }, then: 'hi' }] },
          },
        }),
  ...
  describe('stylesheet integration', function () {
    ...
    it('maps shapes ordinally, keeping the circle collapse', function () {
      ...
      expect(
        cy._store.column('node.shape')[cy.$id('a')._refs[0].slot],
      ).to.equal(0);
    });
    ...
    it('folds mapped edge opacity into the stored arrow alpha', function () {
      ...
      expect(cy.$id('ab').style('target-arrow-color')).to.match(
        /^rgba\(255,0,0,0\.50/,
      );
    });
    ...
    it('removeData reverts to the fallback', function () {
      const cy = graph({
        nodes: {
          width: { data: 'w', domain: [0, 10], range: [10, 110], fallback: 77 },
        },
      });
    ...
    it('reads mapped label channels on unlabelled nodes', function () {
      const cy = graph({
        nodes: { 'font-size': { data: 'w', domain: [0, 10], range: [10, 30] } },
      });
    ...
    it('accepts the object passthrough for labels, like the data() string', function () {
      const cy = graph({ nodes: { label: { data: 'name' } } }, [
        { data: { id: 'a', name: 'Ada' } },
      ]);
    ...
    it('keeps a paint case mapper CPU-evaluated (not GPU-owned)', function () {
      const cy = cytoscape({
        ...
        style: {
          nodes: {
            'background-color': {
              case: [{ when: { data: 't', eq: 'x' }, then: '#ff0000' }],
              else: '#000000',
            },
          },
        },
    ...
    it('reads the first-class id in conditions', function () {
      const cy = cytoscape({
        ...
        style: {
          nodes: {
            'background-color': {
              case: [{ when: { data: 'id', eq: 'a' }, then: '#00ff00' }],
              else: '#ff0000',
            },
          },
        },
    ...
    it('round-trips case sheets through json()', function () {
      const sheet = {
        nodes: {
          shape: {
            case: [{ when: { data: 't', eq: 'box' }, then: 'rectangle' }],
            else: 'ellipse',
          },
        },
      };
      const cy = cytoscape({
        elements: [{ data: { id: 'a', t: 'box' } }],
        style: sheet,
      });
    ...
    it('throws on invalid mapper placements', function () {
      expect(() =>
        graph({ nodes: { label: { data: 'w', range: [0, 1] } } }),
      ).to.throw(/passthrough/);
      expect(() =>
        graph({
          nodes: {
