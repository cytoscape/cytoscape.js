...
/**
 * Paths a documented command *produces*.  Whether they exist depends on
 * what has been run on this machine, so testing them tests the machine —
 * exactly the trap that made `benchmark/results/` pass in a fresh worktree
 * and fail in the maintainer's checkout, where a benchmark had run.  An
 * exemption by name cannot express that; a prefix can.
 */
const GENERATED = ['build/', 'dist/', 'status/', 'benchmark/results/'];

/** `.mjs` specifiers resolve to `.mts` files — the repo's convention. */
function resolves(path) {
  // a quotation carrying a literal backslash is a spelling, not a path —
  // `dist/\ncytoscape-gpu.d.ts` is round 42's line-wrapped example, and it
  // would otherwise be swallowed by the `dist/` prefix below
  if (!path.includes('\\') && GENERATED.some((p) => path.startsWith(p))) {
    return true;
  }
