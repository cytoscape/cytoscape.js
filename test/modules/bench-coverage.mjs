...
describe('gpu benchmark coverage audit', function () {
  describe('mentions()', function () {
    it('matches call-shaped and quoted uses', function () {
      expect(
        mentions('cy.elementsInBox( 1, 2, 3, 4 )', 'elementsInBox'),
      ).to.equal(true);
      expect(mentions('const f = boundingBox( x )', 'boundingBox')).to.equal(
        true,
      );
    ...
    it('does not match a bare word', function () {
      ...
      expect(mentions('the boundingBox of a node', 'boundingBox')).to.equal(
        false,
      );
      expect(
        mentions('renderedOuterHeight is untested', 'renderedOuterHeight'),
      ).to.equal(false);
    });
    ...
    it('over-detects, which is why the audit never gates', function () {
      ...
      expect(mentions('// we never call data( k ) here', 'data')).to.equal(
        true,
      );
