...
import {
  summarize, sortRuns, pruneRuns, readIndex, loadPublished, parseArgs, newestResults
...
const machine = ( over = {} ) => ( {
  cpu: { model: 'Intel(R) Core(TM) i9-9900K CPU @ 3.60GHz', physicalCores: 8, logicalCores: 16, baseGHz: 3.6, maxGHz: 5 },
  ...
  gpus: [ { vendor: 'AMD', model: 'Radeon RX 580', driver: 'amdgpu', vramBytes: 8589934592 } ],
  probe: { errors: [] },
...
const results = ( over = {} ) => ( {
  meta: {
    date: '2026-08-05T17:00:00.000Z', commit: 'abc1234', commitSubject: 'feat: a thing',
    branch: 'v4', profile: 'quick', suiteFilter: null, dirty: false, totalMs: 426000,
    machine: machine(), adapter: null, ...over
  },
...
describe( 'benchmark archive: summarize', () => {
  it( 'carries the fingerprint, which is the grouping key for a trend', () => {
    ...
    expect( summarize( results(), { file: 'r.json' } ).fingerprint ).to.equal( '2d2ea233' );
  } );
  ...
  it( 'flattens the adapter, and answers null when there was none', () => {
    ...
    const withAdapter = summarize(
      results( { adapter: { vendor: 'amd', architecture: 'gcn-4', description: '' } } ),
      { file: 'r.json' }
    );
  ...
  it( 'keeps the dirty flag, so a published run cannot hide it', () => {
    expect( summarize( results( { dirty: true } ), { file: 'r.json' } ).dirty ).to.equal( true );
  } );

  it( 'survives a results file with no machine block at all', () => {
    // every run published before round 46.5 is one of these
    const entry = summarize( { meta: { date: '2026-07-01T00:00:00.000Z', profile: 'quick' }, jobs: [] }, { file: 'old.json' } );
  ...
  it( 'records the note verbatim', () => {
    expect( summarize( results(), { file: 'r.json', note: 'round 46.5 baseline' } ).note )
      .to.equal( 'round 46.5 baseline' );
  } );
} );

describe( 'benchmark archive: ordering and prune', () => {
  const run = ( date, fingerprint, profile = 'quick' ) => ( { file: `${date}.json`, date, fingerprint, profile } );

  it( 'orders newest first', () => {
    const sorted = sortRuns( [ run( '2026-08-01' ), run( '2026-08-05' ), run( '2026-08-03' ) ] );

    expect( sorted.map( r => r.date ) ).to.eql( [ '2026-08-05', '2026-08-03', '2026-08-01' ] );
  } );
  ...
  it( 'keeps a run per machine, not n runs overall', () => {
    ...
    const { keep, drop } = pruneRuns( [
      run( '2026-08-05', 'aaa' ), run( '2026-08-04', 'aaa' ),
      run( '2026-08-03', 'bbb' ), run( '2026-08-02', 'bbb' )
    ], 1 );
...
describe( 'benchmark archive: on disk', () => {
  ...
  it( 'loads each run with its results attached', () => {
    ...
    writeFileSync( join( dir, 'index.json' ), JSON.stringify( {
      runs: [ { file: 'r1.json', date: '2026-08-05T17:00:00.000Z', profile: 'quick', fingerprint: 'aaa' } ]
    } ) );
  ...
  it( 'skips an index entry whose results file is gone', () => {
    ...
    writeFileSync( join( dir, 'index.json' ), JSON.stringify( {
      runs: [ { file: 'missing.json', date: '2026-08-05T17:00:00.000Z', profile: 'quick' } ]
    } ) );
...
describe( 'benchmark archive: argument parsing', () => {
  it( 'reads a value flag and a positional in one scan', () => {
    const { flags, positional } = parseArgs( [ '--note', 'a run', 'results.json', '--dry-run' ] );
