...
import { parseDoc, sectionTitle, factoryStatics } from '../../scripts/docs-generate.mjs';
...
const block = lines => [ '  /**', ...lines.map( l => `   *${l ? ` ${l}` : ''}` ), '   */' ].join( '\n' );
...
describe( 'docs generator: parseDoc', () => {
  ...
  it( 'splits a @param on the em dash, not on the first space', () => {
    ...
    expect( doc.params ).to.deep.equal( [
      { name: 'events', descr: 'one or more space-separated names — comma-free' }
    ] );
  } );
  ...
  it( 'joins a @param description continued on the following lines', () => {
    ...
    expect( doc.params ).to.deep.equal( [
      {
        name: 'predicateOrCb',
        descr: 'the delegation predicate when `callback` is given, otherwise the handler itself'
      },
  ...
  it( 'keeps @returns, @throws and @see as their own fields', () => {
    ...
    expect( doc.see ).to.deep.equal( [ 'Core#off — removing takes the same triple' ] );
  } );
...
describe( 'docs generator: sectionTitle', () => {
  ...
  it( 'drops a trailing round reference', () => {
    ...
    expect( sectionTitle( 'compound hierarchy (round 14.2)' ) ).to.equal( 'Compound hierarchy' );
  } );
  ...
  it( 'leaves any other parenthetical alone', () => {
    ...
    expect( sectionTitle( 'animation (viewport)' ) ).to.equal( 'Animation (viewport)' );
  } );
} );
...
describe( 'docs generator: factoryStatics', () => {
  it( 'reads the statics off the entry point rather than a hand list', () => {
    ...
    expect( [ ...statics ].sort() ).to.deep.equal(
      [ 'deserializeElements', 'serializeElements', 'toColumnarElements' ]
    );
  } );
} );
