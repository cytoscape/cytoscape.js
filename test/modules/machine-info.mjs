...
import {
  parseProcCpuinfo, parseMemTotal, parseOsRelease, parseLspciMm, parseDrmUevent,
  parseSystemProfilerDisplays, parseAppleVram, parseWindowsCpu, parseWindowsGpu,
  describeMachine, machineLine, fingerprint, fmtBytes
...
const linuxIo = ( overrides = {} ) => ( {
  read: path => {
    if( path === '/proc/cpuinfo' ){ return PROC_CPUINFO; }
    if( path === '/proc/meminfo' ){ return PROC_MEMINFO; }
    if( path === '/etc/os-release' ){ return OS_RELEASE; }
    if( path === '/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_max_freq' ){ return '5000000\n'; }
    if( path === '/sys/class/drm/card1/device/uevent' ){ return UEVENT_INTEL; }
    if( path === '/sys/class/drm/card2/device/uevent' ){ return UEVENT_AMD; }
    if( path === '/sys/class/drm/card2/device/mem_info_vram_total' ){ return '8589934592\n'; }
...
describe( 'machine-info: /proc parsers', () => {
  ...
  it( 'reads the model name', () => {
    expect( parseProcCpuinfo( PROC_CPUINFO ).model ).to.equal( 'Intel(R) Core(TM) i9-9900K CPU @ 3.60GHz' );
  } );
  ...
  it( 'reads the distro name and version', () => {
    expect( parseOsRelease( OS_RELEASE ) ).to.equal( 'Fedora Linux 43 (Workstation Edition)' );
  } );
} );
...
describe( 'machine-info: lspci', () => {
  ...
  it( 'splits vendor from device on the quoting, not on free text', () => {
    ...
    expect( amd.model ).to.equal( 'Ellesmere [Radeon RX 470/480/570/570X/580/580X/590]' );
  } );

  it( 'keeps the PCI slot, which is the join key for VRAM', () => {
    expect( parseLspciMm( LSPCI_MM ).map( g => g.slot ) ).to.eql( [ '00:02.0', '01:00.0' ] );
  } );

  it( 'ignores non-display devices and unparseable lines', () => {
    const text = '00:1f.3 "Audio device" "Intel Corporation" "Cannon Lake PCH cAVS"\ngarbage\n';
...
describe( 'machine-info: drm uevent', () => {
  it( 'strips the PCI domain so the slot matches lspci\'s short form', () => {
    ...
    expect( parseDrmUevent( UEVENT_AMD ) ).to.eql( { slot: '01:00.0', driver: 'amdgpu' } );
  } );
...
describe( 'machine-info: describeMachine on Linux', () => {
  it( 'joins VRAM onto the card it belongs to', () => {
    ...
    expect( gpus.find( g => g.driver === 'amdgpu' ).vramBytes ).to.equal( 8589934592 );
  ...
  it( 'never throws when every probe fails, and says which failed', () => {
    const io = {
      ...
      drmCards: () => { throw new Error( 'ENOENT' ); }
    };
...
describe( 'machine-info: Windows parsers', () => {
  it( 'sums cores across sockets', () => {
    const cpu = parseWindowsCpu( JSON.stringify( [
      { Name: 'Intel Xeon Gold 6248R', NumberOfCores: 24, NumberOfLogicalProcessors: 48, MaxClockSpeed: 3000 },
      { Name: 'Intel Xeon Gold 6248R', NumberOfCores: 24, NumberOfLogicalProcessors: 48, MaxClockSpeed: 3000 }
    ] ) );
  ...
  it( 'handles the single-CPU case, which CIM emits as an object not an array', () => {
    const cpu = parseWindowsCpu( JSON.stringify( { Name: 'AMD Ryzen 9 5950X', NumberOfCores: 16, NumberOfLogicalProcessors: 32 } ) );
  ...
  it( 'refuses AdapterRAM when it has saturated its signed 32-bit field', () => {
    ...
    const [ big ] = parseWindowsGpu( JSON.stringify( { Name: 'NVIDIA RTX 4090', AdapterRAM: 4294967295 } ) );

    expect( big.vramBytes ).to.equal( null );
    expect( big.vramUnreliable ).to.equal( true );

    const [ small ] = parseWindowsGpu( JSON.stringify( { Name: 'Intel UHD 630', AdapterRAM: 1073741824 } ) );
...
describe( 'machine-info: presentation', () => {
  ...
  it( 'summarizes a machine in one line', () => {
    const line = machineLine( describeMachine( { platform: 'linux', io: linuxIo() } ) );
