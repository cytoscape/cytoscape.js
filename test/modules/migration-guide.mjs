...
function propertyTableRows(){
  ...
  expect( start, 'the property-table section is gone from MIGRATING.md' ).to.be.greaterThan( -1 );
...
describe( 'the migration guide: the property table', () => {
  ...
  it( 'names only properties v4 actually rejects', () => {
    for( const row of rows ){
      ...
      for( const name of backticked( left ) ){
        ...
        expect( nameKnown( cy, name ), `MIGRATING.md says v4 dropped '${name}', but the sheet accepts it` )
          .to.equal( false );
      }
    }
  } );
  ...
  it( 'names replacements v4 actually accepts', () => {
    ...
    for( const [ , name ] of replacements ){
      expect( nameKnown( cy, name ), `MIGRATING.md offers '${name}' as a replacement, but v4 rejects it` )
        .to.equal( true );
    }
  } );
  ...
  it( 'quotes counts that add up', () => {
    ...
    const registered = Number( GUIDE.match( /registers \*\*(\d+)\*\* property names/ )[1] );
    const accepted = Number( GUIDE.match( /accepts\s*\n?\*\*(\d+)\*\* of them/ )[1] );
    ...
    expect( accepted + rejected, 'accepted + rejected must be the registered total' )
      .to.equal( registered );
    expect( numbered + remaining, 'numbered + remaining must be the rejected total' )
      .to.equal( rejected );
    expect( rows.length, 'the table should carry the remaining rejections' )
...
describe( 'the migration guide: behavioural claims', () => {
  ...
  it( 'still rejects `cose`, listing the built-ins the guide names', () => {
    ...
    for( const name of [ 'grid', 'preset', 'circle', 'concentric', 'breadthfirst', 'random', 'force' ] ){
      expect( () => cy.layout( { name } ), `${name} should be built in` ).to.not.throw();
    }
  } );
  ...
  it( 'still has no classes and no cy.$', () => {
    ...
    for( const m of [ 'addClass', 'removeClass', 'toggleClass', 'hasClass', 'flashClass' ] ){
