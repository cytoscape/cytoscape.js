...
function manifestPaths(){
  const out = new Set();
  const add = v => { if( typeof v === 'string' && v.startsWith( 'dist/' ) ) out.add( v ); };
  ...
  const walk = node => {
    if( typeof node === 'string' ){ add( node.replace( /^\.\//, '' ) ); return; }
...
describe( 'packaging: the tarball', () => {
  ...
  it( 'ships no development tree', () => {
    ...
    for( const prefix of dev ){
      ...
      expect( hits, `${prefix} must not ship (${hits.slice( 0, 3 ).join( ', ' )})` )
        .to.have.lengthOf( 0 );
    }
  } );
...
describe( 'packaging: the manifest resolves to build output', () => {
  ...
  it( 'names only files the build chain produces', async () => {
    ...
    for( const p of manifestPaths() ){
      ...
      expect( produced.has( base ), `${p} is named by package.json but no build script produces it` )
        .to.equal( true );
    }
  } );
  ...
  */
  ...
  it( 'is a conditions map keyed by relative subpaths', () => {
    ...
    for( const [ key, value ] of entries ){
      ...
      expect( value, `${key} does not map to a conditions object` ).to.be.an( 'object' );
  ...
  it( 'puts `types` first wherever it appears', () => {
    ...
    for( const [ key, value ] of subpaths() ){
      ...
      if( conditions.includes( 'types' ) ){
        expect( conditions[ 0 ], `${key}: types must be the first condition` ).to.equal( 'types' );
      }
    }
  } );
  ...
  it( 'matches import to ESM and require to CJS', () => {
    for( const [ key, value ] of subpaths() ){
      if( value.import ) expect( value.import, `${key}.import` ).to.match( /\.mjs$/ );
      if( value.require ) expect( value.require, `${key}.require` ).to.match( /\.js$/ );
  ...
  it( 'points the CDN fields at a real bundle the build produces', () => {
    for( const field of [ 'unpkg', 'jsdelivr' ] ){
      ...
      expect( distCopyFiles(), `${field} names a file dist:copy does not produce` )
