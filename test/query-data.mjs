...
describe('gpu/query: data predicates', function(){
  ...
  it('ordinal comparisons over numbers', function(){
    expect( ids( cy.nodes({ data: { weight: { gt: 0.5 } } }) ) ).to.deep.equal([ 'b', 'c' ]);
    expect( ids( cy.nodes({ data: { weight: { gte: 0.75 } } }) ) ).to.deep.equal([ 'b', 'c' ]);
    expect( ids( cy.nodes({ data: { weight: { lt: 0.5 } } }) ) ).to.deep.equal([ 'a' ]);
    expect( ids( cy.nodes({ data: { weight: { lte: 0.75 } } }) ) ).to.deep.equal([ 'a', 'b' ]);
  });

  it('ne and in', function(){
    // case-mapper semantics: a missing value fails every op, ne included
    expect( ids( cy.nodes({ data: { kind: { ne: 'x' } } }) ) ).to.deep.equal([ 'b', 'c' ]);
    expect( ids( cy.nodes({ data: { kind: { in: [ 'x', 'y' ] } } }) ) ).to.deep.equal([ 'a', 'b', 'c' ]);
  });

  it('multiple keys AND together', function(){
    expect( ids( cy.nodes({ data: { kind: 'y', weight: { gt: 1 } } }) ) ).to.deep.equal([ 'c' ]);
  });

  it('composes with group and selected', function(){
    cy.$id('b').select();

    expect( ids( cy.elements({ group: 'nodes', data: { weight: { gt: 0.5 } }, selected: true }) ) )
      .to.deep.equal([ 'b' ]);
    expect( ids( cy.elements({ group: 'edges', data: { weight: { gte: 10 } } }) ) )
      .to.deep.equal([ 'bc' ]);
  });

  it('works on edges via cy.edges and filter', function(){
    expect( ids( cy.edges({ data: { weight: { lt: 8 } } }) ) ).to.deep.equal([ 'ab' ]);
    expect( ids( cy.filter({ data: { weight: 5 } }) ) ).to.deep.equal([ 'ab' ]);
  });

  it('works on collection filter (planMatchesRef path)', function(){
    expect( ids( cy.nodes().filter({ data: { kind: 'y' } }) ) ).to.deep.equal([ 'b', 'c' ]);
  });

  it('reflects data writes immediately', function(){
    cy.$id('d').data( 'kind', 'y' );

    expect( ids( cy.nodes({ data: { kind: 'y' } }) ) ).to.deep.equal([ 'b', 'c', 'd' ]);
  ...
  it('unknown keys and malformed conditions throw', function(){
    ...
    expect( () => cy.nodes({ data: { weight: { gt: 1, lt: 2 } } }) ).to.throw(/exactly one comparison/);
    expect( () => cy.nodes({ data: { weight: { between: 1 } } }) ).to.throw(/Unknown data condition op/);
    expect( () => cy.nodes({ data: { weight: { gt: 'high' } } }) ).to.throw(/numeric value/);
    expect( () => cy.nodes({ data: { kind: { in: [] } } }) ).to.throw(/non-empty array/);
  });
