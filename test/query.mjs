...
import { compileQuery, planMatchesRef, MATCH_ALL } from '../src/matcher.mjs';
import { FLAG_SELECTED } from '../src/contract.mjs';

describe('gpu/matcher', function () {
  var store;

  var refOf = (id) => store.lookup(id);
  var matches = (id, query) =>
    planMatchesRef(store, refOf(id), compileQuery(query));
  ...
  describe('compileQuery', function () {
    it('compiles the empty query to per-group match-alls', function () {
      expect(compileQuery({})).to.deep.equal({
        nodes: MATCH_ALL,
        edges: MATCH_ALL,
        data: null,
      });
    ...
    it('compiles group restrictions', function () {
      expect(compileQuery({ group: 'nodes' })).to.deep.equal({
        nodes: MATCH_ALL,
        edges: null,
        data: null,
      });
      expect(compileQuery({ group: 'edges' })).to.deep.equal({
        nodes: null,
        edges: MATCH_ALL,
        data: null,
      });
    ...
    it('narrows to the restrict group', function () {
      expect(compileQuery({}, 'nodes')).to.deep.equal({
        nodes: MATCH_ALL,
        edges: null,
        data: null,
      });
      expect(compileQuery({ selected: true }, 'edges')).to.deep.equal({
        nodes: null,
        edges: { mask: FLAG_SELECTED, want: FLAG_SELECTED },
        data: null,
      });
    });

    it('compiles a contradicting group + restrict to a match-nothing plan', function () {
      expect(compileQuery({ group: 'edges' }, 'nodes')).to.deep.equal({
        nodes: null,
        edges: null,
        data: null,
      });
    ...
    it('throws on unknown query keys', function () {
      expect(function () {
        compileQuery({ selectd: true });
      }).to.throw(/Unknown query key/);
      expect(function () {
        compileQuery({ classes: 'foo' });
      }).to.throw(/Unknown query key/);
    });

    it('throws on unknown groups', function () {
      expect(function () {
        compileQuery({ group: 'node' });
      }).to.throw(/Unknown query group/);
    });
  });
