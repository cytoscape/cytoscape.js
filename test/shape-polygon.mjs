...
describe('gpu/style: shape-polygon-points (round 13 C3)', function () {
  ...
  it('accepts a space-separated string and defaults to the unit square', function () {
    ...
    expect(cy.$id('n').style('shape-polygon-points')).to.equal(
      '-1 0 0 -1 1 0 0 1',
    );
  ...
  it('throws on odd counts, short lists and out-of-range values', function () {
    expect(() => makeCy({ 'shape-polygon-points': [0, -1, 1] })).to.throw(
      /even number/,
    );
    expect(() => makeCy({ 'shape-polygon-points': [0, -1, 1, 1] })).to.throw(
      /at least 3/,
    );
    expect(() =>
      makeCy({ 'shape-polygon-points': [0, -2, 1, 1, -1, 1] }),
  ...
  it('stores the points in the poly blob with a packed borderGeom ref', function () {
    ...
    expect(Array.from(store.polygonPointsAt(slot))).to.eql([
      0, -1, 1, 1, -1, 1,
    ]);
  });
  ...
  it('throws on the edges group', function () {
    expect(() =>
      cytoscape({
        style: { edges: { 'shape-polygon-points': [0, -1, 1, 1, -1, 1] } },
      }),
