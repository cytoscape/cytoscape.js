...
describe('soak: multi-instance isolation', () => {
  it('gives each instance its own id space', () => {
    ...
    expect(
      b.$id('x').data('tag'),
      'a write reached the other instance',
    ).to.equal('b');
  ...
  it('refuses to compare elements across instances (round 48.4)', () => {
    ...
    for (const [name, call] of Object.entries(crossing)) {
      expect(
        call,
        `${name}() accepted a collection from another instance`,
  ...
  it('keeps writes, style and selection to one instance', () => {
    ...
    expect(b.$id('x').style('width'), 'style crossed instances').to.not.equal(
      77,
    );
    expect(b.$id('x').selected(), 'selection crossed instances').to.equal(
      false,
    );
    expect(
      b.$id('y').position().x,
      'a position write crossed instances',
    ).to.equal(10);
  ...
  it('leaves its neighbours working when one is destroyed, in either order', () => {
    ...
    for (const i of [0, 2, 3]) {
      expect(
        instances[i].destroyed(),
        `instance ${i} died with its neighbour`,
      ).to.equal(false);
  ...
  it('compacts one instance without disturbing another', () => {
    ...
    expect(
      b.nodes().length,
      'compacting one instance changed another',
    ).to.equal(3000);
