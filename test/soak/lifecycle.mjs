...
const aliveOf = refs => refs.filter( ref => ref.deref() !== undefined ).length;
...
describe( 'soak: instance lifecycle', () => {
  it( 'has a working reachability probe (the control for everything below)', async () => {
    ...
    expect( aliveOf( heldRefs ), 'a held instance was collected — the probe is broken' )
      .to.equal( 40 );
    expect( aliveOf( droppedRefs ), 'unheld instances were not collected — gc is not running' )
      .to.be.at.most( 1 );
  ...
  it( 'releases a destroyed instance', async () => {
    ...
    expect( aliveOf( refs ), 'destroyed instances are being retained somewhere' )
      .to.be.at.most( 1 );
  } );
  ...
  it( 'holds nothing after destroy even when the app kept a collection', async () => {
    ...
    // must go.
    expect( aliveOf( refs ), 'more than the deliberately-held instance survived' )
      .to.be.at.most( 2 );
  ...
  it( 'removes every listener on destroy', () => {
    ...
    expect( cy._emitter.listeners.length, 'destroy left listeners behind' ).to.equal( 0 );
  } );
  ...
  it( 'returns to zero listeners when handlers are removed one by one', () => {
    ...
    expect( cy._emitter.listeners.length, 'off() did not remove what on() added' )
      .to.equal( before );
  ...
  it( 'does not grow without bound over a thousand cycles', async () => {
    ...
    expect( perCycle, `heap grew ${perCycle.toFixed( 0 )} B per create/destroy cycle` )
      .to.be.below( 20000 );
  } );
} );
