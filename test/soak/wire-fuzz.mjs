...
describe('soak: wire-format fuzzing', () => {
    ...
    it(`survives corruption in the ${name}`, () => {
      ...
      for (let i = 0; i < CASES; i++) {
        ...
        try {
          ...
          expect(e, `case ${i} @${at} threw a non-Error`).to.be.an.instanceOf(
            Error,
          );
          outcomes.threw++;
        }
        ...
        if (parsed != null) {
          try {
            ...
            expect(
              e,
              `case ${i} @${at} threw a non-Error on load`,
        ...
        expect(
          elapsed,
          `case ${i} @${at} took ${elapsed.toFixed(0)} ms`,
        ).to.be.below(250);
      }

      // Non-vacuity, both ways: a run where nothing was rejected, or
      // nothing loaded, is exercising one path and calling it two.
      expect(outcomes.threw + outcomes.loadThrew + outcomes.loaded).to.equal(
        CASES,
      );
      expect(
        outcomes.threw + outcomes.loadThrew,
        `${name}: nothing was rejected`,
      ).to.be.greaterThan(0);
      ...
      if (name === 'header') {
        expect(
          outcomes.threw,
          'header corruption should usually be rejected',
        ...
        expect(
          outcomes.loaded,
          'body corruption is often a legal payload byte',
        ).to.be.greaterThan(CASES / 10);
      }

      expect(
        slowest,
        'no case should be anywhere near the per-case bound',
      ).to.be.below(250);
    });
  }
  ...
  it('never loads a graph it cannot then answer for', () => {
    ...
    for (let i = 0; i < CASES; i++) {
      ...
      expect(
        ids.length,
        `case ${i}: elements() disagrees with its own length`,
      ).to.equal(cy.elements().length);
      expect(new Set(ids).size, `case ${i}: duplicate ids after load`).to.equal(
        ids.length,
      );

      cy.edges().forEach((edge) => {
        expect(edge.source().length, `case ${i}: edge with no source`).to.equal(
          1,
        );
        expect(edge.target().length, `case ${i}: edge with no target`).to.equal(
          1,
        );
      });
