...
describe('gpu/query: structural keys (round 14.7)', function () {
  ...
  it('composes with the other query keys', function () {
    ...
    expect(ids(cy.nodes({ parent: true, selected: true }))).to.deep.equal([
      'q',
    ]);
  ...
  it('filters collections through the same plan', function () {
    ...
    expect(
      ids(cy.$id('p').descendants().filter({ child: true })),
    ).to.deep.equal(['a', 'b', 'q']);
    expect(
      ids(cy.$id('p').descendants().filter({ parent: true })),
    ).to.deep.equal(['q']);
  });
  ...
  it('evaluates structural case conditions', function () {
    const cy = make({
      nodes: {
        backgroundColor: {
          case: [{ when: { child: true }, then: '#111111' }],
          else: '#222222',
        },
      },
    });
  ...
  it('rejects malformed structural conditions', function () {
    expect(() =>
      make({
        nodes: {
          borderWidth: {
            case: [{ when: { parent: true, data: 'x', eq: 1 }, then: 1 }],
            else: 0,
          },
        },
      }),
