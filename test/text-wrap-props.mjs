...
describe('gpu/style: text wrap props (round 16.2)', function () {
  ...
  it('re-estimates dims when a mapped label refreshes on a data write', function () {
    const cy = mk({ nodes: { label: { data: 'name' } } }, [
      { data: { id: 'a', name: 'ab' } },
    ]);
  ...
  it('rejects an unsupported keyword, listing the supported set', function () {
    expect(() => mk({ nodes: { label: 'x', 'text-wrap': 'wrapp' } })).to.throw(
      /text-wrap.*unsupported.*none, wrap, ellipsis/,
    );
    expect(() =>
      mk({ nodes: { label: 'x', 'text-overflow-wrap': 'break-word' } }),
    ).to.throw(/text-overflow-wrap.*unsupported/);
    expect(() =>
      mk({ nodes: { label: 'x', 'text-justification': 'justify' } }),
