...
describe('gpu/wire', function () {
  describe('round trip', function () {
    ...
    it('preserves id holes for auto-generation', function () {
      const out = deserializeElements(
        serializeElements([{ data: {} }, { data: { id: 'x' } }]),
      );
    ...
    it('round-trips an empty graph and a nodes-only graph', function () {
      ...
      const nodesOnly = deserializeElements(
        serializeElements([{ data: { id: 'n' } }]),
      );
    ...
    it('accepts a view and realigns a misaligned one', function () {
      ...
      expect(idsOf(deserializeElements(aligned).nodes.ids)).to.deep.equal([
        'a',
        'b',
        'c',
      ]);
    ...
    it('ignores trailing padding past the recorded length', function () {
      ...
      expect(idsOf(deserializeElements(padded).edges.ids)).to.deep.equal([
        'ab',
        'bc',
      ]);
    });
  ...
  describe('data() columns', function () {
    ...
    it('round-trips mixed columns through the JSON fallback', function () {
      ...
      expect(out.nodes.data.flagged).to.deep.equal([
        true,
        undefined,
        undefined,
      ]);
    ...
    it('ingests to full data() parity with the defs path', function () {
      const viaDefs = cytoscape({ elements: DATA_FIXTURE });
      const viaWire = cytoscape({ elements: serializeElements(DATA_FIXTURE) });

      for (const id of ['a', 'b', 'c', 'ab']) {
        expect(viaWire.$id(id).data(), id).to.deep.equal(
          viaDefs.$id(id).data(),
        );
      }
    });
    ...
    it('survives a 4-but-not-8-aligned view (f64 realign)', function () {
      ...
      const out = deserializeElements(
        new Uint8Array(shifted.buffer, 4, buffer.byteLength),
      );
  ...
  */
  describe('graph-level data (round 39.2)', function () {
    it('round-trips through serialize/deserialize', function () {
      const payload = {
        ...toColumnarElements(FIXTURE),
        data: { name: 'g', n: 7, deep: { k: [1, 2] } },
      };
    ...
    it('costs nothing when there is none', function () {
      ...
      expect(serializeElements(withEmpty).byteLength).to.equal(
        serializeElements(payload).byteLength,
      );
      expect(deserializeElements(serializeElements(payload)).data).to.equal(
        undefined,
      );
    });

    it('carries cy.data() out through cy.serialize()', function () {
      const cy = cytoscape({ elements: FIXTURE });

      cy.data({ title: 'my graph', version: 2 });

      expect(deserializeElements(cy.serialize()).data).to.deep.equal({
        title: 'my graph',
        version: 2,
      });
    ...
    it('applies it through options.elements', function () {
      const source = cytoscape({ elements: FIXTURE });

      source.data('title', 'my graph');

      const loaded = cytoscape({ elements: source.serialize() });
    ...
    it('ignores it in cy.add(), leaving the target graph data alone', function () {
      ...
      const target = cytoscape({ elements: [] });
      ...
      expect(
        target.data('title'),
        'add() overwrote the target graph data',
  ...
  describe('validation', function () {
    it('rejects a buffer without the magic header', function () {
      expect(() => deserializeElements(new ArrayBuffer(64))).to.throw(
        /serialized elements/i,
      );
      expect(() => deserializeElements(new ArrayBuffer(4))).to.throw(
        /serialized elements/i,
      );
    });
    ...
    it('rejects a truncated buffer', function () {
      ...
      expect(() =>
        deserializeElements(buffer.slice(0, buffer.byteLength - 4)),
      ).to.throw(/truncated/);
    });
    ...
    it('rejects short columns at serialize time', function () {
      expect(() =>
        serializeElements({
          columnar: true,
          nodes: { count: 2, positions: new Float32Array(2) },
        }),
      ...
      expect(() =>
        serializeElements({
          columnar: true,
          nodes: { count: 2, selected: new Uint8Array(1) },
        }),
  ...
  describe('ingest', function () {
    it('matches the definition path column for column', function () {
      ...
      const viaWire = cytoscape({ elements: serializeElements(FIXTURE) });
      ...
      for (const id of [
        ...
        expect(Array.from(viaWire._store.column(id)), id).to.deep.equal(
          Array.from(viaDefs._store.column(id)),
        );
      }

      expect(viaWire._store.slotsOrdered('nodes')).to.deep.equal(
        viaDefs._store.slotsOrdered('nodes'),
      );
      expect(viaWire._store.slotsOrdered('edges')).to.deep.equal(
        viaDefs._store.slotsOrdered('edges'),
      );
    });
    ...
    it('add() accepts a serialized buffer and a view over one', function () {
      ...
      expect(
        cy2.add(new Uint8Array(serializeElements(FIXTURE))),
      ).to.have.length(5);
    });

    it('selection state survives the wire', function () {
      const cy = cytoscape({ elements: serializeElements(FIXTURE) });
  ...
  describe('malformed input', function () {
    it('rejects an unknown data column kind rather than reading past it', function () {
      ...
      expect(
        new Uint32Array(buffer, kindAt, 1)[0],
        'kind 0 = numeric',
      ).to.equal(0);

      new Uint32Array(buffer, kindAt, 1)[0] = 7;

      expect(() => deserializeElements(buffer)).to.throw(
        /Unknown serialized data column kind 7/,
      );
    });
    ...
    it('rejects packed ids whose offsets are short of count + 1', function () {
      ...
      expect(() => serializeElements(short)).to.throw(
        /Packed ids must have 4 offsets/,
      );
    ...
    it('rejects a data block that declares more keys than could fit (round 48.3)', function () {
      ...
      expect(() => deserializeElements(buffer)).to.throw(
        /declares 4294967295 data keys but has only 0 bytes left/,
      );
    ...
    it('rejects a dictionary index past the end of its dictionary (round 48.3)', function () {
      ...
      expect(() => cytoscape({ elements: corrupt })).to.throw(
        /dictionary index 2566914049 at element 0, beyond its 2-entry dictionary/,
      );
      ...
      expect(() => cy.add(second)).to.throw(
        /dictionary index 99 at element 1, beyond its 2-entry dictionary/,
      );
