...
// An offscreen texture sheet onto which several element images are packed.
interface Texture {
  eleCaches: EleCache[];
  height: number;
  width: number;
  usedWidth: number;
  invalidatedWidth: number;
  fullnessChecks: number;
  retired?: boolean;
  canvas: HTMLCanvasElement | OffscreenCanvas;
  context: CanvasRenderingContext2D;
}

// A single cached element image, located within a Texture.
interface EleCache {
  x: number;
  texture: Texture;
  level: number;
  ...
  width: number;
  height: number;
  scaledLabelShown: boolean;
  invalidated?: boolean;
  key?: string;
}

// A queued request to (re)render an element's image at a given level.
interface EleQueueReq {
  eles: Collection; // a collection of eles sharing the same key
  level: number;
  reqs: number;
  key: string;
}
...
interface ElementTextureCacheInstance {
  renderer: Renderer;
  onDequeues: Array<( deqd: EleQueueReq[] ) => void>;
  lookup: ElementTextureCacheLookup;
  eleImgCaches?: Record<string, Texture[]> & { retired?: Record<string, Texture[]> };
  eleCacheQueue?: Heap<EleQueueReq>;
  eleKeyToCacheQueue?: Record<string, EleQueueReq | null>;
  getKey( ele: Element ): string;
  getBoundingBox( ele: Element ): BoundingBox;
  drawElement( context: CanvasRenderingContext2D, ele: Element, bb: BoundingBox, scaledLabelShown: boolean, useEleOpacity: boolean ): void;
  isVisible( ele: Element, scaledLabelShown: boolean ): boolean;
  allowEdgeTxrCaching: boolean;
  allowParentTxrCaching: boolean;
  setupDequeueing(): void;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- open-ended prototype-mixin surface and assigned init options
  [key: string]: any;
}
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- constructor function init-options bag is open-ended
const ElementTextureCache = function( this: ElementTextureCacheInstance, renderer: Renderer, initOptions: any ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias

  self.renderer = renderer;
  self.onDequeues = [];

  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- merged init options; getKey/doesEleInvalidateKey are supplied at runtime, overriding the null defaults
  let opts = initDefaults(initOptions) as any;
...
} as unknown as { new ( renderer: Renderer, initOptions: unknown ): ElementTextureCacheInstance; prototype: ElementTextureCacheInstance };
...
ETCp.getTextureQueue = function( this: ElementTextureCacheInstance, txrH: number ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias
...
ETCp.getRetiredTextureQueue = function( this: ElementTextureCacheInstance, txrH: number ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias
...
ETCp.getElementQueue = function( this: ElementTextureCacheInstance ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias

  let q = self.eleCacheQueue = self.eleCacheQueue || new Heap(function( a: EleQueueReq, b: EleQueueReq ){
...
ETCp.getElementKeyToQueue = function( this: ElementTextureCacheInstance ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias
...
ETCp.getElement = function( this: ElementTextureCacheInstance, ele: Element, bb: BoundingBox | null, pxRatio: number, lvl: number | null, reason?: string ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias
  let r = this.renderer;
  let zoom = r.cy.zoom();
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- caches are opaque entries from the lookup (typed `any`)
  let scalableFrom = function( otherCache: any ){
...
ETCp.invalidateElements = function( this: ElementTextureCacheInstance, eles: Element[] ){
  for( let i = 0; i < eles.length; i++ ){
    this.invalidateElement(eles[i]);
  }
};

ETCp.invalidateElement = function( this: ElementTextureCacheInstance, ele: Element ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias
...
ETCp.checkTextureUtility = function( this: ElementTextureCacheInstance, txr: Texture ){
...
ETCp.checkTextureFullness = function( this: ElementTextureCacheInstance, txr: Texture ){
...
ETCp.retireTexture = function( this: ElementTextureCacheInstance, txr: Texture ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias
  let txrH = txr.height;
  let txrQ = self.getTextureQueue( txrH );
  let lookup = this.lookup;
...
ETCp.addTexture = function( this: ElementTextureCacheInstance, txrH: number, minW: number ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias
  let txrQ = self.getTextureQueue( txrH );
  let txr = {} as Texture;
  ...
  txr.context = txr.canvas.getContext('2d') as CanvasRenderingContext2D;

  return txr;
};

ETCp.recycleTexture = function( this: ElementTextureCacheInstance, txrH: number, minW: number ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias
  let txrQ = self.getTextureQueue( txrH );
  let rtxtrQ = self.getRetiredTextureQueue( txrH );
...
ETCp.queueElement = function( this: ElementTextureCacheInstance, ele: Element, lvl: number ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias
...
ETCp.dequeue = function( this: ElementTextureCacheInstance, pxRatio: number /*, extent*/ ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias
  let q = self.getElementQueue();
  let k2q = self.getElementKeyToQueue();
  let dequeued: EleQueueReq[] = [];
  let lookup = self.lookup;
...
ETCp.removeFromQueue = function( this: ElementTextureCacheInstance, ele: Element ){
  let self = this; // eslint-disable-line @typescript-eslint/no-this-alias
  let q = self.getElementQueue();
  let k2q = self.getElementKeyToQueue();
  let key = this.getKey(ele);
...
ETCp.onDequeue = function( this: ElementTextureCacheInstance, fn: ( deqd: EleQueueReq[] ) => void ){ this.onDequeues.push( fn ); };
ETCp.offDequeue = function( this: ElementTextureCacheInstance, fn: ( deqd: EleQueueReq[] ) => void ){ removeFromArray( this.onDequeues, fn ); };
...
ETCp.setupDequeueing = defs.setupDequeueing({
  ...
  deq: function( self: ElementTextureCacheInstance, pxRatio: number, extent: BoundingBox ){
    return self.dequeue( pxRatio, extent );
  },
  onDeqd: function( self: ElementTextureCacheInstance, deqd: EleQueueReq[] ){
  ...
  shouldRedraw: function( self: ElementTextureCacheInstance, deqd: EleQueueReq[], pxRatio: number, extent: BoundingBox ){
  ...
  priority: function( self: ElementTextureCacheInstance ){
