...
import type { AtlasTexture } from './webgl-util.mjs';
...
// A "location" is a pointer into the atlas (a row + x), and the rectangle a texture occupies.
...
// A bounding box; only w/h are required by the atlas packing math.
interface BB {
  w: number;
  h: number;
}

// Offscreen canvas augmented with the helpers attached in webgl-util.createTextureCanvas.
interface TextureCanvas extends HTMLCanvasElement {
  context: CanvasRenderingContext2D;
  clear(): void;
}

// Factory signature matching webgl-util.createTextureCanvas (plus an optional cache key).
type CreateTextureCanvasFn = (r: Renderer, w: number, h: number, scratch?: string) => TextureCanvas;

// eslint-disable-next-line @typescript-eslint/no-explicit-any -- callback driven by the canvas renderer
type DoDrawing = (context: CanvasRenderingContext2D, bb: BB) => any;
...
export class Atlas {
  ...
  needsBuffer: boolean;
  freePointer: { x: number, row: number };
  keyToLocation: Map<string, [AtlasLocation | null, AtlasLocation | null]>;
  canvas: TextureCanvas | null;
  scratch: TextureCanvas | null;

  constructor(r: Renderer, texSize: number, texRows: number, createTextureCanvas: CreateTextureCanvasFn) {
  ...
  draw(key: string, bb: BB, doDrawing?: DoDrawing): [AtlasLocation | null, AtlasLocation | null] | false {
    if(this.locked)
    ...
    const drawAt = (location: { x: number, row: number }, canvas: TextureCanvas | null) => {
      if(doDrawing && canvas) {
    ...
    const locations: [AtlasLocation | null, AtlasLocation | null] = [ null, null ];
  ...
  bufferIfNeeded(gl: WebGL2RenderingContext) {
    if(!this.texture) {
      this.texture = util.createTexture(gl, this.debugID);
    }
    if(this.needsBuffer) {
      // canvas is non-null whenever needsBuffer is true at this point
      this.texture.buffer(this.canvas as TextureCanvas);
...
export class AtlasCollection {
  ...
  texSize: number;
  texRows: number;
  createTextureCanvas: CreateTextureCanvasFn;
  atlases: Atlas[];
  styleKeyToAtlas: Map<string, Atlas>;
  markedKeys: Set<string>;
  scratch?: TextureCanvas;
  ...
  draw(key: string, bb: BB, doDrawing?: DoDrawing) {
  ...
  gc() {
    ...
    const newAtlases: Atlas[] = [];
    const newStyleKeyToAtlas = new Map<string, Atlas>();

    let newAtlas: Atlas | null = null;
  ...
  _copyTextureToNewAtlas(key: string, oldAtlas: Atlas, newAtlas: Atlas) {
    const [ s1, s2 ] = oldAtlas.getOffsets(key)! as [AtlasLocation, AtlasLocation];
...
function intersection<T>(set1: Set<T>, set2: Set<T>): Set<T> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Set.prototype.intersection isn't in our lib target
  if((set1 as any).intersection)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return (set1 as any).intersection(set2);
  else
    return new Set([...set1].filter(x => set2.has(x)));
}

// Render type options are configuration bags supplied by the drawing layer.
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- caller-provided option bag with many optional callbacks
type RenderTypeOpts = any;
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- global renderer options bag
type GlobalOptions = any;
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- a cytoscape element
type Ele = any;
...
export class AtlasManager {
  ...
  atlasSize: number;
  maxAtlasesPerBatch: number;
  renderTypes: Map<string, RenderTypeOpts>;
  collections: Map<string, AtlasCollection>;
  typeAndIdToKey: Map<string, string[]>;

  constructor(r: Renderer, globalOptions: GlobalOptions) {
    this.r = r;
  ...
  addAtlasCollection(collectionName: string, atlasCollectionOptions: RenderTypeOpts) {
  ...
  addRenderType(type: string, renderTypeOptions: RenderTypeOpts) {
  ...
  getRenderTypeOpts(type: string) {
    return this.renderTypes.get(type);
  }

  getAtlasCollection(name: string) {
    return this.collections.get(name);
  }

  _cacheScratchCanvas(createTextureCanvas: CreateTextureCanvasFn): CreateTextureCanvasFn {
    ...
    let prevW = -1;
    let prevH = -1;
    let scratchCanvas: TextureCanvas | null = null;

    return (r: Renderer, w: number, h: number, scratch?: string) => {
      if(scratch) {
  ...
  _key(renderType: string, id: string) {
  ...
  invalidate(
    ...
    {
      forceRedraw = false,
      filterEle = (() => true) as (ele: Ele) => boolean,
      filterType = (() => true) as (type: string) => boolean
    } = {}
  ): boolean {
    let needGC = false;
    let runGCNow = false;
      ...
      if(filterEle(ele)) {
        ...
        for(const opts of this.renderTypes.values()) {
          ...
          if(filterType(renderType)) {
            ...
            const keyArray: string[] = Array.isArray(key) ? key : [key];
  ...
  getOrCreateAtlas(ele: Ele, type: string, bb: BB, styleKey: string) {
  ...
  getAtlasInfo(ele: Ele, type: string) {
    const opts = this.renderTypes.get(type);
    const key = opts.getKey(ele);
    const keyArray: string[] = Array.isArray(key) ? key : [key];
...
export class AtlasBatchManager {
  ...
  canAddToCurrentBatch(atlas: Atlas) {
  ...
  getAtlasIndexForBatch(atlas: Atlas) {
